...
- 2026-06-01 experimental demo broker-action start:
  ...
  - 14 experimental demo executor charts were attached on qualified symbols with explicit arming inputs `InpDryRunOnly=false`, `InpBrokerActionAllowed=true`, fixed `0.01` lot, hard SL/TP, one open position per instance, and demo-server guard checks. The hardened 2026-06-02 source adds stricter guards; existing terminal attachments should not be assumed to contain the new guard inputs until redeployed with explicit owner authorization.
