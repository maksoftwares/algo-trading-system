...
- 2026-06-01 experimental demo broker-action start:
  ...
  - 14 experimental demo executor charts are attached on qualified symbols with explicit arming inputs `InpDryRunOnly=false`, `InpBrokerActionAllowed=true`, fixed `0.01` lot, hard SL/TP, one open position per instance, and demo-server guard checks.
...
- Phase 2 paper-mode implementation is blocked by pending measured-cost evidence, VPS evidence, demo preflight, and owner approval. Phase 1 acceptance is closed; continue Phase 1 dry-run and passive spread logging; do not add broker-side execution.
