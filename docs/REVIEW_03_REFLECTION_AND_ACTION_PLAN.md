...
| V2 | Re-run D2 against the full candidate universe. | Re-run completed again after `daily_pivot_reclaim_v0`. `breakout_retest` remains winner across 20 non-empty matrix-ledger candidates. White p=0.0200, max pairwise SPA p=0.0326. | PASS | `xau-usd/xauusd-phase0/outputs/reports/PHASE0_REALITY_CHECK.md` |
| V3 | Aggregate rejected-candidate gate reasons to test frequency bias. | Audit generated after `daily_pivot_reclaim_v0`. 19 rejected/research candidates were audited. 4 had sample-size failures, all 19 had multi-cell expectancy failures, and 0 were frequency-only failures. | PASS | `xau-usd/xauusd-phase0/outputs/reports/PHASE0_REJECTED_CANDIDATE_GATE_AUDIT.md` |
...
| Non-empty matrix-ledger candidates | 20 |
...
| Multi-cell expectancy failures | 19 |
| Both expectancy and sample-size failures | 4 |
| Frequency-only failures | 0 |

Interpretation: the current evidence does not show candidates being rejected only because they were low-frequency. Frequency pressure exists for some candidates, but every rejected/research candidate also failed multi-cell expectancy survival. `daily_pivot_reclaim_v0` was rejected on expectancy despite adequate trade count.
