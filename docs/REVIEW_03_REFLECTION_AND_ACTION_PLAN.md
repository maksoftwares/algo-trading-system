...
| V2 | Re-run D2 against the full candidate universe. | Re-run completed again after `round_number_retest_v0`. `breakout_retest` remains winner across 23 non-empty matrix-ledger candidates. White p=0.0200, max pairwise SPA p=0.0326. | PASS | `xau-usd/xauusd-phase0/outputs/reports/PHASE0_REALITY_CHECK.md` |
| V3 | Aggregate rejected-candidate gate reasons to test frequency bias. | Audit generated after `round_number_retest_v0`. 22 rejected/research candidates were audited; 21 have multi-cell expectancy failures and `round_number_retest_v0` is non-matrix pending because it passed matrix gates but awaits Gate 9. | PASS | `xau-usd/xauusd-phase0/outputs/reports/PHASE0_REJECTED_CANDIDATE_GATE_AUDIT.md` |
...
| Non-empty matrix-ledger candidates | 23 |
...
Interpretation: the current evidence does not show candidates being rejected only because they were low-frequency. Frequency pressure exists for some candidates, but every rejected candidate also failed multi-cell expectancy survival. `round_number_retest_v0` is the first new non-rejected row in this audit, but it remains pending Gate 9 manual review.
