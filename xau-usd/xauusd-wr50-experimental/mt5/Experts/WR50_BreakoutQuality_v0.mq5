...
#property description "WR50 demo-only experimental breakout-retest quality lane. Non-canonical, no live authorization."
...
input double InpBreakAtrMultiple = 0.5;
input double InpMinBodyToRange = 0.50;
input double InpMaxOpposingWickToRange = 0.35;
...
const string EA_ID = "wr50_bqv0";
const string EA_NAME = "WR50_BreakoutQuality_v0";
const string EA_VERSION = "v0";
const string EA_SHORT_CODE = "BQV0";
const string STRATEGY_FAMILY = "breakout_retest_wr50_experimental";
const int EA_MAGIC = WR50_BQV0_ACTIVE_MAGIC;
const int EA_MAGIC_START = WR50_BQV0_MAGIC_START;
const int EA_MAGIC_END = WR50_BQV0_MAGIC_END;
...
int OnInit()
{
   ...
   Comment("WR50 BQV0 demo experiment armed. Non-canonical.");
...
void OnTick()
{
   ...
   if(!WR50_GetBreakoutRetestSignal(_Symbol, 1.5, InpBreakAtrMultiple, true, InpMinBodyToRange,
                                    InpMaxOpposingWickToRange, 5, 0.1,
                                    "WR50_BQV0_LONG", "WR50_BQV0_SHORT", signal))
   {
      WR50_LogBlock(EA_ID, EA_SHORT_CODE, EA_VERSION, STRATEGY_FAMILY, InpExperimentId, InpRunId, EA_MAGIC,
                    "WR50_BQV0_NO_SIGNAL", signal.block_reason, spread_points, InpMaxSpreadPoints);
      return;
   }
