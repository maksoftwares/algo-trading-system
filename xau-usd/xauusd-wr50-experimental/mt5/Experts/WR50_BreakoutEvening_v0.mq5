...
#property description "WR50 demo-only experimental breakout-retest evening lane. Non-canonical, no live authorization."
...
input int    InpSessionAStartHour = 18;
input int    InpSessionAEndHour = 23;
input int    InpSessionBStartHour = 0;
input int    InpSessionBEndHour = 6;
...
const string EA_ID = "wr50_bev0";
const string EA_NAME = "WR50_BreakoutEvening_v0";
const string EA_VERSION = "v0";
const string EA_SHORT_CODE = "BEV0";
const string STRATEGY_FAMILY = "breakout_retest_wr50_experimental";
const int EA_MAGIC = WR50_BEV0_ACTIVE_MAGIC;
const int EA_MAGIC_START = WR50_BEV0_MAGIC_START;
const int EA_MAGIC_END = WR50_BEV0_MAGIC_END;
...
int OnInit()
{
   ...
   Comment("WR50 BEV0 demo experiment armed. Non-canonical.");
...
void OnTick()
{
   ...
   if(!WR50_InConfiguredSession(InpSessionAStartHour, InpSessionAEndHour, InpSessionBStartHour, InpSessionBEndHour))
   {
      WR50_LogBlock(EA_ID, EA_SHORT_CODE, EA_VERSION, STRATEGY_FAMILY, InpExperimentId, InpRunId, EA_MAGIC,
                    "WR50_BEV0_SESSION", "outside_evening_night_session", spread_points, InpMaxSpreadPoints);
      return;
   }

   WR50Signal signal;
   WR50_ResetSignal(signal);
   if(!WR50_GetBreakoutRetestSignal(_Symbol, 1.5, 0.3, false, 0.50, 0.35, 5, 0.1,
                                    "WR50_BEV0_LONG", "WR50_BEV0_SHORT", signal))
   {
      WR50_LogBlock(EA_ID, EA_SHORT_CODE, EA_VERSION, STRATEGY_FAMILY, InpExperimentId, InpRunId, EA_MAGIC,
                    "WR50_BEV0_NO_SIGNAL", signal.block_reason, spread_points, InpMaxSpreadPoints);
      return;
   }
