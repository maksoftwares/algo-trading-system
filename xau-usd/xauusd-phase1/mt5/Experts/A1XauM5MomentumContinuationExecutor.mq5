   ...
   if(InpRegimeRouterMode == REGIME_ROUTER_R3_COMPRESSION_RELEASE_SHOCK_BLOCK)
     {
      if(InpSignalMode != SIGNAL_D1_COMPRESSION_H4_EXPANSION &&
        ...
        {
         block_reason = "regime_router_block_r3_compression_release_shock_block_incompatible_signal_mode";
         return false;
        }
      return true;
     }
   ...
   if(InpRegimeRouterMode == REGIME_ROUTER_R3_INSIDE_COMPRESSION_TREND_SHOCK_BLOCK)
     {
      if(InpSignalMode != SIGNAL_R3_INSIDE_COMPRESSION_H1_BOUNDARY_M15_SWEEP_RECLAIM)
        {
         block_reason = "regime_router_block_r3_inside_compression_trend_shock_block_incompatible_signal_mode";
         return false;
        }
   ...
   if(sent)
     {
      ...
      const bool exact_hard_risk_log =
         InpSignalMode == SIGNAL_R1_SECOND_CONTINUATION_HIGHER_LOW_LONG || r3_chop_mode;
      LogOrder("ORDER_SEND_OK", direction, order_lots, bid, ask, spread_points, entry_reference, sl, tp, stop_points, estimated_cost_r, retcode, retcode_description, g_trade.ResultOrder(), g_trade.ResultDeal(), g_trade.ResultPrice(), "pass", logged_actual_risk_usd, exact_hard_risk_log ? InpRiskAmountUsd : -1.0, exact_hard_risk_log ? "OrderCalcProfit" : "");
     }
   else
     {
