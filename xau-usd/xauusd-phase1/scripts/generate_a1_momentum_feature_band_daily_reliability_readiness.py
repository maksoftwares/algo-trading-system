...
OUTPUT_STEM = "A1_XAU_M5_MOMENTUM_FEATURE_BAND_DAILY_RELIABILITY_READINESS_2026_07_02"

DRAFT = PHASE1_ROOT / "docs" / "A1_XAU_M5_MOMENTUM_FEATURE_BAND_DAILY_RELIABILITY_FORWARD_DRAFT_2026_07_02.md"
MANIFEST = DRAFT.with_suffix(DRAFT.suffix + ".sha256.json")
DAY_STATE_JSON = REPORTS_DIR / "A1_XAU_M5_MOMENTUM_FEATURE_BAND_DAY_STATE_SEARCH_2026_07_02.json"
EA_SOURCE = PHASE1_ROOT / "mt5" / "Experts" / "A1XauM5MomentumContinuationExecutor.mq5"

EXPECTED_SHA = "693d070050666ccd066a834f71b666b3f865829e6a05c8942190be7da9c1729b"
EXPECTED_MAGICS = {
    "feature_band_daily_reliability_long": 932294,
    "feature_band_daily_reliability_v13_both": 932295,
}
...
def build_payload() -> dict[str, Any]:
    ...
    day_state = load_json(DAY_STATE_JSON)
    candidate = day_state.get("best", {})
    ...
    checks: list[dict[str, str]] = [
        ...
        check(
            "candidate_is_cooldown_15",
            candidate.get("name") == "owner_target_50_max6_cooldown_after_loss_15"
            and candidate.get("cooldown_after_loss_minutes") == 15,
            f"name={candidate.get('name')}, cooldown={candidate.get('cooldown_after_loss_minutes')}",
        ),
        check(
            "candidate_meets_business_frequency",
        ...
        check(
            "candidate_meets_reliability_bar",
            candidate.get("win_rate_pct", 0) >= 60.0
            and candidate.get("profit_factor", 0) >= 1.25
            and candidate.get("net_usd", 0) >= 1000.0
            ...
            (
                ...
                f"net={candidate.get('net_usd')}, pos_day={candidate.get('positive_day_pct')}"
            ),
        ),
        check(
            "ea_default_off_cooldown_input",
            "input int    InpPortfolioCooldownAfterLossMinutes = 0;" in ea_text,
            "EA exposes portfolio cooldown-after-loss default-off.",
        ),
        check(
            "ea_guard_logs_cooldown_blocks",
            "portfolio_cooldown_after_loss_active" in ea_text,
            "EA logs portfolio cooldown blocks.",
        ),
    ]
    ...
    for variant, magic in EXPECTED_MAGICS.items():
        ...
        checks.extend(
            [
                ...
                check(
                    f"{variant}_cooldown_guard",
                    config.get("portfolio_daily_guard_enabled") == "true"
                    and config.get("portfolio_guard_magic_csv") == "932294,932295"
    ...
    return {
        ...
        "day_state_report": rel(REPORTS_DIR / "A1_XAU_M5_MOMENTUM_FEATURE_BAND_DAY_STATE_SEARCH_2026_07_02.md"),
        "candidate": candidate,
        "planned_variants": {
            variant: {
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# A1 XAU M5 Momentum Feature-Band Daily Reliability Readiness - 2026-07-02",
        "",
        ...
        "## Candidate",
        "",
        "Daily reliability package: `+50 USD` portfolio target, max `6` package trades/day, and `15` minute package cooldown after any closed package loss.",
        "",
        "| Metric | Value |",
        "|---|---:|",
        ...
        f"| Active days | {candidate.get('active_days', 'n/a')} |",
        f"| Trades / active day | {candidate.get('trades_per_active_day', 'n/a')} |",
        f"| 3+ trade days | {candidate.get('three_plus_trade_day_pct', 'n/a')}% |",
    ...
    lines.extend(
        [
            ...
            f"- Day-state report: `{payload['day_state_report']}`",
        ]
    )
    return "\n".join(lines) + "\n"
