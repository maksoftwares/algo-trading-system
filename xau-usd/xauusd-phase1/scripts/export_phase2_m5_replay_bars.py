...
def export_phase2_m5_replay_bars(
    ...
    try:  # pragma: no cover - exercised against the user's local terminal
        ...
        for symbol in symbols:
            ...
            rows = _rate_rows(rates, symbol=symbol, terminal_exe=terminal_exe)
            rows_by_symbol[symbol] = rows
            _write_csv(output_dir / f"{symbol}_M5_20260601_to_latest.csv", rows, BAR_FIELDS)
    finally:
        mt5.shutdown()

    report_rows = [
        _continuity(symbol=symbol, rows=rows, requested_start=start, requested_end=end)
...
def _rate_rows(rates: Any, *, symbol: str, terminal_exe: Path) -> list[dict[str, str]]:
    if rates is None:
        return []
    rows: list[dict[str, str]] = []
    for item in rates:
        start = datetime.fromtimestamp(int(item["time"]), tz=timezone.utc)
        end = datetime.fromtimestamp(int(item["time"]) + 300, tz=timezone.utc)
        rows.append(
            {
...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["symbols"]:
        lines.append(
            f"| {row['symbol']} | {row['status']} | {row['rows']} | {row['first_bar_utc']} | "
