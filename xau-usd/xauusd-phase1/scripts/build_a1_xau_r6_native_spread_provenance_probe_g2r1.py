"""Build the deterministic privacy-safe zero-action NP1-G2R1 native spread probe."""
...
SOURCE = ROOT / "mt5" / "Experts" / "A1XauR6NativeSpreadProvenanceProbeG2R1.mq5"
...
PROBE_SOURCE = r'''//+------------------------------------------------------------------+
//| A1XauR6NativeSpreadProvenanceProbeG2R1.mq5                       |
//| NP1-G2R1 ZERO-ACTION PRIVACY-SAFE PROVENANCE PROBE               |
//+------------------------------------------------------------------+
#property copyright "maksoftwares - NP1-G2R1 diagnostic only"
...
bool EnvironmentPass()
  {
   bool login_present=AccountInfoInteger(ACCOUNT_LOGIN)>0;
   bool login_matches=InpExpectedLogin>0 && AccountInfoInteger(ACCOUNT_LOGIN)==InpExpectedLogin;
   bool server_present=StringLen(AccountInfoString(ACCOUNT_SERVER))>0;
   bool server_matches=StringLen(InpExpectedAccountServer)>0 && AccountInfoString(ACCOUNT_SERVER)==InpExpectedAccountServer;
   return MQLInfoInteger(MQL_TESTER) && _Symbol==TARGET_SYMBOL && _Period==PERIOD_M5 &&
          login_present && login_matches && server_present && server_matches &&
...
int OnInit()
  {
   ...
   AssertRow("account_login_present",login_present,login_present?"pass":"fail","pass");
   AssertRow("account_login_matches",login_matches,login_matches?"pass":"fail","pass");
   AssertRow("account_server_present",server_present,server_present?"pass":"fail","pass");
   AssertRow("account_server_matches",server_matches,server_matches?"pass":"fail","pass");
...
def assert_source_safety(text: str) -> None:
    ...
    required = (
        ...
        "InpExpectedLogin=0", 'InpExpectedAccountServer=""',
        'AssertRow("account_login_present"', 'AssertRow("account_login_matches"',
        'AssertRow("account_server_present"', 'AssertRow("account_server_matches"',
    )
    if any(token not in text for token in required):
        raise RuntimeError("probe source is missing a required provenance or zero-action primitive")
    forbidden_literal_patterns = (
        r"InpExpectedLogin\s*=\s*[1-9][0-9]*",
        r'InpExpectedAccountServer\s*=\s*"[^\"]+"',
        r"AccountInfoInteger\(ACCOUNT_LOGIN\)\s*==\s*[1-9][0-9]*",
        r'AccountInfoString\(ACCOUNT_SERVER\)\s*==\s*"[^\"]+"',
    )
    if any(re.search(pattern, text) for pattern in forbidden_literal_patterns):
        raise RuntimeError("probe source contains a hardcoded account selector")
...
def build_probe(output: Path = SOURCE, manifest: Path | None = None) -> dict[str, object]:
    ...
    payload = {
        "schema_version": "a1_xau_r6_native_spread_probe_g2r1_source_manifest_v1",
        "source": output.name,
