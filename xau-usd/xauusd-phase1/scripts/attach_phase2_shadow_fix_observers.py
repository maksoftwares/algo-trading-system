...
DEFAULT_PORTABLE_ROOT = Path("C:/MT5PortableShadowFixObservers")
DEFAULT_TERMINAL_DATA_DIR = DEFAULT_PORTABLE_ROOT
DEFAULT_TERMINAL_EXE = DEFAULT_PORTABLE_ROOT / "terminal64.exe"
DEFAULT_METAEDITOR_EXE = Path("C:/Program Files/MetaTrader 5/MetaEditor64.exe")
DEFAULT_OUTPUT_JSON = Path("outputs") / "reports" / "PHASE2_SHADOW_FIX_OBSERVER_ATTACHMENTS.json"
DEFAULT_OUTPUT_MD = Path("outputs") / "reports" / "PHASE2_SHADOW_FIX_OBSERVER_ATTACHMENTS.md"
...
def attach_phase2_shadow_fix_observers(
    phase1_root: Path,
    ...
    _guard_not_standard_demo_terminal(terminal_data_dir, terminal_exe)
    ...
    if not attachments:
        raise RuntimeError("No qualified shadow-fix observer attachments were found.")
    ...
    payload: dict[str, Any] = {
        ...
        "authority": (
            "Shadow-fix observer setup only. Attachments log telemetry and explicitly set "
            "broker_action_allowed=false. This does not touch current demo trading EAs, does not place "
            "orders, and does not mark canonical Phase 2 as passed."
        ),
        "run_id": RUN_ID,
        "terminal": {
    ...
    return AttachOutput("SHADOW_FIX_OBSERVERS_ATTACHED_TO_ISOLATED_TERMINAL", output_json, output_md, len(attachments))
...
def _guard_not_standard_demo_terminal(terminal_data_dir: Path, terminal_exe: Path) -> None:
    if terminal_data_dir.resolve() == STANDARD_DEMO_TERMINAL_DATA_DIR.resolve():
        raise RuntimeError("Refusing to attach shadow-fix observers to the standard demo trading terminal data folder.")
    if terminal_exe.resolve() == STANDARD_DEMO_TERMINAL_EXE.resolve():
        raise RuntimeError("Refusing to attach shadow-fix observers with the standard demo trading terminal executable.")


def _archive_existing_observer_logs(terminal_data_dir: Path) -> Path | None:
    files_dir = terminal_data_dir / "MQL5" / "Files"
    candidates = [
        *files_dir.glob("shadow_fix_observer_signal_log*.csv"),
        *files_dir.glob("shadow_fix_observer_startup*.csv"),
    ]
    existing = [path for path in candidates if path.exists()]
    if not existing:
        return None
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    backup_dir = terminal_data_dir / "_codex_quarantine" / "observer_logs" / f"shadow_fix_observer_logs_{stamp}"
...
def _replace_default_profile(terminal_data_dir: Path, attachments: list[AttachmentRow]) -> Path:
    ...
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    backup_dir = backup_root / f"default_profile_before_shadow_fix_attach_{stamp}"
...
def _attachment_log_name(row: AttachmentRow) -> str:
    return f"shadow_fix_observer_signal_log_{_instance_slug(row)}.csv"


def _startup_log_name(row: AttachmentRow) -> str:
    return f"shadow_fix_observer_startup_{_instance_slug(row)}.csv"
...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Standard demo terminal touched: `{payload['terminal']['standard_demo_terminal_touched']}`",
        f"Standard demo terminal closed/restarted: `{payload['terminal']['standard_demo_terminal_closed_or_restarted']}`",
        f"Shadow policy: `{payload['ea']['shadow_policy_version']}`",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Attach shadow-fix observers to an isolated MT5 demo terminal.")
    ...
    output = attach_phase2_shadow_fix_observers(
