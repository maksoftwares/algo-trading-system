...
DEFAULT_OUTPUT_JSON = Path("outputs") / "reports" / "PHASE2_EXPERIMENTAL_DEMO_EXECUTOR_ATTACHMENTS.json"
DEFAULT_OUTPUT_MD = Path("outputs") / "reports" / "PHASE2_EXPERIMENTAL_DEMO_EXECUTOR_ATTACHMENTS.md"
...
def attach_phase2_experimental_demo_executors(
    ...
    payload: dict[str, Any] = {
        ...
        "authority": (
            "Experimental demo executor setup only. Attachments may place small orders only on a demo "
            "server after broker_action_allowed=true; this does not authorize live trading."
        ),
        "run_id": RUN_ID,
        "terminal": {
            ...
            "executor_log_backup_dir": str(executor_log_backup_dir) if executor_log_backup_dir else "none",
    ...
    return AttachOutput("EXECUTORS_ATTACHED_TO_DEMO_TERMINAL", output_json, output_md, len(attachments))


def _archive_existing_executor_logs(terminal_data_dir: Path) -> Path | None:
    files_dir = terminal_data_dir / "MQL5" / "Files"
    candidates = [
        *files_dir.glob("experimental_demo_executor_signal_log*.csv"),
        *files_dir.glob("experimental_demo_executor_startup*.csv"),
        *files_dir.glob("experimental_demo_executor_order_log*.csv"),
    ]
    existing = [path for path in candidates if path.exists()]
    if not existing:
        return None
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    backup_dir = terminal_data_dir / "_codex_quarantine" / "executor_logs" / f"experimental_demo_executor_logs_{stamp}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Attach experimental demo executors to the standard MT5 demo terminal.")
