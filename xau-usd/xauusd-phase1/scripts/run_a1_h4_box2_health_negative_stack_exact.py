...
PREREG = PHASE1_ROOT / "docs" / "A1_XAU_H4_BOX2_HEALTH_NEGATIVE_STACK_EXACT_PREREG_2026_07_08.md"
OUTPUT_STEM = "A1_XAU_H4_BOX2_HEALTH_NEGATIVE_STACK_EXACT_202207_202606"
TAG = "OWNER_GOAL_H4_BOX2_HEALTH_NEGATIVE_STACK_EXACT_202207_202606"
...
def build_variant() -> tuple[a1.Variant, dict[str, Any]]:
    ...
    variant = a1.Variant(
        name="prevhealth_box2_negative_stack_guard",
        label="H4/D1 box2 supportive + previous-month health gate + negative-stack guard",
        run_id="BT_A1_XAU_H4_BOX2_HEALTH_NEGATIVE_STACK",
        tester_inputs=inputs,
    )
    metadata = {
        "probe": "prevhealth_negative_stack_guard",
        "component_key": "box2",
...
def decide(row: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    if core_ok and risk_ok:
        return "H4_BOX2_NEGATIVE_STACK_REVIEW_CANDIDATE"
    if core_ok and row["worst_week"] > baseline["worst_week"]:
        return "H4_BOX2_NEGATIVE_STACK_WATCHLIST_PARTIAL_TAIL_REPAIR"
    if core_ok:
        return "H4_BOX2_NEGATIVE_STACK_CORE_ONLY"
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Negative-stack blocks: `{reasons.get('h4_d1_negative_stack_guard', 0)}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact-MT5 H4 box2 previous-month health gate with negative-stack guard.")
    ...
    result = evaluate("prevhealth_box2_negative_stack_broad_quarantined", kept, dropped, baseline)

    write_signal_csv(kept_csv, result["kept_rows"])
    write_signal_csv(dropped_csv, dropped)
    write_results(results_csv, [baseline, result])

    if result["decision"] == "H4_BOX2_NEGATIVE_STACK_REVIEW_CANDIDATE":
        status = result["decision"]
        interpretation = "The negative-stack guard repaired the weekly tail without breaking the monthly/core candidate. Keep research-only and send for review."
    elif result["decision"] == "H4_BOX2_NEGATIVE_STACK_WATCHLIST_PARTIAL_TAIL_REPAIR":
        status = result["decision"]
        interpretation = "The negative-stack guard partially improved the weekly tail but did not pass the full preregistered review gate. Keep watchlist-only."
    elif result["decision"] == "H4_BOX2_NEGATIVE_STACK_CORE_ONLY":
        status = result["decision"]
        interpretation = "The negative-stack guard preserved the core but did not repair the weekly tail. The weekly damage is not sufficiently caused by underwater stacking."
    else:
        status = "NO_H4_BOX2_NEGATIVE_STACK_SURVIVOR"
        interpretation = "The negative-stack guard broke the core candidate. Do not promote."
