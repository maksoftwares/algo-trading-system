...
PREREG = PHASE1_ROOT / "docs" / "A1_XAU_H4_BOX2_HEALTH_DAILY_STACK_CAP_EXACT_PREREG_2026_07_08.md"
OUTPUT_STEM = "A1_XAU_H4_BOX2_HEALTH_DAILY_STACK_CAP_EXACT_202207_202606"
TAG = "OWNER_GOAL_H4_BOX2_HEALTH_DAILY_STACK_CAP_EXACT_202207_202606"
...
def build_variant() -> tuple[a1.Variant, dict[str, Any]]:
    ...
    variant = a1.Variant(
        name="prevhealth_box2_daily_stack_cap2",
        label="H4/D1 box2 supportive + previous-month health gate + max 2 entries per day",
        run_id="BT_A1_XAU_H4_BOX2_HEALTH_DAILY_STACK_CAP2",
        tester_inputs=inputs,
    )
    metadata = {
        "probe": "prevhealth_daily_stack_cap2",
        "component_key": "box2",
...
def decide(row: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    if core_ok and risk_ok:
        return "H4_BOX2_DAILY_STACK_CAP_REVIEW_CANDIDATE"
    if core_ok and row["worst_week"] > baseline["worst_week"]:
        return "H4_BOX2_DAILY_STACK_CAP_WATCHLIST_PARTIAL_TAIL_REPAIR"
    if core_ok:
        return "H4_BOX2_DAILY_STACK_CAP_CORE_ONLY"
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Daily max-trade blocks: `{reasons.get('daily_trade_cap_reached', 0)}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact-MT5 H4 box2 previous-month health gate with daily stack cap.")
    ...
    result = evaluate("prevhealth_box2_daily_stack_cap_broad_quarantined", kept, dropped, baseline)

    write_signal_csv(kept_csv, result["kept_rows"])
    write_signal_csv(dropped_csv, dropped)
    write_results(results_csv, [baseline, result])

    if result["decision"] == "H4_BOX2_DAILY_STACK_CAP_REVIEW_CANDIDATE":
        status = result["decision"]
        interpretation = "The daily stack cap repaired the weekly tail without breaking the monthly/core candidate. Keep research-only and send for review."
    elif result["decision"] == "H4_BOX2_DAILY_STACK_CAP_WATCHLIST_PARTIAL_TAIL_REPAIR":
        status = result["decision"]
        interpretation = "The daily stack cap partially improved the weekly tail but did not pass the full preregistered review gate. Keep watchlist-only."
    elif result["decision"] == "H4_BOX2_DAILY_STACK_CAP_CORE_ONLY":
        status = result["decision"]
        interpretation = "The daily stack cap preserved the core but did not repair the weekly tail. The weekly damage is not primarily a same-day burst issue."
    else:
        status = "NO_H4_BOX2_DAILY_STACK_CAP_SURVIVOR"
        interpretation = "The daily stack cap broke the core candidate. Do not promote."
