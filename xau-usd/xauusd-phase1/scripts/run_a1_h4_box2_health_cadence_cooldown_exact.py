...
PREREG = PHASE1_ROOT / "docs" / "A1_XAU_H4_BOX2_HEALTH_CADENCE_COOLDOWN_EXACT_PREREG_2026_07_08.md"
OUTPUT_STEM = "A1_XAU_H4_BOX2_HEALTH_CADENCE_COOLDOWN_EXACT_202207_202606"
TAG = "OWNER_GOAL_H4_BOX2_HEALTH_CADENCE_COOLDOWN_EXACT_202207_202606"
...
def build_variant() -> tuple[a1.Variant, dict[str, Any]]:
    ...
    variant = a1.Variant(
        name="prevhealth_box2_cadence_cooldown_480",
        label="H4/D1 box2 supportive + previous-month health gate + 480-minute cooldown",
        run_id="BT_A1_XAU_H4_BOX2_HEALTH_CADENCE_COOLDOWN_480",
        tester_inputs=inputs,
    )
    metadata = {
        "probe": "prevhealth_cadence_cooldown_480",
        "component_key": "box2",
...
def status_for(decision: str) -> tuple[str, str]:
    if decision == "H4_BOX2_DAILY_STACK_CAP_REVIEW_CANDIDATE":
        return (
            "H4_BOX2_CADENCE_COOLDOWN_REVIEW_CANDIDATE",
            "The cadence cooldown repaired the weekly tail without breaking the monthly/core candidate. Keep research-only and send for review.",
        )
    if decision == "H4_BOX2_DAILY_STACK_CAP_WATCHLIST_PARTIAL_TAIL_REPAIR":
        return (
            "H4_BOX2_CADENCE_COOLDOWN_WATCHLIST_PARTIAL_TAIL_REPAIR",
            "The cadence cooldown partially improved the weekly tail but did not pass the full preregistered review gate. Keep watchlist-only.",
        )
    if decision == "H4_BOX2_DAILY_STACK_CAP_CORE_ONLY":
        return (
            "H4_BOX2_CADENCE_COOLDOWN_CORE_ONLY",
            "The cadence cooldown preserved the core but did not repair the weekly tail. Simple H4 spacing is not enough.",
        )
    return "NO_H4_BOX2_CADENCE_COOLDOWN_SURVIVOR", "The cadence cooldown broke the core candidate. Do not promote."
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Cooldown blocks: `{reasons.get('cooldown_active', 0)}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact-MT5 H4 box2 health gate with cadence cooldown.")
    ...
    result = evaluate("prevhealth_box2_cadence_cooldown_broad_quarantined", kept, dropped, baseline)
