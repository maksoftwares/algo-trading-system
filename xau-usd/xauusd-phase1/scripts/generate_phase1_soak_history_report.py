...
def _render_report(
    ...
    return "\n".join(
        [
            ...
            f"- Longest active streak: {latest.get('soak_longest_streak_hours', 'n/a') if latest else 'n/a'}h",
            f"- Current active streak: {latest.get('soak_current_streak_hours', 'n/a') if latest else 'n/a'}h",
            f"- Weekend policy: {latest.get('soak_weekend_policy', 'n/a') if latest else 'n/a'}",
            f"- Process uptime streak: {latest.get('soak_process_uptime_streak_hours', 'n/a') if latest else 'n/a'}h",
            f"- Code-freeze hours: {latest.get('soak_code_freeze_hours', 'n/a') if latest else 'n/a'}h",
                ...
                [
                    {
                        ...
                        "Longest h": row.get("soak_longest_streak_hours", ""),
                        "Freeze h": row.get("soak_code_freeze_hours", ""),
