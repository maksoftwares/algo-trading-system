...
def attach_r1_forward_research_demo(
    ...
    *,
    ...
    standard_include = _standard_include_source(root)
    ...
    deployed = _deploy(data_path, source, Path(packet["artifacts"]["preset"]), standard_include)
...
def _preflight_checks(
    ...
    return [
        ...
        _check(
            "standard_trade_include_exists",
            all((standard_include / relative).exists() for relative in ("Trade/Trade.mqh", "Object.mqh", "StdLibErr.mqh")),
            str(standard_include),
        ),
...
def _deploy(data_path: Path, source: Path, preset: Path, standard_include: Path) -> list[str]:
    expert_dir = data_path / "MQL5" / "Experts"
    preset_dir = data_path / "MQL5" / "Presets"
    target_include = data_path / "MQL5" / "Include"
    target_trade_include = target_include / "Trade"
    ...
    shutil.copytree(standard_include / "Trade", target_trade_include, dirs_exist_ok=True)
    shutil.copy2(standard_include / "Object.mqh", target_include / "Object.mqh")
    shutil.copy2(standard_include / "StdLibErr.mqh", target_include / "StdLibErr.mqh")
    return [
        str(target_source),
        str(target_preset),
        str(target_trade_include),
        str(target_include / "Object.mqh"),
        str(target_include / "StdLibErr.mqh"),
    ]


def _standard_include_source(root: Path) -> Path:
    ...
    return a1_data_path / "MQL5" / "Include"
