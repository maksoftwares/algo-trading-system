...
OUTPUT_STEM = "A1_XAU_M5_MOMENTUM_FEATURE_BAND_RESIDUAL_PLUS75_HIGH_NET_READINESS_2026_07_02"
...
EXPECTED_SHA = "de637fb4be82b0328ea98e8725936a1bf307810a28ab3dc58fcddfe932c4c39a"
EXPECTED_MAGICS = {
    "feature_band_residual_plus75_high_net_long": 932300,
    "feature_band_residual_plus75_high_net_v13_both": 932301,
}
...
def build_payload() -> dict[str, Any]:
    ...
    candidate = optimizer.get("named_candidates", {}).get("max_net", {})
    ...
    checks: list[dict[str, str]] = [
        ...
        check(
            "candidate_is_plus75_high_net",
            candidate.get("profit_target_usd") == 75.0
            and candidate.get("max_trades_per_day_guard") is None
        ...
        check(
            ...
            candidate.get("trades", 0) >= 2200
            and candidate.get("trades_per_active_day", 0) >= 3.5
        ...
        check(
            "candidate_has_higher_net_tradeoff_declared",
            candidate.get("net_usd", 0) >= 2300.0 and candidate.get("positive_day_pct", 100) < 62.0,
            (
                f"net={candidate.get('net_usd')}, positive_day_pct={candidate.get('positive_day_pct')}; "
                "this is intentionally higher-net and slightly lower positive-day-rate than +50/10m"
            ),
        ),
        check(
        ...
        checks.extend(
            [
                ...
                check(
                    ...
                    and config.get("portfolio_guard_magic_csv") == "932300,932301"
                    and config.get("portfolio_daily_profit_target_usd") == "75.00"
                    and config.get("portfolio_max_trades_per_day") == "0"
    ...
    return {
        ...
        "decision": "review_ready_not_attached_high_net_alternative",
        "tradeoff": (
            "Higher net and higher trade cadence than the +50/10m owner-target package, with a lower "
            "positive active-day rate and larger closed drawdown."
        ),
        "draft": rel(DRAFT),
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# A1 XAU M5 Momentum Feature-Band Residual +75 High-Net Readiness - 2026-07-02",
        "",
        ...
        "## Candidate",
        "",
        "High-net package: `+75 USD` portfolio target, no shared package max-trade cap, `10` minute package cooldown after any closed package loss, LONG hour `18` blocked, and SHORT close-to-recent-extreme band `-2.51..-0.92`.",
        "",
        f"Trade-off: {payload['tradeoff']}",
        "",
        "| Metric | Candidate |",
        "|---|---:|",
