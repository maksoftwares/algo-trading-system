...
PREREG = PHASE1_ROOT / "docs" / "A1_XAU_H4_D1_COMPRESSION_RAW_FREQUENCY_LADDER_PREREG_2026_07_05.md"
BASELINE_JSON = REPORTS / "A1_XAU_H4_D1_COMPRESSION_FREQUENCY_MECHANICS_202207_202606.json"
FROM_DATE = "2022.07.01"
TO_DATE = "2026.06.30"
TAG = "OWNER_GOAL_H4_D1_COMPRESSION_RAW_FREQUENCY_LADDER_202207_202606"
...
def variant(name: str, atr_cap: str, box_days: str, range_max: str, body_min: str) -> a1.Variant:
    return a1.Variant(
        name=name,
        label=(
            "D1 compression/H4 expansion raw-frequency ladder, "
            f"ATR cap {atr_cap}, box days {box_days}, range max {range_max}, body min {body_min}"
        ),
        run_id=f"BT_A1_XAU_H4_D1_COMP_RAW_FREQ_{name.upper()}_OWNER_GOAL",
        tester_inputs={
            "InpSignalMode": "7",
            "InpDirectionMode": "0",
...
VARIANTS = [
    variant("d1_comp_max16_atr45", "45.00", "5", "1.00", "0.50"),
    variant("d1_comp_max16_atr60", "60.00", "5", "1.00", "0.50"),
    variant("d1_comp_max16_box3_atr45", "45.00", "3", "1.00", "0.50"),
    variant("d1_comp_max16_range125_atr45", "45.00", "5", "1.25", "0.50"),
    variant("d1_comp_max16_body035_atr45", "45.00", "5", "1.00", "0.35"),
    variant("d1_comp_max16_broad_box3_atr60_range125_body035", "60.00", "3", "1.25", "0.35"),
]
...
def load_baseline_context() -> dict[str, Any]:
    ...
    baseline = next(
        item for item in payload["variants"] if item["name"] == "d1_compression_h4_expansion_rr2p0_max16"
    )
...
def enrich_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["scope"]["family"] = "A1 H4 D1 compression raw-frequency ladder"
    payload["scope"]["period"] = f"{FROM_DATE} -> {TO_DATE}"
    payload["scope"]["anti_overfit_boundary"] = (
        "Six preregistered frequency-expansion cells only; no optimizer, no post-result threshold search."
    )
...
def choose_winner(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if core_hits:
        ...
        return {"status": "CORE_SHAPE_SURVIVES_RAW_FREQUENCY_GAP", "best_variant": best["name"]}
    near = [
        result
    ...
    if near:
        ...
        return {"status": "NEAR_MISS_RAW_FREQUENCY_LADDER", "best_variant": best["name"]}
    best = max(results, key=lambda item: item["owner_goal_metrics"]["manual_pnl"]) if results else None
    return {"status": "REJECT_RAW_FREQUENCY_LADDER_BREAKS_CORE_SHAPE", "best_variant": best["name"] if best else ""}
...
def render(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# A1 XAU H4 D1 Compression Raw-Frequency Ladder Exact Probe",
        "",
    ...
    lines.extend(["", "## Ledger Counts", "", "### `baseline_max16_prior_exact`", ""])
    ...
    if payload["status"] == "CORE_SHAPE_SURVIVES_RAW_FREQUENCY_GAP":
        lines.append(
            "At least one frequency-expanded row preserved the core WR/W-L shape, but active-day frequency still misses the owner goal. Treat it as a component clue unless a later independent source closes frequency."
        )
    ...
    elif payload["status"] == "NEAR_MISS_RAW_FREQUENCY_LADDER":
        lines.append(
            "The ladder produced only a near miss. Do not spend the reviewer token on this result."
        )
    else:
        lines.append(
            "The raw-frequency expansion did not preserve WR >= 50% and W/L >= 2.0. Reject this expansion branch for owner-goal use."
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact MT5 D1 compression/H4 expansion raw-frequency ladder.")
    ...
    report_md = REPORTS / "A1_XAU_H4_D1_COMPRESSION_RAW_FREQUENCY_LADDER_202207_202606.md"
