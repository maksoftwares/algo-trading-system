...
PREREG = PHASE1_ROOT / "docs" / "A1_XAU_H4_D1_LONG_ONLY_FREQUENCY_STRESS_PREREG_2026_07_05.md"
BASELINE_JSON = REPORTS / "A1_XAU_H4_D1_COMPRESSION_RAW_FREQUENCY_LADDER_202207_202606.json"
FROM_DATE = "2022.07.01"
TO_DATE = "2026.06.30"
TAG = "OWNER_GOAL_H4_D1_LONG_ONLY_FREQUENCY_STRESS_202207_202606"
...
def variant(name: str, atr_cap: str, box_days: str, range_max: str, body_min: str) -> a1.Variant:
    return a1.Variant(
        name=name,
        label=(
            "D1/H4 long-only breakout frequency stress, "
            f"ATR cap {atr_cap}, box days {box_days}, range max {range_max}, body min {body_min}"
        ),
        run_id=f"BT_A1_XAU_H4_D1_LONG_FREQ_{name.upper()}_OWNER_GOAL",
        tester_inputs={
            "InpSignalMode": "7",
            "InpDirectionMode": "1",
...
VARIANTS = [
    variant("long_broad_box3_atr60_range125_body035", "60.00", "3", "1.25", "0.35"),
    variant("long_box2_atr60_range125_body035", "60.00", "2", "1.25", "0.35"),
    variant("long_box2_atr80_range150_body035", "80.00", "2", "1.50", "0.35"),
    variant("long_box2_atr100_range200_body025", "100.00", "2", "2.00", "0.25"),
    variant("long_box2_atr100_norange_body010", "100.00", "2", "999.00", "0.10"),
    variant("long_box2_atr100_norange_body000", "100.00", "2", "999.00", "0.00"),
]
...
def load_prior_long_diagnostic() -> dict[str, Any]:
    payload = json.loads(BASELINE_JSON.read_text(encoding="utf-8"))
    broad = next(
        item for item in payload["variants"] if item["name"] == "d1_comp_max16_broad_box3_atr60_range125_body035"
    )
    rows = [row for row in read_trade_csv(Path(broad["trade_csv"])) if row.get("direction") == "LONG"]
    return {
        "source_variant": broad["name"],
        "source_trade_csv": broad["trade_csv"],
        "owner_goal_metrics": owner_metrics(rows, FROM_DATE, TO_DATE),
        "last12_owner_goal_metrics": last12_metrics(rows, TO_DATE),
    }
...
def enrich_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["prior_long_diagnostic"] = load_prior_long_diagnostic()
    ...
    payload["scope"]["family"] = "A1 H4 D1 long-only frequency stress"
    payload["scope"]["period"] = f"{FROM_DATE} -> {TO_DATE}"
    payload["scope"]["anti_overfit_boundary"] = (
        "Post-diagnostic long-only frequency stress; six preregistered cells only, no optimizer."
    )
    payload["scope"]["review_spend_rule"] = (
        "Do not spend reviewer unless active-day frequency materially improves while WR >= 50% and W/L >= 2.0 survive."
    )
...
def choose_winner(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if core_hits:
        ...
        return {"status": "LONG_ONLY_CORE_SHAPE_FREQUENCY_GAP", "best_variant": best["name"]}
    near = [
        result
    ...
    if near:
        ...
        return {"status": "LONG_ONLY_NEAR_MISS_FREQUENCY_STRESS", "best_variant": best["name"]}
    best = max(results, key=lambda item: item["owner_goal_metrics"]["manual_pnl"]) if results else None
    return {"status": "REJECT_LONG_ONLY_FREQUENCY_STRESS_BREAKS_CORE_SHAPE", "best_variant": best["name"] if best else ""}
...
def render(payload: dict[str, Any]) -> str:
    prior = payload["prior_long_diagnostic"]
    prior_metrics = prior["owner_goal_metrics"]
    prior_last12 = prior["last12_owner_goal_metrics"]
    lines = [
        "# A1 XAU H4 D1 Long-Only Frequency Stress Exact Probe",
        "",
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |",
        render_metric_row(
            "prior_broad_exact_long_subset",
            prior_metrics,
            prior_last12,
            row_decision(prior_metrics),
        ),
    ]
    for result in payload["variants"]:
    ...
    if payload["status"] == "LONG_ONLY_CORE_SHAPE_FREQUENCY_GAP":
        lines.append(
            "A long-only row preserved the core WR/W-L shape, but active-day frequency still misses the owner goal. Treat this as a post-diagnostic component clue, not a demo candidate."
        )
    elif payload["status"] == "OWNER_GOAL_HIT_REVIEW_REQUIRED":
        lines.append(
            "An exact MT5 row reached the owner goal. Freeze artifacts and prepare a reviewer package before any demo specification."
        )
    elif payload["status"] == "LONG_ONLY_NEAR_MISS_FREQUENCY_STRESS":
        lines.append(
            "The long-only stress produced only a near miss. Do not spend the reviewer token on this result."
        )
    else:
        lines.append(
            "The long-only frequency stress did not preserve WR >= 50% and W/L >= 2.0. Stop this branch."
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact MT5 D1/H4 long-only frequency stress.")
    ...
    report_md = REPORTS / "A1_XAU_H4_D1_LONG_ONLY_FREQUENCY_STRESS_202207_202606.md"
