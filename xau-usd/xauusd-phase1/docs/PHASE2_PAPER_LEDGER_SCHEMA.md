...
- The breakout-retest family is currently `COST_SUSPENDED_CANONICAL`; `breakout_retest` and same-family variants remain execution-blocked until future reviewed evidence repairs measured-cost eligibility. `swing_breakout_retest_v0` remains observer-only telemetry and inherits the family-level cost state.
