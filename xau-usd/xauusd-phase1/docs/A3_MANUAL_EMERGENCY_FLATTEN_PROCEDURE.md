...
3. Create `A3_EXECUTION_KILL.txt` in the terminal `MQL5/Files` folder with the text `KILL` to block new broker actions while preserving telemetry.
4. Create `A3_FULL_STOP.txt` only when the A3 EAs must refuse init entirely.
5. Use the MT5 terminal Trade tab to close A3 positions manually if the owner decides flattening is required.
6. Record ticket IDs, close times, reason, screenshots if available, and account equity before/after.
7. Do not edit EA source, committed presets, or locked hypotheses during the active window.
8. Regenerate `A3_RUNTIME_RECONCILIATION.md` and the next daily guard-attribution report.
