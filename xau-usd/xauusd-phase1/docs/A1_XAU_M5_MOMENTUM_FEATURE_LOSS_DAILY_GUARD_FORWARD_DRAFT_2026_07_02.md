# A1 XAU M5 Momentum Feature-Loss Daily Guard Forward Draft - 2026-07-02

Status: FREQUENCY_FIRST_REVIEW_CANDIDATE_NOT_ATTACHED

Boundary: demo-only forward-test draft. No canonical Phase 2 approval, no live trading, no real capital, and no MT5 runtime attachment is authorized by this document alone.

## Purpose

The owner clarified that sparse strategies do not satisfy the project goal. The primary target is an active XAUUSD M5 intraday engine that can produce multiple trades on active days while keeping win rate above 50%, positive PF/net, and better day-to-day reliability.

This draft freezes the best current frequency-first diagnostic from the feature-loss daily guard optimizer. It combines one long weak-hour lane with one feature-filtered V13 lane and applies a shared package-level daily guard.

## Candidate

Base portfolio:

```text
freq_h1_h4_long_rr0p7_cost005_block_weak_hours_v1
+
v13_feature_loss_short_extreme_rr0p6
```

Feature-loss rule:

```text
Block SHORT entries when close_to_recent_extreme >= -0.75.
```

Shared daily guard:

```text
portfolio max trades per day: 6
portfolio daily loss stop: -20 USD
portfolio profit target: none
max loss count rule: none
```

## Evidence

Source reports:

- `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_M5_MOMENTUM_FEATURE_LOSS_DAILY_GUARD_OPTIMIZER_2026_07_02.md`
- `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_M5_MOMENTUM_FEATURE_LOSS_PORTFOLIO_VERDICT_2026_07_02.md`
- `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_M5_MOMENTUM_VARIANT_BACKTEST_FOUR_YEAR_FEATURE_LOSS_THRESHOLD_SWEEP_20260702.md`

| Metric | Optimized feature-loss daily guard |
|---|---:|
| Trades | 1995 |
| Win rate | 66.17% |
| Net USD | +1440.33 |
| Profit factor | 1.34 |
| Active days | 611 |
| Trades / active day | 3.27 |
| 3+ trade active days | 53.68% |
| Positive active days | 57.28% |
| Median active-day PnL | +2.10 USD |
| Positive / negative months | 39 / 9 |
| Worst month | -24.09 USD |
| Top 100 winners removed | +399.07 USD |
| Max closed drawdown | 90.65 USD |
| Older split net / PF | +328.37 / 1.23 |
| Newer split net / PF | +1111.96 / 1.40 |

Interpretation: this is the best current frequency-first diagnostic because it preserves more than 3 trades per active day, keeps win rate above 60%, improves month stability versus the older daily guard, and stays positive after top-winner removal. It is still selected from historical simulation and must be forward-tested before promotion.

## Lane 1 - Long Weak-Hours Block With Optimized Portfolio Guard

Magic: 932280

Order comment: `A1_XAU_M5_MOM_FG_L`

Run id: `A1_XAU_M5_MOMENTUM_FEATURE_GUARD_LONG_WEAK_HOURS_20260702`

Inputs:

| Input | Value |
|---|---|
| Symbol | XAUUSD |
| Timeframe | M5 |
| Direction | LONG only |
| Signal mode | Break-and-run default |
| H1 trend filter | true |
| H4 trend filter | true |
| Risk reward | 0.70 |
| Max estimated cost R | 0.05 |
| Blocked server hours | 2,9,10,11,12,17,22,23 |
| Individual max trades per day | 12 |
| Cooldown minutes | 5 |
| Feature-loss filter | false |
| Portfolio daily guard | true |
| Portfolio guard magic CSV | 932280,932281 |
| Portfolio max trades/day | 6 |
| Portfolio daily loss stop USD | 20.00 |

## Lane 2 - V13 Feature-Loss Both-Direction Lane With Optimized Portfolio Guard

Magic: 932281

Order comment: `A1_XAU_M5_MOM_FG_B`

Run id: `A1_XAU_M5_MOMENTUM_FEATURE_GUARD_V13_BOTH_20260702`

Inputs:

| Input | Value |
|---|---|
| Symbol | XAUUSD |
| Timeframe | M5 |
| Direction | BOTH |
| Signal mode | M5 EMA trend continuation |
| H1 trend filter | true |
| H4 trend filter | true |
| Risk reward | 0.60 |
| Max estimated cost R | 0.05 |
| Blocked server hours | 0,2,4,9,10,11,12,16,19,20 |
| Blocked long server hours | 6,7,8 |
| Blocked short server hours | 13,14,15,17,18 |
| M5 EMA fast / slow | 8 / 21 |
| M5 slope bars | 3 |
| M5 min slope ATR | 0.03 |
| M5 max distance ATR | 1.20 |
| Min range ATR | 0.35 |
| Min body fraction | 0.30 |
| Long close location | 0.58 |
| Short close location | 0.42 |
| Min 3-bar move ATR | 0.10 |
| Feature-loss filter | true |
| Feature-loss shadow-only | false |
| Short close-to-recent-extreme block min | -0.75 |
| Individual max trades per day | 24 |
| Cooldown minutes | 0 |
| Portfolio daily guard | true |
| Portfolio guard magic CSV | 932280,932281 |
| Portfolio max trades/day | 6 |
| Portfolio daily loss stop USD | 20.00 |

## Forward-Test Rules

- Account: A1 demo account only unless owner explicitly approves another account.
- Lot: 0.01 fixed.
- Broker action: disabled until owner approval and reviewer signoff.
- No parameter changes during the forward test.
- Existing sparse RR2 lane should not be stacked with this package by default.
- Existing older daily-guard variants `932270/932271` should not run at the same time as this package unless explicitly approved as shadow-only.
- Daily and weekly reports must separate the two magics and also score the combined package.
- Duplicate scoring must use same-minute same-direction de-duplication.
- The daily scoreboard must track trade-level and active-day-level performance.
- This package must be judged as a portfolio; neither lane should be promoted alone from this draft.

## Promotion Bar

- At least 300 closed forward trades.
- At least 6 trading weeks.
- At least 3.0 trades per active day.
- At least 53% of active days have 3 or more trades.
- Portfolio WR >= 60%.
- Portfolio PF >= 1.25.
- Positive active-day rate >= 56%.
- Portfolio net positive after removing top 10 winners.
- No single day contributes more than 30% of net profit.
- No lane creates a persistent negative drag for two consecutive weeks.

## Kill Rules

- Portfolio net negative after 150 trades.
- Rolling 100-trade PF below 0.90.
- Closed drawdown exceeds 120 USD at 0.01 lot.
- Positive active-day rate below 50% after 150 trades.
- Shared portfolio guard does not block after 6 same-day package entries.
- Shared portfolio guard does not block after closed same-day package PnL <= -20 USD.
- Any safety/broker-action mismatch appears.
- Runtime identity differs from the magics/comments in this document.

## Current Decision

This package is `FREQUENCY_FIRST_REVIEW_CANDIDATE_NOT_ATTACHED`.

It is currently the best candidate matching the owner's stated objective: multiple intraday trades, win rate above 50%, positive PF/net, and improved day-to-day reliability. It still needs independent review, owner approval, and a frozen forward-test launch packet before demo runtime replacement.
