...
The impulse-veto research lane is shadow-only and offline: `scripts/generate_phase2_impulse_veto_shadow_report.py` reads `PHASE2_DEMO_ACTUAL_BROKER_TRADES.csv` plus exported M5 bars, then writes `PHASE2_IMPULSE_VETO_SHADOW_REPORT.md/json` and `PHASE2_IMPULSE_VETO_SHADOW_ROWS.csv`. Its dated hypothesis is `docs/FORWARD_WEEK_IMPULSE_VETO_HYPOTHESIS_2026_06_15.md`. It scores `impulse_alignment` thresholds for weak round/session families only; it does not modify running EAs, and it must not be applied to `breakout_retest` without a separate owner-approved packet.

Dynamic-exit deployment status is recorded in `docs/DYNAMIC_EXIT_DEPLOYMENT_VERDICT_2026_06_09.md`: Partial+BE and BE-only are rejected for broker-action deployment from the exact logged-path replay; ATR-trail remains blocked until M5 high/low plus ATR history is available and beats the plain 1.5R hold on net-R-after-measured-cost. Guard re-arm and de-dilution runtime work is prepared in `docs/EXPERIMENT_GUARD_REARM_AND_DEDILUTION_PLAN_2026_06_09.md` but requires owner runtime confirmation before any chart inputs are changed.
