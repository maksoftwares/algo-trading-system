...
void LogDealTransaction(const ulong deal_ticket)
  {
   ...
   if(!HistoryDealSelect(deal_ticket))
      return;
   const long deal_magic = (long)HistoryDealGetInteger(deal_ticket, DEAL_MAGIC);
...
double NormalizeHedgeVolume(const double requested)
  {
...
bool PrimaryLifetimeRealizedProfit(double &realized_profit)
  {
   realized_profit = 0.0;
...
void ManageClusterEquityHedge()
  {
   ...
   if(balance <= 0.0)
      return;
   const double primary_volume_step = SymbolInfoDouble(InpTargetSymbol, SYMBOL_VOLUME_STEP);
   ...
   if(hedge_volume > 0.0)
     {
      if(drawdown_pct <= InpClusterEquityHedgeReleasePct)
        {
   ...
   if(!g_cluster_hedge_rearm_ready)
      return;
   if(drawdown_pct >= InpClusterEquityHedgeTriggerPct)
     {
...
int OnInit()
  {
   ...
   if(InpClusterEquityHedgeEnabled && AccountInfoInteger(ACCOUNT_MARGIN_MODE) != ACCOUNT_MARGIN_MODE_RETAIL_HEDGING)
     {
...
void OnTradeTransaction(
...
)
  {
   ...
   if(!HistoryDealSelect(trans.deal))
      return;
   const long transaction_magic = (long)HistoryDealGetInteger(trans.deal, DEAL_MAGIC);
