...
OUTPUT_PATH = ROOT / "data" / "reference" / "futures" / "hg_gc_daily_yahoo_2015_2025.csv"
SOURCE_URL = "https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"
SOURCE_SYMBOLS = {"HG=F": "hg", "GC=F": "gc"}
...
def main() -> int:
        ...
        for row in rows:
            ...
            target = by_date.setdefault(
                date,
                {
                    ...
                    "date_utc": date,
                    "source": "Yahoo Finance chart API; public non-primary HG=F/GC=F futures OHLCV proxy",
    ...
    rows = [
        ...
        and row["hg_close"] is not None
        and row["gc_close"] is not None
    ]
    rows.sort(key=lambda item: str(item["timestamp_utc"]))
    if len(rows) < 1800:
        raise RuntimeError(f"Expected at least 1800 usable merged HG/GC daily rows, got {len(rows)}.")
