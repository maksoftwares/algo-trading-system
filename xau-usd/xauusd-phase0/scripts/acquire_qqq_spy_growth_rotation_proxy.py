...
OUTPUT_PATH = ROOT / "data" / "reference" / "etf" / "qqq_spy_daily_yahoo_2015_2025.csv"
SOURCE_URL = "https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"
SOURCE_SYMBOLS = ("QQQ", "SPY")
...
def main() -> int:
        ...
        for row in rows:
            ...
            target = by_date.setdefault(
                date,
                {
                    ...
                    "date_utc": date,
                    "source": "Yahoo Finance chart API; public non-primary QQQ/SPY ETF OHLCV proxy",
    ...
    rows = [
        ...
        and row["qqq_close"] is not None
        and row["spy_close"] is not None
    ]
    rows.sort(key=lambda item: str(item["timestamp_utc"]))
    if len(rows) < 1800:
        raise RuntimeError(f"Expected at least 1800 usable merged QQQ/SPY daily rows, got {len(rows)}.")
