...
OUTPUT_PATH = ROOT / "data" / "reference" / "etf" / "tlt_uup_daily_yahoo_2015_2025.csv"
SOURCE_URL = "https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"
SOURCE_SYMBOLS = ("TLT", "UUP")
...
def main() -> int:
        ...
        for row in rows:
            ...
            target = by_date.setdefault(
                date,
                {
                    ...
                    "date_utc": date,
                    "source": "Yahoo Finance chart API; public non-primary TLT/UUP ETF OHLCV proxy",
    ...
    if len(rows) < 1800:
        raise RuntimeError(f"Expected at least 1800 usable merged TLT/UUP daily rows, got {len(rows)}.")
