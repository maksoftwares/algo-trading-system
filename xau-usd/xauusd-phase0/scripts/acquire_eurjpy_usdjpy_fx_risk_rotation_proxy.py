...
OUTPUT_PATH = ROOT / "data" / "reference" / "fx" / "eurjpy_usdjpy_daily_yahoo_2015_2025.csv"
...
def main() -> int:
        ...
        for row in rows:
            ...
            target = by_date.setdefault(
                date,
                {
                    ...
                    "date_utc": date,
                    "source": "Yahoo Finance chart API; public non-primary EURJPY/USDJPY daily FX proxy",
    ...
    rows = [
        ...
        and row["eurjpy_close"] is not None
        and row["usdjpy_close"] is not None
    ]
    rows.sort(key=lambda item: str(item["timestamp_utc"]))
    if len(rows) < 1800:
        raise RuntimeError(f"Expected at least 1800 usable merged EURJPY/USDJPY daily rows, got {len(rows)}.")
