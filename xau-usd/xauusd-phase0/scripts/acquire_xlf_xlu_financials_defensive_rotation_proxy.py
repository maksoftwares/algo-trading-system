...
OUTPUT_PATH = ROOT / "data" / "reference" / "etf" / "xlf_xlu_daily_yahoo_2015_2025.csv"
SOURCE_URL = "https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"
SOURCE_SYMBOLS = ("XLF", "XLU")
...
def main() -> int:
        ...
        for row in rows:
            ...
            target = by_date.setdefault(
                date,
                {
                    ...
                    "date_utc": date,
                    "source": "Yahoo Finance chart API; public non-primary XLF/XLU ETF OHLCV proxy",
    ...
    rows = [
        ...
        and row["xlf_close"] is not None
        and row["xlu_close"] is not None
    ]
    rows.sort(key=lambda item: str(item["timestamp_utc"]))
    if len(rows) < 1800:
        raise RuntimeError(f"Expected at least 1800 usable merged XLF/XLU daily rows, got {len(rows)}.")
