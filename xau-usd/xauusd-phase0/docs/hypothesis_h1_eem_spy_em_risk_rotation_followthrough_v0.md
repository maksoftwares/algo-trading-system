# H1 EEM/SPY EM Risk Rotation Follow-Through v0 Hypothesis

Hypothesis date: 2026-05-30
Hypothesis version: v0
Author / owner: maksoftwares / Codex
Mechanic family: H1 emerging-market risk rotation / XAU follow-through
...
Expected decisions per week: 0-14 during EM-risk rotation regimes
...
Expected R-multiple distribution: sparse-to-moderate events with small losses and clustered 1.50R winners when EEM/SPY risk rotation and XAU H1 confirmation align.
...
- `EEM`: emerging-market equity ETF / external-risk and USD-funding stress proxy.
- `SPY`: broad US equity proxy.

The EEM/SPY features are shifted by one completed daily observation before merging into XAU H1 decisions.

EM-risk rotation:

```text
eem_return_5d = log(EEM close / EEM close 5d ago)
spy_return_5d = log(SPY close / SPY close 5d ago)
em_risk_rotation_5d = eem_return_5d - spy_return_5d
abs(em_risk_rotation_5d) >= 0.0100
abs(em_risk_rotation_z126) >= 0.35
em_risk_rotation_abs_percentile252 >= 0.55
```

Long setup:

```text
EEM underperforms SPY, so em_risk_rotation_5d <= -0.0100
XAU close > EMA50
EMA21 >= EMA50
...
```text
EEM outperforms SPY, so em_risk_rotation_5d >= +0.0100
XAU close < EMA50
EMA21 <= EMA50
XAU H1 12-bar return <= -0.0015
XAU H1 6-bar return <= +0.0005
...
The strategy should capture XAU follow-through when emerging-market underperformance marks external-risk or USD-funding stress and XAU confirms higher, or when EM outperformance marks risk appetite and XAU confirms lower.

## Why This Hypothesis Should Exist

EEM/SPY EM-risk rotation is distinct from retest, round-number, FX-cross, crypto, GLD-flow, GDX/GLD miner-relative, credit-risk, rates-dollar, commodity-dollar, sector-defensive, precious-beta, QQQ/SPY growth rotation, IWM/SPY size rotation, and calendar candidates. It tests global risk transfer rather than level-break mechanics or a domestic US style/sector proxy.

## What Would Falsify It

Reject v0 if fewer than 7/9 matrix cells reach PF >= 1.30, if trade count is insufficient, if the effect is broker-specific, if concentration or activity gates fail, if cost sensitivity fails, or if EEM/SPY features are not shifted before XAU H1 decisions. Do not tune v0 after results are known.
