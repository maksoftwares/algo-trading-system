...
| H4 trend continuation after D1 pullback | D1 setup, H4 decision | 20-100 | 8-72h | >= 250 points | <= 0.30R | XAUUSD H4/D1 bars, EMA/ADX/ATR features | REJECTED_FIRST_PASS | `h4_d1_contraction_trend_continuation_v0` was SHA256-locked, implemented, smoke-tested, and real matrix-tested; 0/9 PF cells reached 1.30 and every cell had PF below 1.0. Do not tune v0. |
| H4/D1 volatility contraction then expansion | H4/D1 | 15-80 | 12-96h | >= 300 points | <= 0.25R | XAUUSD H4/D1 bars, range compression and ATR percentile features | REJECTED_FIRST_PASS | SHA256-locked, implemented, smoke-tested, and real matrix-tested; 0/9 PF cells reached 1.30 despite 75-165 trades per cell. Do not tune v0. |
...
`d1_compression_h4_expansion_v0` was already tested and rejected, so do not duplicate or tune it. `h4_d1_volatility_contraction_expansion_v0` and `h4_d1_contraction_trend_continuation_v0` both passed lower-cost structural prechecks and smoke, but both failed real 9-cell first pass with 0/9 PF cells >= 1.30. Next step: continue with a new versioned, pre-registered lower-cost independent candidate, preferably using a stronger data class than OHLC-only range/ATR/EMA structure.
