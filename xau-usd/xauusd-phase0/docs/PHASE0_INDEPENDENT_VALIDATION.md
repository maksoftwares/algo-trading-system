...
| D2 - White Reality Check / SPA-style bootstrap | PASS | Post-`round_number_retest_v0` rerun: `breakout_retest` remained the family winner against the 23 non-empty matrix-ledger candidates; White Reality Check p-value 0.0200 and max pairwise SPA p-value 0.0326. |
...
| Non-empty candidate universe | 23 |
...
That audit found 22 rejected/research candidates, 4 with sample-size failures, 21 with multi-cell expectancy failures, 0 frequency-only failures, and one non-matrix pending candidate: `round_number_retest_v0`.
