...
def load_spy_tlt_risk_rotation_context(
    config: ProjectConfig,
    required_start: object,
    required_end: object,
) -> pd.DataFrame:
    path = config.root / SPY_TLT_RISK_ROTATION_REFERENCE_PATH
    if not path.exists():
        raise ConfigError(
            f"{EXPERT_NAME} requires {path}. Run scripts/acquire_spy_tlt_risk_rotation_proxy.py "
            "before any real matrix run. This is a public SPY/TLT ETF daily OHLCV proxy, "
            "not primary equity/rates futures or order-flow data."
        )
    try:
        frame = pd.read_csv(path)
    except Exception as exc:
        raise ConfigError(f"Failed to read {EXPERT_NAME} SPY/TLT proxy file {path}: {exc}") from exc
    ...
    if missing:
        raise ConfigError(f"{path} missing required SPY/TLT column(s): {', '.join(sorted(missing))}.")
    ...
    if frame.empty:
        raise ConfigError(f"{path} has no usable SPY/TLT rows.")
...
def _assert_coverage(
    ...
    if coverage_start > needed_start and coverage_start - needed_start > allowed_start_gap:
        raise ConfigError(
            f"{EXPERT_NAME} SPY/TLT data in {source} start {coverage_start.isoformat()}, "
            f"but required {needed_start.isoformat()}."
        )
    if coverage_end < needed_end and needed_end - coverage_end > allowed_end_gap:
        raise ConfigError(
            f"{EXPERT_NAME} SPY/TLT data in {source} end {coverage_end.isoformat()}, "
