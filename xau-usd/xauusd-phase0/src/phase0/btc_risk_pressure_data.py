...
def load_btc_risk_pressure_context(
    config: ProjectConfig,
    required_start: object,
    required_end: object,
) -> pd.DataFrame:
    path = config.root / BTC_RISK_PRESSURE_REFERENCE_PATH
    if not path.exists():
        raise ConfigError(
            f"{EXPERT_NAME} requires {path}. Run scripts/acquire_btc_risk_pressure_proxy.py "
            "before any real matrix run. This is a public BTC-USD daily OHLCV proxy, "
            "not primary crypto exchange order-flow data."
        )
    try:
        frame = pd.read_csv(path)
    except Exception as exc:
        raise ConfigError(f"Failed to read {EXPERT_NAME} BTC proxy file {path}: {exc}") from exc
    ...
    if missing:
        raise ConfigError(f"{path} missing required BTC column(s): {', '.join(sorted(missing))}.")
    ...
    if frame.empty:
        raise ConfigError(f"{path} has no usable BTC rows.")
...
def _assert_coverage(
    ...
    if coverage_start > needed_start and coverage_start - needed_start > allowed_start_gap:
        raise ConfigError(
            f"{EXPERT_NAME} BTC data in {source} start {coverage_start.isoformat()}, "
            f"but required {needed_start.isoformat()}."
        )
    if coverage_end < needed_end and needed_end - coverage_end > allowed_end_gap:
        raise ConfigError(
            f"{EXPERT_NAME} BTC data in {source} end {coverage_end.isoformat()}, "
