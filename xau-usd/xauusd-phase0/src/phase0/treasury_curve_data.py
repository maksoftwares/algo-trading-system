...
DGS10_FILE = "FRED_DGS10.csv"
T10Y2Y_FILE = "FRED_T10Y2Y.csv"


def load_treasury_curve_context(
    config: ProjectConfig,
    required_start: object,
    required_end: object,
) -> pd.DataFrame:
    directory = config.root / TREASURY_CURVE_RAW_DIR
    dgs2 = _load_fred_series(directory / DGS2_FILE, "DGS2", "dgs2")
    dgs10 = _load_fred_series(directory / DGS10_FILE, "DGS10", "dgs10")
    t10y2y = _load_fred_series(directory / T10Y2Y_FILE, "T10Y2Y", "treasury_10y2y")
    frame = (
        dgs2.merge(dgs10, on="timestamp_utc", how="outer")
        .merge(t10y2y, on="timestamp_utc", how="outer")
        .sort_values("timestamp_utc")
        .ffill()
        .dropna(subset=["dgs2", "dgs10", "treasury_10y2y"])
        .reset_index(drop=True)
    )
    _assert_treasury_curve_coverage(frame, directory, required_start, required_end)
    return frame
...
def _assert_treasury_curve_coverage(
    frame: pd.DataFrame,
    source: Path,
    required_start: object,
    required_end: object,
