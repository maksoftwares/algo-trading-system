...
from phase0.strategies.h1_credit_spread_shock_reversal_v0 import _credit_features_for_h1


class H1CreditSpreadShockFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 corporate credit-spread shock follow-through candidate."""

    name = "h1_credit_spread_shock_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(credit, pd.DataFrame):
            raise ConfigError(
                "h1_credit_spread_shock_followthrough_v0 requires "
                "data_context['credit_spread'] with shifted FRED BAA10Y/AAA10Y observations."
            )
        ...
        for position in range(100, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_CREDIT_SPREAD_SHOCK_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported credit shock follow-through direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid credit shock follow-through trade plan risk.")
