...
class H4BtcRiskPressureGoldReversalV1Strategy(StrategyBase):
    """Research-only broader H4 XAU reversal candidate after shifted BTC stress."""

    name = "h4_btc_risk_pressure_gold_reversal_v1"
    version = "0.1-research-disabled"

    risk_reward = 1.40
    pressure_threshold = 0.065
    pressure_z_threshold = 0.30
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h4_btc_risk_pressure_gold_reversal_v1 requires "
        ...
        for position in range(160, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_BTC_RISK_PRESSURE_GOLD_REVERSAL_V1_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported BTC H4 reversal v1 direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid BTC H4 reversal v1 trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        if (
            ...
            and h4_return_6 <= 0.0600
            and close < open_price
            and close_location <= 0.50
            and ema40_distance_atr <= 3.25
        ):
