...
from phase0.strategies.h1_tip_ief_real_yield_rotation_followthrough_v0 import _rotation_features_for_h1
from phase0.tip_ief_real_yield_rotation_data import TIP_IEF_REAL_YIELD_ROTATION_FRAME_KEY


class H4TipIefRealYieldRotationReversalV0Strategy(StrategyBase):
    """Research-only H4 XAU reversal candidate using TIP/IEF real-yield rotation proxies."""

    name = "h4_tip_ief_real_yield_rotation_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h4_tip_ief_real_yield_rotation_reversal_v0 requires "
        ...
        for position in range(180, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_TIP_IEF_REAL_YIELD_ROTATION_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported TIP/IEF H4 reversal direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid TIP/IEF H4 reversal trade plan risk.")
