...
class D1MultiDayExhaustionReversionV0Strategy(StrategyBase):
    """Disabled research strategy for the locked D1 multi-day exhaustion / H4 reversal v0 hypothesis."""

    name = "d1_multi_day_exhaustion_reversion_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if "range5_high" not in d1:
            d1["range5_high"] = pd.to_numeric(d1["high"], errors="coerce").rolling(
                5, min_periods=5
            ).max()
        if "range5_low" not in d1:
            d1["range5_low"] = pd.to_numeric(d1["low"], errors="coerce").rolling(5, min_periods=5).min()
        if "range5_close_position" not in d1:
            range5_width = (d1["range5_high"] - d1["range5_low"]).replace(0.0, pd.NA)
            d1["range5_close_position"] = (
                pd.to_numeric(d1["close"], errors="coerce") - d1["range5_low"]
            ) / range5_width
    ...
    def generate_signals(self, data_context: dict[str, Any]) -> list[Signal]:
        ...
        signals: list[Signal] = []
        used_extensions: set[tuple[int, str]] = set()

        for h4_position in range(30, len(h4)):
            row = h4.iloc[h4_position]
            timestamp = pd.Timestamp(row["timestamp_utc"])
            extension = self._d1_extension_at_timestamp(d1, timestamp)
            if extension is None:
                continue

            extension_key = (int(extension["d1_index"]), str(extension["direction"]))
            if extension_key in used_extensions:
                continue

            setup = self._setup_at_position(h4, h4_position, extension)
            if setup is None:
                continue

            used_extensions.add(extension_key)
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"D1_MULTI_DAY_EXHAUSTION_REVERSION_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported D1 multi-day exhaustion reversion direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid D1 multi-day exhaustion reversion v0 trade plan risk.")
    ...
    def _d1_extension_at_timestamp(self, d1: pd.DataFrame, timestamp: pd.Timestamp) -> dict[str, Any] | None:
        ...
        if hours_after_close <= 0 or hours_after_close > 72:
            return None
        ...
        range5_close_position = float(row["range5_close_position"])
        if d1_atr <= 0:
            return None

        if momentum5 >= 1.75 * d1_atr and up_closes5 >= 4 and range5_close_position >= 0.85:
            return {
                "direction": "SHORT",
                ...
                "d1_range5_close_position": range5_close_position,
            }

        if momentum5 <= -1.75 * d1_atr and down_closes5 >= 4 and range5_close_position <= 0.15:
            return {
                "direction": "LONG",
    ...
    def _setup_at_position(
        ...
        if candle_range < 0.75 * h4_atr or body_ratio < 0.25:
            return None

        direction = str(extension["direction"])
        if direction == "SHORT":
            if close < open_price and close_position <= 0.35 and upper_wick_ratio >= 0.15:
                return {
                    **extension,
                    "direction": "SHORT",
                    "h4_atr14": h4_atr,
        ...
        if direction == "LONG":
            if close > open_price and close_position >= 0.65 and lower_wick_ratio >= 0.15:
                return {
                    **extension,
                    "direction": "LONG",
                    "h4_atr14": h4_atr,
