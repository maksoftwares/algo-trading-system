...
from phase0.strategies.h1_broker_fx_usd_pressure_followthrough_v0 import (
    _proxy_frame,
    _setup_metadata,
    _usd_pressure_features_for_h1,
)


class H1BrokerFxUsdPressureConflictReversionV0Strategy(StrategyBase):
    """Research-only H1 XAU conflict-reversion candidate using broker-consistent FX pressure."""

    name = "h1_broker_fx_usd_pressure_conflict_reversion_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(proxy, dict):
            raise ConfigError(
                "h1_broker_fx_usd_pressure_conflict_reversion_v0 requires "
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_BROKER_FX_USD_PRESSURE_CONFLICT_REVERSION_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported broker FX USD-pressure conflict direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid broker FX USD-pressure conflict-reversion trade plan risk.")
