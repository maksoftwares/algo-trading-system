...
class H1FxeUupEuroDollarFxRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using FXE/UUP euro-dollar FX rotation."""

    name = "h1_fxe_uup_euro_dollar_fx_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(FXE_UUP_EURO_DOLLAR_FX_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_fxe_uup_euro_dollar_fx_rotation_followthrough_v0 requires "
                "data_context['fxe_uup_euro_dollar_fx_rotation'] with shifted FXE/UUP daily observations."
            )
        ...
        rotation_features = _euro_dollar_fx_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_FXE_UUP_EURO_DOLLAR_FX_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported FXE/UUP euro-dollar FX rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid FXE/UUP euro-dollar FX rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["euro_dollar_fx_rotation_5d"],
            row["euro_dollar_fx_rotation_z126"],
            row["euro_dollar_fx_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["euro_dollar_fx_rotation_5d"])
        rotation_z = float(row["euro_dollar_fx_rotation_z126"])
        rotation_percentile = float(row["euro_dollar_fx_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _euro_dollar_fx_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "fxe_close", "fxe_volume", "uup_close", "uup_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("fxe_close", "fxe_volume", "uup_close", "uup_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "fxe_close", "uup_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["fxe_return_5d"] = np.log(frame["fxe_close"] / frame["fxe_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["euro_dollar_fx_rotation_5d"] = frame["fxe_return_5d"] - frame["uup_return_5d"]
    frame["euro_dollar_fx_rotation_z126"] = _rolling_zscore(frame["euro_dollar_fx_rotation_5d"], 126)
    frame["euro_dollar_fx_rotation_abs_percentile252"] = _rolling_percentile(
        frame["euro_dollar_fx_rotation_5d"].abs(),
        252,
    )
    frame["fxe_volume_z126"] = _rolling_zscore(np.log(frame["fxe_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "fxe_return_5d",
        "uup_return_5d",
        "euro_dollar_fx_rotation_5d",
        "euro_dollar_fx_rotation_z126",
        "euro_dollar_fx_rotation_abs_percentile252",
        "fxe_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "fxe_close": float(row["fxe_close"]),
        "uup_close": float(row["uup_close"]),
        "fxe_return_5d": float(row["fxe_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "euro_dollar_fx_rotation_5d": float(row["euro_dollar_fx_rotation_5d"]),
        "euro_dollar_fx_rotation_z126": float(row["euro_dollar_fx_rotation_z126"]),
        "euro_dollar_fx_rotation_abs_percentile252": float(row["euro_dollar_fx_rotation_abs_percentile252"]),
        "fxe_volume_z126": float(row["fxe_volume_z126"]),
