...
class M15TwoBarExhaustionReversalV0Strategy(StrategyBase):
    """Research-only M15 two-bar exhaustion reversal candidate."""

    name = "m15_two_bar_exhaustion_reversal_v0"
        ...
        for position in range(30, len(m15)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"M15_TWO_BAR_EXHAUSTION_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported M15 exhaustion direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid M15 two-bar exhaustion trade plan risk.")
