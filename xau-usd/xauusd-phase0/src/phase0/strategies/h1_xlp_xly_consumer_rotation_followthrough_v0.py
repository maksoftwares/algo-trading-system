...
class H1XlpXlyConsumerRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XLP/XLY consumer rotation."""

    name = "h1_xlp_xly_consumer_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XLP_XLY_CONSUMER_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xlp_xly_consumer_rotation_followthrough_v0 requires "
                "data_context['xlp_xly_consumer_rotation'] with shifted XLP/XLY daily observations."
            )
        ...
        rotation_features = _consumer_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XLP_XLY_CONSUMER_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLP/XLY consumer-rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLP/XLY consumer-rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["consumer_rotation_5d"],
            row["consumer_rotation_z126"],
            row["consumer_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["consumer_rotation_5d"])
        rotation_z = float(row["consumer_rotation_z126"])
        rotation_percentile = float(row["consumer_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _consumer_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xlp_close", "xlp_volume", "xly_close", "xly_volume"]].copy()
    ...
    frame["xlp_return_5d"] = np.log(frame["xlp_close"] / frame["xlp_close"].shift(5))
    frame["xly_return_5d"] = np.log(frame["xly_close"] / frame["xly_close"].shift(5))
    frame["consumer_rotation_5d"] = frame["xlp_return_5d"] - frame["xly_return_5d"]
    frame["consumer_rotation_z126"] = _rolling_zscore(frame["consumer_rotation_5d"], 126)
    frame["consumer_rotation_abs_percentile252"] = _rolling_percentile(
        frame["consumer_rotation_5d"].abs(),
        252,
    )
    frame["xlp_volume_z126"] = _rolling_zscore(np.log(frame["xlp_volume"].replace(0.0, np.nan)), 126)
    frame["xly_volume_z126"] = _rolling_zscore(np.log(frame["xly_volume"].replace(0.0, np.nan)), 126)
    feature_columns = [
        "xlp_close",
        "xlp_volume",
        "xly_close",
        ...
        "xlp_return_5d",
        "xly_return_5d",
        "consumer_rotation_5d",
        "consumer_rotation_z126",
        "consumer_rotation_abs_percentile252",
        "xlp_volume_z126",
        "xly_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xlp_close": float(row["xlp_close"]),
        "xly_close": float(row["xly_close"]),
        "xlp_return_5d": float(row["xlp_return_5d"]),
        "xly_return_5d": float(row["xly_return_5d"]),
        "consumer_rotation_5d": float(row["consumer_rotation_5d"]),
        "consumer_rotation_z126": float(row["consumer_rotation_z126"]),
        "consumer_rotation_abs_percentile252": float(
            row["consumer_rotation_abs_percentile252"]
        ),
        "xlp_volume_z126": float(row["xlp_volume_z126"]),
        "xly_volume_z126": float(row["xly_volume_z126"]),
    }
