...
class H1FxaUupAussieDollarFxRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using FXA/UUP Aussie-dollar FX rotation."""

    name = "h1_fxa_uup_aussie_dollar_fx_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(FXA_UUP_AUSSIE_DOLLAR_FX_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_fxa_uup_aussie_dollar_fx_rotation_followthrough_v0 requires "
                "data_context['fxa_uup_aussie_dollar_fx_rotation'] with shifted FXA/UUP daily observations."
            )
        ...
        rotation_features = _aussie_dollar_fx_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"h1_fxa_uup_aussie_dollar_fx_rotation_followthrough_v0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported FXA/UUP Aussie-dollar FX rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid FXA/UUP Aussie-dollar FX rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["aussie_dollar_fx_rotation_5d"],
            row["aussie_dollar_fx_rotation_z126"],
            row["aussie_dollar_fx_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["aussie_dollar_fx_rotation_5d"])
        rotation_z = float(row["aussie_dollar_fx_rotation_z126"])
        rotation_percentile = float(row["aussie_dollar_fx_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _aussie_dollar_fx_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "fxa_close", "fxa_volume", "uup_close", "uup_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("fxa_close", "fxa_volume", "uup_close", "uup_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "fxa_close", "uup_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["fxa_return_5d"] = np.log(frame["fxa_close"] / frame["fxa_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["aussie_dollar_fx_rotation_5d"] = frame["fxa_return_5d"] - frame["uup_return_5d"]
    frame["aussie_dollar_fx_rotation_z126"] = _rolling_zscore(frame["aussie_dollar_fx_rotation_5d"], 126)
    frame["aussie_dollar_fx_rotation_abs_percentile252"] = _rolling_percentile(
        frame["aussie_dollar_fx_rotation_5d"].abs(),
        252,
    )
    frame["fxa_volume_z126"] = _rolling_zscore(np.log(frame["fxa_volume"].replace(0.0, np.nan)), 126)
    frame["uup_volume_z126"] = _rolling_zscore(np.log(frame["uup_volume"].replace(0.0, np.nan)), 126)
    feature_columns = [
        "fxa_close",
        "fxa_volume",
        "uup_close",
        ...
        "fxa_return_5d",
        "uup_return_5d",
        "aussie_dollar_fx_rotation_5d",
        "aussie_dollar_fx_rotation_z126",
        "aussie_dollar_fx_rotation_abs_percentile252",
        "fxa_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "fxa_close": float(row["fxa_close"]),
        "uup_close": float(row["uup_close"]),
        "fxa_return_5d": float(row["fxa_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "aussie_dollar_fx_rotation_5d": float(row["aussie_dollar_fx_rotation_5d"]),
        "aussie_dollar_fx_rotation_z126": float(row["aussie_dollar_fx_rotation_z126"]),
        "aussie_dollar_fx_rotation_abs_percentile252": float(row["aussie_dollar_fx_rotation_abs_percentile252"]),
        "fxa_volume_z126": float(row["fxa_volume_z126"]),
        "uup_volume_z126": float(row["uup_volume_z126"]),
    }
