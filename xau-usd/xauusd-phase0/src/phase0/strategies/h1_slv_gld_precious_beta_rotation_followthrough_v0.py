...
class H1SlvGldPreciousBetaRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using SLV/GLD precious-beta rotation."""

    name = "h1_slv_gld_precious_beta_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(SLV_GLD_PRECIOUS_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_slv_gld_precious_beta_rotation_followthrough_v0 requires "
                "data_context['slv_gld_precious_rotation'] with shifted SLV/GLD daily observations."
            )
        ...
        rotation_features = _precious_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_SLV_GLD_PRECIOUS_BETA_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported SLV/GLD precious-beta rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid SLV/GLD precious-beta rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["precious_rotation_5d"],
            row["precious_rotation_z126"],
            row["precious_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["precious_rotation_5d"])
        rotation_z = float(row["precious_rotation_z126"])
        rotation_percentile = float(row["precious_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _precious_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "slv_close", "slv_volume", "gld_close", "gld_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("slv_close", "slv_volume", "gld_close", "gld_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "slv_close", "gld_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["slv_return_5d"] = np.log(frame["slv_close"] / frame["slv_close"].shift(5))
    frame["gld_return_5d"] = np.log(frame["gld_close"] / frame["gld_close"].shift(5))
    frame["precious_rotation_5d"] = frame["slv_return_5d"] - frame["gld_return_5d"]
    frame["precious_rotation_z126"] = _rolling_zscore(frame["precious_rotation_5d"], 126)
    frame["precious_rotation_abs_percentile252"] = _rolling_percentile(
        frame["precious_rotation_5d"].abs(),
        252,
    )
    frame["slv_volume_z126"] = _rolling_zscore(np.log(frame["slv_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "slv_return_5d",
        "gld_return_5d",
        "precious_rotation_5d",
        "precious_rotation_z126",
        "precious_rotation_abs_percentile252",
        "slv_volume_z126",
        "gld_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "slv_close": float(row["slv_close"]),
        "gld_close": float(row["gld_close"]),
        "slv_return_5d": float(row["slv_return_5d"]),
        "gld_return_5d": float(row["gld_return_5d"]),
        "precious_rotation_5d": float(row["precious_rotation_5d"]),
        "precious_rotation_z126": float(row["precious_rotation_z126"]),
        "precious_rotation_abs_percentile252": float(row["precious_rotation_abs_percentile252"]),
        "slv_volume_z126": float(row["slv_volume_z126"]),
