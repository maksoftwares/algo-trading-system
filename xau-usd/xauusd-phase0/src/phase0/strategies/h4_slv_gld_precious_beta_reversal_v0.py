...
from phase0.strategies.h1_slv_gld_precious_beta_rotation_followthrough_v0 import (
    _precious_rotation_features_for_h1,
)


class H4SlvGldPreciousBetaReversalV0Strategy(StrategyBase):
    """Research-only H4 XAU reversal candidate using SLV/GLD precious-beta rotation."""

    name = "h4_slv_gld_precious_beta_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h4_slv_gld_precious_beta_reversal_v0 requires "
        ...
        rotation_features = _precious_rotation_features_for_h1(h4, rotation)
        h4 = pd.concat(
            [
        ...
        for position in range(180, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_SLV_GLD_PRECIOUS_BETA_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported H4 SLV/GLD precious-beta reversal direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid H4 SLV/GLD precious-beta reversal trade plan risk.")
