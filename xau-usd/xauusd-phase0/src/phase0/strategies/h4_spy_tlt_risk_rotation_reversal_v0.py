...
from phase0.strategies.h1_spy_tlt_risk_rotation_followthrough_v0 import _rotation_features_for_h1


class H4SpyTltRiskRotationReversalV0Strategy(StrategyBase):
    """Research-only H4 XAU reversal candidate using SPY/TLT risk-rotation proxies."""

    name = "h4_spy_tlt_risk_rotation_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h4_spy_tlt_risk_rotation_reversal_v0 requires "
        ...
        rotation_features = _rotation_features_for_h1(h4, rotation)
        h4 = pd.concat(
            [
        ...
        for position in range(180, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_SPY_TLT_RISK_ROTATION_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported SPY/TLT H4 reversal direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid SPY/TLT H4 reversal trade plan risk.")
