...
class H1GvzVixVolPremiumFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 gold-volatility-premium follow-through candidate."""

    name = "h1_gvz_vix_vol_premium_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(gvz, pd.DataFrame):
            raise ConfigError(
                "h1_gvz_vix_vol_premium_followthrough_v0 requires "
        ...
        for position in range(80, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_GVZ_VIX_VOL_PREMIUM_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported GVZ/VIX vol-premium follow-through direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid GVZ/VIX vol-premium follow-through trade plan risk.")
