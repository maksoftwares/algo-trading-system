...
class H4MacroMomentumConfluenceV2Strategy(H4MacroMomentumConfluenceV0Strategy):
    """Research-only strict-macro, broader-H4 confluence candidate."""

    name = "h4_macro_momentum_confluence_v2"
    ...
    def _setup_at_row(self, row: pd.Series) -> dict | None:
        ...
        if (
            ...
            and d1_return_5 >= 0.0
            and low <= h4_ema50 + 1.35 * h4_atr
            and close >= h4_ema50 - 0.25 * h4_atr
            and close > open_price
            and h4_return_3 >= -0.0005
            and h4_return_12 >= -0.0180
            and close_location >= 0.50
            and ema50_distance_atr <= 3.50
        ):
