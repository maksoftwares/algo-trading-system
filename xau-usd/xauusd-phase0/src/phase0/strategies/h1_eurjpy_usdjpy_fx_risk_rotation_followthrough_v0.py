...
class H1EurjpyUsdjpyFxRiskRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using EURJPY/USDJPY FX risk rotation."""

    name = "h1_eurjpy_usdjpy_fx_risk_rotation_followthrough_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    rotation_threshold = 0.0070
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(EURJPY_USDJPY_FX_RISK_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_eurjpy_usdjpy_fx_risk_rotation_followthrough_v0 requires "
                "data_context['eurjpy_usdjpy_fx_risk_rotation'] with shifted EURJPY/USDJPY daily observations."
            )
        ...
        rotation_features = _fx_risk_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_EURJPY_USDJPY_FX_RISK_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported EURJPY/USDJPY risk rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid EURJPY/USDJPY risk-rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["fx_risk_rotation_5d"],
            row["fx_risk_rotation_z126"],
            row["fx_risk_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["fx_risk_rotation_5d"])
        rotation_z = float(row["fx_risk_rotation_z126"])
        rotation_percentile = float(row["fx_risk_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _fx_risk_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "eurjpy_close", "usdjpy_close"]].copy()
    ...
    frame["eurjpy_return_5d"] = np.log(frame["eurjpy_close"] / frame["eurjpy_close"].shift(5))
    frame["usdjpy_return_5d"] = np.log(frame["usdjpy_close"] / frame["usdjpy_close"].shift(5))
    frame["fx_risk_rotation_5d"] = frame["eurjpy_return_5d"] - frame["usdjpy_return_5d"]
    frame["fx_risk_rotation_z126"] = _rolling_zscore(frame["fx_risk_rotation_5d"], 126)
    frame["fx_risk_rotation_abs_percentile252"] = _rolling_percentile(
        frame["fx_risk_rotation_5d"].abs(),
        252,
    )
    feature_columns = [
        "eurjpy_close",
        ...
        "usdjpy_return_5d",
        "fx_risk_rotation_5d",
        "fx_risk_rotation_z126",
        "fx_risk_rotation_abs_percentile252",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "eurjpy_close": float(row["eurjpy_close"]),
        "usdjpy_close": float(row["usdjpy_close"]),
        "eurjpy_return_5d": float(row["eurjpy_return_5d"]),
        "usdjpy_return_5d": float(row["usdjpy_return_5d"]),
        "fx_risk_rotation_5d": float(row["fx_risk_rotation_5d"]),
        "fx_risk_rotation_z126": float(row["fx_risk_rotation_z126"]),
        "fx_risk_rotation_abs_percentile252": float(row["fx_risk_rotation_abs_percentile252"]),
    }
