...
class H1SpyTltRiskRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using SPY/TLT risk-rotation proxies."""

    name = "h1_spy_tlt_risk_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(SPY_TLT_RISK_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_spy_tlt_risk_rotation_followthrough_v0 requires "
                "data_context['spy_tlt_risk_rotation'] with shifted SPY/TLT daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_SPY_TLT_RISK_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported SPY/TLT risk-rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid SPY/TLT risk-rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["risk_rotation_5d"],
            row["risk_rotation_z126"],
            row["risk_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["risk_rotation_5d"])
        rotation_z = float(row["risk_rotation_z126"])
        rotation_percentile = float(row["risk_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "spy_close", "spy_volume", "tlt_close", "tlt_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("spy_close", "spy_volume", "tlt_close", "tlt_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "spy_close", "tlt_close"]).sort_values("timestamp_utc")
    ...
    frame["risk_rotation_5d"] = frame["tlt_return_5d"] - frame["spy_return_5d"]
    frame["risk_rotation_z126"] = _rolling_zscore(frame["risk_rotation_5d"], 126)
    frame["risk_rotation_abs_percentile252"] = _rolling_percentile(frame["risk_rotation_5d"].abs(), 252)
    ...
    feature_columns = [
        ...
        "risk_rotation_5d",
        "risk_rotation_z126",
        "risk_rotation_abs_percentile252",
        "spy_volume_z126",
        "tlt_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "risk_rotation_5d": float(row["risk_rotation_5d"]),
        "risk_rotation_z126": float(row["risk_rotation_z126"]),
        "risk_rotation_abs_percentile252": float(row["risk_rotation_abs_percentile252"]),
