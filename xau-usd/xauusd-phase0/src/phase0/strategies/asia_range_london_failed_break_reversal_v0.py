...
class AsiaRangeLondonFailedBreakReversalV0Strategy(StrategyBase):
    """Disabled research strategy for the locked Asia-range London failed-break reversal v0 hypothesis."""

    name = "asia_range_london_failed_break_reversal_v0"
        ...
        for m5_position in range(72, len(m5)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"ASIA_RANGE_LONDON_FAILED_BREAK_REVERSAL_V0_{direction}",
                    metadata={
                        **setup,
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "SHORT":
            ...
            raise ConfigError(f"Unsupported Asia-range failed-break reversal direction {signal.direction!r}.")
        if risk_price <= 0:
            raise ConfigError("Invalid Asia-range failed-break reversal v0 trade plan risk.")
        return TradePlan(
            expert=self.name,
    ...
    def _setup_at_position(
        ...
        if high >= asia_high + 0.25 * m5_atr and close <= asia_high - 0.10 * m5_atr:
        ...
        if low <= asia_low - 0.25 * m5_atr and close >= asia_low + 0.10 * m5_atr:
