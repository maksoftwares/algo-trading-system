...
from phase0.strategies.h1_real_yield_inflation_mix_reversal_v0 import (
    _inflation_features_for_h1,
    _macro_features_for_h1,
)


class H1RealYieldInflationMixFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 follow-through from real-yield and breakeven inflation mix shifts."""

    name = "h1_real_yield_inflation_mix_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(macro, pd.DataFrame):
            raise ConfigError(
                "h1_real_yield_inflation_mix_followthrough_v0 requires "
                "data_context['macro_proxy'] with FRED DFII10 and DTWEXBGS observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_REAL_YIELD_INFLATION_MIX_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(
                f"Unsupported H1 real-yield inflation follow-through direction {signal.direction!r}."
            )

        if risk_price <= 0:
            raise ConfigError("Invalid H1 real-yield inflation follow-through trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        if (
            gold_supportive
            and h1_return_24 >= 0.003
            and h1_return_8 >= 0.0015
            and close > open_price
            and close_location >= 0.58
            and ema50 < close <= ema50 * 1.025
        ):
