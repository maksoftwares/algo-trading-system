...
class H1BtcRiskPressureGoldFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using public BTC-USD risk pressure."""

    name = "h1_btc_risk_pressure_gold_followthrough_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    pressure_threshold = 0.0800
    pressure_z_threshold = 0.50
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h1_btc_risk_pressure_gold_followthrough_v0 requires "
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_BTC_RISK_PRESSURE_GOLD_FOLLOWTHROUGH_V0_{direction}",
