...
class H1GldFlowStressFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 GLD flow-stress follow-through candidate."""

    name = "h1_gld_flow_stress_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(gld_flow, pd.DataFrame):
            raise ConfigError(
                "h1_gld_flow_stress_followthrough_v0 requires data_context['gld_etf_flow'] "
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_GLD_FLOW_STRESS_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported GLD flow stress follow-through direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid GLD flow stress follow-through trade plan risk.")
