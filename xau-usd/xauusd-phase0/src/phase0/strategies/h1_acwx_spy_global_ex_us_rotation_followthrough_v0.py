...
class H1AcwxSpyGlobalExUsRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using ACWX/SPY global ex-US rotation."""

    name = "h1_acwx_spy_global_ex_us_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(ACWX_SPY_GLOBAL_EX_US_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_acwx_spy_global_ex_us_rotation_followthrough_v0 requires "
                "data_context['acwx_spy_global_ex_us_rotation'] with shifted ACWX/SPY daily observations."
            )
        ...
        rotation_features = _global_ex_us_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_ACWX_SPY_GLOBAL_EX_US_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported ACWX/SPY global ex-US rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid ACWX/SPY global ex-US rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["global_ex_us_rotation_5d"],
            row["global_ex_us_rotation_z126"],
            row["global_ex_us_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["global_ex_us_rotation_5d"])
        rotation_z = float(row["global_ex_us_rotation_z126"])
        rotation_percentile = float(row["global_ex_us_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _global_ex_us_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "acwx_close", "acwx_volume", "spy_close", "spy_volume"]].copy()
    ...
    frame["acwx_return_5d"] = np.log(frame["acwx_close"] / frame["acwx_close"].shift(5))
    frame["spy_return_5d"] = np.log(frame["spy_close"] / frame["spy_close"].shift(5))
    frame["global_ex_us_rotation_5d"] = frame["acwx_return_5d"] - frame["spy_return_5d"]
    frame["global_ex_us_rotation_z126"] = _rolling_zscore(frame["global_ex_us_rotation_5d"], 126)
    frame["global_ex_us_rotation_abs_percentile252"] = _rolling_percentile(
        frame["global_ex_us_rotation_5d"].abs(),
        252,
    )
    frame["acwx_volume_z126"] = _rolling_zscore(np.log(frame["acwx_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "acwx_return_5d",
        "spy_return_5d",
        "global_ex_us_rotation_5d",
        "global_ex_us_rotation_z126",
        "global_ex_us_rotation_abs_percentile252",
        "acwx_volume_z126",
        "spy_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "acwx_close": float(row["acwx_close"]),
        "spy_close": float(row["spy_close"]),
        "acwx_return_5d": float(row["acwx_return_5d"]),
        "spy_return_5d": float(row["spy_return_5d"]),
        "global_ex_us_rotation_5d": float(row["global_ex_us_rotation_5d"]),
        "global_ex_us_rotation_z126": float(row["global_ex_us_rotation_z126"]),
        "global_ex_us_rotation_abs_percentile252": float(row["global_ex_us_rotation_abs_percentile252"]),
        "acwx_volume_z126": float(row["acwx_volume_z126"]),
