...
from phase0.strategies.h1_xlu_xlk_defensive_rotation_followthrough_v0 import (
    _defensive_rotation_features_for_h1,
)
from phase0.xlu_xlk_defensive_rotation_data import XLU_XLK_DEFENSIVE_ROTATION_FRAME_KEY


class H4XluXlkDefensiveRotationReversalV0Strategy(StrategyBase):
    """Research-only H4 XAU reversal candidate using XLU/XLK defensive-sector rotation."""

    name = "h4_xlu_xlk_defensive_rotation_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h4_xlu_xlk_defensive_rotation_reversal_v0 requires "
        ...
        rotation_features = _defensive_rotation_features_for_h1(h4, rotation)
        h4 = pd.concat(
            [
        ...
        for position in range(180, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_XLU_XLK_DEFENSIVE_ROTATION_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLU/XLK H4 reversal direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLU/XLK H4 reversal trade plan risk.")
...
def _setup_metadata(
    ...
    return {
        ...
        "defensive_rotation_abs_percentile252": float(row["defensive_rotation_abs_percentile252"]),
