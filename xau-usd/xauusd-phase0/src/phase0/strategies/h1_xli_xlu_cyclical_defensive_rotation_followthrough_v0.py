...
class H1XliXluCyclicalDefensiveRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XLI/XLU sector rotation."""

    name = "h1_xli_xlu_cyclical_defensive_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XLI_XLU_CYCLICAL_DEFENSIVE_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xli_xlu_cyclical_defensive_rotation_followthrough_v0 requires "
                "data_context['xli_xlu_cyclical_defensive_rotation'] with shifted XLI/XLU daily observations."
            )
        ...
        rotation_features = _cyclical_defensive_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XLI_XLU_CYCLICAL_DEFENSIVE_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLI/XLU cyclical-defensive direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLI/XLU cyclical-defensive trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["cyclical_defensive_rotation_5d"],
            row["cyclical_defensive_rotation_z126"],
            row["cyclical_defensive_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["cyclical_defensive_rotation_5d"])
        rotation_z = float(row["cyclical_defensive_rotation_z126"])
        rotation_percentile = float(row["cyclical_defensive_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _cyclical_defensive_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xli_close", "xli_volume", "xlu_close", "xlu_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("xli_close", "xli_volume", "xlu_close", "xlu_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "xli_close", "xlu_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["xli_return_5d"] = np.log(frame["xli_close"] / frame["xli_close"].shift(5))
    frame["xlu_return_5d"] = np.log(frame["xlu_close"] / frame["xlu_close"].shift(5))
    frame["cyclical_defensive_rotation_5d"] = frame["xli_return_5d"] - frame["xlu_return_5d"]
    frame["cyclical_defensive_rotation_z126"] = _rolling_zscore(
        frame["cyclical_defensive_rotation_5d"],
        126,
    )
    frame["cyclical_defensive_rotation_abs_percentile252"] = _rolling_percentile(
        frame["cyclical_defensive_rotation_5d"].abs(),
        252,
    )
    frame["xli_volume_z126"] = _rolling_zscore(np.log(frame["xli_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "xli_return_5d",
        "xlu_return_5d",
        "cyclical_defensive_rotation_5d",
        "cyclical_defensive_rotation_z126",
        "cyclical_defensive_rotation_abs_percentile252",
        "xli_volume_z126",
        "xlu_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xli_close": float(row["xli_close"]),
        "xlu_close": float(row["xlu_close"]),
        "xli_return_5d": float(row["xli_return_5d"]),
        "xlu_return_5d": float(row["xlu_return_5d"]),
        "cyclical_defensive_rotation_5d": float(row["cyclical_defensive_rotation_5d"]),
        "cyclical_defensive_rotation_z126": float(row["cyclical_defensive_rotation_z126"]),
        "cyclical_defensive_rotation_abs_percentile252": float(
            row["cyclical_defensive_rotation_abs_percentile252"]
        ),
        "xli_volume_z126": float(row["xli_volume_z126"]),
