...
class H1XluXlkDefensiveRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XLU/XLK defensive-sector rotation."""

    name = "h1_xlu_xlk_defensive_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XLU_XLK_DEFENSIVE_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xlu_xlk_defensive_rotation_followthrough_v0 requires "
                "data_context['xlu_xlk_defensive_rotation'] with shifted XLU/XLK daily observations."
            )
        ...
        rotation_features = _defensive_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XLU_XLK_DEFENSIVE_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLU/XLK defensive-rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLU/XLK defensive-rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["defensive_rotation_5d"],
            row["defensive_rotation_z126"],
            row["defensive_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["defensive_rotation_5d"])
        rotation_z = float(row["defensive_rotation_z126"])
        rotation_percentile = float(row["defensive_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _defensive_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xlu_close", "xlu_volume", "xlk_close", "xlk_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("xlu_close", "xlu_volume", "xlk_close", "xlk_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "xlu_close", "xlk_close"]).sort_values("timestamp_utc")
    ...
    frame["xlk_return_5d"] = np.log(frame["xlk_close"] / frame["xlk_close"].shift(5))
    frame["defensive_rotation_5d"] = frame["xlu_return_5d"] - frame["xlk_return_5d"]
    frame["defensive_rotation_z126"] = _rolling_zscore(frame["defensive_rotation_5d"], 126)
    frame["defensive_rotation_abs_percentile252"] = _rolling_percentile(
        frame["defensive_rotation_5d"].abs(),
        252,
    )
    frame["xlu_volume_z126"] = _rolling_zscore(np.log(frame["xlu_volume"].replace(0.0, np.nan)), 126)
    frame["xlk_volume_z126"] = _rolling_zscore(np.log(frame["xlk_volume"].replace(0.0, np.nan)), 126)
    feature_columns = [
        "xlu_close",
        "xlu_volume",
        "xlk_close",
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xlk_close": float(row["xlk_close"]),
        "xlu_return_5d": float(row["xlu_return_5d"]),
        "xlk_return_5d": float(row["xlk_return_5d"]),
        "defensive_rotation_5d": float(row["defensive_rotation_5d"]),
        "defensive_rotation_z126": float(row["defensive_rotation_z126"]),
        "defensive_rotation_abs_percentile252": float(
            row["defensive_rotation_abs_percentile252"]
        ),
        "xlu_volume_z126": float(row["xlu_volume_z126"]),
        "xlk_volume_z126": float(row["xlk_volume_z126"]),
    }
