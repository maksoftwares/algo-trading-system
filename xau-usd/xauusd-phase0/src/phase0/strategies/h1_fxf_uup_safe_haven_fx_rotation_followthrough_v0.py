...
class H1FxfUupSafeHavenFxRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using FXF/UUP safe-haven FX rotation."""

    name = "h1_fxf_uup_safe_haven_fx_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(FXF_UUP_SAFE_HAVEN_FX_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_fxf_uup_safe_haven_fx_rotation_followthrough_v0 requires "
                "data_context['fxf_uup_safe_haven_fx_rotation'] with shifted FXF/UUP daily observations."
            )
        ...
        rotation_features = _safe_haven_fx_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_FXF_UUP_SAFE_HAVEN_FX_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported FXF/UUP safe-haven FX rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid FXF/UUP safe-haven FX rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["safe_haven_fx_rotation_5d"],
            row["safe_haven_fx_rotation_z126"],
            row["safe_haven_fx_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["safe_haven_fx_rotation_5d"])
        rotation_z = float(row["safe_haven_fx_rotation_z126"])
        rotation_percentile = float(row["safe_haven_fx_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _safe_haven_fx_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "fxf_close", "fxf_volume", "uup_close", "uup_volume"]].copy()
    ...
    frame["fxf_return_5d"] = np.log(frame["fxf_close"] / frame["fxf_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["safe_haven_fx_rotation_5d"] = frame["fxf_return_5d"] - frame["uup_return_5d"]
    frame["safe_haven_fx_rotation_z126"] = _rolling_zscore(frame["safe_haven_fx_rotation_5d"], 126)
    frame["safe_haven_fx_rotation_abs_percentile252"] = _rolling_percentile(
        frame["safe_haven_fx_rotation_5d"].abs(),
        252,
    )
    frame["fxf_volume_z126"] = _rolling_zscore(np.log(frame["fxf_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "fxf_return_5d",
        "uup_return_5d",
        "safe_haven_fx_rotation_5d",
        "safe_haven_fx_rotation_z126",
        "safe_haven_fx_rotation_abs_percentile252",
        "fxf_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "fxf_close": float(row["fxf_close"]),
        "uup_close": float(row["uup_close"]),
        "fxf_return_5d": float(row["fxf_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "safe_haven_fx_rotation_5d": float(row["safe_haven_fx_rotation_5d"]),
        "safe_haven_fx_rotation_z126": float(row["safe_haven_fx_rotation_z126"]),
        "safe_haven_fx_rotation_abs_percentile252": float(row["safe_haven_fx_rotation_abs_percentile252"]),
        "fxf_volume_z126": float(row["fxf_volume_z126"]),
