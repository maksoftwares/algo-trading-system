...
from phase0.strategies.h4_vix_risk_off_reversal_v0 import _vix_features_for_h4
from phase0.vix_risk_data import VIX_FRAME_KEY


class H4VixRiskOffFollowthroughV0Strategy(StrategyBase):
    """Research-only H4 equity-risk implied-volatility followthrough candidate."""

    name = "h4_vix_risk_off_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(vix, pd.DataFrame):
            raise ConfigError(
                "h4_vix_risk_off_followthrough_v0 requires data_context['vix_risk'] "
        ...
        for position in range(80, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_VIX_RISK_OFF_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported VIX risk-off followthrough direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid VIX risk-off followthrough trade plan risk.")
