...
class H1IwmSpySizeRiskRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using IWM/SPY size-risk rotation."""

    name = "h1_iwm_spy_size_risk_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(IWM_SPY_SIZE_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_iwm_spy_size_risk_rotation_followthrough_v0 requires "
                "data_context['iwm_spy_size_rotation'] with shifted IWM/SPY daily observations."
            )
        ...
        rotation_features = _size_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_IWM_SPY_SIZE_RISK_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported IWM/SPY size-risk rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid IWM/SPY size-risk rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["size_rotation_5d"],
            row["size_rotation_z126"],
            row["size_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["size_rotation_5d"])
        rotation_z = float(row["size_rotation_z126"])
        rotation_percentile = float(row["size_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _size_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "iwm_close", "iwm_volume", "spy_close", "spy_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("iwm_close", "iwm_volume", "spy_close", "spy_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "iwm_close", "spy_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["iwm_return_5d"] = np.log(frame["iwm_close"] / frame["iwm_close"].shift(5))
    frame["spy_return_5d"] = np.log(frame["spy_close"] / frame["spy_close"].shift(5))
    frame["size_rotation_5d"] = frame["iwm_return_5d"] - frame["spy_return_5d"]
    frame["size_rotation_z126"] = _rolling_zscore(frame["size_rotation_5d"], 126)
    frame["size_rotation_abs_percentile252"] = _rolling_percentile(
        frame["size_rotation_5d"].abs(),
        252,
    )
    frame["iwm_volume_z126"] = _rolling_zscore(np.log(frame["iwm_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "size_rotation_5d",
        "size_rotation_z126",
        "size_rotation_abs_percentile252",
        "iwm_volume_z126",
        "spy_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "iwm_return_5d": float(row["iwm_return_5d"]),
        "spy_return_5d": float(row["spy_return_5d"]),
        "size_rotation_5d": float(row["size_rotation_5d"]),
        "size_rotation_z126": float(row["size_rotation_z126"]),
        "size_rotation_abs_percentile252": float(row["size_rotation_abs_percentile252"]),
        "iwm_volume_z126": float(row["iwm_volume_z126"]),
