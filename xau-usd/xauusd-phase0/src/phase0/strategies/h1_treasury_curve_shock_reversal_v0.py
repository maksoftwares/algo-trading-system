...
class H1TreasuryCurveShockReversalV0Strategy(StrategyBase):
    """Research-only H1 Treasury curve/rates shock reversal candidate."""

    name = "h1_treasury_curve_shock_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(treasury, pd.DataFrame):
            raise ConfigError(
                "h1_treasury_curve_shock_reversal_v0 requires "
        ...
        for position in range(100, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_TREASURY_CURVE_SHOCK_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported Treasury curve shock reversal direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid Treasury curve shock reversal trade plan risk.")
...
def _treasury_features_for_h1(h1: pd.DataFrame, treasury: pd.DataFrame) -> pd.DataFrame:
    ...
    treasury_frame["dgs2_change_10d"] = treasury_frame["dgs2"] - treasury_frame["dgs2"].shift(10)
    treasury_frame["dgs10_change_10d"] = treasury_frame["dgs10"] - treasury_frame["dgs10"].shift(10)
    treasury_frame["treasury_10y2y_change_10d"] = (
        treasury_frame["treasury_10y2y"] - treasury_frame["treasury_10y2y"].shift(10)
    )
    treasury_frame["dgs2_change_z252"] = _rolling_zscore(
        treasury_frame["dgs2_change_10d"],
        252,
    )
