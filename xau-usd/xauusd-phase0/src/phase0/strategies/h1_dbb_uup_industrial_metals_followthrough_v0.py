...
class H1DbbUupIndustrialMetalsFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using DBB/UUP industrial-metals pressure."""

    name = "h1_dbb_uup_industrial_metals_followthrough_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    pressure_threshold = 0.0080
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        pressure = data_context.get(DBB_UUP_INDUSTRIAL_METALS_FRAME_KEY)
        if not isinstance(pressure, pd.DataFrame):
            raise ConfigError(
                "h1_dbb_uup_industrial_metals_followthrough_v0 requires "
                "data_context['dbb_uup_industrial_metals'] with shifted DBB/UUP daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_DBB_UUP_INDUSTRIAL_METALS_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported DBB/UUP industrial-metals direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid DBB/UUP industrial-metals trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["industrial_metals_pressure_5d"],
            row["industrial_metals_pressure_z126"],
            row["industrial_metals_pressure_abs_percentile252"],
        )
        ...
        pressure = float(row["industrial_metals_pressure_5d"])
        pressure_z = float(row["industrial_metals_pressure_z126"])
        pressure_percentile = float(row["industrial_metals_pressure_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _pressure_features_for_h1(h1: pd.DataFrame, pressure: pd.DataFrame) -> pd.DataFrame:
    frame = pressure[["timestamp_utc", "dbb_close", "dbb_volume", "uup_close", "uup_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("dbb_close", "dbb_volume", "uup_close", "uup_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "dbb_close", "uup_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["dbb_return_5d"] = np.log(frame["dbb_close"] / frame["dbb_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["industrial_metals_pressure_5d"] = frame["dbb_return_5d"] - frame["uup_return_5d"]
    frame["industrial_metals_pressure_z126"] = _rolling_zscore(frame["industrial_metals_pressure_5d"], 126)
    frame["industrial_metals_pressure_abs_percentile252"] = _rolling_percentile(
        frame["industrial_metals_pressure_5d"].abs(),
        252,
    )
    frame["dbb_volume_z126"] = _rolling_zscore(np.log(frame["dbb_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "dbb_return_5d",
        "uup_return_5d",
        "industrial_metals_pressure_5d",
        "industrial_metals_pressure_z126",
        "industrial_metals_pressure_abs_percentile252",
        "dbb_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "dbb_close": float(row["dbb_close"]),
        "uup_close": float(row["uup_close"]),
        "dbb_return_5d": float(row["dbb_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "industrial_metals_pressure_5d": float(row["industrial_metals_pressure_5d"]),
        "industrial_metals_pressure_z126": float(row["industrial_metals_pressure_z126"]),
        "industrial_metals_pressure_abs_percentile252": float(
            row["industrial_metals_pressure_abs_percentile252"]
        ),
        "dbb_volume_z126": float(row["dbb_volume_z126"]),
