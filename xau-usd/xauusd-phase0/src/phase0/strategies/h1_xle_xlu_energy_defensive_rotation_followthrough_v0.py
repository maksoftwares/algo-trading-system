...
from phase0.xle_xlu_energy_defensive_rotation_data import (
    XLE_XLU_ENERGY_DEFENSIVE_ROTATION_FRAME_KEY,
)


class H1XleXluEnergyDefensiveRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XLE/XLU energy-defensive rotation."""

    name = "h1_xle_xlu_energy_defensive_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XLE_XLU_ENERGY_DEFENSIVE_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xle_xlu_energy_defensive_rotation_followthrough_v0 requires "
                "data_context['xle_xlu_energy_defensive_rotation'] with shifted XLE/XLU daily observations."
            )
        ...
        rotation_features = _energy_defensive_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XLE_XLU_ENERGY_DEFENSIVE_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLE/XLU energy-defensive direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLE/XLU energy-defensive trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["energy_defensive_rotation_5d"],
            row["energy_defensive_rotation_z126"],
            row["energy_defensive_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["energy_defensive_rotation_5d"])
        rotation_z = float(row["energy_defensive_rotation_z126"])
        rotation_percentile = float(row["energy_defensive_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _energy_defensive_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xle_close", "xle_volume", "xlu_close", "xlu_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("xle_close", "xle_volume", "xlu_close", "xlu_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "xle_close", "xlu_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["xle_return_5d"] = np.log(frame["xle_close"] / frame["xle_close"].shift(5))
    frame["xlu_return_5d"] = np.log(frame["xlu_close"] / frame["xlu_close"].shift(5))
    frame["energy_defensive_rotation_5d"] = frame["xle_return_5d"] - frame["xlu_return_5d"]
    frame["energy_defensive_rotation_z126"] = _rolling_zscore(
        frame["energy_defensive_rotation_5d"],
        126,
    )
    frame["energy_defensive_rotation_abs_percentile252"] = _rolling_percentile(
        frame["energy_defensive_rotation_5d"].abs(),
        252,
    )
    frame["xle_volume_z126"] = _rolling_zscore(np.log(frame["xle_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "xle_return_5d",
        "xlu_return_5d",
        "energy_defensive_rotation_5d",
        "energy_defensive_rotation_z126",
        "energy_defensive_rotation_abs_percentile252",
        "xle_volume_z126",
        "xlu_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xle_close": float(row["xle_close"]),
        "xlu_close": float(row["xlu_close"]),
        "xle_return_5d": float(row["xle_return_5d"]),
        "xlu_return_5d": float(row["xlu_return_5d"]),
        "energy_defensive_rotation_5d": float(row["energy_defensive_rotation_5d"]),
        "energy_defensive_rotation_z126": float(row["energy_defensive_rotation_z126"]),
        "energy_defensive_rotation_abs_percentile252": float(row["energy_defensive_rotation_abs_percentile252"]),
        "xle_volume_z126": float(row["xle_volume_z126"]),
