...
class H1MacroCompositePullbackV0Strategy(StrategyBase):
    """Research-only H1 pullback-continuation candidate under fixed macro composite state."""

    name = "h1_macro_composite_pullback_v0"
        ...
        for position in range(120, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_MACRO_COMPOSITE_PULLBACK_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported H1 macro pullback direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid H1 macro pullback trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        near_ema21 = low <= ema21 + self.pullback_atr_multiple * h1_atr and high >= ema21 - 0.35 * h1_atr
