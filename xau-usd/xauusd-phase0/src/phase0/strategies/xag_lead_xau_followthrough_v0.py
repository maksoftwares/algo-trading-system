...
class XagLeadXauFollowthroughV0Strategy(StrategyBase):
    """Research-only XAG lead-lag continuation candidate."""

    name = "xag_lead_xau_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(relative_value, dict):
            raise ConfigError(
                "xag_lead_xau_followthrough_v0 requires data_context['relative_value'] "
        ...
        features = _lead_lag_features(h1, xag)
        ...
        for position in range(300, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"XAG_LEAD_XAU_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XAG lead XAU follow-through direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XAG lead XAU follow-through trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        relative_impulse_z = float(row["relative_impulse_z"])
        if xau_atr14 <= 0:
            return None

        if (
            xag_return_6h >= 0.0010
            and xag_return_24h >= 0.0025
            and relative_impulse_z >= 0.45
            and xau_return_6h >= -0.0015
            and xau_return_24h >= -0.0030
            and close > open_price
            and close >= xau_ema20 - 0.65 * xau_atr14
        ):
...
def _relative_frame(relative_value: dict[str, Any], symbol: str) -> pd.DataFrame:
    ...
    if not isinstance(frame, pd.DataFrame):
        raise ConfigError(f"xag_lead_xau_followthrough_v0 missing {symbol} H1 frame.")
    return require_frame({symbol: frame}, symbol)


def _lead_lag_features(h1: pd.DataFrame, xag: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["relative_impulse"] = frame["xag_return_6h"] - frame["xau_return_6h"]
    frame["relative_impulse_z"] = _rolling_zscore(frame["relative_impulse"], 250)
    return frame[
        [
            "timestamp_utc",
            "xag_close",
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float) -> dict[str, Any]:
    return {
        ...
        "xau_return_24h": float(row["xau_return_24h"]),
        "xag_return_24h": float(row["xag_return_24h"]),
        "relative_impulse": float(row["relative_impulse"]),
        "relative_impulse_z": float(row["relative_impulse_z"]),
    }
