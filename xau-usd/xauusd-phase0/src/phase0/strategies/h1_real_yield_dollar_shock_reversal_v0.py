...
class H1RealYieldDollarShockReversalV0Strategy(StrategyBase):
    """Research-only H1 reversal after real-yield and broad-dollar macro shocks."""

    name = "h1_real_yield_dollar_shock_reversal_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(macro, pd.DataFrame):
            raise ConfigError(
                "h1_real_yield_dollar_shock_reversal_v0 requires data_context['macro_proxy'] "
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_REAL_YIELD_DOLLAR_SHOCK_REVERSAL_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported H1 real-yield shock direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid H1 real-yield shock reversal trade plan risk.")
