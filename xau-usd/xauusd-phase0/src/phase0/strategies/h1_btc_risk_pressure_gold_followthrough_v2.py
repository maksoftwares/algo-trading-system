...
class H1BtcRiskPressureGoldFollowthroughV2Strategy(StrategyBase):
    """Research-only H1 XAU continuation candidate using BTC stress transitions."""

    name = "h1_btc_risk_pressure_gold_followthrough_v2"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    pressure_threshold = 0.070
    pressure_z_threshold = 0.35
    pressure_percentile_threshold = 0.60
    pressure_volume_z_threshold = 0.20
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h1_btc_risk_pressure_gold_followthrough_v2 requires "
        ...
        btc_features = _btc_features_for_h1_v2(h1, btc)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_BTC_RISK_PRESSURE_GOLD_FOLLOWTHROUGH_V2_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported BTC risk-pressure direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid BTC risk-pressure trade plan risk.")
        ...
        return TradePlan(
            ...
            metadata={**signal.metadata, "estimated_entry_price": estimated_entry, "max_holding_bars": 100},
        )
...
def _btc_features_for_h1_v2(h1: pd.DataFrame, btc: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["btc_abs_return_percentile252"] = _rolling_percentile(
        frame["btc_return_5d"].abs(),
        252,
    )
...
def _setup_metadata(
    ...
    return {
        ...
        "h1_ema21": float(row["h1_ema21"]),
        "h1_ema50": float(row["h1_ema50"]),
        "h1_return_6": float(h1_return_6),
        "h1_return_12": float(h1_return_12),
