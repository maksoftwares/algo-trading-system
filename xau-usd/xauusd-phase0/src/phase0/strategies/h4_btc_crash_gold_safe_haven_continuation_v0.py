...
class H4BtcCrashGoldSafeHavenContinuationV0Strategy(StrategyBase):
    """Research-only H4 XAU continuation candidate after shifted BTC crash stress."""

    name = "h4_btc_crash_gold_safe_haven_continuation_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.35
    btc_crash_threshold = -0.075
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h4_btc_crash_gold_safe_haven_continuation_v0 requires "
        ...
        h4["prior_h4_high_12"] = high.shift(1).rolling(12, min_periods=8).max()
        ...
        for position in range(180, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction="LONG",
                    reason_code="H4_BTC_CRASH_GOLD_SAFE_HAVEN_CONTINUATION_V0_LONG",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if risk_price <= 0:
            raise ConfigError("Invalid BTC crash safe-haven continuation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        prior_high = float(row["prior_h4_high_12"])
        ...
        breakout_atr = (close - prior_high) / h4_atr

        crash_active = (
            btc_return <= self.btc_crash_threshold
        ...
        if (
            ...
            and 0.0 <= ema40_distance_atr <= 3.40
            and breakout_atr <= 1.65
        ):
            return {
                "direction": "LONG",
