...
class H4BtcVolatilityRegimeGoldBreakoutV0Strategy(StrategyBase):
    """Research-only H4 XAU expansion gated by shifted BTC volatility regime."""

    name = "h4_btc_volatility_regime_gold_breakout_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h4_btc_volatility_regime_gold_breakout_v0 requires "
        ...
        for position in range(160, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_BTC_VOLATILITY_REGIME_GOLD_BREAKOUT_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            stop_loss = estimated_entry - 1.50 * h4_atr
            ...
            raise ConfigError(f"Unsupported BTC volatility-regime direction {signal.direction!r}.")
        if risk_price <= 0:
            raise ConfigError("Invalid BTC volatility-regime trade plan risk.")
        return TradePlan(
            expert=self.name,
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        required = (
            ...
            row["h4_return_3"],
            row["h4_return_12"],
            row["btc_realized_vol_10d"],
            row["btc_realized_vol_40d"],
        ...
        range_atr = candle_range / h4_atr
        ema40_distance_atr = (close - h4_ema40) / h4_atr
        volatility_regime = (
            btc_vol_ratio >= 1.18
            and btc_vol_percentile >= 0.68
            and btc_abs_return_percentile >= 0.55
        )
        ...
        if (
            close > open_price
            and close > h4_ema40
            and h4_return_3 >= 0.0018
            and h4_return_12 >= -0.0120
            and close_location >= 0.62
            and range_atr >= 0.70
            and -0.35 <= ema40_distance_atr <= 3.25
        ):
            return _setup_metadata(row, "LONG", close, close_location, range_atr, ema40_distance_atr)
        ...
        if (
            ...
            and range_atr >= 0.70
            and -3.25 <= ema40_distance_atr <= 0.35
        ):
            return _setup_metadata(row, "SHORT", close, close_location, range_atr, ema40_distance_atr)

        return None


def _btc_volatility_features_for_h4(h4: pd.DataFrame, btc: pd.DataFrame) -> pd.DataFrame:
