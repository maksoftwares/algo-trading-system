...
class H1HygIefCreditRiskRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using HYG/IEF credit-risk rotation."""

    name = "h1_hyg_ief_credit_risk_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_hyg_ief_credit_risk_rotation_followthrough_v0 requires "
        ...
        rotation_features = _credit_risk_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_HYG_IEF_CREDIT_RISK_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported HYG/IEF credit-risk rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid HYG/IEF credit-risk rotation trade plan risk.")
...
def _credit_risk_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "hyg_close", "hyg_volume", "ief_close", "ief_volume"]].copy()
    ...
    frame["hyg_return_5d"] = np.log(frame["hyg_close"] / frame["hyg_close"].shift(5))
    frame["ief_return_5d"] = np.log(frame["ief_close"] / frame["ief_close"].shift(5))
    frame["credit_stress_5d"] = frame["ief_return_5d"] - frame["hyg_return_5d"]
    frame["credit_stress_z126"] = _rolling_zscore(frame["credit_stress_5d"], 126)
    frame["credit_stress_abs_percentile252"] = _rolling_percentile(frame["credit_stress_5d"].abs(), 252)
    frame["hyg_volume_z126"] = _rolling_zscore(np.log(frame["hyg_volume"].replace(0.0, np.nan)), 126)
