...
class H1XlfXluFinancialsDefensiveRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XLF/XLU sector rotation."""

    name = "h1_xlf_xlu_financials_defensive_rotation_followthrough_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    rotation_threshold = 0.0090
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XLF_XLU_FINANCIALS_DEFENSIVE_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xlf_xlu_financials_defensive_rotation_followthrough_v0 requires "
                "data_context['xlf_xlu_financials_defensive_rotation'] with shifted XLF/XLU daily observations."
            )
        ...
        rotation_features = _financials_defensive_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XLF_XLU_FINANCIALS_DEFENSIVE_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XLF/XLU financials-defensive direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XLF/XLU financials-defensive trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["financials_defensive_rotation_5d"],
            row["financials_defensive_rotation_z126"],
            row["financials_defensive_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["financials_defensive_rotation_5d"])
        rotation_z = float(row["financials_defensive_rotation_z126"])
        rotation_percentile = float(row["financials_defensive_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _financials_defensive_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xlf_close", "xlf_volume", "xlu_close", "xlu_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("xlf_close", "xlf_volume", "xlu_close", "xlu_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "xlf_close", "xlu_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["xlf_return_5d"] = np.log(frame["xlf_close"] / frame["xlf_close"].shift(5))
    frame["xlu_return_5d"] = np.log(frame["xlu_close"] / frame["xlu_close"].shift(5))
    frame["financials_defensive_rotation_5d"] = frame["xlf_return_5d"] - frame["xlu_return_5d"]
    frame["financials_defensive_rotation_z126"] = _rolling_zscore(
        frame["financials_defensive_rotation_5d"],
        126,
    )
    frame["financials_defensive_rotation_abs_percentile252"] = _rolling_percentile(
        frame["financials_defensive_rotation_5d"].abs(),
        252,
    )
    frame["xlf_volume_z126"] = _rolling_zscore(np.log(frame["xlf_volume"].replace(0.0, np.nan)), 126)
    frame["xlu_volume_z126"] = _rolling_zscore(np.log(frame["xlu_volume"].replace(0.0, np.nan)), 126)
    feature_columns = [
        "xlf_close",
        "xlf_volume",
        "xlu_close",
        ...
        "xlf_return_5d",
        "xlu_return_5d",
        "financials_defensive_rotation_5d",
        "financials_defensive_rotation_z126",
        "financials_defensive_rotation_abs_percentile252",
        "xlf_volume_z126",
        "xlu_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xlf_close": float(row["xlf_close"]),
        "xlu_close": float(row["xlu_close"]),
        "xlf_return_5d": float(row["xlf_return_5d"]),
        "xlu_return_5d": float(row["xlu_return_5d"]),
        "financials_defensive_rotation_5d": float(row["financials_defensive_rotation_5d"]),
        "financials_defensive_rotation_z126": float(row["financials_defensive_rotation_z126"]),
        "financials_defensive_rotation_abs_percentile252": float(
            row["financials_defensive_rotation_abs_percentile252"]
        ),
        "xlf_volume_z126": float(row["xlf_volume_z126"]),
        "xlu_volume_z126": float(row["xlu_volume_z126"]),
    }
