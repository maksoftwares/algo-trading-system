...
class H1GldFlowMomentumPullbackV0Strategy(StrategyBase):
    """Research-only H1 GLD flow-aligned momentum pullback candidate."""

    name = "h1_gld_flow_momentum_pullback_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(gld_flow, pd.DataFrame):
            raise ConfigError(
                "h1_gld_flow_momentum_pullback_v0 requires data_context['gld_etf_flow'] "
        ...
        for position in range(160, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_GLD_FLOW_MOMENTUM_PULLBACK_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported GLD flow momentum direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid GLD flow momentum trade plan risk.")
...
def _gld_flow_features_for_h1(h1: pd.DataFrame, gld_flow: pd.DataFrame) -> pd.DataFrame:
