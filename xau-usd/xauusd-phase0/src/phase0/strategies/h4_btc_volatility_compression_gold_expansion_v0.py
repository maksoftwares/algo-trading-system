...
class H4BtcVolatilityCompressionGoldExpansionV0Strategy(StrategyBase):
    """Research-only H4 XAU expansion after shifted BTC volatility compression."""

    name = "h4_btc_volatility_compression_gold_expansion_v0"
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported BTC compression expansion direction {signal.direction!r}.")
        if risk_price <= 0:
            raise ConfigError("Invalid BTC compression expansion trade plan risk.")
        return TradePlan(
            expert=self.name,
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        btc_compression = (
            btc_vol_ratio <= 0.88
            and btc_vol_percentile <= 0.42
            and btc_abs_return_percentile <= 0.55
        )
        if not btc_compression:
            return None
        ...
        if (
            ...
            and h4_return_12 >= 0.0010
            and range_atr >= 0.70
            and close_location >= 0.62
            and -0.35 <= ema40_distance_atr <= 3.00
        ):
            return _metadata(row, "LONG", close, range_atr, close_location, ema40_distance_atr)
        ...
        if (
            ...
            and -3.00 <= ema40_distance_atr <= 0.35
        ):
            return _metadata(row, "SHORT", close, range_atr, close_location, ema40_distance_atr)
