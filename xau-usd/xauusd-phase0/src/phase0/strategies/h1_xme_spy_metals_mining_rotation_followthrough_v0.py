...
class H1XmeSpyMetalsMiningRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using XME/SPY metals-mining rotation."""

    name = "h1_xme_spy_metals_mining_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(XME_SPY_METALS_MINING_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_xme_spy_metals_mining_rotation_followthrough_v0 requires "
                "data_context['xme_spy_metals_mining_rotation'] with shifted XME/SPY daily observations."
            )
        ...
        rotation_features = _metals_mining_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_XME_SPY_METALS_MINING_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported XME/SPY metals-mining rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid XME/SPY metals-mining rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["metals_mining_rotation_5d"],
            row["metals_mining_rotation_z126"],
            row["metals_mining_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["metals_mining_rotation_5d"])
        rotation_z = float(row["metals_mining_rotation_z126"])
        rotation_percentile = float(row["metals_mining_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _metals_mining_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "xme_close", "xme_volume", "spy_close", "spy_volume"]].copy()
    ...
    frame["xme_return_5d"] = np.log(frame["xme_close"] / frame["xme_close"].shift(5))
    frame["spy_return_5d"] = np.log(frame["spy_close"] / frame["spy_close"].shift(5))
    frame["metals_mining_rotation_5d"] = frame["xme_return_5d"] - frame["spy_return_5d"]
    frame["metals_mining_rotation_z126"] = _rolling_zscore(frame["metals_mining_rotation_5d"], 126)
    frame["metals_mining_rotation_abs_percentile252"] = _rolling_percentile(
        frame["metals_mining_rotation_5d"].abs(),
        252,
    )
    frame["xme_volume_z126"] = _rolling_zscore(np.log(frame["xme_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "xme_return_5d",
        "spy_return_5d",
        "metals_mining_rotation_5d",
        "metals_mining_rotation_z126",
        "metals_mining_rotation_abs_percentile252",
        "xme_volume_z126",
        "spy_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "xme_close": float(row["xme_close"]),
        "spy_close": float(row["spy_close"]),
        "xme_return_5d": float(row["xme_return_5d"]),
        "spy_return_5d": float(row["spy_return_5d"]),
        "metals_mining_rotation_5d": float(row["metals_mining_rotation_5d"]),
        "metals_mining_rotation_z126": float(row["metals_mining_rotation_z126"]),
        "metals_mining_rotation_abs_percentile252": float(row["metals_mining_rotation_abs_percentile252"]),
        "xme_volume_z126": float(row["xme_volume_z126"]),
