...
class H1UsoUupOilDollarFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using USO/UUP oil-dollar pressure."""

    name = "h1_uso_uup_oil_dollar_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        pressure = data_context.get(USO_UUP_OIL_DOLLAR_FRAME_KEY)
        if not isinstance(pressure, pd.DataFrame):
            raise ConfigError(
                "h1_uso_uup_oil_dollar_followthrough_v0 requires "
                "data_context['uso_uup_oil_dollar'] with shifted USO/UUP daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_USO_UUP_OIL_DOLLAR_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported USO/UUP oil-dollar direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid USO/UUP oil-dollar trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["oil_dollar_pressure_5d"],
            row["oil_dollar_pressure_z126"],
            row["oil_dollar_pressure_abs_percentile252"],
        )
        ...
        pressure = float(row["oil_dollar_pressure_5d"])
        pressure_z = float(row["oil_dollar_pressure_z126"])
        pressure_percentile = float(row["oil_dollar_pressure_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _pressure_features_for_h1(h1: pd.DataFrame, pressure: pd.DataFrame) -> pd.DataFrame:
    frame = pressure[["timestamp_utc", "uso_close", "uso_volume", "uup_close", "uup_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("uso_close", "uso_volume", "uup_close", "uup_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "uso_close", "uup_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["uso_return_5d"] = np.log(frame["uso_close"] / frame["uso_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["oil_dollar_pressure_5d"] = frame["uso_return_5d"] - frame["uup_return_5d"]
    frame["oil_dollar_pressure_z126"] = _rolling_zscore(frame["oil_dollar_pressure_5d"], 126)
    frame["oil_dollar_pressure_abs_percentile252"] = _rolling_percentile(
        frame["oil_dollar_pressure_5d"].abs(),
        252,
    )
    frame["uso_volume_z126"] = _rolling_zscore(np.log(frame["uso_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "uso_return_5d",
        "uup_return_5d",
        "oil_dollar_pressure_5d",
        "oil_dollar_pressure_z126",
        "oil_dollar_pressure_abs_percentile252",
        "uso_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "uso_close": float(row["uso_close"]),
        "uup_close": float(row["uup_close"]),
        "uso_return_5d": float(row["uso_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "oil_dollar_pressure_5d": float(row["oil_dollar_pressure_5d"]),
        "oil_dollar_pressure_z126": float(row["oil_dollar_pressure_z126"]),
        "oil_dollar_pressure_abs_percentile252": float(
            row["oil_dollar_pressure_abs_percentile252"]
        ),
        "uso_volume_z126": float(row["uso_volume_z126"]),
