...
from phase0.indicators import adx, atr, ema, slope
...
class H4D1ContractionTrendContinuationV0Strategy(StrategyBase):
    """Disabled research strategy for the locked H4/D1 contraction trend-continuation v0 hypothesis."""

    name = "h4_d1_contraction_trend_continuation_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if "ema50_slope6" not in h4:
            h4["ema50_slope6"] = slope(h4["ema50"], 6)
        if "adx14" not in h4:
            h4["adx14"] = adx(h4["high"], h4["low"], h4["close"], 14)
        if "range" not in h4:
        ...
        if "prior60_atr40" not in d1:
            d1["prior60_atr40"] = d1["atr14"].shift(1).rolling(60, min_periods=60).quantile(0.40)
        ...
        for h4_position in range(60, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_D1_CONTRACTION_TREND_CONTINUATION_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            stop_loss = float(signal.metadata["pullback_low"]) - 0.65 * h4_atr
            risk_price = estimated_entry - stop_loss
            take_profit = estimated_entry + 1.70 * risk_price
        elif direction == "SHORT":
            stop_loss = float(signal.metadata["pullback_high"]) + 0.65 * h4_atr
            risk_price = stop_loss - estimated_entry
            take_profit = estimated_entry - 1.70 * risk_price
        else:
            raise ConfigError(f"Unsupported H4/D1 contraction trend direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid H4/D1 contraction trend continuation v0 trade plan risk.")
        if risk_price / point_size < 325.0:
            raise ConfigError("H4/D1 contraction trend continuation v0 stop is below 325 points.")
    ...
    def _d1_contraction_at_timestamp(self, d1: pd.DataFrame, timestamp: pd.Timestamp) -> dict[str, Any] | None:
        ...
        prior60_atr40 = float(row["prior60_atr40"])
        prior40_range3_median = float(row["prior40_range3_median"])
        if d1_atr <= 0 or range3_width <= 0 or prior60_atr40 <= 0 or prior40_range3_median <= 0:
            return None
        if d1_atr > prior60_atr40:
            return None
    ...
    def _setup_at_position(
        ...
        ema50_slope6 = float(row["ema50_slope6"])
        adx14 = float(row["adx14"])
        candle_range = float(row["range"])
        close_position = float(row["close_position"])
        if h4_atr <= 0 or candle_range <= 0 or adx14 < 18.0:
            return None

        long_pullback = low <= ema21_value + 0.35 * h4_atr
        short_pullback = high >= ema21_value - 0.35 * h4_atr

        if (
            close > open_price
            and close > ema21_value
            and close > ema50_value
            and ema50_slope6 > 0
            and close_position >= 0.60
            and long_pullback
        ):
            stop_loss = low - 0.65 * h4_atr
            stop_distance_points = (close - stop_loss) / point_size
            if stop_distance_points < 325.0:
                return None
            return {
                **contraction,
                "direction": "LONG",
                "h4_atr14": h4_atr,
                "h4_adx14": adx14,
                "h4_ema21": ema21_value,
                "h4_ema50": ema50_value,
                "h4_ema50_slope6": ema50_slope6,
                "pullback_high": high,
                "pullback_low": low,
                "estimated_entry_price": close,
                "pullback_range": candle_range,
