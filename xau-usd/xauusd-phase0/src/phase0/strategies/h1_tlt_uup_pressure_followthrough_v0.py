...
from phase0.strategies.h1_tlt_uup_pressure_reversion_v0 import _pressure_features_for_h1
from phase0.indicators import atr, ema
from phase0.tlt_uup_macro_pressure_data import TLT_UUP_PRESSURE_FRAME_KEY


class H1TltUupPressureFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using TLT/UUP market-pressure proxies."""

    name = "h1_tlt_uup_pressure_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(pressure, pd.DataFrame):
            raise ConfigError(
                "h1_tlt_uup_pressure_followthrough_v0 requires "
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_TLT_UUP_PRESSURE_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported TLT/UUP pressure follow-through direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid TLT/UUP pressure follow-through trade plan risk.")
