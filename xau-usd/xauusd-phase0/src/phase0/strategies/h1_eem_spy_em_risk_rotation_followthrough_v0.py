...
class H1EemSpyEmRiskRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using EEM/SPY EM-risk rotation."""

    name = "h1_eem_spy_em_risk_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(EEM_SPY_EM_RISK_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_eem_spy_em_risk_rotation_followthrough_v0 requires "
                "data_context['eem_spy_em_risk_rotation'] with shifted EEM/SPY daily observations."
            )
        ...
        rotation_features = _em_risk_rotation_features_for_h1(h1, rotation)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_EEM_SPY_EM_RISK_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported EEM/SPY EM-risk rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid EEM/SPY EM-risk rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["em_risk_rotation_5d"],
            row["em_risk_rotation_z126"],
            row["em_risk_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["em_risk_rotation_5d"])
        rotation_z = float(row["em_risk_rotation_z126"])
        rotation_percentile = float(row["em_risk_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _em_risk_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "eem_close", "eem_volume", "spy_close", "spy_volume"]].copy()
    frame["timestamp_utc"] = pd.to_datetime(frame["timestamp_utc"], utc=True, errors="coerce")
    for column in ("eem_close", "eem_volume", "spy_close", "spy_volume"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["timestamp_utc", "eem_close", "spy_close"]).sort_values("timestamp_utc")
    frame = frame.drop_duplicates("timestamp_utc").reset_index(drop=True)
    frame["eem_return_5d"] = np.log(frame["eem_close"] / frame["eem_close"].shift(5))
    frame["spy_return_5d"] = np.log(frame["spy_close"] / frame["spy_close"].shift(5))
    frame["em_risk_rotation_5d"] = frame["eem_return_5d"] - frame["spy_return_5d"]
    frame["em_risk_rotation_z126"] = _rolling_zscore(frame["em_risk_rotation_5d"], 126)
    frame["em_risk_rotation_abs_percentile252"] = _rolling_percentile(
        frame["em_risk_rotation_5d"].abs(),
        252,
    )
    frame["eem_volume_z126"] = _rolling_zscore(np.log(frame["eem_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "spy_return_5d",
        "em_risk_rotation_5d",
        "em_risk_rotation_z126",
        "em_risk_rotation_abs_percentile252",
        "eem_volume_z126",
        "spy_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "eem_close": float(row["eem_close"]),
        "spy_close": float(row["spy_close"]),
        "eem_return_5d": float(row["eem_return_5d"]),
        "spy_return_5d": float(row["spy_return_5d"]),
        "em_risk_rotation_5d": float(row["em_risk_rotation_5d"]),
        "em_risk_rotation_z126": float(row["em_risk_rotation_z126"]),
        "em_risk_rotation_abs_percentile252": float(row["em_risk_rotation_abs_percentile252"]),
        "eem_volume_z126": float(row["eem_volume_z126"]),
