...
class H4BreakevenInflationMomentumV0Strategy(StrategyBase):
    """Research-only H4 breakeven inflation expectations momentum candidate."""

    name = "h4_breakeven_inflation_momentum_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(inflation, pd.DataFrame):
            raise ConfigError(
                "h4_breakeven_inflation_momentum_v0 requires "
        ...
        inflation_features = _inflation_features_for_h4(h4, inflation)
        h4 = pd.concat(
            [
        ...
        for position in range(100, len(h4)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H4_BREAKEVEN_INFLATION_MOMENTUM_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported breakeven inflation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid breakeven inflation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        required = (
            ...
            row["breakeven_5y"],
            row["breakeven_10y"],
            row["breakeven_5y_change_20d"],
            row["breakeven_10y_change_20d"],
            row["breakeven_5y_change_z252"],
            row["breakeven_5y_10y_spread"],
        )
        ...
        breakeven_5y_change = float(row["breakeven_5y_change_20d"])
        breakeven_10y_change = float(row["breakeven_10y_change_20d"])
        breakeven_5y_change_z = float(row["breakeven_5y_change_z252"])
        if h4_atr <= 0:
            return None

        inflation_momentum = (
            breakeven_5y_change >= 0.12 and breakeven_10y_change >= 0.08
        ) or breakeven_5y_change_z >= 0.50
        disinflation_momentum = (
            breakeven_5y_change <= -0.12 and breakeven_10y_change <= -0.08
        ) or breakeven_5y_change_z <= -0.50

        if inflation_momentum and close > h4_ema40 and close > open_price and h4_return_6 > 0.0:
            return _setup_metadata(row, "LONG", close)

        if disinflation_momentum and close < h4_ema40 and close < open_price and h4_return_6 < 0.0:
            return _setup_metadata(row, "SHORT", close)

        return None


def _inflation_features_for_h4(h4: pd.DataFrame, inflation: pd.DataFrame) -> pd.DataFrame:
    ...
    inflation_frame["breakeven_5y_change_20d"] = (
        inflation_frame["breakeven_5y"] - inflation_frame["breakeven_5y"].shift(20)
    )
    inflation_frame["breakeven_10y_change_20d"] = (
        inflation_frame["breakeven_10y"] - inflation_frame["breakeven_10y"].shift(20)
    )
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float) -> dict[str, Any]:
    return {
        ...
        "breakeven_10y_change_20d": float(row["breakeven_10y_change_20d"]),
