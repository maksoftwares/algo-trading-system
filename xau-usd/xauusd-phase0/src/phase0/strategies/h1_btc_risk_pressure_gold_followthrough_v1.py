...
class H1BtcRiskPressureGoldFollowthroughV1Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using BTC volatility-pressure asymmetry."""

    name = "h1_btc_risk_pressure_gold_followthrough_v1"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    pressure_threshold = 0.0700
    pressure_z_threshold = 0.40
    pressure_percentile_threshold = 0.60
    pressure_volume_z_threshold = 0.50
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(btc, pd.DataFrame):
            raise ConfigError(
                "h1_btc_risk_pressure_gold_followthrough_v1 requires "
        ...
        btc_features = _btc_features_for_h1(h1, btc)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_BTC_RISK_PRESSURE_GOLD_FOLLOWTHROUGH_V1_{direction}",
...
def _btc_features_for_h1(h1: pd.DataFrame, btc: pd.DataFrame) -> pd.DataFrame:
