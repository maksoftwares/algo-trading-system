...
class H1DbcUupCommodityDollarFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using DBC/UUP commodity-dollar pressure."""

    name = "h1_dbc_uup_commodity_dollar_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        pressure = data_context.get(DBC_UUP_COMMODITY_DOLLAR_FRAME_KEY)
        if not isinstance(pressure, pd.DataFrame):
            raise ConfigError(
                "h1_dbc_uup_commodity_dollar_followthrough_v0 requires "
                "data_context['dbc_uup_commodity_dollar'] with shifted DBC/UUP daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_DBC_UUP_COMMODITY_DOLLAR_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported DBC/UUP commodity-dollar direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid DBC/UUP commodity-dollar trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["commodity_dollar_pressure_5d"],
            row["commodity_dollar_pressure_z126"],
            row["commodity_dollar_pressure_abs_percentile252"],
        )
        ...
        pressure = float(row["commodity_dollar_pressure_5d"])
        pressure_z = float(row["commodity_dollar_pressure_z126"])
        pressure_percentile = float(row["commodity_dollar_pressure_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _pressure_features_for_h1(h1: pd.DataFrame, pressure: pd.DataFrame) -> pd.DataFrame:
    frame = pressure[["timestamp_utc", "dbc_close", "dbc_volume", "uup_close", "uup_volume"]].copy()
    ...
    frame["dbc_return_5d"] = np.log(frame["dbc_close"] / frame["dbc_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["commodity_dollar_pressure_5d"] = frame["dbc_return_5d"] - frame["uup_return_5d"]
    frame["commodity_dollar_pressure_z126"] = _rolling_zscore(frame["commodity_dollar_pressure_5d"], 126)
    frame["commodity_dollar_pressure_abs_percentile252"] = _rolling_percentile(
        frame["commodity_dollar_pressure_5d"].abs(),
        252,
    )
    frame["dbc_volume_z126"] = _rolling_zscore(np.log(frame["dbc_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "dbc_return_5d",
        "uup_return_5d",
        "commodity_dollar_pressure_5d",
        "commodity_dollar_pressure_z126",
        "commodity_dollar_pressure_abs_percentile252",
        "dbc_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "dbc_close": float(row["dbc_close"]),
        "uup_close": float(row["uup_close"]),
        "dbc_return_5d": float(row["dbc_return_5d"]),
        "uup_return_5d": float(row["uup_return_5d"]),
        "commodity_dollar_pressure_5d": float(row["commodity_dollar_pressure_5d"]),
        "commodity_dollar_pressure_z126": float(row["commodity_dollar_pressure_z126"]),
        "commodity_dollar_pressure_abs_percentile252": float(
            row["commodity_dollar_pressure_abs_percentile252"]
        ),
        "dbc_volume_z126": float(row["dbc_volume_z126"]),
