...
class H1GldSpySafeHavenRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using GLD/SPY rotation."""

    name = "h1_gld_spy_safe_haven_rotation_followthrough_v0"
    version = "0.1-research-disabled"

    risk_reward = 1.50
    rotation_threshold = 0.0120
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        qqq_spy = data_context.get(QQQ_SPY_GROWTH_ROTATION_FRAME_KEY)
        if not isinstance(gld_flow, pd.DataFrame) or not isinstance(qqq_spy, pd.DataFrame):
            raise ConfigError(
                "h1_gld_spy_safe_haven_rotation_followthrough_v0 requires "
                "data_context['gld_etf_flow'] and data_context['qqq_spy_growth_rotation'] "
                "with shifted GLD/SPY daily observations."
            )
        ...
        rotation_features = _gld_spy_rotation_features_for_h1(h1, gld_flow, qqq_spy)
        h1 = pd.concat(
            [
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_GLD_SPY_SAFE_HAVEN_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported GLD/SPY safe-haven rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid GLD/SPY safe-haven rotation trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["h1_return_12"],
            row["h1_return_24"],
            row["gld_spy_rotation_5d"],
            row["gld_spy_rotation_z126"],
            row["gld_spy_rotation_abs_percentile252"],
        )
        ...
        rotation = float(row["gld_spy_rotation_5d"])
        rotation_z = float(row["gld_spy_rotation_z126"])
        rotation_percentile = float(row["gld_spy_rotation_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _gld_spy_rotation_features_for_h1(
    h1: pd.DataFrame,
    gld_flow: pd.DataFrame,
    qqq_spy: pd.DataFrame,
) -> pd.DataFrame:
    gld = gld_flow[["timestamp_utc", "close", "volume"]].copy()
    gld["timestamp_utc"] = pd.to_datetime(gld["timestamp_utc"], utc=True, errors="coerce")
    gld["gld_close"] = pd.to_numeric(gld["close"], errors="coerce")
    gld["gld_volume"] = pd.to_numeric(gld["volume"], errors="coerce")
    gld = gld.drop(columns=["close", "volume"]).dropna(subset=["timestamp_utc", "gld_close"])

    spy = qqq_spy[["timestamp_utc", "spy_close", "spy_volume"]].copy()
    spy["timestamp_utc"] = pd.to_datetime(spy["timestamp_utc"], utc=True, errors="coerce")
    spy["spy_close"] = pd.to_numeric(spy["spy_close"], errors="coerce")
    spy["spy_volume"] = pd.to_numeric(spy["spy_volume"], errors="coerce")
    spy = spy.dropna(subset=["timestamp_utc", "spy_close"])

    frame = pd.merge(
        gld.sort_values("timestamp_utc"),
        spy.sort_values("timestamp_utc"),
        on="timestamp_utc",
        how="inner",
    )
    ...
    frame["gld_spy_rotation_5d"] = frame["gld_return_5d"] - frame["spy_return_5d"]
    frame["gld_spy_rotation_z126"] = _rolling_zscore(frame["gld_spy_rotation_5d"], 126)
    frame["gld_spy_rotation_abs_percentile252"] = _rolling_percentile(
        frame["gld_spy_rotation_5d"].abs(),
        252,
    )
    frame["gld_volume_z126"] = _rolling_zscore(np.log(frame["gld_volume"].replace(0.0, np.nan)), 126)
    ...
    feature_columns = [
        ...
        "spy_return_5d",
        "gld_spy_rotation_5d",
        "gld_spy_rotation_z126",
        "gld_spy_rotation_abs_percentile252",
        "gld_volume_z126",
        "spy_volume_z126",
    ]
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "gld_spy_rotation_5d": float(row["gld_spy_rotation_5d"]),
        "gld_spy_rotation_z126": float(row["gld_spy_rotation_z126"]),
        "gld_spy_rotation_abs_percentile252": float(row["gld_spy_rotation_abs_percentile252"]),
