...
class H1FxyUupSafeHavenFxRotationFollowthroughV0Strategy(StrategyBase):
    """Research-only H1 XAU follow-through candidate using FXY/UUP safe-haven FX rotation."""

    name = "h1_fxy_uup_safe_haven_fx_rotation_followthrough_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        rotation = data_context.get(FXY_UUP_SAFE_HAVEN_FX_ROTATION_FRAME_KEY)
        if not isinstance(rotation, pd.DataFrame):
            raise ConfigError(
                "h1_fxy_uup_safe_haven_fx_rotation_followthrough_v0 requires "
                "data_context['fxy_uup_safe_haven_fx_rotation'] with shifted FXY/UUP daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_FXY_UUP_SAFE_HAVEN_FX_ROTATION_FOLLOWTHROUGH_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported FXY/UUP safe-haven FX rotation direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid FXY/UUP safe-haven FX rotation trade plan risk.")
...
def _safe_haven_fx_rotation_features_for_h1(h1: pd.DataFrame, rotation: pd.DataFrame) -> pd.DataFrame:
    frame = rotation[["timestamp_utc", "fxy_close", "fxy_volume", "uup_close", "uup_volume"]].copy()
    ...
    frame["fxy_return_5d"] = np.log(frame["fxy_close"] / frame["fxy_close"].shift(5))
    frame["uup_return_5d"] = np.log(frame["uup_close"] / frame["uup_close"].shift(5))
    frame["safe_haven_fx_rotation_5d"] = frame["fxy_return_5d"] - frame["uup_return_5d"]
    ...
    frame["fxy_volume_z126"] = _rolling_zscore(np.log(frame["fxy_volume"].replace(0.0, np.nan)), 126)
...
def _setup_metadata(row: pd.Series, direction: str, estimated_entry: float, close_location: float) -> dict[str, Any]:
    return {
        ...
        "fxy_close": float(row["fxy_close"]),
        "uup_close": float(row["uup_close"]),
        "fxy_return_5d": float(row["fxy_return_5d"]),
        ...
        "fxy_volume_z126": float(row["fxy_volume_z126"]),
