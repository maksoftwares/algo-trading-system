...
class H1TltUupPressureReversionV0Strategy(StrategyBase):
    """Research-only H1 XAU reversion candidate using TLT/UUP market-pressure proxies."""

    name = "h1_tlt_uup_pressure_reversion_v0"
    ...
    def prepare_features(self, data_context: dict[str, Any]) -> dict[str, Any]:
        ...
        pressure = data_context.get(TLT_UUP_PRESSURE_FRAME_KEY)
        if not isinstance(pressure, pd.DataFrame):
            raise ConfigError(
                "h1_tlt_uup_pressure_reversion_v0 requires "
                "data_context['tlt_uup_macro_pressure'] with shifted TLT/UUP daily observations."
            )
        ...
        for position in range(180, len(h1)):
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    direction=direction,
                    reason_code=f"H1_TLT_UUP_PRESSURE_REVERSION_V0_{direction}",
    ...
    def build_trade_plan(self, signal: Signal, data_context: dict[str, Any]) -> TradePlan:
        ...
        if direction == "LONG":
            ...
            raise ConfigError(f"Unsupported TLT/UUP pressure direction {signal.direction!r}.")

        if risk_price <= 0:
            raise ConfigError("Invalid TLT/UUP pressure reversion trade plan risk.")
    ...
    def _setup_at_row(self, row: pd.Series) -> dict[str, Any] | None:
        ...
        required = (
            ...
            row["tlt_return_5d"],
            row["uup_return_5d"],
            row["gold_pressure_5d"],
            row["gold_pressure_z126"],
            row["gold_pressure_abs_percentile252"],
        )
        ...
        pressure = float(row["gold_pressure_5d"])
        pressure_z = float(row["gold_pressure_z126"])
        pressure_percentile = float(row["gold_pressure_abs_percentile252"])
        if h1_atr <= 0:
            return None
...
def _pressure_features_for_h1(h1: pd.DataFrame, pressure: pd.DataFrame) -> pd.DataFrame:
    frame = pressure[["timestamp_utc", "tlt_close", "tlt_volume", "uup_close", "uup_volume"]].copy()
    ...
    frame["gold_pressure_5d"] = frame["tlt_return_5d"] - frame["uup_return_5d"]
    frame["gold_pressure_z126"] = _rolling_zscore(frame["gold_pressure_5d"], 126)
    frame["gold_pressure_abs_percentile252"] = _rolling_percentile(frame["gold_pressure_5d"].abs(), 252)
    ...
    feature_columns = [
        ...
        "gold_pressure_5d",
        "gold_pressure_z126",
        "gold_pressure_abs_percentile252",
        "tlt_volume_z126",
        "uup_volume_z126",
    ]
...
def _setup_metadata(
    ...
    return {
        ...
        "gold_pressure_5d": float(row["gold_pressure_5d"]),
        "gold_pressure_z126": float(row["gold_pressure_z126"]),
        "gold_pressure_abs_percentile252": float(row["gold_pressure_abs_percentile252"]),
