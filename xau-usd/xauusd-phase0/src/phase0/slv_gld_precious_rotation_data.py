...
def load_slv_gld_precious_rotation_context(
    config: ProjectConfig,
    required_start: object,
    required_end: object,
) -> pd.DataFrame:
    path = config.root / SLV_GLD_PRECIOUS_ROTATION_REFERENCE_PATH
    if not path.exists():
        raise ConfigError(
            f"{EXPERT_NAME} requires {path}. Run scripts/acquire_slv_gld_precious_rotation_proxy.py "
            "before any real matrix run. This is a public SLV/GLD ETF daily OHLCV proxy, "
            "not primary COMEX silver/gold futures order-flow data."
        )
    try:
        frame = pd.read_csv(path)
    except Exception as exc:
        raise ConfigError(f"Failed to read {EXPERT_NAME} SLV/GLD proxy file {path}: {exc}") from exc
    ...
    if missing:
        raise ConfigError(f"{path} missing required SLV/GLD column(s): {', '.join(sorted(missing))}.")
    ...
    if frame.empty:
        raise ConfigError(f"{path} has no usable SLV/GLD rows.")
...
def _assert_coverage(
    ...
    if coverage_start > needed_start and coverage_start - needed_start > allowed_start_gap:
        raise ConfigError(
            f"{EXPERT_NAME} SLV/GLD data in {source} start {coverage_start.isoformat()}, "
            f"but required {needed_start.isoformat()}."
        )
    if coverage_end < needed_end and needed_end - coverage_end > allowed_end_gap:
        raise ConfigError(
            f"{EXPERT_NAME} SLV/GLD data in {source} end {coverage_end.isoformat()}, "
