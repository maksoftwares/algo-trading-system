...
def test_h1_financial_conditions_shock_followthrough_synthetic_smoke():
    strategy = get_strategy(
        "h1_financial_conditions_shock_followthrough_v0",
        allow_research_candidate=True,
    )
    context = synthetic_context_for_expert("h1_financial_conditions_shock_followthrough_v0")

    signals = strategy.generate_signals(context)

    assert signals
    assert signals[-1].expert == "h1_financial_conditions_shock_followthrough_v0"
