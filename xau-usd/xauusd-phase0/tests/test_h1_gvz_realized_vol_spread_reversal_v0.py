...
def test_h1_gvz_realized_vol_spread_reversal_synthetic_smoke():
    strategy = get_strategy("h1_gvz_realized_vol_spread_reversal_v0", allow_research_candidate=True)
    context = synthetic_context_for_expert("h1_gvz_realized_vol_spread_reversal_v0")

    signals = strategy.generate_signals(context)

    assert signals
    assert signals[-1].expert == "h1_gvz_realized_vol_spread_reversal_v0"
