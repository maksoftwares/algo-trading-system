...
def test_h1_move_vix_bond_vol_shock_reversal_generates_synthetic_trade_plan() -> None:
    strategy = get_strategy(
        "h1_move_vix_bond_vol_shock_reversal_v0",
        allow_research_candidate=True,
    )
    context = synthetic_context_for_expert("h1_move_vix_bond_vol_shock_reversal_v0")

    signals = strategy.generate_signals(context)

    assert signals
    signal = signals[-1]
    assert signal.expert == "h1_move_vix_bond_vol_shock_reversal_v0"
