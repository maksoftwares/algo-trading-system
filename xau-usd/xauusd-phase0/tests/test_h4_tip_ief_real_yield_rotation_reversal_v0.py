...
def test_h4_tip_ief_real_yield_rotation_reversal_v0_generates_synthetic_trade_plan() -> None:
    strategy = get_strategy(
        "h4_tip_ief_real_yield_rotation_reversal_v0",
        allow_research_candidate=True,
    )
    context = synthetic_context_for_expert("h4_tip_ief_real_yield_rotation_reversal_v0")

    signals = strategy.generate_signals(context)

    assert signals
    assert signals[-1].expert == "h4_tip_ief_real_yield_rotation_reversal_v0"
    assert signals[-1].direction == "LONG"
    assert signals[-1].metadata["real_yield_rotation_5d"] >= 0.0035
    assert signals[-1].metadata["close_location"] >= 0.60

    plan = strategy.build_trade_plan(signals[-1], context)

    assert plan.expert == "h4_tip_ief_real_yield_rotation_reversal_v0"
