...
def test_h4_gvz_vix_vol_premium_reversal_v0_generates_synthetic_trade_plan() -> None:
    strategy = get_strategy(
        "h4_gvz_vix_vol_premium_reversal_v0",
        allow_research_candidate=True,
    )
    context = synthetic_context_for_expert("h4_gvz_vix_vol_premium_reversal_v0")

    signals = strategy.generate_signals(context)

    assert signals
    assert signals[-1].expert == "h4_gvz_vix_vol_premium_reversal_v0"
    ...
    assert plan.expert == "h4_gvz_vix_vol_premium_reversal_v0"
