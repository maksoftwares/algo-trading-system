...
def test_h1_gdx_gld_trend_confirmation_smoke_signal() -> None:
    strategy = H1GdxGldTrendConfirmationV0Strategy()

    signals = strategy.generate_signals(synthetic_context_for_expert("h1_gdx_gld_trend_confirmation_v0"))

    assert len(signals) == 1
    assert signals[0].expert == "h1_gdx_gld_trend_confirmation_v0"
    assert signals[0].direction == "LONG"
    assert signals[0].metadata["miner_relative_return_5d"] >= strategy.relative_return_5d_threshold
    assert signals[0].metadata["miner_relative_z126"] >= strategy.relative_z_threshold


def test_h1_gdx_gld_trend_confirmation_builds_valid_plan() -> None:
    strategy = H1GdxGldTrendConfirmationV0Strategy()
    context = synthetic_context_for_expert("h1_gdx_gld_trend_confirmation_v0")
...
def test_h1_gdx_gld_trend_confirmation_requires_context() -> None:
    strategy = H1GdxGldTrendConfirmationV0Strategy()
    context = synthetic_context_for_expert("h1_gdx_gld_trend_confirmation_v0")
...
def test_h1_gdx_gld_trend_confirmation_no_unshifted_same_day_data() -> None:
    strategy = H1GdxGldTrendConfirmationV0Strategy()
    context = synthetic_context_for_expert("h1_gdx_gld_trend_confirmation_v0")
