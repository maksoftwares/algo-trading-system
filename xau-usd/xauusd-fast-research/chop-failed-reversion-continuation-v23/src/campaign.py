...
SOURCE_MECHANICS = {
    "CHOP_M15_VARIANCE_RATIO_EXTREME_CONTINUATION": "VARIANCE_RATIO_EXTREME",
    "CHOP_M15_COUNTERFLOW_EXTREME_CONTINUATION": "COUNTERFLOW_EXTREME",
    "CHOP_M15_Z_EXPANSION_CONTINUATION": "Z_EXPANSION",
    "CHOP_M15_PERSISTENT_EXTREME_CONTINUATION": "PERSISTENT_EXTREME",
    "CHOP_M15_STATIONARITY_BREAK_CONTINUATION": "STATIONARITY_BREAK",
}
    ...
    while len(result) < count:
        ...
        for name, items in options.items():
            digest = hashlib.sha256(
                f"chop-failed-reversion-continuation-v23|{mechanic}|{nonce}|{name}".encode(
                    "ascii"
                )
            ).digest()
...
def parameter_space(mechanic: str, candidate_count: int = 400) -> list[dict[str, Any]]:
    ...
    if source == "VARIANCE_RATIO_EXTREME":
        options = dict(
            stationarity_window=(192, 384, 768),
            variance_horizon=(4, 16),
            variance_ratio_max=(0.70, 0.90, 1.10, 1.30),
            z_abs_min=(0.75, 1.00, 1.25, 1.50),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            return_acf_max=(-0.10, 0.0, 0.10, 0.20),
            m5_alignment_min=(-0.03, 0.0, 0.03),
            flow_alignment_min=(-0.03, 0.0, 0.01),
        )
    elif source == "COUNTERFLOW_EXTREME":
        options = dict(
            stationarity_window=(192, 384, 768),
            variance_horizon=(4, 16),
            variance_ratio_max=(0.70, 0.90, 1.10, 1.30),
            ...
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            return_acf_max=(-0.10, 0.0, 0.10, 0.20),
            counter_move_max=(0.0, 0.03, 0.06),
            counter_flow_max=(-0.03, -0.01, 0.0),
        )
    elif source == "Z_EXPANSION":
        options = dict(
            stationarity_window=(192, 384, 768),
            z_abs_min=(0.75, 1.00, 1.25, 1.50),
            z_delta_min=(0.0, 0.10, 0.25, 0.50),
            variance_ratio_max=(0.90, 1.10, 1.30),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            m5_alignment_min=(-0.03, 0.0, 0.03),
            flow_alignment_min=(-0.03, 0.0, 0.01),
        )
    elif source == "PERSISTENT_EXTREME":
        options = dict(
            stationarity_window=(192, 384, 768),
            z_abs_min=(0.75, 1.00, 1.25, 1.50),
            lag_bars=(1, 2, 4),
            lag_z_abs_min=(0.50, 0.75, 1.00),
            variance_ratio_max=(0.90, 1.10, 1.30),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            m5_alignment_min=(-0.03, 0.0, 0.03),
            flow_alignment_min=(-0.03, 0.0, 0.01),
        )
    elif source == "STATIONARITY_BREAK":
        options = dict(
            stationarity_window=(192, 384, 768),
            z_abs_min=(0.75, 1.00, 1.25),
            variance_ratio_min=(0.70, 0.90, 1.10),
            variance_ratio_delta_min=(0.0, 0.05, 0.10),
            hurst_min=(0.30, 0.45, 0.60),
            return_acf_min=(-0.10, 0.0, 0.10),
            m5_alignment_min=(-0.03, 0.0, 0.03),
            flow_alignment_min=(-0.03, 0.0, 0.01),
        )
    else:
...
def signal_mask_direction(
    ...
    if source == "VARIANCE_RATIO_EXTREME":
        ...
        z = frame[f"z_{window}"]
        lag_z = frame[f"z_lag_{lag}_{window}"]
        direction = pd.Series(
            np.sign(z.fillna(0.0)).astype(int), index=frame.index
        )
        mask = (
            z.abs().ge(float(params["z_abs_min"]))
            & lag_z.abs().ge(float(params["lag_z_abs_min"]))
            & z.mul(lag_z).gt(0.0)
            & frame[f"variance_ratio_4_{window}"].le(
                float(params["variance_ratio_max"])
            )
        ...
        mask = (
            z.abs().ge(float(params["z_abs_min"]))
            & variance_ratio.ge(float(params["variance_ratio_min"]))
            & frame[f"variance_ratio_delta_4_{window}"].ge(
                float(params["variance_ratio_delta_min"])
            )
            & frame[f"hurst_{window}"].ge(float(params["hurst_min"]))
            & frame[f"return_acf_1_{window}"].ge(
                float(params["return_acf_min"])
            )
        )
        confirmation = (
            direction.mul(m5_move).ge(float(params["m5_alignment_min"]))
