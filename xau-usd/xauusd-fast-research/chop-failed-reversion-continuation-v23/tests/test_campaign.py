...
def _variance_params() -> dict[str, object]:
    return {
...
def test_deterministic_design_has_1000_unique_rows() -> None:
    rows = parameter_space(
        "CHOP_M15_VARIANCE_RATIO_EXTREME_CONTINUATION", 1000
    )
    assert len(rows) == 1000
    assert len({repr(sorted(row.items())) for row in rows}) == 1000


def test_variance_signal_is_restricted_to_chop() -> None:
    mask, direction = signal_mask_direction(
        _signal_frame(),
        "CHOP_M15_VARIANCE_RATIO_EXTREME_CONTINUATION",
        _variance_params(),
    )
...
def test_counterflow_signal_trades_against_attempted_fade() -> None:
    ...
    params = {
        **_variance_params(),
        "counter_move_max": 0.0,
        "counter_flow_max": -0.01,
    }
    params.pop("m5_alignment_min")
    params.pop("flow_alignment_min")
    mask, direction = signal_mask_direction(
        frame, "CHOP_M15_COUNTERFLOW_EXTREME_CONTINUATION", params
    )
    assert mask.tolist() == [False, True, True, True, False, True]
    assert direction.loc[mask].eq(1).all()
