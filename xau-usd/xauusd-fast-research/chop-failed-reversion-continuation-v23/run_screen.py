...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Failed-Reversion Continuation V23 Result",
        "",
    ...
    lines.extend(
        (
            ...
            "Failed-reversion diagnostics use completed M15 bars and H1 risk.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_failed_reversion_continuation_v23.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V23 outcomes already exist")
    ...
    decision = (
        "CHOP_FAILED_REVERSION_CONTINUATION_V23_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_FAILED_REVERSION_CONTINUATION_V23_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_FAILED_REVERSION_CONTINUATION_V23_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_failed_reversion_v23_artifacts",
            "files": {
                name: {
