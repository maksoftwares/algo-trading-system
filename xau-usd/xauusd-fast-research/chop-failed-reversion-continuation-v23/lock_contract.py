...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/chop_failed_reversion_continuation_v23.json",
    "src/__init__.py",
    "src/campaign.py",
    "prepare_manifest.py",
    "lock_contract.py",
...
def main() -> int:
    config = json.loads(
        (ROOT / "config" / "chop_failed_reversion_continuation_v23.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if lock_path.exists():
        raise FileExistsError("V23 contract lock already exists")
    payload: dict[str, Any] = {
        "schema_version": "xauusd_chop_failed_reversion_v23_contract",
