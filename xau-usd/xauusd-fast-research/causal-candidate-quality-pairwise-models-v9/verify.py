...
from src.adaptive_models import build_result_comparison
...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != lock["definition_contract_sha256"]:
        raise ValueError("Pairwise V9 manifest references a different contract")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Verified Pairwise V9 base method contract changed")
    ...
    if generated_interactions != interactions:
        raise ValueError("Verified Pairwise V9 interaction order changed")
    source = add_event_target(source)
    pair_source = build_pairwise_rows(
        source,
    ...
    if inventory != {
        ...
        raise ValueError("Pairwise V9 feature inventory changed")
    expected_target_audit = pairwise_target_audit(source, pair_source)
    stored_target_audit = json.loads(
        (output / config["outputs"]["target_audit"]).read_text(encoding="utf-8")
    )
    if stored_target_audit != expected_target_audit:
        raise ValueError("Pairwise V9 target audit changed")
    source_lookup = source.set_index("candidate_id", drop=False)
    pair_source_lookup = pair_source.set_index("pair_id", drop=False)
    ...
    if comparison_v5 != build_result_comparison(result, reference_result):
        raise ValueError("Pairwise V9 versus Adaptive V5 comparison replay mismatch")
    ...
    if len(policies) != int(config["expected"]["calibration_policy_rows"]):
        raise ValueError("Pairwise V9 calibration policy count changed")
    ...
    if pair_predictions["pair_id"].duplicated().any():
        raise ValueError("Out-of-time pair predictions are duplicated")
    ...
    for lane in lanes:
        for fold_id in folds:
            ...
            pair_fit = partition_for(
                pair_source, splits, fold_id=fold_id, lane=lane, partition="FIT"
            )
            ...
            for variant_id, variant in variants.items():
                ...
                best = choose_pairwise_action(scored)
                for quantile in config["retention_quantiles"]:
                    threshold = float(best["event_score"].quantile(float(quantile)))
                    selected_cal = best.loc[best["event_score"].ge(threshold)]
                    fraction = float(
                    ...
                    if len(stored_policy) != 1:
                        raise ValueError("Pairwise V9 calibration policy key mismatch")
                    ...
                    if bool(stored_row["calibration_gate_pass"]) != bool(
                        ...
                        raise ValueError("Pairwise V9 calibration gate replay mismatch")
                    if (
                        json.loads(stored_row["fit_metadata_json"])
                        != model.fit_metadata
                    ):
                        raise ValueError("Pairwise V9 fit metadata replay mismatch")
            ...
            if len(stored_chosen) != 1:
                raise ValueError("Pairwise V9 chosen policy key mismatch")
            ...
            if (
                ...
                raise ValueError("Pairwise V9 chosen policy replay mismatch")
    ...
    for policy in chosen.itertuples(index=False):
        ...
        stored_pairs = pair_predictions.loc[
            pair_predictions["model_lane"].eq(policy.model_lane)
            & pair_predictions["fold_id"].eq(policy.fold_id)
        ].sort_values("pair_id", kind="mergesort")
        replay_source = source_lookup.loc[stored["candidate_id"]].reset_index(drop=True)
        replay_pair_source = pair_source_lookup.loc[
            stored_pairs["pair_id"]
        ...
        if replay_pair_source["pair_id"].tolist() != stored_pairs["pair_id"].tolist():
            raise ValueError(f"Replay pair order mismatch: {name}")
        replay, replay_pairs = score_pairwise_model(
            model,
            replay_source,
            replay_pair_source,
            event_features=event_features,
        ...
        replay = replay.sort_values("candidate_id", kind="mergesort")
        replay_pairs = replay_pairs.sort_values("pair_id", kind="mergesort")
        for column in (
            "event_score",
            ...
            if not np.allclose(
                replay[column].to_numpy(),
                stored[column].to_numpy(),
                rtol=1e-12,
                atol=1e-12,
            ):
                raise ValueError(f"Serialized {column} replay mismatch: {name}")
        if not np.allclose(
            replay_pairs["left_win_probability"].to_numpy(),
            stored_pairs["left_win_probability"].to_numpy(),
            rtol=1e-12,
            atol=1e-12,
        ):
            raise ValueError(f"Serialized pair-probability replay mismatch: {name}")
        refitted = refitted_models[
            (policy.model_lane, policy.fold_id, policy.variant_id)
        ]
        refitted_scores, refitted_pairs = score_pairwise_model(
            refitted,
            replay_source,
            replay_pair_source,
            event_features=event_features,
        ...
        refitted_scores = refitted_scores.sort_values(
            "candidate_id", kind="mergesort"
        )
        refitted_pairs = refitted_pairs.sort_values("pair_id", kind="mergesort")
        for column in (
            "event_score",
            "pairwise_borda_score",
            "action_advantage_score",
            ...
            if not np.allclose(
                refitted_scores[column].to_numpy(),
                stored[column].to_numpy(),
                rtol=1e-12,
                atol=1e-12,
            ):
                raise ValueError(f"Independent {column} refit mismatch: {name}")
        if not np.allclose(
            refitted_pairs["left_win_probability"].to_numpy(),
            stored_pairs["left_win_probability"].to_numpy(),
            rtol=1e-12,
            atol=1e-12,
        ):
            raise ValueError(f"Independent pair-probability refit mismatch: {name}")
        best = choose_pairwise_action(refitted_scores)
        ...
        expected_selected = set(
            best.loc[
                best["event_score"].ge(float(policy.score_threshold)), "candidate_id"
            ]
        )
    ...
    print(
        json.dumps(
            {
                "decision": "PAIRWISE_V9_VERIFICATION_PASS",
                ...
                "constituent_stage_models_independently_refit": 2
                * len(refitted_models),
                "calibration_policies_replayed": len(policies),
                "prediction_rows": len(predictions),
                "pair_prediction_rows": len(pair_predictions),
