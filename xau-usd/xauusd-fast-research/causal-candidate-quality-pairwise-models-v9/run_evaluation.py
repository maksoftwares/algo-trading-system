...
from src.pairwise import (
    ...
    chosen_action_outcome_auc,
    choose_pairwise_action,
    event_tradeability_auc,
    build_pairwise_rows,
    fit_pairwise_model,
    pairwise_feature_columns,
    pairwise_target_audit,
    pairwise_test_auc,
    score_pairwise_model,
)
...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Pairwise V9 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Pairwise V9 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Pairwise V9 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Pairwise V9 implementation changed after lock: {name}")
...
def policy_rank(row: dict[str, Any], variant_order: dict[str, int]) -> tuple[Any, ...]:
    return (
...
def incremental_checks(
    metrics: dict[str, Any], reference: dict[str, Any], gates: dict[str, Any]
) -> dict[str, bool]:
    selected_pf = metrics["selected"]["weighted_profit_factor"]
    reference_pf = reference["selected"]["weighted_profit_factor"]
    reference_frequency = reference["selected"]["events_per_weekday"]
    frequency_ratio = (
        metrics["selected"]["events_per_weekday"] / reference_frequency
        if reference_frequency > 0.0
        else 0.0
    )
    return {
        "incremental_selected_mean_stress_r": (
            metrics["selected"]["weighted_mean_stress_r"]
            - reference["selected"]["weighted_mean_stress_r"]
            >= float(gates["minimum_selected_mean_stress_r_delta"])
        ),
        "incremental_selected_profit_factor": (
            selected_pf is not None
            and reference_pf is not None
            and float(selected_pf) - float(reference_pf)
            >= float(gates["minimum_selected_profit_factor_delta"])
        ),
        "incremental_frequency_retention": frequency_ratio
        >= float(gates["minimum_selected_events_per_weekday_ratio"]),
        "incremental_latest_fold_mean_stress_r": (
            metrics["latest_fold"]["selected_weighted_mean_stress_r"]
            - reference["latest_fold"]["selected_weighted_mean_stress_r"]
            >= float(gates["minimum_latest_fold_mean_stress_r_delta"])
        ),
    }
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Causal Pairwise Models V9 Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Pairwise V9 evaluation")
    ...
    reference_result = json.loads(
        inputs["reference_adaptive_v5_result"].read_text(encoding="utf-8")
    )
    base_method_hash = assert_adaptive_v5_population_parity(
        config, reference_config
    )
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Pairwise V9 base method contract changed after lock")
    base_features = list(v4_config["model_features"])
    if canonical_json_sha256(base_features) != str(
        config["expected"]["base_model_feature_sha256"]
    ):
        raise ValueError("Pairwise V9 base feature surface changed")
    event_features = event_feature_columns(base_features)
    expected_interactions = interaction_feature_columns(base_features)
    action_features = [*base_features, *expected_interactions]
    pair_features = pairwise_feature_columns(action_features)
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Pairwise V9 feature hash is inconsistent")
    if canonical_json_sha256(pair_features) != str(
        config["expected"]["pairwise_feature_sha256"]
    ):
        raise ValueError("Pairwise V9 pair feature surface changed")
    dataset = prepare_dataset(
        pd.read_parquet(inputs["v4_action_dataset"]), config, base_features
    )
    dataset, generated_interactions = add_action_interactions(dataset, base_features)
    if generated_interactions != expected_interactions:
        raise ValueError("Pairwise V9 generated interaction order changed")
    dataset = add_event_target(dataset)
    pair_dataset = build_pairwise_rows(
        dataset,
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("Pairwise V9 split fold IDs changed")
    ...
    for lane in lanes:
        for fold_id in fold_ids:
            ...
            pair_fit = partition_for(
                pair_dataset, splits, fold_id=fold_id, lane=lane, partition="FIT"
            )
            pair_calibration = partition_for(
                pair_dataset,
                splits,
                fold_id=fold_id,
                lane=lane,
            ...
            pair_test = partition_for(
                pair_dataset, splits, fold_id=fold_id, lane=lane, partition="TEST"
            )
            ...
            fit_boundary = windows[fold_id]["CALIBRATION"][0]
            for variant in config["training_variants"]:
                ...
                fitted_models[variant_id] = model
                scored_cal, _ = score_pairwise_model(
                    model,
                    calibration,
                    pair_calibration,
                    event_features=event_features,
                ...
                best_cal = choose_pairwise_action(scored_cal)
                for quantile in config["retention_quantiles"]:
                    threshold = float(best_cal["event_score"].quantile(float(quantile)))
                    selected_cal = best_cal.loc[
                        best_cal["event_score"].ge(threshold)
                    ].copy()
                    ...
                    row = {
                        ...
                        "fit_metadata_json": json.dumps(
                            model.fit_metadata, sort_keys=True, separators=(",", ":")
                        ),
            ...
            model = fitted_models[variant_id]
            model_path = models_dir / f"{lane}_{fold_id}_{variant_id}.joblib"
            joblib.dump(model, model_path)
            model_paths.append(model_path)
            scored_test, scored_pair_test = score_pairwise_model(
                model,
                test,
                pair_test,
                event_features=event_features,
            ...
            best_test = choose_pairwise_action(scored_test)
            selected_test = best_test.loc[
                best_test["event_score"].ge(float(chosen["score_threshold"]))
            ].copy()
            ...
            fold_auc = event_tradeability_auc(scored_test)
            outcome_auc = chosen_action_outcome_auc(best_test)
            action_accuracy = action_choice_accuracy(scored_test, best_test)
            pair_auc = pairwise_test_auc(scored_pair_test)
            fold_metric_rows.append(
                {
                    ...
                    "fit_pair_rows": int(len(pair_fit)),
                    "calibration_pair_rows": int(len(pair_calibration)),
                    "test_pair_rows": int(len(pair_test)),
                    "test_action_weight_sum": float(test["structural_weight"].sum()),
                    "test_pair_weight_sum": float(pair_test["pair_weight"].sum()),
                    "weighted_test_auc": fold_auc,
                    "pairwise_test_auc": pair_auc,
                    "chosen_action_outcome_auc": outcome_auc,
                    "action_choice_accuracy": action_accuracy,
                    "selected_fraction": selected_fraction,
                    **flatten("selected", selected_metrics),
            ...
            scored_test["chosen_variant_id"] = variant_id
            scored_test["score_threshold"] = float(chosen["score_threshold"])
            scored_pair_test["fold_id"] = fold_id
            scored_pair_test["chosen_variant_id"] = variant_id
            pair_prediction_frames.append(
                scored_pair_test[
                    [
                        "fold_id",
                        "model_lane",
            ...
            selected_test["chosen_variant_id"] = variant_id
    ...
    policies = pd.DataFrame(policy_rows).sort_values(
        ["model_lane", "fold_id", "variant_id", "retention_quantile"],
        kind="mergesort",
    )
    predictions = pd.concat(prediction_frames, ignore_index=True)
    pair_predictions = pd.concat(pair_prediction_frames, ignore_index=True)
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        pair_auc_valid = folds_lane["pairwise_test_auc"].notna()
        weighted_pairwise_test_auc = (
            float(
                np.average(
                    folds_lane.loc[pair_auc_valid, "pairwise_test_auc"],
                    weights=folds_lane.loc[pair_auc_valid, "test_pair_weight_sum"],
                )
            )
            if pair_auc_valid.any()
            else None
        )
        ...
        metrics = {
            ...
            "weighted_pairwise_test_auc": weighted_pairwise_test_auc,
            "positive_folds": int(
        ...
        decision = (
            "PAIRWISE_MODEL_V9_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "PAIRWISE_MODEL_V9_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "PAIRWISE_MODEL_V9_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "PAIRWISE_MODEL_V9_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "PAIRWISE_MODEL_V9_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
        ...
        "chosen_variant_counts": {
            str(key): int(value)
            for key, value in fold_metrics["variant_id"].value_counts().items()
        },
        "event_stage_feature_count": len(event_features),
        "action_stage_feature_count": len(action_features),
        "pairwise_feature_count": len(pair_features),
        "base_feature_count": len(base_features),
        "event_feature_count": len(event_features),
        "interaction_feature_count": len(generated_interactions),
        "model_stages_per_serialized_policy": 2,
    ...
    paths = {
        ...
        "interaction_inventory": output / config["outputs"]["interaction_inventory"],
        "target_audit": output / config["outputs"]["target_audit"],
        ...
        "pair_predictions": output / config["outputs"]["pair_predictions"],
        ...
        "comparison_v5": output / config["outputs"]["comparison_v5"],
    }
    write_json(
        paths["interaction_inventory"],
        {
            "base_features": base_features,
            "event_features": event_features,
            "interaction_features": generated_interactions,
            "action_model_features": action_features,
            "pairwise_features": pair_features,
        },
    )
    write_json(
        paths["target_audit"], pairwise_target_audit(dataset, pair_dataset)
    )
    ...
    pair_predictions.to_parquet(paths["pair_predictions"], index=False)
    ...
    write_json(
        paths["comparison_v5"], build_result_comparison(result, reference_result)
    )
