...
from src.two_stage import (
    ...
    fit_two_stage_model,
    score_two_stage_model,
    two_stage_target_audit,
)
...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Two-Stage V8 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Two-Stage V8 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Two-Stage V8 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Two-Stage V8 implementation changed after lock: {name}")
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Causal Two-Stage Models V8 Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Two-Stage V8 evaluation")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Two-Stage V8 base method contract changed after lock")
    ...
    if canonical_json_sha256(base_features) != str(
        ...
        raise ValueError("Two-Stage V8 base feature surface changed")
    ...
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Two-Stage V8 feature hash is inconsistent")
    ...
    if generated_interactions != expected_interactions:
        raise ValueError("Two-Stage V8 generated interaction order changed")
    dataset = add_two_stage_targets(dataset, config)
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("Two-Stage V8 split fold IDs changed")
    ...
    for lane in lanes:
        for fold_id in fold_ids:
            ...
            for variant in config["training_variants"]:
                ...
                scored_cal = score_two_stage_model(
                    model,
                    calibration,
                    event_features=event_features,
                    action_features=action_features,
                )
                best_cal = choose_two_stage_action(scored_cal)
            ...
            scored_test = score_two_stage_model(
                model,
                test,
                event_features=event_features,
                action_features=action_features,
            )
            best_test = choose_two_stage_action(scored_test)
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        decision = (
            "TWO_STAGE_MODEL_V8_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "TWO_STAGE_MODEL_V8_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "TWO_STAGE_MODEL_V8_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "TWO_STAGE_MODEL_V8_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "TWO_STAGE_MODEL_V8_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
    ...
    write_json(paths["target_audit"], two_stage_target_audit(dataset))
