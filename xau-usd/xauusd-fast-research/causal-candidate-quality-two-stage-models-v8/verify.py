...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != lock["definition_contract_sha256"]:
        raise ValueError("Two-Stage V8 manifest references a different contract")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Verified Two-Stage V8 base method contract changed")
    ...
    if generated_interactions != interactions:
        raise ValueError("Verified Two-Stage V8 interaction order changed")
    source = add_two_stage_targets(source, config)
    ...
    if inventory != {
        ...
        raise ValueError("Two-Stage V8 interaction inventory changed")
    expected_target_audit = two_stage_target_audit(source)
    ...
    if stored_target_audit != expected_target_audit:
        raise ValueError("Two-Stage V8 target audit changed")
    ...
    if comparison_v5 != build_result_comparison(result, reference_result):
        raise ValueError("Two-Stage V8 versus Adaptive V5 comparison replay mismatch")
    ...
    if len(policies) != int(config["expected"]["calibration_policy_rows"]):
        raise ValueError("Two-Stage V8 calibration policy count changed")
    ...
    for lane in lanes:
        for fold_id in folds:
            ...
            for variant_id, variant in variants.items():
                ...
                scored = score_two_stage_model(
                    model,
                    calibration,
                    event_features=event_features,
                    action_features=action_features,
                )
                best = choose_two_stage_action(scored)
                for quantile in config["retention_quantiles"]:
                    ...
                    if len(stored_policy) != 1:
                        raise ValueError("Two-Stage V8 calibration policy key mismatch")
                    ...
                    if bool(stored_row["calibration_gate_pass"]) != bool(
                        ...
                        raise ValueError("Two-Stage V8 calibration gate replay mismatch")
                    if (
                        json.loads(stored_row["fit_metadata_json"])
                        != model.fit_metadata
                    ):
                        raise ValueError("Two-Stage V8 fit metadata replay mismatch")
            ...
            if len(stored_chosen) != 1:
                raise ValueError("Two-Stage V8 chosen policy key mismatch")
            ...
            if (
                ...
                raise ValueError("Two-Stage V8 chosen policy replay mismatch")
    ...
    for policy in chosen.itertuples(index=False):
        ...
        replay = score_two_stage_model(
            model,
            replay_source,
            event_features=event_features,
            action_features=action_features,
        ).sort_values("candidate_id", kind="mergesort")
        for column in ("event_score", "action_advantage_score", "model_score"):
            if not np.allclose(
        ...
        refitted_scores = score_two_stage_model(
            refitted,
            replay_source,
            event_features=event_features,
            action_features=action_features,
        ...
        best = choose_two_stage_action(refitted_scores)
    ...
    print(
        json.dumps(
            {
                "decision": "TWO_STAGE_V8_VERIFICATION_PASS",
