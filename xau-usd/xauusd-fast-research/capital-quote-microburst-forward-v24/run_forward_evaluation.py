...
def main() -> int:
    ...
    inventory = {
        "schema_version": "xauusd_v24_forward_inventory",
        "contract_sha256": contract["contract_sha256"],
    ...
    if validation_audit_path.exists():
        ...
        if len(validation_dates) < validation_days:
            print(
                json.dumps(
                    {
                        "decision": "V24_CONTINUE_SEALED_FORWARD_COLLECTION",
        ...
        validation_audit["decision"] = (
            "V24_FORWARD_VALIDATION_PASS_CONFIRMATION_REMAINS_SEALED"
            if validation_audit["gate_passed"]
            else "V24_FORWARD_VALIDATION_FAIL_TERMINAL"
        )
    ...
    if len(confirmation_dates) < confirmation_days:
        print(
            json.dumps(
                {
                    "decision": "V24_VALIDATION_PASSED_CONTINUE_SEALED_CONFIRMATION_COLLECTION",
    ...
    confirmation_audit["decision"] = (
        "V24_FORWARD_CONFIRMATION_PASS_RESEARCH_SHADOW_NOMINATION_ONLY"
        if confirmation_audit["gate_passed"]
        else "V24_FORWARD_CONFIRMATION_FAIL_TERMINAL"
    )
