...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop M15 Stationarity Campaign V21 Result",
        "",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_m15_stationarity_campaign_v21.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V21 outcomes already exist")
    ...
    decision = (
        "CHOP_M15_STATIONARITY_V21_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_M15_STATIONARITY_V21_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_M15_STATIONARITY_V21_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_m15_stationarity_v21_artifacts",
            "files": {
                name: {
