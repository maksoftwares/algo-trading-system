...
TARGET_FREQUENCIES = tuple(round(0.85 + 0.025 * index, 3) for index in range(10))
...
V89 = _load_module(
    "executable_event_nearmiss_ranker_v99_v89_metrics",
...
def parameter_space(mechanic: str) -> list[dict[str, Any]]:
    ...
    stop, target, hold = EXECUTION_PROFILE
    for model_id, event_pool_id, frequency in product(
        range(len(MODEL_SPECS)), range(len(EVENT_POOLS)), TARGET_FREQUENCIES
    ):
        rows.append(
            {
...
def _complete_m15(m5: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"V99 M5 source is missing columns: {missing}")
...
def _candidate_frame(m5: pd.DataFrame, config: Mapping[str, Any]) -> pd.DataFrame:
    ...
    if not rows:
        raise ValueError("V99 candidate protocol produced no events")
...
def generate_manifest(
    ...
    if raw_signals < minimum_raw_signals:
        raise ValueError("V99 event universe lacks preregistered Discovery density")
    ...
    for mechanic in MECHANICS:
        ...
        if len(policies) != policies_per_mechanic:
            raise ValueError(f"V99 policy count changed for {mechanic}")
    ...
    if len(manifest) != expected or manifest["policy_id"].duplicated().any():
        raise ValueError("Invalid V99 policy manifest")
    return manifest
...
def _calibrate_threshold(
    ...
    if target <= 0 or target >= len(candidates):
        return float(np.nextafter(candidates["score"].min(), -np.inf))
...
def _score_folds(
    ...
    for fold in config["model_protocol"]["folds"][stage]:
        ...
        if (
            ...
            raise ValueError(f"V99 fold lacks model support: {fold['fold_id']}")
...
def evaluate_policies(
    ...
    rows: list[dict[str, Any]] = []
    grouped: dict[tuple[tuple[float, float, int], str, int, int], list[Any]] = {}
    for policy in manifest.itertuples(index=False):
        ...
        key = (
            ...
            str(policy.mechanic),
            int(params["model_id"]),
            int(params["event_pool_id"]),
        )
        grouped.setdefault(key, []).append(policy)
    for (profile, feature_set, model_id, event_pool_id), policies in grouped.items():
        ...
        pooled_actions = actions.loc[
            actions["event_type"].isin(EVENT_POOLS[event_pool_id])
        ].copy()
        scored_folds = _score_folds(
            pooled_actions, config, stage, feature_set, model_id
        )
        for policy in policies:
            ...
            if not trades.empty:
                trades = trades.assign(
                    ...
                    event_pool_id=int(params["event_pool_id"]),
