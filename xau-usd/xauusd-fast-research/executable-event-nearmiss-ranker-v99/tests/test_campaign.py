...
def test_policy_registry_is_exactly_one_thousand() -> None:
    ...
    assert campaign.EXECUTION_PROFILE == (1.0, 1.5, 3)
    assert len(campaign.EVENT_POOLS) == 5
    assert campaign.TARGET_FREQUENCIES[0] == 0.85
    assert campaign.TARGET_FREQUENCIES[-1] == 1.075


def test_calibration_saturates_when_target_exceeds_support() -> None:
    start = pd.Timestamp("2024-01-01T00:00:00Z")
    end = pd.Timestamp("2024-02-01T00:00:00Z")
    scored = pd.DataFrame(
        {
            "signal_time": pd.date_range(start, periods=4, freq="D"),
            "score": [0.9, 0.8, 0.7, 0.6],
            "event_type": ["IMPULSE_RETEST"] * 4,
            "direction_value": [1, -1, 1, -1],
        }
    )
    threshold = campaign._calibrate_threshold(scored, start, end, 1.0)
    assert threshold < scored["score"].min()
...
def test_source_only_manifest_has_exact_attempt_registry() -> None:
    ...
    assert manifest["attempt_no"].tolist() == list(range(130001, 131001))
...
def test_config_fixes_ceiling_attempts_and_research_only_training() -> None:
    config = json.loads(
        (ROOT / "config" / "executable_event_nearmiss_ranker_v99.json").read_text()
    )
    controls = config["research_controls"]
    assert controls["attempt_first"] == 130001
    assert controls["attempt_last"] == 131000
...
def test_v99_requires_artifact_bound_terminal_v98_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    result_path = tmp_path / "V98_RESULT.json"
    manifest_path = tmp_path / "V98_ARTIFACT_MANIFEST.json"
    result = {
        "attempt_first": 129001,
        "attempt_last": 130000,
        "registered_policy_count": 1000,
        "contract_sha256": "locked-v98",
    ...
    monkeypatch.setattr(run_research, "V98_RESULT_PATH", result_path)
    monkeypatch.setattr(run_research, "V98_ARTIFACT_MANIFEST_PATH", manifest_path)
    evidence = run_research._verify_v98_terminal_failure()
    assert evidence["v98_terminal_reason"] == "V98_ENGINEERING_INVALIDATED_TERMINAL"

    result["decision"] = "V98_DISCOVERY_PASS_ADVANCE"
    result_path.write_text(json.dumps(result))
    with pytest.raises(RuntimeError):
        run_research._verify_v98_terminal_failure()
