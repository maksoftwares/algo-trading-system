...
from fx_consensus import (  # noqa: E402
...
STAGES = ("development", "confirmation", "validation")
...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"FX_CONSENSUS_XAU_V79_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V79 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V79 {stage} sealed because {prior} failed")
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V79 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if bool(quality["eligible_full_weekday"]):
            ...
            source_candidates = generate_candidates(
                features, policy=policy, family="V78_SOURCE_DIRECTION"
            )
        ...
        if number % 25 == 0:
            print(f"V79 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V79_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V79_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_fx_consensus_xau_v79_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V79 stage")
