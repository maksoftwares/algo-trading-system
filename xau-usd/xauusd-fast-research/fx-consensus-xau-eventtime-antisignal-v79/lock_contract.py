...
V78_ROOT = ROOT.parent / "fx-consensus-xau-eventtime-v78"
...
CONFIG = ROOT / "config" / "fx_consensus_xau_eventtime_antisignal_v79.json"
PACKAGE_FILES = (
    "README.md",
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/fx_consensus_xau_eventtime_antisignal_v79.json",
    "src/__init__.py",
    "src/antisignal.py",
    "lock_contract.py",
    "run_stage.py",
...
def verified_inheritance(config: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for key, hash_key in mapping.items():
        if sha256_file(paths[key]) != inherited[hash_key]:
            raise ValueError(f"V79 inherited V78 source changed: {paths[key]}")
    ...
    if canonical_hash(contract, "contract_sha256") != contract.get("contract_sha256"):
        raise ValueError("V78 inherited contract self-hash changed")
    config_record = next(
        (
            row
    ...
    if config_record is None:
        raise ValueError("V78 contract does not lock its source configuration")
    v78_config_path = V78_ROOT / str(config_record["path"])
    if sha256_file(v78_config_path) != config_record["sha256"]:
        raise ValueError("V78 locked source configuration changed")
    v78_config = load_json(v78_config_path)
    inherited_source = {
        key: v78_config["source"][key]
        for key in (
    ...
    if config["source"] != inherited_source:
        raise ValueError("V79 source semantics differ from V78")
    if config["spot_source"] != v78_config["spot_source"]:
        raise ValueError("V79 completed-M5 source semantics differ from V78")
    if (
        development.get("decision") != "V78_DEVELOPMENT_FAIL_TERMINAL"
        or canonical_hash(development, "audit_sha256") != development.get("audit_sha256")
    ):
        raise ValueError("V78 terminal development audit is invalid")
    ...
    if dates and max(dates) >= exposed_end:
        raise ValueError("V78 outcomes overlap V79 development")
    if pd.to_datetime(config["splits"]["development"][0], utc=True) != exposed_end:
        raise ValueError("V79 development does not begin at the fresh boundary")
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    v78, development = verified_inheritance(config)
    inherited_rule = dict(v78["candidate_rule"])
    inherited_rule["family"] = config["candidate_rule"]["family"]
    if inherited_rule != config["candidate_rule"]:
        raise ValueError("V79 changed a V78 candidate rule")
    if v78["execution"] != config["execution"]:
        raise ValueError("V79 execution differs from V78")
    if next(iter(v78["families"].values())) != next(iter(config["families"].values())):
        raise ValueError("V79 exit geometry differs from V78")
    if without_minimum_sample(v78["gates"]) != without_minimum_sample(config["gates"]):
        raise ValueError("V79 changed a V78 economic gate")
    for row in v78["dependencies"]:
        path = REPO_ROOT / str(row["path"])
        if sha256_file(path) != row["sha256"]:
            raise ValueError(f"V79 inherited dependency changed: {path}")
    ...
    if sha256_file(atr_path) != config["spot_source"]["m5_feature_sha256"]:
        raise ValueError("V79 completed-M5 ATR cache changed")
    contract: dict[str, Any] = {
        "schema_version": "xauusd_fx_consensus_xau_v79_contract_lock",
        ...
        "v78_source_audit": record(
            REPO_ROOT / str(config["inherited_v78"]["source_audit_path"]), REPO_ROOT
        ),
        "v78_development_audit": record(
            REPO_ROOT / str(config["inherited_v78"]["development_audit_path"]), REPO_ROOT
        ),
        "selected_policy": v78["selected_policy"],
        "v78_terminal_decision": development["decision"],
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V79 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V79 contract already exists")
    if list(output.glob("FX_CONSENSUS_XAU_V79_*_AUDIT.json")):
        raise ValueError("V79 stage outputs existed before lock")
