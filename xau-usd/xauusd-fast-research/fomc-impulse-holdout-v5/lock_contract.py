...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    paths = {
        ...
        "config/fomc_impulse_holdout_v5.json": CONFIG_PATH,
        "prepare_candidates.py": ROOT / "prepare_candidates.py",
        "lock_contract.py": ROOT / "lock_contract.py",
        "run_holdout.py": ROOT / "run_holdout.py",
        "tests/test_fomc_impulse_holdout_v5.py": ROOT
        / "tests"
        / "test_fomc_impulse_holdout_v5.py",
        ...
        "parent/config.json": parent
        / "config"
        / "event_reaction_corrected_v4.json",
        "parent/contract_lock": parent_output / "CORRECTED_EVENT_CONTRACT_LOCK.json",
        "parent/calendar": parent_output / "CORRECTED_EVENT_CALENDAR.csv",
        "parent/calendar_manifest": parent_output
        / "CORRECTED_EVENT_CALENDAR_MANIFEST.json",
        "parent/candidates": parent_output / "CORRECTED_EVENT_CANDIDATES.parquet",
        "parent/candidate_manifest": parent_output
        / "CORRECTED_EVENT_CANDIDATE_MANIFEST.json",
        "parent/historical_outcomes": parent_output
        / "CORRECTED_EVENT_HISTORICAL_REPLICATION_OUTCOMES.parquet",
        "parent/historical_metrics": parent_output
        / "CORRECTED_EVENT_HISTORICAL_REPLICATION_METRICS.csv",
        "parent/historical_result": parent_output
        / "CORRECTED_EVENT_HISTORICAL_REPLICATION_RESULT.json",
        "parent/historical_advancement": parent_output
        / "CORRECTED_EVENT_HISTORICAL_REPLICATION_ADVANCEMENT_LOCK.json",
        "parent/artifact_manifest": parent_output / "CORRECTED_EVENT_ARTIFACT_MANIFEST.json",
...
def main() -> int:
    ...
    if lock_path.exists() or any(output.glob("*_OUTCOMES_OPENED.json")):
        raise RuntimeError("FOMC impulse V5 was already locked or opened")
    ...
    if missing:
        raise FileNotFoundError(f"Missing FOMC V5 contract inputs: {missing}")
    ticks = [name for name in paths if name.startswith("external/raw/XAUUSD/year=")]
    if len(ticks) != 54:
        raise ValueError(f"Expected 54 holdout tick manifests, found {len(ticks)}")
    manifest = json.loads(
    ...
    if _sha256(output / config["outputs"]["candidates"]) != manifest[
        ...
        raise ValueError("FOMC V5 candidate manifest mismatch")
    policy = config["policy"]
    controls = config["research_controls"]
    attempt = int(policy["attempt_no"])
    if attempt != int(controls["campaign_attempts_before_v5"]) + 1:
        raise ValueError("FOMC V5 attempt number is not contiguous")
    if int(controls["registered_policy_count"]) != 1:
        raise ValueError("FOMC V5 must contain one registered policy")
    body = {
        "schema_version": "xauusd_fomc_impulse_holdout_contract_v5",
        ...
        "economic_and_deployment_gates_separated": True,
