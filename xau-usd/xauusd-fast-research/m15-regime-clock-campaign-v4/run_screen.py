...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD M15 Regime Clock Campaign V4 Result",
        "",
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_m15_regime_clock_v4_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V4 outcomes already exist; same-version reruns are forbidden")
    ...
    frame = prepare_clock_features(frame)
    arrays = CORRECTION.execution_arrays(frame)
    next_gaps = (arrays["starts"][1:] - arrays["signals"][:-1]) / 60_000_000_000
    if np.any(next_gaps < 0.0):
        raise ValueError("Negative next-bar gap after clock normalization")
    ...
    for offset, manifest_row in enumerate(manifest.itertuples(index=False), 1):
        trades = simulate_variant(
            frame, arrays, manifest_row, config["execution"], outcome_cache, TARGET
        )
    ...
    decision = (
        "FDR_SUPPORTED_RAW_TICK_FINALISTS_FOUND"
        if statistical_finalists > 0
        else (
            "ECONOMIC_ONLY_RAW_TICK_FINALISTS_FOUND"
            if not shortlist.empty
            else "NO_M15_REGIME_CLOCK_V4_ECONOMIC_FINALIST"
        )
    )
    result = {
        ...
        "clock_audit": {
            ...
            "pre_asia_information_rows": int(
                frame.loc[frame["hour"].lt(6), "asia_close"].notna().sum()
            ),
        },
        "data_evidence": {
            **evidence,
