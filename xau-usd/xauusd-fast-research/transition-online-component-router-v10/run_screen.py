...
SCORE = load_module(
    "transition_online_router_v10_score",
...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Transition Online Component Router V10 Result",
        "",
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V10 policy outcomes already exist")
    ...
    decision = (
        "TRANSITION_ONLINE_ROUTER_V10_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "TRANSITION_ONLINE_ROUTER_V10_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_TRANSITION_ONLINE_ROUTER_V10_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_transition_online_router_v10_artifacts",
            "files": {
                name: {
