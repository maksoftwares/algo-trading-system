...
from src.adaptive_models import assert_action_v4_parity
...
def main() -> int:
    ...
    base_method_hash = assert_action_v4_parity(config, reference)
    ...
    if feature_hash != expected_feature_hash:
        raise ValueError("Adaptive V5 model feature surface changed")
    shared_code_hash = sha256_file(ROOT / "src" / "action_models.py")
    if (
        shared_code_hash
        != config["adaptation_contract"]["expected_shared_model_code_sha256"]
    ):
        raise ValueError("Adaptive V5 shared model mechanics differ from Action V4")
