...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != lock["definition_contract_sha256"]:
        raise ValueError("Adaptive V5 manifest references a different contract")
    ...
    reference_config = json.loads(
        input_paths["reference_action_v4_config"].read_text(encoding="utf-8")
    )
    base_method_hash = assert_action_v4_parity(config, reference_config)
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Verified Adaptive V5 base method contract changed")
    ...
    comparison_v4 = json.loads(
        (output / config["outputs"]["comparison_v4"]).read_text(encoding="utf-8")
    )
    reference_result = json.loads(
        input_paths["reference_action_v4_result"].read_text(encoding="utf-8")
    )
    if comparison_v4 != build_result_comparison(result, reference_result):
        raise ValueError("Adaptive V5 versus Action V4 comparison replay mismatch")
    ...
    if len(policies) != int(config["expected"]["calibration_policy_rows"]):
        raise ValueError("Adaptive V5 calibration policy count changed")
    ...
    for lane in lanes:
        for fold_id in folds:
            ...
            for variant_id, variant in variants.items():
                ...
                for quantile in config["retention_quantiles"]:
                    ...
                    if len(stored_policy) != 1:
                        raise ValueError("Adaptive V5 calibration policy key mismatch")
                    ...
                    if bool(stored_row["calibration_gate_pass"]) != bool(
                        ...
                        raise ValueError("Adaptive V5 calibration gate replay mismatch")
                    if (
                        json.loads(stored_row["fit_metadata_json"])
                        != model.fit_metadata
                    ):
                        raise ValueError("Adaptive V5 fit metadata replay mismatch")
            ...
            if len(stored_chosen) != 1:
                raise ValueError("Adaptive V5 chosen policy key mismatch")
            ...
            if (
                ...
                raise ValueError("Adaptive V5 chosen policy replay mismatch")
    ...
    print(
        json.dumps(
            {
                "decision": "ADAPTIVE_V5_VERIFICATION_PASS",
