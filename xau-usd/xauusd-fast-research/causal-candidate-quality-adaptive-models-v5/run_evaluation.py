...
CONFIG_PATH = ROOT / "config" / "adaptive_models_v5.json"
...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Adaptive V5 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Adaptive V5 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Adaptive V5 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Adaptive V5 implementation changed after lock: {name}")
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Adaptive Action Models V5 Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Adaptive V5 evaluation")
    ...
    reference_config = json.loads(
        inputs["reference_action_v4_config"].read_text(encoding="utf-8")
    )
    base_method_hash = assert_action_v4_parity(config, reference_config)
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Adaptive V5 base method contract changed after lock")
    features = list(v4_config["model_features"])
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Adaptive V5 feature hash is inconsistent")
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("Adaptive V5 split fold IDs changed")
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        decision = (
            "ADAPTIVE_MODEL_V5_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "ADAPTIVE_MODEL_V5_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "ADAPTIVE_MODEL_V5_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "ADAPTIVE_MODEL_V5_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "ADAPTIVE_MODEL_V5_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
    ...
    paths = {
        ...
        "comparison_v4": output / config["outputs"]["comparison_v4"],
    }
    ...
    reference_result = json.loads(
        inputs["reference_action_v4_result"].read_text(encoding="utf-8")
    )
    write_json(
        paths["comparison_v4"], build_result_comparison(result, reference_result)
    )
