...
CONFIG = json.loads(
    (ROOT / "config" / "adaptive_models_v5.json").read_text(encoding="utf-8")
)


def test_base_method_matches_action_v4() -> None:
    path = REPO_ROOT / CONFIG["inputs"]["reference_action_v4_config"]["path"]
    reference = json.loads(path.read_text(encoding="utf-8"))
    assert assert_action_v4_parity(CONFIG, reference)
    assert (
