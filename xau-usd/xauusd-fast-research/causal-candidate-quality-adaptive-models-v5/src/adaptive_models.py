...
def assert_action_v4_parity(
    ...
    if current_contract != reference_contract:
        raise ValueError("Adaptive V5 changed an unauthorized Action V4 method field")
    ridge = config["ridge_model"]
    reference_ridge = reference["models"][0]
    if (
        ridge["kind"] != reference_ridge["kind"]
        or ridge["parameters"] != reference_ridge["parameters"]
    ):
        raise ValueError("Adaptive V5 ridge model differs from Action V4")
...
def build_result_comparison(
    ...
    for lane, current_family in current["families"].items():
        ...
        for name, path in paths.items():
            ...
            metrics[name] = {
                "action_v4": reference_value,
                "adaptive_v5": current_value,
                "delta_v5_minus_v4": (
                    None
        ...
        lanes[lane] = {
            "action_v4_decision": reference_family["decision"],
            "adaptive_v5_decision": current_family["decision"],
            "metrics": metrics,
        }
    return {
        "schema_version": "xauusd_adaptive_v5_vs_action_v4",
        "action_v4_decision": reference["decision"],
        "adaptive_v5_decision": current["decision"],
        "lanes": lanes,
    }
