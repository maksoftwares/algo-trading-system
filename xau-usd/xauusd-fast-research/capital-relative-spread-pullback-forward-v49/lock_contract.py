...
PACKAGE_FILES = (
    ...
    "README.md",
    "requirements.txt",
    "config/capital_relative_spread_pullback_forward_v49.json",
...
def build_contract() -> dict[str, object]:
    ...
    if canonical_hash(audit, "audit_sha256") != audit["audit_sha256"]:
        raise ValueError("V49 calibration audit self-hash mismatch")
    if not audit["calibration_structure_passed"]:
        raise ValueError("V49 calibration did not select a policy")
...
def main() -> int:
    ...
    if args.verify:
        ...
        if locked != contract:
            raise ValueError("V49 contract lock does not match current files")
