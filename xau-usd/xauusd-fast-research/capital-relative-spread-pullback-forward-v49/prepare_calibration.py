...
def main() -> int:
    ...
    if len(eligible_dates) < int(config["sampling"]["minimum_calibration_weekdays"]):
        raise ValueError("V49 has too few eligible outcome-blind calibration weekdays")
    ...
    manifest = {
        "schema_version": "xauusd_v49_calibration_source_manifest",
    ...
    audit = {
        "schema_version": "xauusd_v49_outcome_blind_calibration_audit",
        "decision": (
            "V49_CALIBRATION_STRUCTURE_PASS_READY_TO_LOCK"
            if selected is not None
            else "V49_CALIBRATION_STRUCTURE_FAIL_TERMINAL"
        ),
