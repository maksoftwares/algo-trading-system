...
OUTPUT = ROOT / "outputs"
CONFIG_PATH = ROOT / "config" / "calendar_session_specialists_v1.json"
PREFIX = "CALENDAR_SESSION_SPECIALISTS"
...
def _verify_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    lock_module = _load_module(
        "calendar_session_contract_verify", ROOT / "lock_contract.py"
    )
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("Calendar/session contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("Calendar/session contract file set changed")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    ...
    _write_json(
        marker,
        {
            "schema_version": "xauusd_calendar_session_stage_open_v1",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_calendar_session_advancement_v1",
        "stage": stage,
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    attempts_before = int(controls["campaign_attempts_before_v1"])
    return {
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "exam":
            return "CALENDAR_SESSION_NEAR_SURVIVOR_REQUIRES_REPLICATION_AND_SHADOW"
        return f"CALENDAR_SESSION_{stage.upper()}_PASS_ADVANCE"
    return f"NO_CALENDAR_SESSION_V1_{stage.upper()}_SURVIVOR"


def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# XAUUSD Calendar And Session Specialists V1 Result",
        "",
    ...
    lines.extend(
        [
            "",
            "Significance uses all source trading days, including zero-trade days. Later stages remain sealed unless the prior hashed advancement lock names an unchanged passer.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    session = _load_module(
        "calendar_session_specialists_run", ROOT / "src" / "session.py"
    )
    data = _load_module(
        "calendar_session_data_run",
    ...
    frame = session.prepare_features(bundle.bars["H1"], config)
    metrics, cache = session.evaluate_policies(
        frame, bundle.bars["M5"], incoming, config, stage
    )
    selected = session.select_advancers(metrics, config["gates"][stage])
    trades = session.selected_trade_ledger(
    ...
    result["data_evidence"] = bundle.evidence
