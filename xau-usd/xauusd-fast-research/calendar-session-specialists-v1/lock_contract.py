...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        "config/calendar_session_specialists_v1.json": CONFIG_PATH,
        "PREREGISTRATION.md": ROOT / "PREREGISTRATION.md",
        "requirements.txt": ROOT / "requirements.txt",
        "src/session.py": ROOT / "src" / "session.py",
        ...
        "upstream/execution_and_metrics.py": (
            ROOT / source["execution_source"]
        ).resolve(),
...
def _verify_external(
    ...
    if manifest["source_digest"] != source["source_digest"]:
        raise ValueError("Dukascopy source digest mismatch")
    if int(manifest["rows"]) != int(source["expected_rows"]):
        raise ValueError("Unexpected Dukascopy row count")
    return manifest


def main() -> int:
    if LOCK_PATH.exists() or MANIFEST_PATH.exists() or CENSUS_PATH.exists():
        raise RuntimeError("Calendar/session V1 was already locked")
    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    paths = contract_paths(config)
    source_manifest = _verify_external(config, paths)
    session = _load_module(
        "calendar_session_specialists_lock", ROOT / "src" / "session.py"
    )
    data = _load_module(
        "calendar_session_data_lock",
    ...
    census = {
        "schema_version": "xauusd_calendar_session_signal_census_v1",
    ...
    body = {
        "schema_version": "xauusd_calendar_session_contract_lock_v1",
        "created_utc": datetime.now(UTC).isoformat(),
