...
def load_day(
    ...
    *,
    ...
    return (
        ...
        bond_store.quote_frame(start_ms, end_ms),
...
def run_calibration() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (feature_path, grid_path, audit_path)):
        raise FileExistsError("V82 calibration outputs already exist")
    if (output / str(config["outputs"]["contract_lock"])).exists():
        raise RuntimeError("V82 calibration cannot run after lock")
    source_path = output / str(config["outputs"]["calibration_source_audit"])
    source_audit = load_json(source_path)
    if (
        source_audit.get("decision") != "V82_CALIBRATION_SOURCE_AUDIT_PASS"
    ...
    for date in pd.date_range(
        ...
        print(f"V82 calibrated {date.date()}", flush=True)
    features = (
    ...
    if len(policies) != 1000:
        raise ValueError(f"V82 registered {len(policies)} policies, expected 1000")
    ...
    decision = (
        "V82_CALIBRATION_POLICY_SELECTED"
        if selected
        else "V82_NO_CALIBRATION_POLICY"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_macro_consensus_xau_catchup_v82_calibration_audit",
