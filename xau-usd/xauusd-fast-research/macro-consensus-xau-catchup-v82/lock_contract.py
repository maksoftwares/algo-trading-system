...
PACKAGE_FILES = (
    ...
    "src/__init__.py",
    "src/macro_consensus.py",
    "run_source_audit.py",
    "run_calibration.py",
    "lock_contract.py",
    "run_stage.py",
    "tests/conftest.py",
    "tests/test_macro_consensus.py",
)
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if sha256_file(feature_path) != calibration["feature_sha256"]:
        raise ValueError("V82 calibration features changed")
    if sha256_file(grid_path) != calibration["grid_sha256"]:
        raise ValueError("V82 calibration grid changed")
    ...
    for dependency in config["locked_dependencies"].values():
        ...
        if sha256_file(path) != dependency["sha256"]:
            raise ValueError(f"V82 dependency changed: {path}")
    ...
    if sha256_file(atr_path) != config["spot_source"]["m5_feature_sha256"]:
        raise ValueError("V82 completed-M5 ATR cache changed")
    contract: dict[str, Any] = {
        "schema_version": "xauusd_macro_consensus_xau_catchup_v82_contract_lock",
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V82 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V82 contract already exists")
    economic_outputs = [
        output / f"MACRO_CONSENSUS_XAU_V82_{stage.upper()}_{suffix}"
        for stage in (
            "development",
            "confirmation",
            "validation",
    ...
    if existing := [str(path) for path in economic_outputs if path.exists()]:
        raise ValueError(f"V82 economic outputs existed before lock: {existing}")
