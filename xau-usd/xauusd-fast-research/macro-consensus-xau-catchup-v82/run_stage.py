...
from macro_consensus import (  # noqa: E402
...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"MACRO_CONSENSUS_XAU_V82_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V82 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V82 {stage} sealed because {prior} failed")
...
def verify_stage_source_audit(
    ...
    if (
        ...
        raise ValueError(f"V82 {stage} source audit is invalid")
    if stage == "development" and sha256_file(path) != contract[
        "development_source_audit"
    ]["sha256"]:
        raise ValueError("V82 locked development source audit changed")
    return audit, path
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V82 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if date.weekday() >= 5:
            continue
        dxy, bond, xag, xau = load_day(
            date,
            dxy_store=stores["DOLLARIDXUSD"],
            bond_store=stores["USTBONDTRUSD"],
            xag_store=stores["XAGUSD"],
            xau_store=stores["XAUUSD"],
            rule=rule,
            maximum_horizon_ms=horizon,
        )
        quality = session_quality(date, dxy, bond, xag, xau, rule)
        ...
        if number % 25 == 0:
            print(f"V82 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V82_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V82_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_macro_consensus_xau_catchup_v82_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V82 stage")
