...
def main() -> int:
    ...
    evaluator = load_module(
        "v60_v11_dynamic_evaluator", resolve(inputs["shared_evaluator"]["path"])
    )
    anti_module = load_module(
        "v60_v11_crossfeed", resolve(inputs["antichase_experiment_source"]["path"])
    )
    ...
    with tempfile.TemporaryDirectory(prefix="v60-persistent-source-health-union-v11-") as temporary:
        ...
        replay_config["report_title"] = "V60 Persistent Source-Health Union V11"
    ...
    result.update(
        {
            ...
            "dynamic_policy": {
                ...
                "v2_persistence": config["v2_persistence"],
            ...
            "limitations": [
                ...
                "The underlying thresholds, outcomes, and V6 cost failure were exposed before V11.",
                "The two-window persistence rule was selected post-hoc and was not tuned after replay.",
    ...
    (OUTPUTS / "RESULT.md").write_text(
        "\n".join(
            [
                "# V60 Persistent Source-Health Union V11 Result",
                "",
                ...
                f"Persistent V2 support: {v2_component['trades']} historical vetoes, ${v2_component['avoided_pnl_usd']:.2f} avoided P/L, veto PF {v2_component['profit_factor']:.3f}.",
