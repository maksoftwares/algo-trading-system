...
def profit_factor(values: list[float]) -> float:
    ...
    gross_loss = -float(numeric[numeric < 0.0].sum())
    return gross_profit / gross_loss if gross_loss > 0.0 else float("inf")


def persistent_profit_factor_state(
    prior_outcomes: list[float],
    lookback: int,
    maximum_profit_factor_exclusive: float,
) -> tuple[bool, float | None, float | None]:
    if lookback <= 0:
        raise ValueError("Lookback must be positive")
    if len(prior_outcomes) < lookback + 1:
        return False, None, None
    current = profit_factor(prior_outcomes[-lookback:])
    previous = profit_factor(prior_outcomes[-lookback - 1 : -1])
    persistent = (
        current < maximum_profit_factor_exclusive
        and previous < maximum_profit_factor_exclusive
    )
    return bool(persistent), current, previous
...
def combined_challenger_class(
    ...
    class DynamicCombinedScenario(replay.Scenario):
        def __init__(
            ...
            **kwargs: Any,
            ...
            self.source_closed: dict[str, deque[float]] = defaultdict(
                lambda: deque(maxlen=int(policy["lookback_closed_trades"]) + 1)
            )
        ...
        def _entry_reason(self, candidate: Any, *args: Any, **kwargs: Any) -> str | None:
            ...
            persistent, persistent_pf, previous_pf = persistent_profit_factor_state(
                list(self.source_closed[candidate.source_id]),
                int(self.veto_policy["lookback_closed_trades"]),
                float(self.veto_policy["maximum_prior_profit_factor_exclusive"]),
            )
            if recent_pf is not None and persistent_pf is not None and not np.isclose(
                recent_pf, persistent_pf, rtol=0.0, atol=1e-12
            ):
                raise ValueError("Persistent PF state disagrees with V2 evaluator")
            v2_veto = bool(v2_veto and persistent)
            ...
            self.veto_audit.append(
                {
                    ...
                    "previous_20_profit_factor": previous_pf,
