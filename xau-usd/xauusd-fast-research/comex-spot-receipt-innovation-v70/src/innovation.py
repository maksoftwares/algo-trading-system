...
def publisher_clock_lead_rows(frame: pd.DataFrame) -> int:
    normalized = normalize_received_trades(frame)
    return int((normalized["ts_recv"] < normalized["ts_event"]).sum())
...
def generate_candidates(
    ...
    if selected["candidate_id"].duplicated().any():
        raise ValueError("V70 candidate IDs are not unique")
