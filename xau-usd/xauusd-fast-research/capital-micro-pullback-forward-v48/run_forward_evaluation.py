...
def verify_contract(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if canonical_hash(contract, "contract_sha256") != contract["contract_sha256"]:
        raise ValueError("V48 contract self-hash mismatch")
    for section in ("package_files", "calibration_files"):
        for record in contract[section]:
            ...
            if sha256_file(candidate) != record["sha256"]:
                raise ValueError(f"V48 locked file changed: {record['path']}")
    return contract
...
def verify_stage(
    ...
    if canonical_hash(audit, "audit_sha256") != audit["audit_sha256"]:
        raise ValueError(f"V48 stage self-hash mismatch: {path}")
    if audit["contract_sha256"] != contract["contract_sha256"]:
        raise ValueError(f"V48 stage contract mismatch: {path}")
    if sha256_file(trades_path) != audit["trades_sha256"]:
        raise ValueError(f"V48 stage trades changed: {trades_path}")
    return audit
...
def main() -> int:
    ...
    inventory = {
        "schema_version": "xauusd_v48_forward_inventory",
        "contract_sha256": contract["contract_sha256"],
    ...
    if validation_path.exists():
        ...
        print(
            json.dumps(
                {
                    "decision": "V48_CONTINUE_SEALED_FORWARD_COLLECTION",
        ...
        validation["decision"] = (
            "V48_FORWARD_VALIDATION_PASS_CONFIRMATION_REMAINS_SEALED"
            if validation["gate_passed"]
            else "V48_FORWARD_VALIDATION_FAIL_TERMINAL"
        )
    ...
    if len(confirmation_dates) < confirmation_count:
        print(
            json.dumps(
                {
                    "decision": "V48_VALIDATION_PASS_CONTINUE_SEALED_CONFIRMATION",
    ...
    confirmation["decision"] = (
        "V48_FORWARD_CONFIRMATION_PASS_RESEARCH_SHADOW_NOMINATION_ONLY"
        if confirmation["gate_passed"]
        else "V48_FORWARD_CONFIRMATION_FAIL_TERMINAL"
    )
