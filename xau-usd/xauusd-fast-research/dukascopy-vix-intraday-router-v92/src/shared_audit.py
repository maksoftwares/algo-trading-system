...
def prepare_v92_ledger(trades: pd.DataFrame) -> pd.DataFrame:
    required = {
        "attempt_no",
        "policy_id",
        "mechanic",
    ...
    if missing:
        raise ValueError(f"V92 trade ledger is missing columns: {missing}")
    result = trades.copy()
    ...
    if result["risk_usd"].le(0.0).any():
        raise ValueError("V92 initial risk must be positive")
    if result["exit_time"].lt(result["entry_time"]).any():
        raise ValueError("V92 exit precedes entry")
    if not result["direction"].isin(("LONG", "SHORT")).all():
        raise ValueError("V92 direction is invalid")
    if result.duplicated(["policy_id", "entry_time"]).any():
        raise ValueError("Duplicate V92 policy entry")
    ...
    if result["fee_stress_open_cost_usd"].lt(-1e-9).any():
        raise ValueError("V92 stress cost creates a rebate")
...
def route_v92_candidates(
    ...
    prepared = prepare_v92_ledger(candidates)
    ...
    maximum_v92_daily = int(limits["maximum_v92_entries_per_utc_date"])
    ...
    for order, row in enumerate(prepared.itertuples(index=False), start=1):
        ...
        if entry_time in accepted_entry_times:
            reason = "DUPLICATE_V92_ENTRY_TIME"
        elif drawdown_suspended:
            reason = "ACCOUNT_DRAWDOWN_SUSPENDED"
        elif accepted_daily.get(day, 0) >= maximum_v92_daily:
            reason = "MAXIMUM_V92_ENTRIES_PER_UTC_DATE"
...
def shared_window_metrics(
    ...
    for window, stage in stage_by_window.items():
        ...
        checks = {
            ...
            "minimum_v92_stress_pf": addon_pf
            >= float(gates[stage]["minimum_stress_pf"]),
            "v92_winner_removed_positive": float(removed.sum()) > 0.0,
        ...
        rows.append(
            {
                ...
                "v92_accepted_trades": int(len(addon_window)),
                ...
                "v92_stress_net_usd": float(addon_values.sum()),
                "v92_stress_pf": addon_pf,
                "v92_winner_removed_stress_net_usd": float(removed.sum()),
