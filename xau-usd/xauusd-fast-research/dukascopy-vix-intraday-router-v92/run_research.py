...
def _verify_contract(config: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    if (
        ...
        raise FileNotFoundError("Run lock_contract.py before opening V92 outcomes")
    lock_module = _load_module(
        "dukascopy_vix_v92_contract_verify", ROOT / "lock_contract.py"
    )
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("V92 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V92 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V92 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V92 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V92 signal census changed")
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V92 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V92 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V92 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V92 {stage} metrics changed")
    if payload["trades_sha256"] != _sha256(_trades_path(stage)):
        raise ValueError(f"V92 {stage} trades changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V92 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V92 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V92 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_dukascopy_vix_v92_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_dukascopy_vix_v92_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V92_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V92_{stage.upper()}_PASS_ADVANCE"
    return f"V92_{stage.upper()}_FAIL_TERMINAL"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V92 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# Dukascopy Intraday VIX Router V92 Result",
        "",
    ...
    lines.extend(
        [
            "",
            "Only completed intraday VIX and XAU bars are used; execution is side-correct on the next XAU M5 bar.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    campaign = _load_module(
        "dukascopy_vix_v92_run_campaign", ROOT / "src" / "campaign.py"
    )
    data = _load_module(
        "dukascopy_vix_v92_run_data",
    ...
    result["data_evidence"] = {
        ...
        "dukascopy_vix_path": str(paths["external/vix_feature_cache"]),
        "dukascopy_vix_sha256": config["vix_source"]["feature_sha256"],
