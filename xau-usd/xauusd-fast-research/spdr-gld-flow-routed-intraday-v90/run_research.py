...
def _verify_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    if not LOCK_PATH.is_file() or not MANIFEST_PATH.is_file() or not CENSUS_PATH.is_file():
        raise FileNotFoundError("Run lock_contract.py before opening V90 outcomes")
    lock_module = _load_module("spdr_gld_v90_contract_verify", ROOT / "lock_contract.py")
    lock = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    claimed = str(lock["contract_sha256"])
    body = dict(lock)
    body.pop("contract_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError("V90 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V90 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V90 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V90 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V90 signal census changed")
    return lock
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V90 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V90 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V90 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V90 {stage} metrics changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V90 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V90 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V90 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_spdr_gld_flow_v90_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_spdr_gld_flow_v90_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V90_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V90_{stage.upper()}_PASS_ADVANCE"
    return f"V90_{stage.upper()}_FAIL_TERMINAL"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V90 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# SPDR GLD Flow-Routed Intraday V90 Result",
        "",
    ...
    lines.extend(
        [
            "",
            "SPDR GLD holdings are lagged by one calendar day. XAU decisions use completed bars and side-correct next-bar execution.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    campaign = _load_module("spdr_gld_v90_run_campaign", ROOT / "src" / "campaign.py")
    data = _load_module(
        "spdr_gld_v90_run_data", (ROOT / config["source"]["data_source"]).resolve()
    )
    bundle = data.load_bundle(config)
    gld_path = Path(config["gld_source"]["path"]).resolve()
    ...
    result["data_evidence"] = {
        ...
        "spdr_gld_path": str(gld_path),
        "spdr_gld_sha256": config["gld_source"]["sha256"],
