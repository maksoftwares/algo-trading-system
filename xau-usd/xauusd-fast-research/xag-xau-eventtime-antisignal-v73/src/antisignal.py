...
def invert_candidates(candidates: pd.DataFrame, *, family: str) -> pd.DataFrame:
    required = {"candidate_id", "direction", "decision_timestamp_ms"}
    if missing := sorted(required - set(candidates.columns)):
        raise ValueError(f"V73 candidates are missing columns: {missing}")
    result = candidates.copy()
    if not result["direction"].isin(["LONG", "SHORT"]).all():
        raise ValueError("V73 source direction is invalid")
    ...
    if result["candidate_id"].duplicated().any():
        raise ValueError("V73 candidate IDs are not unique")
    return result
