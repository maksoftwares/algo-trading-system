...
def load_day(
    ...
    *,
    ...
    return xag_store.quote_frame(start_ms, end_ms), xau_store.quote_frame(start_ms, end_ms)


def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    output = ROOT / str(config["outputs"]["directory"])
    prefix = f"XAG_XAU_V73_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V73 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V73 {stage} sealed because {prior} failed")
    if stage == "exam":
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V73 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if bool(quality["eligible_full_weekday"]):
            ...
            source_candidates = generate_candidates(
                features, policy=policy, family="V72_SOURCE_DIRECTION"
            )
        ...
        if number % 25 == 0:
            print(f"V73 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V73_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V73_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_xag_xau_v73_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V73 stage")
