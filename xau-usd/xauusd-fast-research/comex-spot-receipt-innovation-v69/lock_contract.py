...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/comex_spot_receipt_innovation_v69.json",
    "src/__init__.py",
    "src/innovation.py",
    "run_calibration.py",
    "lock_contract.py",
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if canonical_hash(calibration, "audit_sha256") != str(
        ...
        raise ValueError("V69 calibration audit self-hash changed")
    if calibration.get("decision") != "V69_CALIBRATION_POLICY_SELECTED":
        raise ValueError("V69 calibration did not select a policy")
    if calibration.get("selected_policy") is None:
        raise ValueError("V69 selected policy is missing")
    ...
    if sha256_file(feature_path) != str(calibration["feature_sha256"]):
        raise ValueError("V69 calibration features changed")
    if sha256_file(grid_path) != str(calibration["grid_sha256"]):
        raise ValueError("V69 calibration grid changed")
    dependencies = []
    for dependency in config["locked_dependencies"].values():
        ...
        if sha256_file(path) != str(dependency["sha256"]):
            raise ValueError(f"V69 dependency changed: {dependency['path']}")
    ...
    if sha256_file(manifest) != str(config["source"]["download_manifest_sha256"]):
        raise ValueError("V69 COMEX download manifest changed")
    ...
    if sha256_file(spot_cache) != str(config["spot_source"]["m5_feature_sha256"]):
        raise ValueError("V69 completed-M5 feature cache changed")
    contract = {
        "schema_version": "xauusd_comex_spot_receipt_v69_contract_lock",
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V69 immutable contract verification failed")
    return lock


def main() -> int:
    config = load_json(ROOT / "config" / "comex_spot_receipt_innovation_v69.json")
    output = ROOT / str(config["outputs"]["directory"])
    lock_path = output / str(config["outputs"]["contract_lock"])
    if lock_path.exists():
        raise FileExistsError("V69 contract already exists")
    for stage in config["splits"]:
        if list(output.glob(f"COMEX_SPOT_RECEIPT_V69_{stage.upper()}_*")):
            raise ValueError(f"V69 {stage} outputs existed before lock")
