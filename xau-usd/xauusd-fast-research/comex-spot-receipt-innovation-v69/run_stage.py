...
CONFIG = ROOT / "config" / "comex_spot_receipt_innovation_v69.json"
...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"COMEX_SPOT_RECEIPT_V69_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not audit_path.is_file():
            raise RuntimeError(f"V69 {stage} sealed because {prior} has not run")
        audit = load_json(audit_path)
        if canonical_hash(audit, "audit_sha256") != str(audit.get("audit_sha256")):
            raise RuntimeError(f"V69 {prior} audit self-hash changed")
        if not bool(audit.get("gate_passed")):
            raise RuntimeError(f"V69 {stage} sealed because {prior} failed")
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V69 {stage} outputs already exist")
    ...
    decision = (
        f"V69_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V69_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_comex_spot_receipt_v69_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V69 stage")
