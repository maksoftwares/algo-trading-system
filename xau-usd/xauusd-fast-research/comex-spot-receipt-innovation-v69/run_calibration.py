...
def run_calibration() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (feature_path, grid_path, audit_path)):
        raise FileExistsError("V69 calibration outputs already exist")
    if (output / str(config["outputs"]["contract_lock"])).exists():
        raise RuntimeError("V69 calibration cannot run after contract lock")
    ...
    if len(policies) != 1000:
        raise ValueError(f"V69 registered {len(policies)} policies, expected 1000")
    rows = []
    ...
    decision = (
        "V69_CALIBRATION_POLICY_SELECTED" if selected else "V69_NO_CALIBRATION_POLICY"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_comex_spot_receipt_v69_calibration_audit",
