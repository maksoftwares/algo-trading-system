...
def main() -> int:
    ...
    if sha256_file(Path(__file__)) != config["lock"]["tick_replay_runner_sha256"]:
        raise ValueError("Locked exact tick replay runner changed")
    tick_replay_path = ROOT / "src" / "tick_replay.py"
    if sha256_file(tick_replay_path) != config["lock"]["tick_replay_sha256"]:
    ...
    for path in tick_paths:
        ...
        if first_date <= file_date <= final_date:
            relevant_tick_paths.append(path)
    if not relevant_tick_paths:
        raise FileNotFoundError("No dated tick files cover the resolved trades")
    result = replay_ticks(
        trades,
        iter_tick_files(
            relevant_tick_paths,
            first_time_msc=first_entry,
            final_time_msc=final_exit,
        ),
    ...
    result.update(
        {
            ...
            "tick_files_considered": len(relevant_tick_paths),
            "tick_file_sha256": {
                path.name: sha256_file(path) for path in sorted(relevant_tick_paths)
            },
            "contract_units_per_lot": contract_units_per_lot,
            "deployment_authorized": False,
        }
