...
V24.1 is a structural successor to locked V24. V24 failed before any forward
file or economic outcome existed because its empty candidate frame had no
declared columns. V24.1 changes only that empty-data schema contract and adds a
regression test. Every hypothesis, feature, threshold, block, direction, horizon,
cost, split, and pass/fail gate remains byte-equivalent in meaning.

V24.1 is a single, forward-only XAUUSD quote-microstructure hypothesis. It uses
