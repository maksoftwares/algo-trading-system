# V24.1 Preregistration

V24.1 inherits the complete locked V24 research protocol. Its only correction is
that an empty candidate result retains a stable declared schema, allowing the
forward runner to report `CONTINUE_EVIDENCE` before any forward file exists. V24
failed on that code path before any economic outcome was opened. No research
rule changed.
...
V24.1 contains one candidate rule, one direction mapping, and one horizon. No
threshold, cooldown, session, direction, horizon, feature, label, or cost grid is
registered. Failure is terminal for V24.1. A new version must count as another
attempt and wait for later untouched evidence; V24.1 may not be reversed or tuned
