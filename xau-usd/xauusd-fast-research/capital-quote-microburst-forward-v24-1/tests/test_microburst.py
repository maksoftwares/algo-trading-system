...
def test_empty_ticks_keep_stable_candidate_schema() -> None:
    config = load_config(ROOT)
    candidates, features = generate_candidates(pd.DataFrame(), config)
    assert candidates.empty
    assert features.empty
    assert "tick_time_msc" in candidates.columns
    assert "candidate_side" in candidates.columns
    assert "raw_gate_crossing" in features.columns
