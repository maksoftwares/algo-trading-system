...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD M5 Passive Regime Campaign V5 Result",
        "",
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_m5_passive_regime_v5_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V5 outcomes already exist; same-version reruns are forbidden")
    ...
    frame = prepare_passive_features(frame)
    arrays = m5_execution_arrays(m5)
    signal_ns = frame["timestamp_utc"].astype("int64").to_numpy()
    first_m5 = np.searchsorted(arrays["starts"], signal_ns, side="left")
    in_bounds = first_m5 < len(arrays["starts"])
    gaps = np.full(len(signal_ns), np.nan)
    gaps[in_bounds] = (
        arrays["starts"][first_m5[in_bounds]] - signal_ns[in_bounds]
    ) / 60_000_000_000
    if np.nanmin(gaps) < 0.0:
        raise ValueError("Negative pending-order activation gap")
    ...
    decision = (
        ...
        if statistical_finalists > 0
            ...
            else "NO_M5_PASSIVE_REGIME_V5_ECONOMIC_FINALIST"
        )
    )
    result = {
        ...
        "clock_audit": {
            "unit": "ns",
            "negative_activation_gaps": int(np.nansum(gaps < 0.0)),
            "zero_activation_gaps": int(np.nansum(gaps == 0.0)),
            "maximum_activation_gap_minutes": float(np.nanmax(gaps)),
        },
