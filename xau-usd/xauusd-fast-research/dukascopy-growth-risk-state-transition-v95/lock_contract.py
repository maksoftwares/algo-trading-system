...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        "config/dukascopy_growth_risk_state_transition_v95.json": CONFIG_PATH,
...
def verify_sources(
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise FileNotFoundError(f"Missing V95 contract input {name}: {path}")
    source = config["source"]
    ...
    if source_config["start_utc"] != growth_risk["start_utc"] or source_config[
        ...
        raise ValueError("Growth-risk source period differs from V95")
...
def main() -> int:
    ...
    if len(manifest) != int(controls["registered_policy_count"]):
        raise ValueError("V95 policy count mismatch")
    if int(manifest["attempt_no"].min()) != int(controls["attempt_first"]):
        raise ValueError("V95 first attempt mismatch")
    if int(manifest["attempt_no"].max()) != int(controls["attempt_last"]):
        raise ValueError("V95 last attempt mismatch")
    ...
    census = {
        "schema_version": "xauusd_dukascopy_growth_risk_v95_signal_census",
    ...
    body = {
        "schema_version": "xauusd_dukascopy_growth_risk_v95_contract_lock",
        "created_utc": datetime.now(UTC).isoformat(),
