...
def _paths(stage: str) -> tuple[Path, Path, Path]:
    prefix = f"COMEX_SEQUENCE_IGNITION_V45_{stage.upper()}"
    return (
...
def main() -> None:
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            features = build_sequence_features(session, rule=rule)
    ...
    if not candidates.empty:
        ...
        if candidates["candidate_id"].duplicated().any():
            raise RuntimeError("V45 stage candidates contain duplicate IDs.")
    ...
    decision = (
        f"V45_{stage.upper()}_PASS"
        if result["gate_passed"]
        else f"V45_{stage.upper()}_FAIL_TERMINAL"
    )
    payload: dict[str, Any] = {
        "schema_version": "xauusd_comex_sequence_ignition_v45_stage_audit",
