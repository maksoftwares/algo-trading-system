...
def test_millisecond_timestamps_produce_completed_shadow_history() -> None:
    trades = pd.DataFrame(
        [
            {
                "attempt_no": 1,
    ...
    trades["entry_time"] = trades["entry_time"].astype("datetime64[ms, UTC]")
    trades["exit_time"] = trades["exit_time"].astype("datetime64[ms, UTC]")
    cache = build_shadow_cache(trades, [365], [1])
    second_entry = pd.Timestamp(trades["entry_time"].iat[1]).value
    assert cache[(second_entry, 1, 365)].count == 1
    assert cache[(second_entry, 1, 365)].net_r == 1.0
...
def test_manifest_is_deterministic_and_balanced() -> None:
    ...
    assert first["attempt_no"].tolist() == list(range(26239, 27239))
