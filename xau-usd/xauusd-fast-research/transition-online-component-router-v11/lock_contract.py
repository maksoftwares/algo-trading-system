...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/transition_online_component_router_v11.json",
    "src/__init__.py",
    "src/router.py",
    "prepare_manifest.py",
    "lock_contract.py",
...
DEPENDENCIES = ("../regime-mechanism-campaign-v1/src/campaign.py",)
...
def main() -> int:
    ...
    if lock_path.exists():
        raise FileExistsError("V11 contract lock already exists")
    payload: dict[str, Any] = {
        "schema_version": "xauusd_transition_online_router_v11_contract",
        "selection": config["selection"],
        "portfolio": config["portfolio"],
        "policy_space": config["policy_space"],
        ...
        "source_files": [
            _record(ROOT / config["source"][name], REPO) for name in source_names
        ],
        "external_files": [
