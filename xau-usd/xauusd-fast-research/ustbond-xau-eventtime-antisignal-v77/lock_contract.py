...
V76_ROOT = ROOT.parent / "ustbond-xau-eventtime-catchup-v76"
...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/ustbond_xau_eventtime_antisignal_v77.json",
    "src/__init__.py",
    "src/antisignal.py",
    "lock_contract.py",
    "run_stage.py",
...
def verified_inheritance(config: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for key, hash_key in mapping.items():
        if sha256_file(paths[key]) != inherited[hash_key]:
            raise ValueError(f"V77 inherited V76 source changed: {paths[key]}")
    ...
    if canonical_hash(contract, "contract_sha256") != contract.get("contract_sha256"):
        raise ValueError("V76 inherited contract self-hash changed")
    config_record = next(
        (
            row
            for row in contract["package_files"]
            if row["path"] == "config/ustbond_xau_eventtime_catchup_v76.json"
        ),
        None,
    )
    if config_record is None:
        raise ValueError("V76 contract does not lock its source configuration")
    v76_config_path = V76_ROOT / str(config_record["path"])
    if sha256_file(v76_config_path) != config_record["sha256"]:
        raise ValueError("V76 locked source configuration changed")
    v76_config = load_json(v76_config_path)
    inherited_source = {
        key: v76_config["source"][key]
        for key in (
    ...
    if config["source"] != inherited_source:
        raise ValueError("V77 source semantics differ from V76")
    if config["spot_source"] != v76_config["spot_source"]:
        raise ValueError("V77 completed-M5 source semantics differ from V76")
    if (
        development.get("decision") != "V76_DEVELOPMENT_FAIL_TERMINAL"
        or canonical_hash(development, "audit_sha256") != development.get("audit_sha256")
    ):
        raise ValueError("V76 terminal development audit is invalid")
    ...
    if dates and max(dates) >= exposed_end:
        raise ValueError("V76 outcomes overlap V77 development")
    if pd.to_datetime(config["splits"]["development"][0], utc=True) != exposed_end:
        raise ValueError("V77 development does not begin at the fresh boundary")
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    v76, development = verified_inheritance(config)
    inherited_rule = dict(v76["candidate_rule"])
    inherited_rule["family"] = config["candidate_rule"]["family"]
    if inherited_rule != config["candidate_rule"]:
        raise ValueError("V77 changed a V76 candidate rule")
    if v76["execution"] != config["execution"]:
        raise ValueError("V77 execution differs from V76")
    if next(iter(v76["families"].values())) != next(iter(config["families"].values())):
        raise ValueError("V77 exit geometry differs from V76")
    if without_minimum_sample(v76["gates"]) != without_minimum_sample(config["gates"]):
        raise ValueError("V77 changed a V76 economic gate")
    for row in v76["dependencies"]:
        path = REPO_ROOT / str(row["path"])
        if sha256_file(path) != row["sha256"]:
            raise ValueError(f"V77 inherited dependency changed: {path}")
    ...
    contract: dict[str, Any] = {
        "schema_version": "xauusd_ustbond_xau_v77_contract_lock",
        "package_files": [record(path, ROOT) for path in package_paths],
        "v76_contract": record(
            REPO_ROOT / str(config["inherited_v76"]["contract_path"]), REPO_ROOT
        ),
        "v76_source_audit": record(
            REPO_ROOT / str(config["inherited_v76"]["source_audit_path"]), REPO_ROOT
        ),
        "v76_development_audit": record(
            REPO_ROOT / str(config["inherited_v76"]["development_audit_path"]), REPO_ROOT
        ),
        "selected_policy": v76["selected_policy"],
        "v76_terminal_decision": development["decision"],
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V77 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V77 contract already exists")
    if list(output.glob("USTBOND_XAU_V77_*_AUDIT.json")):
        raise ValueError("V77 stage outputs existed before lock")
