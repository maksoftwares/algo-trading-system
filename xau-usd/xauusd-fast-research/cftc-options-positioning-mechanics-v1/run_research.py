...
OUTPUT = ROOT / "outputs"
CONFIG_PATH = ROOT / "config" / "cftc_options_positioning_mechanics_v1.json"
LOCK_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_CONTRACT_LOCK.json"
MANIFEST_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_POLICY_MANIFEST.csv"
CENSUS_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_SIGNAL_CENSUS.json"
RESULT_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_RESULT.json"
RESULT_MD_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_RESULT.md"
ARTIFACT_MANIFEST_PATH = OUTPUT / "CFTC_OPTIONS_POSITIONING_ARTIFACT_MANIFEST.json"
...
def _verify_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    lock_module = _load_module("cftc_options_contract_verify", ROOT / "lock_contract.py")
    lock = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    claimed = str(lock["contract_sha256"])
    body = dict(lock)
    body.pop("contract_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError("CFTC options contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("CFTC options contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"CFTC options contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("CFTC options policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("CFTC options signal census changed")
    return lock


def _marker_path(stage: str) -> Path:
    return OUTPUT / f"CFTC_OPTIONS_POSITIONING_{stage.upper()}_OUTCOMES_OPENED.json"


def _metrics_path(stage: str) -> Path:
    return OUTPUT / f"CFTC_OPTIONS_POSITIONING_{stage.upper()}_METRICS.csv"


def _trades_path(stage: str) -> Path:
    return OUTPUT / f"CFTC_OPTIONS_POSITIONING_{stage.upper()}_TRADES.csv"


def _advancement_path(stage: str) -> Path:
    return OUTPUT / f"CFTC_OPTIONS_POSITIONING_{stage.upper()}_ADVANCEMENT_LOCK.json"
...
def _write_marker(stage: str, contract_hash: str) -> None:
    ...
    _write_json(
        marker,
        {
            "schema_version": "xauusd_cftc_options_stage_open_v1",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_cftc_options_advancement_v1",
        "stage": stage,
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("CFTC options result contract mismatch")
        return result
    controls = config["research_controls"]
    return {
        "schema_version": config["schema_version"],
        "contract_sha256": contract_hash,
        "attempt_first": int(controls["campaign_attempts_before_v1"]) + 1,
        "attempt_last": int(controls["campaign_attempts_before_v1"])
        + int(controls["registered_policy_count"]),
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "exam":
            return "CFTC_OPTIONS_NEAR_SURVIVOR_REQUIRES_OUT_OF_ERA_AND_SHADOW"
        return f"CFTC_OPTIONS_{stage.upper()}_PASS_ADVANCE"
    return f"NO_CFTC_OPTIONS_POSITIONING_V1_{stage.upper()}_SURVIVOR"


def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# CFTC Options-Positioning Mechanics V1 Result",
        "",
    ...
    lines.extend(
        [
            "",
            "Significance uses weekly CFTC report blocks, including zero-trade blocks. Later stages remain sealed unless the prior hashed advancement lock names an unchanged passer.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    campaign = _load_module("cftc_options_campaign_run", ROOT / "src" / "campaign.py")
    data = _load_module(
        "cftc_options_data_run", (ROOT / config["source"]["data_source"]).resolve()
    )
    ...
    frame = campaign.prepare_features(bundle.bars["H1"], positioning, config)
