...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        "config/cftc_options_positioning_mechanics_v1.json": CONFIG_PATH,
        ...
        "upstream/cftc_foundation.py": (ROOT / cftc["foundation_source"]).resolve(),
        "external/dukascopy_feature_cache": source_root / source["feature_cache"],
        "external/dukascopy_feature_manifest": source_root / source["feature_manifest"],
        "external/cftc_curated": cftc_root / cftc["curated_file"],
        "external/cftc_manifest": cftc_root / cftc["manifest_file"],
    }


def _verify_external(config: Mapping[str, Any], paths: Mapping[str, Path]) -> dict[str, Any]:
    ...
    manifest = json.loads(
        paths["external/cftc_manifest"].read_text(encoding="utf-8")
    )
...
def main() -> int:
    if LOCK_PATH.exists() or MANIFEST_PATH.exists() or CENSUS_PATH.exists():
        raise RuntimeError("CFTC options-positioning V1 was already locked")
    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    paths = contract_paths(config)
    cftc_manifest = _verify_external(config, paths)
    campaign = _load_module("cftc_options_campaign_lock", ROOT / "src" / "campaign.py")
    data = _load_module(
        "cftc_options_data_lock", (ROOT / config["source"]["data_source"]).resolve()
    )
    bundle = data.load_bundle(config)
    positioning = pd.read_parquet(paths["external/cftc_curated"])
    ...
    manifest = campaign.generate_manifest(
        ...
        int(controls["campaign_attempts_before_v1"]),
    ...
    census = {
        "schema_version": "xauusd_cftc_options_signal_census_v1",
    ...
    body = {
        "schema_version": "xauusd_cftc_options_positioning_contract_lock_v1",
        "created_utc": datetime.now(UTC).isoformat(),
