...
MECHANICS = (
    "CASH_MOMENTUM_BREAKOUT",
    "CASH_VOLUME_PRESSURE",
    "DEFERRED_BASIS_REVERSION",
    "DEFERRED_OI_EXPANSION",
    "DELIVERY_DIRECTION_PRESSURE",
)
...
def parameter_space(mechanic: str) -> list[dict[str, Any]]:
    ...
    if mechanic == "CASH_MOMENTUM_BREAKOUT":
        return _space(
            **common,
            return_horizon=(1, 2, 3),
            minimum_return_bps=(0.0, 5.0, 10.0, 20.0),
            channel_bars=(3, 6, 12),
            breakout_buffer_atr=(0.0, 0.05, 0.10),
        )
    if mechanic == "CASH_VOLUME_PRESSURE":
        return _space(
            **common,
            return_horizon=(1, 2, 3),
            minimum_return_bps=(0.0, 5.0, 10.0, 20.0),
            volume_z_lookback=(20, 60, 120),
            minimum_volume_z=(-0.5, 0.0, 0.5, 1.0),
            impulse_hours=(3, 6, 12),
            confirmation_min_atr=(0.0, 0.10, 0.25),
        )
    if mechanic == "DEFERRED_BASIS_REVERSION":
        return _space(
            **common,
            basis_z_lookback=(20, 60, 120),
            minimum_basis_z=(0.0, 0.5, 1.0, 1.5),
            impulse_hours=(3, 6, 12, 24),
            dislocation_min_atr=(0.0, 0.25, 0.50),
            confirmation_min_atr=(0.0, 0.10, 0.25),
        )
    if mechanic == "DEFERRED_OI_EXPANSION":
        return _space(
            **common,
            return_horizon=(1, 2, 3),
            minimum_return_bps=(0.0, 5.0, 10.0, 20.0),
            minimum_oi_change_bps=(-10.0, 0.0, 10.0, 25.0),
            impulse_hours=(3, 6, 12),
            trend_min_atr=(0.0, 0.25, 0.50),
            confirmation_min_atr=(0.0, 0.10, 0.25),
        )
    if mechanic == "DELIVERY_DIRECTION_PRESSURE":
        return _space(
            **common,
            delivery_z_lookback=(20, 60, 120),
            minimum_delivery_z=(-0.5, 0.0, 0.5, 1.0),
            impulse_hours=(3, 6, 12),
            trend_min_atr=(0.0, 0.25, 0.50),
            confirmation_min_atr=(0.0, 0.10, 0.25),
        )
...
def load_sge(path: Path, availability_lag_days: int) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Unexpected SGE source schema: {missing}")
    if raw.duplicated(["date", "contract"]).any():
        raise ValueError("Duplicate SGE date-contract row")
    raw = raw.copy()
    raw["date"] = pd.to_datetime(raw["date"], errors="raise")
    core_contracts = {"Au99.99", "Au(T+D)", "mAu(T+D)", "Ag(T+D)"}
    if not core_contracts.issubset(set(raw["contract"])):
        raise ValueError("SGE core contract coverage is incomplete")
    core = raw.loc[raw["contract"].isin(core_contracts)].copy()
    fields = [
        "close",
        "volume_kg",
        "open_interest_lot",
    ...
    prefixes = {
        "Au99.99": "cash",
        "Au(T+D)": "deferred",
        "mAu(T+D)": "mini_deferred",
        "Ag(T+D)": "silver_deferred",
    }
    for contract, prefix in prefixes.items():
        contract_rows = core.loc[core["contract"].eq(contract), ["date", *fields]]
        contract_rows = contract_rows.rename(
            columns={field: f"{prefix}_{field}" for field in fields}
        )
        daily = (
            contract_rows
            if daily is None
            else daily.merge(contract_rows, on="date", how="outer", validate="one_to_one")
        )
    if daily is None:
        raise ValueError("SGE source is empty")
    ...
    if daily["date"].duplicated().any() or not daily["date"].is_monotonic_increasing:
        raise ValueError("Invalid SGE daily ordering")
    daily["available_utc"] = (
        daily["date"] + pd.Timedelta(days=int(availability_lag_days))
    ).dt.tz_localize("UTC")
    if (daily["available_utc"].dt.date <= daily["date"].dt.date).any():
        raise ValueError("Same-day SGE use is forbidden")
    return daily
...
def prepare_features(
    h1: pd.DataFrame, sge: pd.DataFrame, config: Mapping[str, Any]
) -> pd.DataFrame:
    required = {
        "bar_start_utc",
        "bar_end_utc",
    ...
    daily = sge.copy().sort_values("available_utc", kind="mergesort")
    for horizon in map(int, features["return_horizons"]):
        daily[f"cash_return_{horizon}d_bps"] = daily["cash_close"].pct_change(
            horizon, fill_method=None
        ) * 10_000.0
        daily[f"deferred_return_{horizon}d_bps"] = daily[
            "deferred_close"
        ].pct_change(horizon, fill_method=None) * 10_000.0
    daily["deferred_basis_bps"] = (
        daily["deferred_close"] / daily["cash_close"] - 1.0
    ) * 10_000.0
    daily["deferred_oi_change_bps"] = daily[
        "deferred_open_interest_lot"
    ].pct_change(fill_method=None) * 10_000.0
    daily["delivery_direction_sign"] = daily["deferred_direction"].map(
        {"short_to_long": 1.0, "long_to_short": -1.0}
    )
    cash_volume_log = np.log1p(daily["cash_volume_kg"].clip(lower=0.0))
    delivery_log = np.log1p(
        daily["deferred_delivery_volume_lot"].clip(lower=0.0)
    )
    for lookback in map(int, features["state_z_lookbacks"]):
        daily[f"cash_volume_z_{lookback}"] = _causal_z(
            cash_volume_log, lookback
        )
        daily[f"basis_z_{lookback}"] = _causal_z(
            daily["deferred_basis_bps"], lookback
        )
        daily[f"delivery_z_{lookback}"] = _causal_z(delivery_log, lookback)
    ...
    daily_columns = [
        ...
        "deferred_open_interest_lot",
        "deferred_direction",
        "deferred_delivery_volume_lot",
        "deferred_basis_bps",
        "deferred_oi_change_bps",
        "delivery_direction_sign",
        *[
            column
            for horizon in map(int, features["return_horizons"])
            for column in (
                f"cash_return_{horizon}d_bps",
                f"deferred_return_{horizon}d_bps",
            )
        ],
        *[
            column
            for lookback in map(int, features["state_z_lookbacks"])
            for column in (
                f"cash_volume_z_{lookback}",
                f"basis_z_{lookback}",
                f"delivery_z_{lookback}",
            )
        ],
    ]
    frame = pd.merge_asof(
        ...
        direction="backward",
        tolerance=pd.Timedelta(days=float(features["maximum_sge_staleness_days"])),
    )
    observed = frame["available_utc"].notna()
    if (frame.loc[observed, "available_utc"] > frame.loc[observed, "bar_end_utc"]).any():
        raise ValueError("Future SGE state joined to H1 decision")
    if (
        frame.loc[observed, "date"].dt.date
        >= frame.loc[observed, "bar_end_utc"].dt.date
    ).any():
        raise ValueError("Same-date SGE state joined to XAU decision")
    return frame


def _state_feature(mechanic: str, params: Mapping[str, Any]) -> str:
    if mechanic == "CASH_MOMENTUM_BREAKOUT":
        return f"cash_return_{int(params['return_horizon'])}d_bps"
    if mechanic == "CASH_VOLUME_PRESSURE":
        return f"cash_volume_z_{int(params['volume_z_lookback'])}"
    if mechanic == "DEFERRED_BASIS_REVERSION":
        return f"basis_z_{int(params['basis_z_lookback'])}"
    if mechanic == "DEFERRED_OI_EXPANSION":
        return "deferred_oi_change_bps"
    if mechanic == "DELIVERY_DIRECTION_PRESSURE":
        return f"delivery_z_{int(params['delivery_z_lookback'])}"
    raise KeyError(mechanic)
...
def signal_mask_direction(
    ...
    if mechanic in {"CASH_MOMENTUM_BREAKOUT", "CASH_VOLUME_PRESSURE"}:
        state_return = frame[
            f"cash_return_{int(params['return_horizon'])}d_bps"
        ]
        direction = pd.Series(
            np.sign(state_return.fillna(0.0)).astype(int), index=frame.index
        )
    elif mechanic == "DEFERRED_BASIS_REVERSION":
        basis_state = frame[f"basis_z_{int(params['basis_z_lookback'])}"]
        direction = pd.Series(
            -np.sign(basis_state.fillna(0.0)).astype(int), index=frame.index
        )
    elif mechanic == "DEFERRED_OI_EXPANSION":
        state_return = frame[
            f"deferred_return_{int(params['return_horizon'])}d_bps"
        ]
        direction = pd.Series(
            np.sign(state_return.fillna(0.0)).astype(int), index=frame.index
        )
    elif mechanic == "DELIVERY_DIRECTION_PRESSURE":
        direction = frame["delivery_direction_sign"].fillna(0.0).astype(int)
    else:
        raise KeyError(mechanic)
    ...
    if mechanic == "CASH_MOMENTUM_BREAKOUT":
        ...
        mask &= state_return.abs().ge(float(params["minimum_return_bps"]))
    elif mechanic == "CASH_VOLUME_PRESSURE":
        volume_state = frame[f"cash_volume_z_{int(params['volume_z_lookback'])}"]
        impulse = frame[f"impulse_{int(params['impulse_hours'])}_atr"]
        mask = state_return.abs().ge(float(params["minimum_return_bps"]))
        mask &= volume_state.ge(float(params["minimum_volume_z"]))
        mask &= (direction * impulse).ge(0.0)
        mask &= (direction * frame["body_atr"]).ge(
            float(params["confirmation_min_atr"])
        )
    elif mechanic == "DEFERRED_BASIS_REVERSION":
        impulse = frame[f"impulse_{int(params['impulse_hours'])}_atr"]
        mask = basis_state.abs().ge(float(params["minimum_basis_z"]))
        mask &= (direction * impulse).le(-float(params["dislocation_min_atr"]))
        mask &= (direction * frame["body_atr"]).ge(
            float(params["confirmation_min_atr"])
        )
    elif mechanic == "DEFERRED_OI_EXPANSION":
        impulse = frame[f"impulse_{int(params['impulse_hours'])}_atr"]
        mask = state_return.abs().ge(float(params["minimum_return_bps"]))
        mask &= frame["deferred_oi_change_bps"].ge(
            float(params["minimum_oi_change_bps"])
        )
        ...
        delivery_state = frame[
            f"delivery_z_{int(params['delivery_z_lookback'])}"
        ]
        impulse = frame[f"impulse_{int(params['impulse_hours'])}_atr"]
        mask = delivery_state.ge(float(params["minimum_delivery_z"]))
        mask &= (direction * impulse).ge(float(params["trend_min_atr"]))
        mask &= (direction * frame["body_atr"]).ge(
            float(params["confirmation_min_atr"])
        )
...
def generate_manifest(
    ...
    for mechanic in MECHANICS:
        ...
        if admitted != policies_per_mechanic:
            raise ValueError(
                f"Only {admitted} coverage-eligible V91 policies for {mechanic}; "
    ...
    if len(manifest) != expected or manifest["policy_id"].duplicated().any():
        raise ValueError("Invalid V91 policy manifest")
    return manifest
        ...
        selected.append(
            {
                ...
                "mechanic": mechanic,
                "sge_date": pd.Timestamp(row["date"]),
                "sge_available_utc": pd.Timestamp(row["available_utc"]),
                "sge_cash_close": float(row["cash_close"]),
                "sge_deferred_close": float(row["deferred_close"]),
                "sge_state_feature": state_column,
                "sge_state_value": float(row[state_column]),
                "session_slot": slot,
                **outcome,
            }
        )
