...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Paired Anti-Signal Campaign V15 Result",
        "",
    ...
    lines.extend(
        (
            "",
            "Every policy is paired to a V14 source definition with direction reversed.",
            "Signal timing, features, exits, costs, and gates remain paired to V14.",
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V15 outcomes already exist")
    ...
    for offset, item in enumerate(manifest.itertuples(index=False), 1):
        trades = CAMPAIGN.simulate_variant(frame, item, config, cache)
    ...
    decision = (
        "CHOP_ANTISIGNAL_V15_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_ANTISIGNAL_V15_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_ANTISIGNAL_V15_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_antisignal_v15_artifacts",
            "files": {
                name: {
