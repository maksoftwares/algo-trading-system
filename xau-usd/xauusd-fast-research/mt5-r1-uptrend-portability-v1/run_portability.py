...
def _report(result: dict[str, Any], metrics: pd.DataFrame) -> str:
    ...
    for row in metrics.to_dict("records"):
        ...
        lines.append(
            ...
            f"{row['top_winners_removed_stress_net_r']:.3f} | "
...
def main() -> int:
    config_path = ROOT / "config" / "mt5_r1_uptrend_portability_v1.json"
    ...
    primary_failed_checks = {
        stage: [
            check
            for check, passed in audit[primary_id][stage]["checks"].items()
            if not passed
        ]
        for stage in config["windows"]
    }
    failed_check_names = {
        check for checks in primary_failed_checks.values() for check in checks
    }
    if primary_pass:
        decision = "RETROSPECTIVE_R1_SURVIVOR_REQUIRES_FORWARD_SHADOW"
        interpretation = (
            "The constrained R1 mechanism passed every frozen Dukascopy stability gate. "
            "It remains retrospective and requires exact-tick parity plus prospective shadow evidence."
        )
    elif failed_check_names and failed_check_names <= {"minimum_trades"}:
        decision = "R1_NEAR_SURVIVOR_REJECT_INSUFFICIENT_SAMPLE"
        interpretation = (
            "The constrained R1 mechanism passed every frozen economic and concentration gate, "
            "but failed a minimum-sample gate. It remains an unqualified near-survivor; no threshold "
            "is relaxed and additional prospective evidence is required."
        )
    else:
        decision = "REJECT_R1_PORTABILITY"
        interpretation = (
            "The constrained R1 mechanism failed at least one frozen Dukascopy stability gate. "
            "The MT5 edge is not portable enough for specialist qualification."
        )
    result = {
        "schema_version": config["schema_version"],
        "decision": decision,
        "primary_policy": primary_id,
        "primary_failed_checks": primary_failed_checks,
        "gate_audit": audit,
