...
from dxy_silver_consensus import (
...
def test_feature_is_causal_and_requires_dxy_silver_consensus() -> None:
    date = pd.Timestamp("2024-01-02", tz="UTC")
...
def test_policy_grid_has_exactly_one_thousand_members() -> None:
    calibration = {
        ...
        "maximum_signed_xau_response_ratio_grid": [0.0, 0.25, 0.5, 0.75],
...
def test_source_audits_are_sliced_on_registered_stage_boundaries() -> None:
    config = json.loads(
        (ROOT / "config" / "dxy_silver_consensus_xau_v83.json").read_text(
            encoding="utf-8"
        )
    )
    assert SCOPES == (
    ...
    assert source_audit_decision("development") == (
        "V83_DEVELOPMENT_SOURCE_AUDIT_PASS"
    )
    assert source_audit_output_path(config, "exam").name == (
        "DXY_SILVER_XAU_V83_EXAM_SOURCE_AUDIT.json"
    )
