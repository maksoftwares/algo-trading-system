...
def audit_month_bounds(config: dict[str, Any], scope: str) -> tuple[str, str]:
    ...
    if scope not in STAGES:
        raise ValueError(f"unknown V83 source-audit scope: {scope}")
...
def source_audit_decision(scope: str) -> str:
    if scope == "calibration":
        return "V83_CALIBRATION_SOURCE_AUDIT_PASS"
    if scope == "full":
        return "V83_SOURCE_AUDIT_PASS"
    if scope not in STAGES:
        raise ValueError(f"unknown V83 source-audit scope: {scope}")
    return f"V83_{scope.upper()}_SOURCE_AUDIT_PASS"
...
def require_prior_economic_pass(config: dict[str, Any], scope: str) -> None:
    ...
    output = ROOT / str(config["outputs"]["directory"])
    path = output / f"DXY_SILVER_XAU_V83_{prior.upper()}_AUDIT.json"
    if not path.is_file():
        raise RuntimeError(
            f"V83 {scope} source remains sealed until {prior} passes"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"V83 {scope} source remains sealed because {prior} failed"
        )
...
def run_source_audit(scope: str = "full") -> dict[str, Any]:
    ...
    if path.exists():
        raise FileExistsError(f"V83 {scope} source audit already exists")
    ...
    for symbol, spec in expected.items():
        ...
        if (
            ...
            raise ValueError(f"V83 official {symbol} evidence is invalid")
    ...
    if len(manifests) != expected_count:
        raise ValueError("V83 source audit has incomplete symbol-month coverage")
    audit: dict[str, Any] = {
        "schema_version": f"xauusd_dxy_silver_consensus_xau_v83_{scope}_source_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit frozen V83 source months")
