...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Exit-Hazard Antisignal V19 Result",
        "",
    ...
    lines.extend(
        (
            ...
            "State pressure uses only the latest completed H4 and H1 bars.",
            "Every V18 signal direction is reversed with parameters unchanged.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_exit_hazard_antisignal_v19.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V19 outcomes already exist")
    ...
    decision = (
        "CHOP_EXIT_HAZARD_ANTISIGNAL_V19_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_EXIT_HAZARD_ANTISIGNAL_V19_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_EXIT_HAZARD_ANTISIGNAL_V19_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_exit_hazard_antisignal_v19_artifacts",
            "files": {
                name: {
