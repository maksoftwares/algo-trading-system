...
ANTI_FEATURES = ("execution_source_id", "direction", "atr_ratio", "dist_hi_24h", "ret_4h", "ret_24h")
...
def anti_chase_veto(
    ...
    if str(feature["direction"]).upper() != str(rule["direction"]).upper():
        return False
    numeric = np.asarray(
        [feature["atr_ratio"], feature["dist_hi_24h"], feature["ret_4h"], feature["ret_24h"]],
        dtype=float,
    )
    if not np.isfinite(numeric).all():
        return False
    atr_ratio, distance, ret_4h, ret_24h = numeric
