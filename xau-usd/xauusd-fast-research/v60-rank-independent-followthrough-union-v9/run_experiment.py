...
def main() -> int:
    ...
    evaluator = load_module(
        "v60_v9_dynamic_evaluator", resolve(inputs["shared_evaluator"]["path"])
    )
    anti_module = load_module(
        "v60_v9_crossfeed", resolve(inputs["antichase_experiment_source"]["path"])
    )
    ...
    with tempfile.TemporaryDirectory(prefix="v60-rank-independent-union-v9-") as temporary:
        ...
        replay_config["report_title"] = "V60 Rank-Independent Follow-Through Union V9"
    ...
    anti_audit["rank_independent_antichase_proposal"] = anti_base & followthrough
    broker = broker.merge(
        anti_audit[["candidate_id", "rank_independent_antichase_proposal"]],
        on="candidate_id",
        how="left",
        validate="one_to_one",
    )
    broker["v2_baseline_path_proposal"] = broker["would_veto"].astype(str).str.lower().eq("true")
    broker["rank_independent_antichase_proposal"] = broker[
        "rank_independent_antichase_proposal"
    ].fillna(False)
    ...
    result.update(
        {
            ...
            "limitations": [
                ...
                "The follow-through thresholds and rank-removal hypothesis were selected after outcomes were exposed.",
                "This was a single predeclared structural change and was not tuned after replay.",
    ...
    (OUTPUTS / "RESULT.md").write_text(
        "\n".join(
            [
                "# V60 Rank-Independent Follow-Through Union V9 Result",
                "",
