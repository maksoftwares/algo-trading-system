...
def canonical_alpha_health_pnl(
    scenario_pnl_usd: float, incremental_cost_stress_usd: float
) -> float:
    values = np.asarray([scenario_pnl_usd, incremental_cost_stress_usd], dtype=float)
    if not np.isfinite(values).all() or incremental_cost_stress_usd < 0.0:
        raise ValueError("P/L and incremental cost stress must be finite and cost nonnegative")
    return float(scenario_pnl_usd + incremental_cost_stress_usd)
    ...
    class DynamicCombinedScenario(replay.Scenario):
        def __init__(
            ...
            **kwargs: Any,
            ...
            self.source_health_cost_offset_usd = float(source_health_cost_offset_usd)
            canonical_alpha_health_pnl(0.0, self.source_health_cost_offset_usd)
        ...
        def _close(
            ...
            *,
            ...
            if evaluator.policy_targets_source(source_id, self.veto_policy):
                self.source_closed[source_id].append(
                    canonical_alpha_health_pnl(
                        float(pnl), self.source_health_cost_offset_usd
                    )
                )
            ...
            self.veto_audit.append(
                {
                    ...
                    "source_health_cost_offset_usd": self.source_health_cost_offset_usd,
