...
def main() -> int:
    ...
    evaluator = load_module(
        "v60_v12_dynamic_evaluator", resolve(inputs["shared_evaluator"]["path"])
    )
    anti_module = load_module(
        "v60_v12_crossfeed", resolve(inputs["antichase_experiment_source"]["path"])
    )
    ...
    with tempfile.TemporaryDirectory(prefix="v60-canonical-alpha-health-union-v12-") as temporary:
        ...
        replay_config["report_title"] = "V60 Canonical Alpha-Health Union V12"
    ...
    anti_audit["v6_antichase_proposal"] = anti_base & followthrough
    broker = broker.merge(
        anti_audit[["candidate_id", "v6_antichase_proposal"]],
        on="candidate_id",
        how="left",
        validate="one_to_one",
    )
    broker["v2_baseline_path_proposal"] = broker["would_veto"].astype(str).str.lower().eq("true")
    broker["v6_antichase_proposal"] = broker["v6_antichase_proposal"].fillna(False)
    ...
    v2_vetoes = executed_vetoes.loc[
        executed_vetoes["proposal_rule"].astype(str).str.contains(
            "V2_SOURCE_HEALTH", regex=False
        )
    ].copy()
    v2_component = closed_metrics(v2_vetoes["baseline_runtime_pnl_usd"])
    ...
    v2_pf = v2_component["profit_factor"]
    v2_gates = {
        "minimum_executed_vetoes": v2_component["trades"] >= int(config["acceptance"]["minimum_v2_executed_vetoes"]),
        "positive_avoided_pnl": v2_component["avoided_pnl_usd"] > float(config["acceptance"]["minimum_v2_avoided_pnl_usd_exclusive"]),
        "veto_profit_factor_below_limit": v2_pf is not None and v2_pf < float(config["acceptance"]["maximum_v2_veto_profit_factor_exclusive"]),
    }
    gates = {
        ...
        "v2_component_support": bool(all(v2_gates.values())),
    ...
    result.update(
        {
            ...
            "dynamic_policy": {
                ...
                "v2_health_outcome": config["v2_health_outcome"],
            ...
            "limitations": [
                ...
                "The underlying thresholds, outcomes, and V6/V11 cost failures were exposed before V12.",
                "Canonical alpha-health ledger separation was selected post-hoc and was not tuned after replay.",
                "Execution-cost stress still applies fully to portfolio P/L, equity, and drawdown.",
    ...
    (OUTPUTS / "RESULT.md").write_text(
        "\n".join(
            [
                "# V60 Canonical Alpha-Health Union V12 Result",
                "",
                ...
                f"Canonical-health V2 support: {v2_component['trades']} historical vetoes, ${v2_component['avoided_pnl_usd']:.2f} avoided P/L, veto PF {v2_component['profit_factor']:.3f}.",
