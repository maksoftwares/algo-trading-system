...
def _paths(stage: str) -> tuple[Path, Path, Path]:
    prefix = f"COMEX_ROUND_BARRIER_REJECTION_V71_{stage.upper()}"
    return (
...
def main() -> None:
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            features = build_barrier_features(session, rule=rule)
            candidate_frames.append(
                generate_candidates(features, policy=policy, rule=rule)
            )
        if number % 25 == 0:
    ...
    if not candidates.empty:
        ...
        if candidates["candidate_id"].duplicated().any():
            raise RuntimeError("V71 stage candidates contain duplicate IDs.")
    ...
    decision = (
        f"V71_{stage.upper()}_PASS"
        if result["gate_passed"]
        else f"V71_{stage.upper()}_FAIL_TERMINAL"
    )
    payload: dict[str, Any] = {
        "schema_version": "xauusd_comex_round_barrier_rejection_v71_stage_audit",
