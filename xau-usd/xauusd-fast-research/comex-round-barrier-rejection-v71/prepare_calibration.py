...
CONFIG = ROOT / "config" / "comex_round_barrier_rejection_v71.json"
OUTPUTS = ROOT / "outputs"
AUDIT = OUTPUTS / "COMEX_ROUND_BARRIER_REJECTION_V71_CALIBRATION_AUDIT.json"
CANDIDATES = OUTPUTS / "COMEX_ROUND_BARRIER_REJECTION_V71_CALIBRATION_CANDIDATES.csv"
...
def main() -> None:
    if AUDIT.exists() or CANDIDATES.exists():
        raise RuntimeError("Calibration outputs already exist; rerun is prohibited.")
    config = load_config(CONFIG)
    manifest = Path(config["source"]["download_manifest"])
    expected_manifest = str(config["source"]["download_manifest_sha256"]).lower()
    if sha256_file(manifest).lower() != expected_manifest:
        raise RuntimeError(
            "The COMEX download manifest hash does not match the contract."
        )
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            feature_frames.append(build_barrier_features(session, rule=rule))
    ...
    if len(policies) != 1000:
        raise RuntimeError(
            f"Expected exactly 1000 registered policies, got {len(policies)}."
        )
    ...
    decision = (
        "V71_CALIBRATION_PASS_READY_TO_LOCK"
        if selected is not None
        else "V71_CALIBRATION_FREQUENCY_STRUCTURE_FAIL"
    )
    selected_candidates = (
    ...
    payload: dict[str, object] = {
        "schema_version": "xauusd_comex_round_barrier_rejection_v71_calibration_audit",
