...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/chop_three_mechanism_rawtick_v26.json",
    "src/__init__.py",
    "src/confirmation.py",
...
def build_lock(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for attempt, parity in manifest["independent_signal_parity"].items():
        if (
            ...
            raise ValueError(
                f"Independent V26 signal logic does not match V24: {attempt}"
            )
    payload: dict[str, Any] = {
        "schema_version": "xauusd_chop_three_mechanism_rawtick_v26_contract",
