...
def _render(result: Mapping[str, Any], metrics: Mapping[str, Any]) -> str:
    return "\n".join(
        (
            "# Chop Three-Mechanism Raw-Tick V26 Result",
            "",
            f"Decision: `{result['decision']}`",
            "",
            f"Component raw signals: **{result['origin_raw_signals']}**",
            ...
            "",
            "All three signal streams matched V24 exactly before raw outcomes were opened.",
            "This composite was selected after component outcomes were exposed.",
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V26 raw-tick outcomes already exist")
    ...
    if trades.empty:
        raise ValueError("V26 raw-tick replay produced no accepted trades")
    ...
    decision = (
        "RAW_TICK_ECONOMIC_CHOP_SPECIALIST_CANDIDATE_FOUND"
    ...
    result = {
        ...
        "decision": decision,
        "origin_attempts": [
            int(component["origin_attempt"]) for component in config["components"]
        ],
        "origin_raw_signals": int(
            config["composite"]["expected_total_raw_signals"]
        ),
        "origin_m5_trades": int(
        ...
        "execution_audit": {
            ...
            "accepted_trades_by_component": {
                str(int(attempt)): int(count)
                for attempt, count in trades.groupby("origin_attempt").size().items()
            },
        ...
        "authorization": {
            ...
            "composite_selected_after_component_outcomes": True,
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_three_mechanism_rawtick_v26_artifacts",
            "files": {
                name: {
