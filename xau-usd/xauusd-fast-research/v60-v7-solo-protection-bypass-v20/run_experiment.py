...
def render_markdown(result: Mapping[str, Any]) -> str:
    ...
    v20 = result["historical"]["challenger"]
    lines = [
        ...
        "",
        "| Metric | Deployed V60 | Frozen V6 | V20 | V20 vs V6 |",
        "|---|---:|---:|---:|---:|",
        f"| Trades | {v60['trades_closed']} | {v6['trades_closed']} | {v20['trades_closed']} | {v20['trades_closed'] - v6['trades_closed']:+d} |",
        f"| Net P/L | ${v60['net_pnl_usd']:.2f} | ${v6['net_pnl_usd']:.2f} | ${v20['net_pnl_usd']:.2f} | ${v20['net_pnl_usd'] - v6['net_pnl_usd']:+.2f} |",
        f"| Profit factor | {v60['profit_factor']:.4f} | {v6['profit_factor']:.4f} | {v20['profit_factor']:.4f} | {v20['profit_factor'] - v6['profit_factor']:+.4f} |",
        f"| Closed drawdown | ${v60['maximum_lifetime_closed_drawdown_usd']:.2f} | ${v6['maximum_lifetime_closed_drawdown_usd']:.2f} | ${v20['maximum_lifetime_closed_drawdown_usd']:.2f} | ${v20['maximum_lifetime_closed_drawdown_usd'] - v6['maximum_lifetime_closed_drawdown_usd']:+.2f} |",
        f"| Equity drawdown | ${v60['maximum_lifetime_equity_drawdown_usd']:.2f} | ${v6['maximum_lifetime_equity_drawdown_usd']:.2f} | ${v20['maximum_lifetime_equity_drawdown_usd']:.2f} | ${v20['maximum_lifetime_equity_drawdown_usd'] - v6['maximum_lifetime_equity_drawdown_usd']:+.2f} |",
        "",
        "## Mechanism exercise",
        "",
        f"- V7 solo-bypass trade IDs observed: `{len(v20['v7_solo_bypass_trade_ids'])}`.",
        f"- V7-only five-second bypass cycles: `{v20['v7_solo_bypass_cycles']}`.",
        f"- V7/non-V7 mixed-basket cycles delegated unchanged: `{v20['v7_mixed_basket_cycles']}`.",
        "",
        "## Gates",
        "",
    ]
...
def main() -> int:
    ...
    if v17_result["decision"] != "DIAGNOSTIC_SUPPORTS_TARGETED_PROTECTION_RESEARCH":
        raise ValueError("V17 does not support the V20 research lane")
    v18_result = json.loads(
        resolve(config["inputs"]["v18_result"]["path"]).read_text(encoding="utf-8")
    )
    if v18_result["decision"] != "REJECT_KEEP_V60_AND_FROZEN_V6":
        raise ValueError("V18 rejection identity changed")

    evaluator = load_module(
        "v20_shared_evaluator", resolve(config["inputs"]["shared_evaluator"]["path"])
    )
    v6_scenario = load_module(
        "v20_v6_scenario", resolve(config["inputs"]["v6_scenario"]["path"])
    )
    scenario = load_module(
        "v20_local_scenario", resolve(config["inputs"]["v20_scenario"]["path"])
    )
    ...
    with tempfile.TemporaryDirectory(prefix="v60-v20-v7-solo-bypass-") as temporary:
        ...
        baseline_frame, v20_frame = captured
    ...
    monthly = monthly_table(v20_frame, "V20")
    ...
    gates = {
        ...
        * float(config["acceptance"]["minimum_frequency_retention_fraction_vs_v60"]),
        ...
        "mechanism_exercised": (
            len(historical["challenger"]["v7_solo_bypass_trade_ids"]) > 0
            and any(
    ...
    result = {
        ...
        "monthly": {"v20": monthly_observed, "v6": monthly_reference},
        "source": source_table(v20_frame).to_dict("records"),
        ...
        "gates": gates,
        "limitations": [
            "All historical outcomes were exposed before V20 nomination.",
            "V17 nominated V7 and V18's full exemption was rejected before V20.",
            "The historical replay cannot authorize deployment.",
            "No independent Capital.com forward endpoint exists yet for the V7-only bypass.",
    ...
    source_table(v20_frame).to_csv(OUTPUTS / "SOURCE.csv", index=False)
    v20_frame.to_csv(OUTPUTS / "TRADE_AUDIT.csv", index=False)
        ...
        json.dumps(
            {
                ...
                "mechanism": {
                    "solo_bypass_trade_ids": len(
                        historical["challenger"]["v7_solo_bypass_trade_ids"]
                    ),
                    "solo_bypass_cycles": historical["challenger"][
                        "v7_solo_bypass_cycles"
                    ],
                    "mixed_basket_cycles": historical["challenger"][
                        "v7_mixed_basket_cycles"
                    ],
                },
