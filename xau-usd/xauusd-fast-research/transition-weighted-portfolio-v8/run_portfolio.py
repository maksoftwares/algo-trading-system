...
from portfolio import build_weighted_trades  # noqa: E402
...
def verify_lock(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError("Run lock_contract.py before opening portfolio outcomes")
    ...
    _verify_record(lock["component_trade_file"], REPO, "component trades")
    _verify_record(lock["manifest_file"], REPO, "manifest")
    _verify_record(lock["input_evidence"], REPO, "input evidence")
...
def _render(result: Mapping[str, Any], best: Mapping[str, Any]) -> str:
    return "\n".join(
        (
            "# Transition Weighted Portfolio V8 Result",
            "",
            ...
            f"Minimum era average: **{float(best['minimum_era_average_stress_r']):.3f} R**",
            f"Closed drawdown: **{float(best['whole_closed_drawdown_r']):.3f} R**",
            "",
            "This is post-selection portfolio discovery on exposed component outcomes.",
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V8 outcomes already exist")
    ...
    foundation_module = load_module(
        "transition_weighted_v8_run_foundation", macro_root / "src" / "foundation.py"
    )
    ...
    component_trades = pd.read_parquet(output / config["outputs"]["component_trades"])
    manifest = pd.read_csv(output / config["outputs"]["manifest"])
    rows: list[dict[str, Any]] = []
    trade_frames: list[pd.DataFrame] = []
    maximum_daily = int(config["execution"]["maximum_trades_per_portfolio_utc_day"])
    adjustment = int(config["selection"]["selection_adjustment_count"])
    for policy in manifest.itertuples(index=False):
        trades = build_weighted_trades(component_trades, policy, maximum_daily)
        score = foundation_module.SCORE.score_variant(
            trades, foundation.execution_frame, score_config
        )
        adjusted = min(1.0, float(score["daily_pvalue"]) * adjustment)
    ...
    decision = (
        "TRANSITION_WEIGHTED_V8_STATISTICAL_FINALIST_FOUND"
        if not statistical.empty
        else (
            "TRANSITION_WEIGHTED_V8_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_TRANSITION_WEIGHTED_V8_ECONOMIC_FINALIST"
        )
    )
    result = {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_transition_weighted_v8_artifacts",
            "files": {
                name: {
