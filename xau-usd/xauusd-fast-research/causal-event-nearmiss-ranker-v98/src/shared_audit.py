...
V98_SLEEVE = "V98_CAUSAL_EVENT_NEARMISS_MODEL"
...
def prepare_v98_ledger(trades: pd.DataFrame) -> pd.DataFrame:
    required = {
        "attempt_no",
        "policy_id",
        "mechanic",
    ...
    if missing:
        raise ValueError(f"V98 trade ledger is missing columns: {missing}")
    result = trades.copy()
    ...
    if result["risk_usd"].le(0.0).any():
        raise ValueError("V98 initial risk must be positive")
    if result["exit_time"].lt(result["entry_time"]).any():
        raise ValueError("V98 exit precedes entry")
    if not result["direction"].isin(("LONG", "SHORT")).all():
        raise ValueError("V98 direction is invalid")
    if result.duplicated(["policy_id", "entry_time"]).any():
        raise ValueError("Duplicate V98 policy entry")
    ...
    if result["fee_stress_open_cost_usd"].lt(-1e-9).any():
        raise ValueError("V98 stress cost creates a rebate")
...
def route_v98_candidates(
    ...
    prepared = prepare_v98_ledger(candidates)
    ...
    maximum_v98_daily = int(limits["maximum_v98_entries_per_utc_date"])
    ...
    for order, row in enumerate(prepared.itertuples(index=False), start=1):
        ...
        if entry_time in accepted_entry_times:
            reason = "DUPLICATE_V98_ENTRY_TIME"
        elif drawdown_suspended:
            reason = "ACCOUNT_DRAWDOWN_SUSPENDED"
        elif accepted_daily.get(day, 0) >= maximum_v98_daily:
            reason = "MAXIMUM_V98_ENTRIES_PER_UTC_DATE"
...
def shared_window_metrics(
    ...
    for window, stage in stage_by_window.items():
        ...
        checks = {
            ...
            "minimum_v98_stress_pf": addon_pf
            >= float(gates[stage]["minimum_stress_pf"]),
            "v98_winner_removed_positive": float(removed.sum()) > 0.0,
        ...
        rows.append(
            {
                ...
                "v98_accepted_trades": int(len(addon_window)),
                ...
                "v98_stress_net_usd": float(addon_values.sum()),
                "v98_stress_pf": addon_pf,
                "v98_winner_removed_stress_net_usd": float(removed.sum()),
