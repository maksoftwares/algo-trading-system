...
for source in (SRC, BASE_SRC, FOUNDATION):
...
from v33 import generate_candidates, load_config  # noqa: E402
...
def _paths(stage: str) -> tuple[Path, Path, Path]:
    prefix = f"COMEX_SIZE_SEGMENT_V33_{stage.upper()}"
    return (
...
def main() -> None:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise RuntimeError(f"{stage} outputs already exist; rerun is prohibited.")
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            bars = base.build_bar_features(
                session,
                large_trade_size=int(policy["large_trade_size"]),
                rule=rule,
            )
            candidate_frames.append(generate_candidates(bars, policy=policy, rule=rule))
        if number % 50 == 0:
    ...
    if not candidates.empty:
        ...
        if candidates["candidate_id"].duplicated().any():
            raise RuntimeError("V33 stage candidates contain duplicate IDs.")
    ...
    decision = (
        f"V33_{stage.upper()}_PASS"
        if result["gate_passed"]
        else f"V33_{stage.upper()}_FAIL_TERMINAL"
    )
    payload: dict[str, Any] = {
        "schema_version": "xauusd_comex_size_segment_v33_stage_audit",
        ...
        "labels_sha256": base.sha256_file(label_path),
        **result,
