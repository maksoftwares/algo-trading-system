...
V100_SLEEVE = "V100_CAUSAL_HEALTH_GATED_EVENT_SLEEVES"
...
def prepare_v100_ledger(trades: pd.DataFrame) -> pd.DataFrame:
    required = {
        "attempt_no",
        "policy_id",
        "mechanic",
    ...
    if missing:
        raise ValueError(f"V100 trade ledger is missing columns: {missing}")
    result = trades.copy()
    ...
    if result["risk_usd"].le(0.0).any():
        raise ValueError("V100 initial risk must be positive")
    if result["exit_time"].lt(result["entry_time"]).any():
        raise ValueError("V100 exit precedes entry")
    if not result["direction"].isin(("LONG", "SHORT")).all():
        raise ValueError("V100 direction is invalid")
    if result.duplicated(["policy_id", "entry_time"]).any():
        raise ValueError("Duplicate V100 policy entry")
    ...
    result["sleeve_id"] = V100_SLEEVE
    result["pnl_usd"] = result["net_r"] * result["risk_usd"]
    result["fee_stress_pnl_usd"] = result["stress_net_r"] * result["risk_usd"]
    side_pnl = result["side_pnl_before_fees_usd"]
    result["open_cost_usd"] = side_pnl - result["pnl_usd"]
    result["fee_stress_open_cost_usd"] = side_pnl - result["fee_stress_pnl_usd"]
    if result["fee_stress_open_cost_usd"].lt(-1e-9).any():
        raise ValueError("V100 stress cost creates a rebate")
    if result["open_cost_usd"].lt(-1e-9).any():
        raise ValueError("V100 venue cost creates a rebate")
...
def route_v100_candidates(
    ...
    prepared = prepare_v100_ledger(candidates)
    ...
    maximum_v100_daily = int(limits["maximum_v100_entries_per_utc_date"])
    ...
    for order, row in enumerate(prepared.itertuples(index=False), start=1):
        ...
        if entry_time in accepted_entry_times:
            reason = "DUPLICATE_V100_ENTRY_TIME"
        elif drawdown_suspended:
            reason = "ACCOUNT_DRAWDOWN_SUSPENDED"
        elif accepted_daily.get(day, 0) >= maximum_v100_daily:
            reason = "MAXIMUM_V100_ENTRIES_PER_UTC_DATE"
...
def shared_window_metrics(
    ...
    for window, stage in stage_by_window.items():
        ...
        checks = {
            ...
            "minimum_v100_stress_pf": addon_pf
            >= float(gates[stage]["minimum_stress_pf"]),
            "v100_winner_removed_positive": float(removed.sum()) > 0.0,
        ...
        rows.append(
            {
                ...
                "v100_accepted_trades": int(len(addon_window)),
                ...
                "v100_stress_net_usd": float(addon_values.sum()),
                "v100_stress_pf": addon_pf,
                "v100_winner_removed_stress_net_usd": float(removed.sum()),
