...
def _verify_predecessor(config: Mapping[str, Any]) -> dict[str, Any]:
    predecessor = config["predecessor"]
    result_path = (ROOT / predecessor["v99_result"]["path"]).resolve()
    artifact_path = (ROOT / predecessor["v99_artifact_manifest"]["path"]).resolve()
    if _sha256(result_path) != predecessor["v99_result"]["sha256"]:
        raise ValueError("V100 V99 result hash mismatch")
    if _sha256(artifact_path) != predecessor["v99_artifact_manifest"]["sha256"]:
        raise ValueError("V100 V99 artifact manifest hash mismatch")
    result = json.loads(result_path.read_text(encoding="utf-8"))
    if result.get("decision") != predecessor["required_decision"]:
        raise RuntimeError("V99 is not terminal; V100 remains sealed")
    return {
        "v99_contract_sha256": result["contract_sha256"],
        "v99_result_sha256": _sha256(result_path),
        "v99_terminal_reason": result["decision"],
    }
...
def _verify_contract(config: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    if not LOCK_PATH.is_file() or not MANIFEST_PATH.is_file() or not CENSUS_PATH.is_file():
        raise FileNotFoundError("Run lock_contract.py before opening V100 outcomes")
    lock_module = _load_module(
        "causal_health_gated_event_sleeves_v100_contract_verify",
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("V100 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V100 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V100 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V100 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V100 signal census changed")
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V100 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V100 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V100 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V100 {stage} metrics changed")
    if payload["trades_sha256"] != _sha256(_trades_path(stage)):
        raise ValueError(f"V100 {stage} trades changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V100 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V100 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V100 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_causal_health_gated_event_sleeves_v100_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_causal_health_gated_event_sleeves_v100_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V100_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V100_{stage.upper()}_PASS_ADVANCE"
    return f"V100_{stage.upper()}_FAIL_PROGRAM_STOP"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V100 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# V100 Causal Health-Gated Event Sleeves Result",
        "",
    ...
    for stage in STAGES:
        ...
        if row is None:
            lines.append(f"| {stage} | SEALED | SEALED | SEALED | SEALED | SEALED | SEALED | SEALED |")
            continue
        lines.append(
            f"| {stage} | {row['incoming_policy_count']} | {row['gate_pass_policy_count']} | "
            f"{row['advanced_policy_count']} | {row['best_stress_pf']:.3f} | "
            f"{row['best_average_stress_r']:.3f} | {row['best_trades_per_weekday']:.3f} | "
            f"{row['lowest_fdr_qvalue']:.4f} |"
        )
    lines.extend(
        [
            "",
            "V100 is the program ceiling. A failure stops strategy versioning; there is no V101.",
            "",
            "Research only. No model fitting, EA, demo, live, paid-data, Databento, or broker authority is granted.",
            "",
        ]
    )
    return "\n".join(lines)
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    campaign = _load_module(
        "causal_health_gated_event_sleeves_v100_run_campaign",
        ROOT / "src" / "campaign.py",
    )
    frame = campaign.load_source(config, outcomes=True)
    ...
    trades = campaign.selected_trade_ledger(selected, config, cache, stage)
    ...
    finite_pf = pd.to_numeric(metrics["stress_pf"], errors="coerce").replace(
    ...
    result["conditional_predecessor"] = predecessor
    result["data_evidence"] = {
        "episode_action_rows": int(len(frame)),
        "episode_ids": int(frame["episode_id"].nunique()),
        "family_horizon_rows": {
            f"{family}|{int(horizon)}": int(value)
            for (family, horizon), value in frame.groupby(
