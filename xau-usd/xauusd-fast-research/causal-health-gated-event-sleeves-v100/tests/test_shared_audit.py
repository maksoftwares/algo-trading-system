...
def _candidate(
    ...
    return {
        ...
        "net_r": 0.5,
        "stress_net_r": 0.4,
        "side_pnl_before_fees_usd": risk_usd * 0.5 + 1.0,
    }


def test_prepare_v100_ledger_reconciles_stress_cost() -> None:
    result = prepare_v100_ledger(
        pd.DataFrame(
            [
                _candidate(
                    "policy-a",
    ...
    assert result.loc[0, "open_cost_usd"] == 1.0
    assert result.loc[0, "fee_stress_open_cost_usd"] == 2.0
...
def test_router_uses_fixed_priority_and_current_portfolio_limits() -> None:
    ...
    limits = {
        ...
        "maximum_v100_entries_per_utc_date": 2,
        "drawdown_suspend_usd": 225.0,
        "drawdown_resume_usd": 180.0,
    }
    accepted, decisions = route_v100_candidates(baseline, candidates, limits)
    assert accepted["policy_id"].tolist() == ["policy-a"]
    assert decisions["reason"].tolist() == [
        "ACCEPTED",
        "DUPLICATE_V100_ENTRY_TIME",
...
def test_router_does_not_look_ahead_to_a_future_frozen_v59_entry() -> None:
    ...
    accepted, _ = route_v100_candidates(baseline, candidates, limits)
