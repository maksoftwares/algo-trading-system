...
def _load_final_candidates(
    ...
    if not policy_ids:
        raise RuntimeError(
            "V100 shared audit is sealed because Final selected no policy"
        )
    frames = []
    ...
    if not frames:
        raise RuntimeError("Final V100 policies have no staged trade ledger")
    candidates = pd.concat(frames, ignore_index=True)
    if set(candidates["policy_id"].astype(str)) != set(policy_ids):
        raise ValueError("A Final V100 policy is missing from the staged trade ledgers")
    return candidates, policy_ids


def main() -> int:
    if MARKER_PATH.exists() or RESULT_PATH.exists():
        raise RuntimeError("V100 shared audit has already been opened")
    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    run = _load_module(
        "causal_health_gated_event_sleeves_v100_shared_run", ROOT / "run_research.py"
    )
    ...
    _write_json(
        MARKER_PATH,
        {
            "schema_version": "xauusd_causal_health_gated_event_sleeves_v100_shared_stage_open",
    ...
    v60_audit = _load_module(
        "causal_health_gated_event_sleeves_v100_v60_audit",
    ...
    result = {
        "schema_version": "xauusd_causal_health_gated_event_sleeves_v100_shared_audit",
        "created_utc": datetime.now(UTC).isoformat(),
        "contract_sha256": contract_hash,
        "final_policy_ids": policy_ids,
        "candidate_v100_trades": int(len(candidates)),
        "accepted_v100_trades": int(len(accepted)),
        ...
        "checks": checks,
        "passed": passed,
        "decision": "V100_SHARED_V59_V60_PASS" if passed else "V100_SHARED_V59_V60_FAIL",
