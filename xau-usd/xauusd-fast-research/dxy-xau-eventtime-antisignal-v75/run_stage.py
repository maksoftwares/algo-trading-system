...
V74_SRC = ROOT.parent / "dxy-xau-eventtime-catchup-v74" / "src"
BASE_SRC = ROOT.parent / "comex-size-segment-flow-v32" / "src"
LABEL_SRC = ROOT.parent / "comex-futures-foundation-v1" / "src"
for source in (ROOT / "src", V72_SRC, V74_SRC, BASE_SRC, LABEL_SRC):
...
from inverse_lead import build_inverse_features, generate_candidates, session_quality  # noqa: E402
...
def load_day(
    ...
    *,
    ...
    return dxy_store.quote_frame(start_ms, end_ms), xau_store.quote_frame(start_ms, end_ms)


def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    output = ROOT / str(config["outputs"]["directory"])
    prefix = f"DXY_XAU_V75_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V75 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V75 {stage} sealed because {prior} failed")
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V75 {stage} outputs already exist")
    ...
    dxy_store = ManifestTickStore(
        storage, "DOLLARIDXUSD", source["symbols"]["DOLLARIDXUSD"]
    )
    ...
    for number, date in enumerate(
        ...
        quality = session_quality(date, dxy, xau, rule)
        ...
        if bool(quality["eligible_full_weekday"]):
            ...
            source_candidates = generate_candidates(
                features, policy=policy, family="V74_SOURCE_DIRECTION"
            )
        ...
        if number % 25 == 0:
            print(f"V75 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V75_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V75_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_dxy_xau_v75_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V75 stage")
