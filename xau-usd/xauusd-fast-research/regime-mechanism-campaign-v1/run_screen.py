...
def verify_lock(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if R2.sha256_file(manifest_path) != str(lock["manifest_sha256"]):
        raise ValueError("Strategy manifest changed after lock")
    manifest = pd.read_csv(manifest_path)
    if len(manifest) != int(lock["attempt_count"]):
...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD Regime Mechanism Campaign V1 Result",
        "",
        ...
        "",
        "Historical eras in this package are discovery robustness segments, not untouched holdouts.",
        "No finalist is authorized for training or execution.",
        "",
        "## Regime summary",
        "",
        "| Owner | Attempts | Economic passes | FDR passes |",
        "|---|---:|---:|---:|",
    ]
    for item in result["regime_summary"]:
        lines.append(
            f"| {item['regime_owner']} | {item['attempts']} | "
            f"{item['economic_passes']} | {item['statistical_passes']} |"
        )
    ...
    if shortlist.empty:
        ...
        for row in shortlist.itertuples(index=False):
            lines.append(
                ...
                f"{int(row.minimum_era_trades)} | {float(row.daily_fdr_qvalue):.6f} |"
            )
    lines.extend(
        [
            "",
            "## Next decision",
            "",
            "Only frozen definitions in the shortlist may proceed to a separately locked raw-tick confirmation.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_regime_mechanism_campaign_v1_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    config = json.loads(
        (ROOT / "config" / "regime_mechanism_campaign_v1.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError(
            "V1 outcomes already exist; same-version reruns are not authorized"
        )
    ...
    frame = prepare_features(h1, h4, config, ADAPTIVE, REGIMES)
    ...
    for offset, manifest_row in enumerate(manifest.itertuples(index=False), 1):
        trades = simulate_variant(
            frame, manifest_row, config["execution"], outcome_cache
        )
        metrics = score_variant(trades, frame, config)
        rows.append(
            {
        ...
        if offset % 100 == 0:
            print(
                f"scored={offset}/{len(manifest)} outcomes={len(outcome_cache)}",
                flush=True,
            )
    metrics = pd.DataFrame(rows)
    metrics["daily_fdr_qvalue"] = bh_adjust(metrics["daily_pvalue"])
    metrics["statistical_pass"] = metrics["daily_fdr_qvalue"].le(
        float(config["selection"]["false_discovery_rate"])
    )
    finalists = metrics.loc[metrics["economic_pass"]].copy()
    ...
    maximum = int(
        config["selection"]["maximum_economic_finalists_per_mechanic"]
    )
    ...
    decision = (
        ...
        else "NO_REGIME_MECHANISM_V1_ECONOMIC_FINALIST"
    )
    result = {
        ...
        "cached_h1_outcomes": int(len(outcome_cache)),
        "economic_pass_rows": int(metrics["economic_pass"].sum()),
        "economic_finalist_rows": int(len(shortlist)),
        "statistical_finalist_rows": int(shortlist["statistical_pass"].sum())
        if not shortlist.empty
        else 0,
