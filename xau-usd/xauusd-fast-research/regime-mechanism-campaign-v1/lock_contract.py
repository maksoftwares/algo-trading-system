...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/regime_mechanism_campaign_v1.json",
    "src/__init__.py",
    "src/campaign.py",
    "lock_contract.py",
    "run_screen.py",
...
def _external_records(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    old_contract_path = (
        ROOT / str(source["old_final_contract"])
    ).resolve()
    if R2.sha256_file(old_contract_path) != str(
        source["old_final_contract_sha256"]
    ):
...
def build_lock(config: dict[str, Any]) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_regime_mechanism_campaign_v1_contract",
        ...
        "dependency_files": [
            _record(path, REPO) for path in dependency_paths
        ],
...
def main() -> int:
    config_path = ROOT / "config" / "regime_mechanism_campaign_v1.json"
