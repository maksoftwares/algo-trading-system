...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Stationarity Campaign V20 Result",
        "",
    ...
    lines.extend(
        (
            ...
            "Stationarity diagnostics use only completed H1 bars.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_stationarity_campaign_v20.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V20 outcomes already exist")
    ...
    decision = (
        "CHOP_STATIONARITY_V20_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_STATIONARITY_V20_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_STATIONARITY_V20_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_stationarity_v20_artifacts",
            "files": {
                name: {
