...
SOURCE_MECHANICS = {
    "CHOP_AR1_MEAN_REVERSION": "AR1_MEAN_REVERSION",
    "CHOP_VARIANCE_RATIO_REVERSION": "VARIANCE_RATIO_REVERSION",
    "CHOP_HURST_REVERSION": "HURST_REVERSION",
    "CHOP_RETURN_AUTOCORRELATION_FADE": "RETURN_AUTOCORRELATION_FADE",
    "CHOP_ADAPTIVE_LEVEL_REVERSION": "ADAPTIVE_LEVEL_REVERSION",
}
    ...
    while len(result) < count:
        ...
        for name, items in options.items():
            digest = hashlib.sha256(
                f"chop-stationarity-v20|{mechanic}|{nonce}|{name}".encode("ascii")
            ).digest()
...
def parameter_space(mechanic: str, candidate_count: int = 400) -> list[dict[str, Any]]:
    ...
    if source == "AR1_MEAN_REVERSION":
        ...
        options = dict(
            stationarity_window=(48, 96, 192),
            autocorrelation_lag=(1, 4),
            return_acf_max=(-0.20, -0.05, 0.10),
            impulse_h1_atr_min=(0.10, 0.30, 0.60),
            z_alignment_min=(0.0, 0.50, 1.0),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
        )
    elif source == "ADAPTIVE_LEVEL_REVERSION":
        options = dict(
            adaptive_span=(24, 48, 96),
            residual_z_abs_min=(0.75, 1.25, 1.75),
            residual_phi_max=(0.70, 0.85, 0.95),
            residual_half_life_max=(6.0, 18.0, 48.0),
            level_slope_abs_max=(0.15, 0.35, 0.70),
            zero_crossing_min=(0.02, 0.05, 0.10),
        )
    else:
...
def add_h1_stationarity_features(
    h1: pd.DataFrame,
    config: Mapping[str, Any],
    regime_module: Any,
) -> pd.DataFrame:
    result = h1.sort_values("timestamp_utc", kind="mergesort").reset_index(
        drop=True
    ).copy()
    close = result["mid_close"].astype(float)
    result["atr_h1"] = regime_module.atr(
        result, int(config["features"]["h1_atr_period"])
    )
    atr = result["atr_h1"].replace(0.0, np.nan)
    returns = close.diff()
    result["return_h1"] = returns
    for window in (48, 96, 192):
        ...
        variance_2 = close.diff(2).rolling(window, min_periods=window).var(ddof=0)
        variance_8 = close.diff(8).rolling(window, min_periods=window).var(ddof=0)
        ratio = variance_8 / variance_2.replace(0.0, np.nan)
...
def prepare_frame(
    ...
    h1_state = add_h1_stationarity_features(h1, config, regime_module)
    h1_state = h1_state.drop(
        columns=[
            column
            for column in h1_state.columns
            if column not in {"timestamp_utc", "atr_h1", "return_h1"}
    ...
    ).rename(columns={"timestamp_utc": "h1_state_time"})
    attached = pd.merge_asof(
        attached.sort_values("timestamp_utc"),
        h1_state.sort_values("h1_state_time"),
        left_on="timestamp_utc",
        right_on="h1_state_time",
        direction="backward",
        allow_exact_matches=True,
    ).sort_values("timestamp_utc", kind="mergesort").reset_index(drop=True)
    attached["h1_state_age_m5"] = (
        (attached["timestamp_utc"] - attached["h1_state_time"])
        .dt.total_seconds()
        .div(300.0)
        .round()
    )
...
def signal_mask_direction(
    ...
    if source == "AR1_MEAN_REVERSION":
        ...
        impulse = frame["return_h1"].div(risk_atr)
        ...
        span = int(params["adaptive_span"])
        residual_z = frame[f"adaptive_residual_z_{span}"]
        direction = pd.Series(
            -np.sign(residual_z.fillna(0.0)).astype(int), index=frame.index
        )
        mask = (
            residual_z.abs().ge(float(params["residual_z_abs_min"]))
            & frame[f"adaptive_residual_phi_{span}"].le(
                float(params["residual_phi_max"])
            )
            & frame[f"adaptive_residual_half_life_{span}"].le(
                float(params["residual_half_life_max"])
            )
            & frame[f"adaptive_level_slope_atr_{span}"].abs().le(
                float(params["level_slope_abs_max"])
            )
            & frame[f"adaptive_zero_crossing_{span}"].ge(
                float(params["zero_crossing_min"])
            )
        )
    else:
    ...
    valid = (
        ...
        & frame["h1_state_age_m5"].between(
            0, int(params["h1_state_age_m5_max"]), inclusive="both"
        )
        & session
