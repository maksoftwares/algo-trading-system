...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        "config/dukascopy_growth_risk_leadlag_v94.json": CONFIG_PATH,
...
def verify_sources(
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise FileNotFoundError(f"Missing V94 contract input {name}: {path}")
    source = config["source"]
    ...
    if source_config["start_utc"] != growth_risk["start_utc"] or source_config[
        ...
        raise ValueError("Growth-risk source period differs from V94")
...
def main() -> int:
    ...
    if len(manifest) != int(controls["registered_policy_count"]):
        raise ValueError("V94 policy count mismatch")
    if int(manifest["attempt_no"].min()) != int(controls["attempt_first"]):
        raise ValueError("V94 first attempt mismatch")
    if int(manifest["attempt_no"].max()) != int(controls["attempt_last"]):
        raise ValueError("V94 last attempt mismatch")
    ...
    census = {
        "schema_version": "xauusd_dukascopy_growth_risk_v94_signal_census",
    ...
    body = {
        "schema_version": "xauusd_dukascopy_growth_risk_v94_contract_lock",
        "created_utc": datetime.now(UTC).isoformat(),
