...
OUTPUT = ROOT / "outputs"
CONFIG_PATH = ROOT / "config" / "dukascopy_growth_risk_leadlag_v94.json"
PREFIX = "DUKASCOPY_GROWTH_RISK_LEADLAG_V94"
...
V93_OUTPUT = ROOT.parent / "dukascopy-growth-risk-dislocation-v93" / "outputs"
V93_RESULT_PATH = V93_OUTPUT / "DUKASCOPY_GROWTH_RISK_DISLOCATION_V93_RESULT.json"
V93_SHARED_RESULT_PATH = (
    V93_OUTPUT / "DUKASCOPY_GROWTH_RISK_DISLOCATION_V93_SHARED_AUDIT.json"
)
V93_ARTIFACT_MANIFEST_PATH = (
    V93_OUTPUT / "DUKASCOPY_GROWTH_RISK_DISLOCATION_V93_ARTIFACT_MANIFEST.json"
)
...
def _verify_v93_terminal_failure() -> dict[str, Any]:
    for path in (V93_RESULT_PATH, V93_ARTIFACT_MANIFEST_PATH):
        if not path.is_file():
            raise FileNotFoundError(
                "V94 is conditional on a completed terminal V93 failure: "
                f"missing {path}"
            )
    result = json.loads(V93_RESULT_PATH.read_text(encoding="utf-8"))
    artifact_manifest = json.loads(
        V93_ARTIFACT_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    if (
        int(result.get("attempt_first", 0)) != 124001
        or int(result.get("attempt_last", 0)) != 125000
        or int(result.get("registered_policy_count", 0)) != 1000
    ):
        raise ValueError("V93 predecessor attempt registry is invalid")
    if artifact_manifest.get("contract_sha256") != result.get("contract_sha256"):
        raise ValueError("V93 result and artifact manifest contracts differ")

    decision = str(result.get("decision", ""))
    evidence_paths = [V93_RESULT_PATH]
    terminal_reason = decision
    if not decision.endswith("_FAIL_TERMINAL"):
        if not V93_SHARED_RESULT_PATH.is_file():
            raise RuntimeError("V93 has not failed terminally; V94 remains sealed")
        shared = json.loads(V93_SHARED_RESULT_PATH.read_text(encoding="utf-8"))
        if (
            decision != "V93_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
            or bool(shared.get("passed"))
            or shared.get("decision") != "V93_SHARED_V59_V60_FAIL"
            or shared.get("contract_sha256") != result.get("contract_sha256")
        ):
            raise RuntimeError("V93 has not failed terminally; V94 remains sealed")
        evidence_paths.append(V93_SHARED_RESULT_PATH)
    ...
    for path in evidence_paths:
        ...
        if recorded.get("sha256") != _sha256(path):
            raise ValueError(f"V93 terminal evidence is not artifact-bound: {path.name}")
    return {
        "v93_contract_sha256": str(result["contract_sha256"]),
        "v93_terminal_reason": terminal_reason,
        "v93_result_sha256": _sha256(V93_RESULT_PATH),
        "v93_shared_result_sha256": (
            _sha256(V93_SHARED_RESULT_PATH)
            if V93_SHARED_RESULT_PATH in evidence_paths
            else None
        ),
    }
...
def _verify_contract(config: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    if (
        ...
        raise FileNotFoundError("Run lock_contract.py before opening V94 outcomes")
    lock_module = _load_module(
        "dukascopy_growth_risk_v94_contract_verify", ROOT / "lock_contract.py"
    )
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("V94 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V94 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V94 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V94 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V94 signal census changed")
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V94 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V94 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V94 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V94 {stage} metrics changed")
    if payload["trades_sha256"] != _sha256(_trades_path(stage)):
        raise ValueError(f"V94 {stage} trades changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V94 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V94 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V94 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_dukascopy_growth_risk_v94_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_dukascopy_growth_risk_v94_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V94_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V94_{stage.upper()}_PASS_ADVANCE"
    return f"V94_{stage.upper()}_FAIL_TERMINAL"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V94 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# Dukascopy Growth-Risk Lead-Lag V94 Result",
        "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    predecessor_evidence = _verify_v93_terminal_failure()
    ...
    campaign = _load_module(
        "dukascopy_growth_risk_v94_run_campaign", ROOT / "src" / "campaign.py"
    )
    data = _load_module(
        "dukascopy_growth_risk_v94_run_data",
    ...
    frame = campaign.prepare_features(bundle.bars["M5"], growth_risk_m5, config)
