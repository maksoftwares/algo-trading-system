...
def run_source_audit() -> dict[str, Any]:
    ...
    audit_path = output / str(config["outputs"]["source_audit"])
    if audit_path.exists():
        raise FileExistsError("V72 source audit already exists")
    ...
    if (
        evidence.get("code") != "XAG-USD"
        or evidence.get("name") != "XAG/USD"
        or int(evidence.get("priceScale", -1)) != 3
        or not any(row.get("period") == "TICK" for row in evidence.get("histories", []))
    ):
        raise ValueError("V72 official XAG instrument evidence is invalid")
    manifests: list[dict[str, Any]] = []
    for symbol, spec in source["symbols"].items():
        for year, month in month_keys(
            source["first_month"], source["pre_exam_last_month"]
        ):
            _, row = validate_month_manifest(storage, symbol, spec, year, month)
            manifests.append(row)
    audit: dict[str, Any] = {
        "schema_version": "xauusd_xag_xau_v72_source_audit",
        "campaign_id": str(config["campaign_id"]),
        "decision": "V72_SOURCE_AUDIT_PASS",
        "storage_root": str(storage),
        "symbols": source["symbols"],
        "first_month": str(source["first_month"]),
        "last_month": str(source["pre_exam_last_month"]),
