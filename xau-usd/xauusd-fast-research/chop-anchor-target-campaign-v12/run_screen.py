...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Anchor-Target Campaign V12 Result",
        "",
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V12 outcomes already exist")
    ...
    decision = (
        "CHOP_ANCHOR_TARGET_V12_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_ANCHOR_TARGET_V12_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_ANCHOR_TARGET_V12_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_anchor_target_v12_artifacts",
            "files": {
                name: {
