...
def main() -> int:
    ...
    locks = config["lock"]
    if sha256_file(Path(__file__)) != locks["tick_replay_runner_sha256"]:
        raise ValueError("Locked exact tick replay runner changed")
    wrapper = ROOT / "src" / "tick_replay.py"
    base = ROOT.parent / "v60-mature-source-health-rank-veto-prospective-v2" / "src" / "tick_replay.py"
    if sha256_file(wrapper) != locks["tick_replay_sha256"]:
        raise ValueError("Locked exact tick replay wrapper changed")
    if sha256_file(base) != locks["base_tick_replay_sha256"]:
        raise ValueError("Locked base exact tick replay changed")
    ...
    if not trades:
        result = {
            "schema_version": "v60_antichase_exact_tick_equity_replay_v1",
    ...
    units_per_lot = float(account["expected_ounces_at_fixed_lot"]) / float(
        account["fixed_lot"]
    )
    ...
    result.update(
        {
            ...
            "tick_file_sha256": {
                path.name: sha256_file(path) for path in sorted(relevant)
            },
            "contract_units_per_lot": units_per_lot,
            "deployment_authorized": False,
        }
