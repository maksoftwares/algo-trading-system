...
def load_config(path: Path) -> dict:
    ...
    if sha256_file(warm_start) != config["lock"]["warm_start_sha256"]:
        raise ValueError("Locked prospective warm start changed")
    source_config = REPO_ROOT / config["read_only_inputs"][
        "candidate_source_config"
    ]
    if (
        sha256_file(source_config)
        != config["lock"]["candidate_source_config_sha256"]
    ):
...
def read_mt5_observation(config: dict):
    ...
    try:
        ...
        ranker = load_module("v60_antichase_ranker", RANKER)
        ...
        policy = load_module("v60_antichase_policy", POLICY)
...
def run_once(config_path: Path) -> dict:
    ...
    deals, open_positions, decisions, rank_audit, feature_audit = read_mt5_observation(
        config
    )
    decision_observed_at = datetime.now(UTC)
    status, rows = build_snapshot(
        config, deals, now=decision_observed_at, rank_decisions=decisions
    )
    policy = load_module("v60_antichase_policy_apply", POLICY)
    status["policy_audit"] = policy.apply_policy(rows, decisions, config["lock"]["policy"])
    ...
    evidence = load_module("v60_antichase_evidence", EVIDENCE)
    state = json.loads(
    ...
    status["evidence_chain"] = evidence.update_evidence_chain(
        runtime, rows, observed_at=decision_observed_at
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run read-only V60 anti-chase observer")
