...
BASE = load_module(
    "comex_session_swing_base_runner",
...
def render(payload: dict[str, Any], stages: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD COMEX Session-Swing Specialists V1 Result",
        "",
...
def main() -> int:
    config_path = ROOT / "config" / "comex_session_swing_specialists_v1.json"
    ...
    decision = (
        "PROMOTE_SESSION_SWING_SURVIVORS"
        if survivors
        else "REJECT_COMEX_SESSION_SWING_V1"
    )
    payload = {
        ...
        "data_evidence": {
            "gold": gold.evidence,
            "comex_auction": auction_evidence,
        },
    ...
    candidates.to_csv(
        output / config["outputs"]["candidate_ledger"], index=False, lineterminator="\n"
    )
    labels.to_csv(output / "COMEX_SESSION_SWING_OPENED_LABELS.csv", index=False)
    selected.to_csv(
        output / config["outputs"]["selected_trade_ledger"], index=False
    )
