...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Episode-State Campaign V14 Result",
        "",
    ...
    lines.extend(
        (
            ...
            "Prior-regime ancestry uses only the latest completed non-chop regime.",
...
def main() -> int:
    ...
    config = json.loads(
        (ROOT / "config" / "chop_episode_state_campaign_v14.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V14 outcomes already exist")
    ...
    decision = (
        "CHOP_EPISODE_STATE_V14_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_EPISODE_STATE_V14_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_EPISODE_STATE_V14_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_episode_state_v14_artifacts",
            "files": {
                name: {
