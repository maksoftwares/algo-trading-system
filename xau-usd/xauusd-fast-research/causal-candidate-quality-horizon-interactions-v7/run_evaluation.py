...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Horizon V7 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Horizon V7 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Horizon V7 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Horizon V7 implementation changed after lock: {name}")
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Causal Horizon Interaction Models V7 Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Horizon V7 evaluation")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Horizon V7 base method contract changed after lock")
    ...
    if canonical_json_sha256(base_features) != str(
        ...
        raise ValueError("Horizon V7 base feature surface changed")
    expected_interactions = interaction_feature_columns(base_features)
    features = [*base_features, *expected_interactions]
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Horizon V7 feature hash is inconsistent")
    ...
    if generated_interactions != expected_interactions:
        raise ValueError("Horizon V7 generated interaction order changed")
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("Horizon V7 split fold IDs changed")
    ...
    for lane in lanes:
        for fold_id in fold_ids:
            ...
            for variant in config["training_variants"]:
                ...
                scored_cal["model_score"] = predict_adaptive_model(
                    model, scored_cal, features
                )
            ...
            scored_test["model_score"] = predict_adaptive_model(
                model, scored_test, features
            )
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        decision = (
            "HORIZON_MODEL_V7_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "HORIZON_MODEL_V7_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "HORIZON_MODEL_V7_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "HORIZON_MODEL_V7_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "HORIZON_MODEL_V7_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
        ...
        "event_feature_count": len(event_feature_columns(base_features)),
