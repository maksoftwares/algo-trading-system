...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != contract["contract_sha256"]:
        raise ValueError("V15 manifest contract changed")
    for section in ("inputs", "artifacts"):
        for item in manifest[section].values():
            path = REPO_ROOT / item["path"]
            if not path.is_file() or sha256_file(path) != item["sha256"]:
                raise ValueError(f"V15 {section} changed: {path}")
    if manifest["authorization"] != config["authorization"]:
        raise ValueError("V15 manifest authority changed")
    if any(
        ...
        raise ValueError("V15 unexpectedly has runtime authority")
    result_path = (
        ROOT / config["outputs"]["directory"] / config["outputs"]["result_json"]
    )
    result = json.loads(result_path.read_text(encoding="utf-8"))
    if result["v14_changed"] is not False or result["deployment_eligible"] is not False:
        raise ValueError("V15 result changed authority or V14 state")
    print(
        json.dumps(
            {
                "status": "VERIFIED",
