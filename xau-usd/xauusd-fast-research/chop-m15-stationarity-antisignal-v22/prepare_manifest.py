...
def main() -> int:
    config = json.loads(
        (ROOT / "config" / "chop_m15_stationarity_antisignal_v22.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if manifest_path.exists() or evidence_path.exists():
        raise FileExistsError("V22 manifest preflight already exists")
    ...
    source_manifest_path = (
        RESEARCH_ROOT
        / "chop-m15-stationarity-campaign-v21"
        / "outputs"
        / "CHOP_M15_STATIONARITY_V21_MANIFEST.csv"
    )
    ...
    checks = (
        paired["parameters_json_v22"].eq(paired["parameters_json_v21"])
        & paired["raw_signal_count_v22"].eq(paired["raw_signal_count_v21"])
        & paired["minimum_era_raw_signal_count_v22"].eq(
            paired["minimum_era_raw_signal_count_v21"]
        )
        & paired.apply(
            lambda row: CAMPAIGN.V21_MECHANICS[str(row["mechanic_v22"])]
            == str(row["mechanic_v21"]),
            axis=1,
        )
    )
    if len(paired) != len(manifest) or not bool(checks.all()):
        raise ValueError("V22 manifest is not an exact parameter/signal pair of V21")
    with manifest_path.open("w", encoding="utf-8", newline="\n") as handle:
        manifest.to_csv(handle, index=False, lineterminator="\n")
    evidence = {
        "schema_version": "xauusd_chop_m15_stationarity_v22_manifest_evidence",
        ...
        "stationarity_feature_rows": int(frame["z_192"].notna().sum()),
