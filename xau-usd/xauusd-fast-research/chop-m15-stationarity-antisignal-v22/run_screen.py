...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop M15 Stationarity Antisignal V22 Result",
        "",
    ...
    lines.extend(
        (
            "",
            "All signals are owned by causal H4 CHOP at signal time.",
            "Every signal and geometry is paired exactly to V21 with direction inverted.",
            "Stationarity diagnostics use completed M15 bars and H1 risk.",
            "Entries require the next contiguous side-correct M5 open.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_m15_stationarity_antisignal_v22.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V22 outcomes already exist")
    ...
    decision = (
        "CHOP_M15_STATIONARITY_ANTISIGNAL_V22_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_M15_STATIONARITY_ANTISIGNAL_V22_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_M15_STATIONARITY_ANTISIGNAL_V22_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_m15_stationarity_v22_artifacts",
            "files": {
                name: {
