...
from antisignal import load_config, simulate_variant  # noqa: E402
...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD M15 Regime Anti-Signal Campaign V3 Result",
        "",
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_m15_regime_antisignal_v3_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    if result_path.exists():
        raise FileExistsError("V3 outcomes already exist; same-version reruns are forbidden")
    ...
    decision = (
        ...
        else "NO_M15_REGIME_ANTISIGNAL_V3_ECONOMIC_FINALIST"
    )
    result = {
