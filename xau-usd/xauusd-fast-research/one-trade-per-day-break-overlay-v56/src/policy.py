...
def resolve_config(
    repo_root: Path, v56_config_path: Path
) -> tuple[dict[str, Any], dict[str, Any]]:
    overlay = json.loads(v56_config_path.read_text(encoding="utf-8"))
    base_path = repo_root / str(overlay["base_config_path"])
    base = json.loads(base_path.read_text(encoding="utf-8"))
    resolved = dict(base)
    resolved["schema_version"] = overlay["schema_version"]
    resolved["sources"] = {**base["sources"], **overlay["additional_sources"]}
