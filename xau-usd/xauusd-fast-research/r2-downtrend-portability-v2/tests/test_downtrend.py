...
def test_all_candidates_exclude_decisions_at_data_end(monkeypatch) -> None:
    config = contract.load_config()
    end = pd.Timestamp(config["source"]["end_exclusive_utc"])
    frame = pd.DataFrame(
        {
            "decision_time": [end - pd.Timedelta(minutes=5), end],
            "candidate_id": ["inside", "outside"],
            "mechanism_family": ["test", "test"],
            "attempt_no": [1, 2],
        }
    )
    monkeypatch.setattr(
        downtrend, "generate_pullback_candidates", lambda *args: frame.copy()
    )
    monkeypatch.setattr(
        downtrend, "generate_impulse_candidates", lambda *args: pd.DataFrame()
    )
    result = downtrend.generate_all_candidates(None, None, config)
    assert result["candidate_id"].tolist() == ["inside"]
    assert result.attrs["boundary_candidates_removed"] == 1
