...
CONFIG_PATH = PACKAGE / "config" / "r2_downtrend_portability_v2.json"
...
def package_files() -> list[Path]:
    relative = (
        ...
        "PREREGISTRATION.md",
        "requirements.txt",
        "config/r2_downtrend_portability_v2.json",
        "src/__init__.py",
        "src/downtrend.py",
        "src/contract.py",
        "lock_contract.py",
...
def dependency_files(config: Mapping[str, Any]) -> list[Path]:
    ...
    values = [
        str(config["source"]["v1_preoutcome_invalidation"]),
...
def build_definition_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    root = downtrend.storage_root(config)
    lock: dict[str, Any] = {
        "schema_version": "xauusd_r2_downtrend_portability_definition_lock_v2",
...
def validate_definition_lock(
    ...
    if lock.get("schema_version") != "xauusd_r2_downtrend_portability_definition_lock_v2":
