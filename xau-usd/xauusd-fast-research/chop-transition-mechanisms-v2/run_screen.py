...
from campaign import prepare_features, simulate_variant  # noqa: E402
...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD Chop and Transition Mechanisms V2 Result",
        "",
        ...
        "",
        "All historical eras are discovery robustness segments, not untouched holdouts.",
    ...
    for item in result["regime_summary"]:
        lines.append(
            ...
            f"{item['best_minimum_era_stress_pf']:.3f} |"
        )
    ...
    lines.extend(
        [
            ...
            "## Next decision",
            "",
            "Only frozen definitions in this shortlist may proceed to separately locked raw-tick replay.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_chop_transition_mechanisms_v2_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    config = json.loads(
        (ROOT / "config" / "chop_transition_mechanisms_v2.json").read_text(encoding="utf-8")
    )
    ...
    if result_path.exists():
        raise FileExistsError("V2 outcomes already exist; same-version reruns are not authorized")
    ...
    frame = prepare_features(h1, h4, config, ADAPTIVE, REGIMES, BASE)
    ...
    decision = (
        ...
        else "NO_CHOP_TRANSITION_V2_ECONOMIC_FINALIST"
    )
    result = {
