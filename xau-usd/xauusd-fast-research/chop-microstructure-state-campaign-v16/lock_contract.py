...
PACKAGE_FILES = (
    ...
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/chop_microstructure_state_campaign_v16.json",
    "src/__init__.py",
    "src/campaign.py",
    "prepare_manifest.py",
    "lock_contract.py",
...
def main() -> int:
    config = json.loads(
        (ROOT / "config" / "chop_microstructure_state_campaign_v16.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if lock_path.exists():
        raise FileExistsError("V16 contract lock already exists")
    payload: dict[str, Any] = {
        "schema_version": "xauusd_chop_microstructure_state_v16_contract",
