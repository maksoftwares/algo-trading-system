...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Microstructure-State Campaign V16 Result",
        "",
    ...
    lines.extend(
        (
            "",
            "All signals use native M5 tick-derived features inside causal H4 chop.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_microstructure_state_campaign_v16.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V16 outcomes already exist")
    ...
    decision = (
        "CHOP_MICROSTRUCTURE_STATE_V16_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_MICROSTRUCTURE_STATE_V16_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_MICROSTRUCTURE_STATE_V16_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_microstructure_state_v16_artifacts",
            "files": {
                name: {
