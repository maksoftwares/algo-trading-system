...
V96_OUTPUT = ROOT.parent / "dukascopy-growth-risk-state-transition-v96" / "outputs"
V96_RESULT_PATH = V96_OUTPUT / "DUKASCOPY_GROWTH_RISK_STATE_TRANSITION_V96_RESULT.json"
V96_ARTIFACT_MANIFEST_PATH = (
    V96_OUTPUT / "DUKASCOPY_GROWTH_RISK_STATE_TRANSITION_V96_ARTIFACT_MANIFEST.json"
)
...
def _verify_v96_terminal_failure() -> dict[str, Any]:
    for path in (V96_RESULT_PATH, V96_ARTIFACT_MANIFEST_PATH):
        if not path.is_file():
            raise FileNotFoundError(
                "V97 is conditional on a completed terminal V96 failure: "
                f"missing {path}"
            )
    result = json.loads(V96_RESULT_PATH.read_text(encoding="utf-8"))
    artifact_manifest = json.loads(
        V96_ARTIFACT_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    if (
        int(result.get("attempt_first", 0)) != 127001
        or int(result.get("attempt_last", 0)) != 128000
        or int(result.get("registered_policy_count", 0)) != 1000
    ):
        raise ValueError("V96 predecessor attempt registry is invalid")
    if artifact_manifest.get("contract_sha256") != result.get("contract_sha256"):
        raise ValueError("V96 result and artifact manifest contracts differ")
    decision = str(result.get("decision", ""))
    if decision != "V96_DISCOVERY_FAIL_TERMINAL":
        raise RuntimeError("V96 has not failed terminally; V97 remains sealed")
    recorded = artifact_manifest.get("artifacts", {}).get(V96_RESULT_PATH.name, {})
    if recorded.get("sha256") != _sha256(V96_RESULT_PATH):
        raise ValueError("V96 terminal result is not artifact-bound")
    return {
        "v96_contract_sha256": str(result["contract_sha256"]),
        "v96_terminal_reason": decision,
        "v96_result_sha256": _sha256(V96_RESULT_PATH),
    }
...
def _verify_contract(config: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    if (
        ...
        raise FileNotFoundError("Run lock_contract.py before opening V97 outcomes")
    lock_module = _load_module(
        "causal_hourly_action_router_v97_contract_verify", ROOT / "lock_contract.py"
    )
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("V97 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V97 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V97 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V97 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V97 signal census changed")
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V97 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V97 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V97 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V97 {stage} metrics changed")
    if payload["trades_sha256"] != _sha256(_trades_path(stage)):
        raise ValueError(f"V97 {stage} trades changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V97 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V97 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V97 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_causal_hourly_action_router_v97_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_causal_hourly_action_router_v97_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V97_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V97_{stage.upper()}_PASS_ADVANCE"
    return f"V97_{stage.upper()}_FAIL_TERMINAL"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V97 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# V97 Causal Hourly Action Router Result",
        "",
    ...
    lines.extend(
        [
            "",
            "Regularized models use only completed growth-risk and XAU features; execution is side-correct on the next XAU M5 bar.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    predecessor_evidence = _verify_v96_terminal_failure()
    ...
    campaign = _load_module(
        "causal_hourly_action_router_v97_run_campaign", ROOT / "src" / "campaign.py"
    )
    data = _load_module(
        "causal_hourly_action_router_v97_run_data",
    ...
    frame = campaign.prepare_features(
        bundle.bars["H1"], bundle.bars["M5"], growth_risk_m5, config
    )
