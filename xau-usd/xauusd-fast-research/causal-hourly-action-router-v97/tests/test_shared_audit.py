...
def test_prepare_v97_ledger_reconciles_stress_cost() -> None:
    result = prepare_v97_ledger(
        pd.DataFrame(
            [
                _candidate(
                    "policy-a",
...
def test_router_uses_fixed_priority_and_current_portfolio_limits() -> None:
    ...
    limits = {
        ...
        "maximum_v97_entries_per_utc_date": 2,
        "drawdown_suspend_usd": 225.0,
        "drawdown_resume_usd": 180.0,
    }
    accepted, decisions = route_v97_candidates(baseline, candidates, limits)
    assert accepted["policy_id"].tolist() == ["policy-a"]
    assert decisions["reason"].tolist() == [
        "ACCEPTED",
        "DUPLICATE_V97_ENTRY_TIME",
...
def test_router_does_not_look_ahead_to_a_future_frozen_v59_entry() -> None:
    ...
    accepted, _ = route_v97_candidates(baseline, candidates, limits)
