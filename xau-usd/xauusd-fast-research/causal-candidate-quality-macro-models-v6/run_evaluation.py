...
from src.macro_features import MACRO_FEATURE_COLUMNS, enrich_actions, load_macro_state
...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Macro V6 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Macro V6 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Macro V6 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Macro V6 implementation changed after lock: {name}")
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Causal Macro Action Models V6 Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Macro V6 evaluation")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Macro V6 base method contract changed after lock")
    ...
    if canonical_json_sha256(base_features) != str(
        ...
        raise ValueError("Macro V6 base feature surface changed")
    if list(MACRO_FEATURE_COLUMNS) != list(config["macro_features"]):
        raise ValueError("Macro V6 configured feature order changed")
    features = [*base_features, *config["macro_features"]]
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Macro V6 feature hash is inconsistent")
    dataset = prepare_dataset(
        pd.read_parquet(inputs["v4_action_dataset"]), config, base_features
    )
    macro_state, macro_source_evidence = load_macro_state(config)
    dataset, macro_availability = enrich_actions(
        dataset,
        macro_state,
        tolerance_minutes=int(config["macro_geometry"]["maximum_staleness_minutes"]),
    )
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("Macro V6 split fold IDs changed")
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        decision = (
            "MACRO_MODEL_V6_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "MACRO_MODEL_V6_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "MACRO_MODEL_V6_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "MACRO_MODEL_V6_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "MACRO_MODEL_V6_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
        ...
        "macro_feature_count": len(config["macro_features"]),
    ...
    paths = {
        ...
        "macro_availability": output / config["outputs"]["macro_availability"],
    ...
    write_json(
        paths["macro_availability"],
        {"source": macro_source_evidence, "join": macro_availability},
    )
