...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != lock["definition_contract_sha256"]:
        raise ValueError("Macro V6 manifest references a different contract")
    ...
    if base_method_hash != lock["base_method_contract_sha256"]:
        raise ValueError("Verified Macro V6 base method contract changed")
    base_features = list(v4_config["model_features"])
    if list(MACRO_FEATURE_COLUMNS) != list(config["macro_features"]):
        raise ValueError("Verified Macro V6 feature implementation changed")
    ...
    source, macro_join_evidence = enrich_actions(
        source,
        macro_state,
        tolerance_minutes=int(config["macro_geometry"]["maximum_staleness_minutes"]),
    )
    stored_macro_evidence = json.loads(
        (output / config["outputs"]["macro_availability"]).read_text(encoding="utf-8")
    )
    if stored_macro_evidence != {
        "source": macro_source_evidence,
        "join": macro_join_evidence,
    }:
        raise ValueError("Macro V6 source or causal-join evidence changed")
    ...
    if comparison_v5 != build_result_comparison(result, reference_result):
        raise ValueError("Macro V6 versus Adaptive V5 comparison replay mismatch")
    ...
    if len(policies) != int(config["expected"]["calibration_policy_rows"]):
        raise ValueError("Macro V6 calibration policy count changed")
    ...
    for lane in lanes:
        for fold_id in folds:
            ...
            for variant_id, variant in variants.items():
                ...
                for quantile in config["retention_quantiles"]:
                    ...
                    if len(stored_policy) != 1:
                        raise ValueError("Macro V6 calibration policy key mismatch")
                    ...
                    if bool(stored_row["calibration_gate_pass"]) != bool(
                        ...
                        raise ValueError("Macro V6 calibration gate replay mismatch")
                    if (
                        json.loads(stored_row["fit_metadata_json"])
                        != model.fit_metadata
                    ):
                        raise ValueError("Macro V6 fit metadata replay mismatch")
            ...
            if len(stored_chosen) != 1:
                raise ValueError("Macro V6 chosen policy key mismatch")
            ...
            if (
                ...
                raise ValueError("Macro V6 chosen policy replay mismatch")
    ...
    print(
        json.dumps(
            {
                "decision": "MACRO_V6_VERIFICATION_PASS",
