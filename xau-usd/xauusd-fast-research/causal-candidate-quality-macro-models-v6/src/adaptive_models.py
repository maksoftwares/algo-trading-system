...
def assert_adaptive_v5_parity(
    ...
    if current_contract != reference_contract:
        raise ValueError("Macro V6 changed an unauthorized Adaptive V5 method field")
    if config["ridge_model"] != reference["ridge_model"]:
        raise ValueError("Macro V6 ridge model differs from Adaptive V5")
    if config["training_variants"] != reference["training_variants"]:
        raise ValueError("Macro V6 training variants differ from Adaptive V5")
...
def build_result_comparison(
    ...
    for lane, current_family in current["families"].items():
        ...
        lanes[lane] = {
            ...
            "macro_v6_decision": current_family["decision"],
            "metrics": metrics,
        }
    return {
        "schema_version": "xauusd_macro_v6_vs_adaptive_v5",
        "adaptive_v5_decision": reference["decision"],
        "macro_v6_decision": current["decision"],
        "lanes": lanes,
    }
