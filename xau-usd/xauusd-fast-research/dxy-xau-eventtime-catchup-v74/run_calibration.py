...
from inverse_lead import (  # noqa: E402
...
def run_calibration() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (feature_path, grid_path, audit_path)):
        raise FileExistsError("V74 calibration outputs already exist")
    if (output / str(config["outputs"]["contract_lock"])).exists():
        raise RuntimeError("V74 calibration cannot run after lock")
    ...
    for date in pd.date_range(start.normalize(), end.normalize(), inclusive="left", freq="D"):
        ...
        print(f"V74 calibrated {date.date()}", flush=True)
    ...
    if len(policies) != 1000:
        raise ValueError(f"V74 registered {len(policies)} policies, expected 1000")
    ...
    decision = "V74_CALIBRATION_POLICY_SELECTED" if selected else "V74_NO_CALIBRATION_POLICY"
    audit: dict[str, Any] = {
        "schema_version": "xauusd_dxy_xau_v74_calibration_audit",
