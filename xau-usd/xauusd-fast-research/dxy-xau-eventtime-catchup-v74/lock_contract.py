...
CONFIG = ROOT / "config" / "dxy_xau_eventtime_catchup_v74.json"
PACKAGE_FILES = (
    "README.md",
    "PREREGISTRATION.md",
    "requirements.txt",
    "config/dxy_xau_eventtime_catchup_v74.json",
    "src/__init__.py",
    "src/inverse_lead.py",
    "run_source_audit.py",
    "run_calibration.py",
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        source_audit.get("decision") != "V74_SOURCE_AUDIT_PASS"
        or canonical_hash(source_audit, "audit_sha256") != source_audit.get("audit_sha256")
    ):
        raise ValueError("V74 source audit is invalid")
    if (
        calibration.get("decision") != "V74_CALIBRATION_POLICY_SELECTED"
        or calibration.get("selected_policy") is None
        or canonical_hash(calibration, "audit_sha256") != calibration.get("audit_sha256")
    ):
        raise ValueError("V74 calibration did not select a policy")
    ...
    if sha256_file(feature_path) != calibration["feature_sha256"]:
        raise ValueError("V74 calibration features changed")
    if sha256_file(grid_path) != calibration["grid_sha256"]:
        raise ValueError("V74 calibration grid changed")
    ...
    for dependency in config["locked_dependencies"].values():
        ...
        if sha256_file(path) != dependency["sha256"]:
            raise ValueError(f"V74 dependency changed: {path}")
    ...
    if sha256_file(atr_path) != config["spot_source"]["m5_feature_sha256"]:
        raise ValueError("V74 completed-M5 ATR cache changed")
    contract: dict[str, Any] = {
        "schema_version": "xauusd_dxy_xau_v74_contract_lock",
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V74 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V74 contract already exists")
    if list(output.glob("DXY_XAU_V74_*_DEVELOPMENT_*")):
        raise ValueError("V74 development outputs existed before lock")
