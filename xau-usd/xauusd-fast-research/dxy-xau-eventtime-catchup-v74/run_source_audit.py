...
def run_source_audit() -> dict[str, Any]:
    ...
    if path.exists():
        raise FileExistsError("V74 source audit already exists")
    ...
    if (
        evidence.get("code") != "DOLLAR.IDX-USD"
        or evidence.get("name") != "DOLLAR.IDX/USD"
        or int(evidence.get("priceScale", -1)) != 3
        or not any(row.get("period") == "TICK" for row in evidence.get("histories", []))
    ):
        raise ValueError("V74 official DXY instrument evidence is invalid")
    ...
    audit: dict[str, Any] = {
        "schema_version": "xauusd_dxy_xau_v74_source_audit",
        "campaign_id": config["campaign_id"],
        "decision": "V74_SOURCE_AUDIT_PASS",
