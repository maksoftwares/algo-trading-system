...
from inverse_lead import build_inverse_features, generate_candidates, policy_grid
...
def test_dxy_up_maps_to_short_and_uses_strict_prior_xau() -> None:
    ...
    prefilter = {
        "minimum_absolute_dxy_move_bps": 0.2,
        "minimum_directional_innovation_bps": 0.1,
        "maximum_signed_xau_response_ratio": 0.75,
        "minimum_dxy_quote_count": 2,
    }
    result = build_inverse_features(
        date, dxy, xau, horizons_ms=[1000], rule=rule, prefilter=prefilter
    )
    assert not result.empty
    assert result.iloc[0]["direction"] == "SHORT"
    assert result.iloc[0]["xau_current_timestamp_ms"] < result.iloc[0]["decision_timestamp_ms"]
