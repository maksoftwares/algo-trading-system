...
def _load_final_candidates(
    ...
    if not policy_ids:
        raise RuntimeError(
            "V93 shared audit is sealed because Final selected no policy"
        )
    frames = []
    ...
    if not frames:
        raise RuntimeError("Final V93 policies have no staged trade ledger")
    candidates = pd.concat(frames, ignore_index=True)
    if set(candidates["policy_id"].astype(str)) != set(policy_ids):
        raise ValueError("A Final V93 policy is missing from the staged trade ledgers")
    return candidates, policy_ids


def main() -> int:
    if MARKER_PATH.exists() or RESULT_PATH.exists():
        raise RuntimeError("V93 shared audit has already been opened")
    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    run = _load_module("dukascopy_growth_risk_v93_shared_run", ROOT / "run_research.py")
    ...
    _write_json(
        MARKER_PATH,
        {
            "schema_version": "xauusd_dukascopy_growth_risk_v93_shared_stage_open",
    ...
    v60_audit = _load_module(
        "dukascopy_growth_risk_v93_v60_audit", paths["shared/v60_audit_module"]
    )
    ...
    result = {
        "schema_version": "xauusd_dukascopy_growth_risk_v93_shared_audit",
        "created_utc": datetime.now(UTC).isoformat(),
        "contract_sha256": contract_hash,
        "final_policy_ids": policy_ids,
        "candidate_v93_trades": int(len(candidates)),
        "accepted_v93_trades": int(len(accepted)),
        ...
        "checks": checks,
        "passed": passed,
        "decision": "V93_SHARED_V59_V60_PASS" if passed else "V93_SHARED_V59_V60_FAIL",
