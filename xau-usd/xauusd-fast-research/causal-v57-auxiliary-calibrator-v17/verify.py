...
def main() -> int:
    ...
    if manifest["definition_contract_sha256"] != contract["contract_sha256"]:
        raise ValueError("V17 manifest contract changed")
    for section in ("inputs", "artifacts"):
        for item in manifest[section].values():
            path = REPO_ROOT / item["path"]
            if not path.is_file() or sha256_file(path) != item["sha256"]:
                raise ValueError(f"V17 {section} changed: {path}")
    if manifest["authorization"] != config["authorization"]:
        raise ValueError("V17 manifest authority changed")
    forbidden = (
    ...
    if any(bool(config["authorization"][key]) for key in forbidden):
        raise ValueError("V17 unexpectedly has runtime authority")
    result_path = (
    ...
    if (
        ...
        or result["v16_changed"] is not False
        or result["deployment_eligible"] is not False
    ):
        raise ValueError("V17 result changed authority or frozen lanes")
    print(
        json.dumps(
            {
                "status": "VERIFIED",
