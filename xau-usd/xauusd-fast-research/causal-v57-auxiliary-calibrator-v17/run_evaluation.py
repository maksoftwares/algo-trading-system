...
CONFIG_PATH = ROOT / "config" / "v57_auxiliary_calibrator_v17.json"
OUTPUT = ROOT / "outputs"
CONTRACT_PATH = OUTPUT / "V57_AUX_V17_CONTRACT_LOCK.json"
...
def verify_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    if not CONTRACT_PATH.is_file():
        raise FileNotFoundError("Run lock_contract.py before V17 evaluation")
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    if contract["contract_sha256"] != canonical_hash(contract):
        raise ValueError("V17 contract self-hash changed")
    for base, records in (
        ...
        for item in records:
            ...
            if (
                ...
                raise ValueError(f"Locked V17 dependency changed: {path}")
    if contract["authorization"] != config["authorization"]:
        raise ValueError("V17 authority changed after lock")
    return contract
...
def choose_specialist_threshold(
    ...
    *,
        ...
        rows.append(
            {
                ...
                **{f"constraint_{key}": value for key, value in constraints.items()},
                ...
                "v17_vetoed_rows": int(candidate["v17_veto"].sum()),
                "v57_re_admitted_rows": int(candidate["v57_re_admitted"].sum()),
                ...
                "profit_improvement_usd_vs_raw_v57": metrics[
    ...
    return (
        ...
        "MAXIMUM_ELIGIBLE_V57_CALIBRATION_NORMALIZED_USD",
        grid,
    )
...
def build_predictions(
    ...
    expected_r = load_module("v17_expected_r", paths["expected_r_module"])
    policy = load_module("v17_profit_policy", paths["profit_policy_module"])
    transfer = load_module("v17_transfer", paths["v15_transfer_module"])
    ...
    if observed != config["expected"]:
        raise ValueError(f"V17 population changed: {observed}")
    ...
    for fold_id in canonical_config["outer_evaluation"]["folds"]:
        ...
        v57_fit = fit.loc[fit["family_id"].eq(specialist_id)].copy()
        v57_calibration = calibration.loc[
            calibration["family_id"].eq(specialist_id)
        ].copy()
        v57_test_mask = test["family_id"].eq(specialist_id)
        supported = (
            len(v57_fit) >= int(config["specialist"]["minimum_fit_rows"])
            and len(v57_calibration)
            >= int(config["specialist"]["minimum_calibration_rows"])
        )
        for frame in (fit, calibration, test):
            frame["v57_model_score"] = np.nan
        model = None
        if supported:
            model = SpecialistExpectedR.fit(
                v57_fit,
                alpha=float(config["specialist"]["ridge_alpha"]),
                target_clip=tuple(
                    float(value)
                    for value in config["specialist"]["target_clip_r"]
                ),
            )
            fit.loc[
                fit["family_id"].eq(specialist_id), "v57_model_score"
            ] = model.predict(v57_fit)
            calibration.loc[
                calibration["family_id"].eq(specialist_id),
                "v57_model_score",
            ] = model.predict(v57_calibration)
            test.loc[v57_test_mask, "v57_model_score"] = model.predict(
                test.loc[v57_test_mask]
            )
            ...
            threshold = None
            action = "V57_RETAIN_ALL_INSUFFICIENT_SPECIALIST_SUPPORT"
            grid = pd.DataFrame(
                [
                    {
                        "quantile": np.nan,
        ...
        grid["specialist_fit_rows"] = len(v57_fit)
        grid["specialist_calibration_rows"] = len(v57_calibration)
        grid["selection_reason"] = action
        threshold_rows.append(grid)

        selected_test = apply_v57_confirmation(
            test,
            specialist_id=specialist_id,
            threshold=threshold,
            fallback_retain_all=True,
        )
        selected_test["v17_threshold"] = threshold
        selected_test["v17_action"] = action
        selected_for_policy = selected_test.rename(
            columns={"v17_selected": "selected"}
        )
        ...
        v57_selected_test = selected_test.loc[v57_test_mask]
        fold_rows.append(
            {
                "fold_id": fold_id,
                ...
                "v57_fit_rows": int(len(v57_fit)),
                "v57_calibration_rows": int(len(v57_calibration)),
                "v57_test_rows": int(v57_test_mask.sum()),
                ...
                "b123_v57_test_vetoes": int(
                    (~v57_selected_test["b123_selected"]).sum()
                ),
                "v17_total_test_vetoes": int(selected_test["v17_veto"].sum()),
                "v17_v57_test_vetoes": int(
                    v57_selected_test["v17_veto"].sum()
                ),
                "v57_test_re_admitted": int(
                    v57_selected_test["v57_re_admitted"].sum()
                ),
                "chosen_threshold": threshold,
                "selection_reason": action,
                "selected_rows": int(selected_test["v17_selected"].sum()),
                ...
                "v57_model_test_auc": weighted_auc(
                    v57_selected_test, "v57_model_score"
                ),
                "aux_linear_v57_test_auc": weighted_auc(
                    v57_selected_test, AUXILIARY_SCORES[0]
                ),
                "aux_nonlinear_v57_test_auc": weighted_auc(
                    v57_selected_test, AUXILIARY_SCORES[1]
                ),
                "aux_win_v57_test_auc": weighted_auc(
                    v57_selected_test, AUXILIARY_SCORES[2]
                ),
                "v57_model_coef_linear": (
                    None if model is None else float(model.estimator.coef_[0])
                ),
                "v57_model_coef_nonlinear": (
                    None if model is None else float(model.estimator.coef_[1])
                ),
                "v57_model_coef_win": (
                    None if model is None else float(model.estimator.coef_[2])
                ),
            }
        )
        predictions.append(
    ...
    if prediction_frame["candidate_id"].duplicated().any():
        raise ValueError("V17 out-of-time predictions are duplicated")
    dataset_audit = {
        "schema_version": "xauusd_v57_auxiliary_calibrator_v17_dataset_audit",
        **observed,
        **overlap_audit,
        "specialist_family_id": specialist_id,
        "journey_rows_used": 0,
        "outcome_or_identity_features_used": False,
        "specialist_model_feature_count": len(AUXILIARY_SCORES),
        "specialist_model_features": list(AUXILIARY_SCORES),
...
def exact_v60_replay(
    ...
    helper = load_module("v17_v60_replay", paths["v60_replay_module"])
    ...
    trades["model_score"] = trades["v57_model_score"]
    trades["v12_prediction_available"] = trades["fold_id"].notna()
    trades["v12_retained"] = trades["v17_selected"].fillna(True).astype(bool)
    trades["v12_action"] = trades["v17_action"].fillna(
...
def combined_windows(
    ...
    current = v17.set_index("period")
    if list(current.index) != list(locked.index):
        raise ValueError("Locked B123 and V17 windows differ")
    for column in (
        "raw_trades",
        "raw_net_pnl_usd",
        "raw_profit_factor",
    ...
    result = current[
        ...
        columns={
            "ml_trades": "v17_trades",
            "ml_net_pnl_usd": "v17_net_pnl_usd",
            "ml_win_rate_pct": "v17_win_rate_pct",
            "ml_profit_factor": "v17_profit_factor",
            "ml_closed_trade_drawdown_usd": "v17_closed_trade_drawdown_usd",
        }
    )
    ...
    result["v17_delta_vs_raw_usd"] = (
        result["v17_net_pnl_usd"] - result["raw_net_pnl_usd"]
    )
    result["v17_delta_vs_locked_b123_usd"] = (
        result["v17_net_pnl_usd"] - result["locked_b123_net_pnl_usd"]
    )
    result["v17_trade_retention"] = result["v17_trades"] / result["raw_trades"]
...
def build_result(
    ...
    return {
        ...
        "out_of_time": {
            ...
            "b123_v57_vetoes": int(folds["b123_v57_test_vetoes"].sum()),
            "v17_total_vetoes": int(folds["v17_total_test_vetoes"].sum()),
            "v17_v57_vetoes": int(folds["v17_v57_test_vetoes"].sum()),
            "v57_re_admitted": int(folds["v57_test_re_admitted"].sum()),
            ...
            "mean_supported_v57_model_test_auc": float(
                folds["v57_model_test_auc"].mean()
            ),
            "mean_auxiliary_linear_v57_test_auc": float(
                folds["aux_linear_v57_test_auc"].mean()
            ),
            "mean_auxiliary_nonlinear_v57_test_auc": float(
                folds["aux_nonlinear_v57_test_auc"].mean()
            ),
            "mean_auxiliary_win_v57_test_auc": float(
                folds["aux_win_v57_test_auc"].mean()
            ),
        },
        "exact_v60": {
            period: {
        ...
        "limitations": [
            "Historical outcomes were exposed before V17 was designed.",
            "The V57 scope was chosen after observing V16's latest-window error.",
            "Only three frozen auxiliary scores enter the V57 calibrator.",
            "Only untouched prospective evidence could support any later runtime role.",
            "V14 remains the locked prospective lane; V14, V15, and V16 were not modified.",
        ],
    }
...
def result_markdown(result: Mapping[str, Any], windows: pd.DataFrame) -> str:
    ...
    for period in ("3M", "6M", "1Y", "2Y", "5Y", "10Y", "ALL"):
        ...
        lines.append(
            ...
            f"${row['v17_net_pnl_usd']:.2f} | "
            f"${row['v17_delta_vs_raw_usd']:.2f} | "
            f"${row['v17_delta_vs_locked_b123_usd']:.2f} | "
            f"{int(row['v17_trades'])} | {row['v17_win_rate_pct']:.2f}% | "
            f"{row['v17_profit_factor']:.3f} | "
            f"${row['v17_closed_trade_drawdown_usd']:.2f} |"
        )
    lines.extend(
        [
            "",
            f"B123 V57 vetoes: {result['out_of_time']['b123_v57_vetoes']}. "
            f"V17 V57 vetoes: {result['out_of_time']['v17_v57_vetoes']}. "
            f"V57 re-admitted: {result['out_of_time']['v57_re_admitted']}.",
            "",
...
def main() -> int:
    ...
    v17_windows, trade_audit, replay_checks = exact_v60_replay(
        predictions, paths
    )
    windows = combined_windows(v17_windows, paths["locked_b123_windows"])
    ...
    manifest = {
        "schema_version": "xauusd_v57_auxiliary_calibrator_v17_artifact_manifest",
        "definition_contract_sha256": contract["contract_sha256"],
