...
def run_calibration() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (feature_path, grid_path, audit_path)):
        raise FileExistsError("V78 calibration outputs already exist")
    if (output / str(config["outputs"]["contract_lock"])).exists():
        raise RuntimeError("V78 calibration cannot run after lock")
    ...
    for date in pd.date_range(start.normalize(), end.normalize(), inclusive="left", freq="D"):
        ...
        print(f"V78 calibrated {date.date()}", flush=True)
    ...
    if len(policies) != 1000:
        raise ValueError(f"V78 registered {len(policies)} policies, expected 1000")
    ...
    decision = "V78_CALIBRATION_POLICY_SELECTED" if selected else "V78_NO_CALIBRATION_POLICY"
    audit: dict[str, Any] = {
        "schema_version": "xauusd_fx_consensus_xau_v78_calibration_audit",
