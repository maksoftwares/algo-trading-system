...
def run_source_audit() -> dict[str, Any]:
    ...
    if path.exists():
        raise FileExistsError("V78 source audit already exists")
    ...
    for symbol, relative in source["instrument_evidence"].items():
        evidence_path = storage / str(relative)
        ...
        if (
            ...
            or not any(
                row.get("period") == "TICK" for row in evidence.get("histories", [])
            )
        ):
            raise ValueError(f"V78 official {symbol} instrument evidence is invalid")
    ...
    audit: dict[str, Any] = {
        "schema_version": "xauusd_fx_consensus_xau_v78_source_audit",
        "campaign_id": config["campaign_id"],
        "decision": "V78_SOURCE_AUDIT_PASS",
