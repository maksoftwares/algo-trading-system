...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"FX_CONSENSUS_XAU_V78_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V78 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V78 {stage} sealed because {prior} failed")
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V78 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if number % 25 == 0:
            print(f"V78 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V78_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V78_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_fx_consensus_xau_v78_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V78 stage")
