...
CONFIG = ROOT / "config" / "crossasset_volatility_pending_breakout_v86.json"
PACKAGE_FILES = (
    "README.md",
    "PREREGISTRATION.md",
    ...
    "requirements.txt",
    "config/crossasset_volatility_pending_breakout_v86.json",
    "src/__init__.py",
    "src/pending_breakout.py",
    "run_source_audit.py",
    "run_calibration.py",
    "lock_contract.py",
    "run_stage.py",
    "tests/conftest.py",
    "tests/test_pending_breakout.py",
)
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if sha256_file(feature_path) != calibration["feature_sha256"]:
        raise ValueError("V86 calibration features changed")
    if sha256_file(grid_path) != calibration["grid_sha256"]:
        raise ValueError("V86 calibration grid changed")
    ...
    for dependency in config["locked_dependencies"].values():
        ...
        if sha256_file(path) != dependency["sha256"]:
            raise ValueError(f"V86 dependency changed: {path}")
    ...
    if sha256_file(atr_path) != config["spot_source"]["m5_feature_sha256"]:
        raise ValueError("V86 completed-M5 ATR cache changed")
    contract: dict[str, Any] = {
        "schema_version": "xauusd_crossasset_volatility_pending_breakout_v86_contract_lock",
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V86 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V86 contract already exists")
    economic_outputs = [
        output / f"PENDING_BREAKOUT_V86_{stage.upper()}_{suffix}"
        for stage in (
            "development",
            "confirmation",
            "validation",
    ...
    if existing := [str(path) for path in economic_outputs if path.exists()]:
        raise ValueError(f"V86 economic outputs existed before lock: {existing}")
