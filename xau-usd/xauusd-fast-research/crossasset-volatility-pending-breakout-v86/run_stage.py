...
from pending_breakout import (  # noqa: E402
...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"PENDING_BREAKOUT_V86_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V86 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V86 {stage} sealed because {prior} failed")
...
def verify_stage_source_audit(
    ...
    if (
        ...
        raise ValueError(f"V86 {stage} source audit is invalid")
    if stage == "development" and sha256_file(path) != contract[
        "development_source_audit"
    ]["sha256"]:
        raise ValueError("V86 locked development source audit changed")
    return audit, path
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V86 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if bool(quality["eligible_full_weekday"]):
            features = build_pending_breakout_features(
                ...
                xau,
                horizons_ms=[horizon],
                breakout_thresholds_bps=[float(policy["breakout_move_bps"])],
                rule=rule,
                prefilter={
                    **policy,
                    "minimum_source_quote_count": int(
                        rule["minimum_source_quote_count"]
                    ),
                },
            )
        ...
        if number % 25 == 0:
            print(f"V86 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V86_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V86_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_crossasset_volatility_pending_breakout_v86_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V86 stage")
