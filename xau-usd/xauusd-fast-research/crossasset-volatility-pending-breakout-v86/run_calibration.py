...
def load_day(
    ...
    *,
    ...
    end_ms += int(rule["trigger_window_ms"])
    return (
...
def run_calibration() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (feature_path, grid_path, audit_path)):
        raise FileExistsError("V86 calibration outputs already exist")
    if (output / str(config["outputs"]["contract_lock"])).exists():
        raise RuntimeError("V86 calibration cannot run after lock")
    source_path = output / str(config["outputs"]["calibration_source_audit"])
    source_audit = load_json(source_path)
    if (
        source_audit.get("decision") != "V86_CALIBRATION_SOURCE_AUDIT_PASS"
    ...
    for date in pd.date_range(
        ...
        if bool(quality["eligible_full_weekday"]):
            features = build_pending_breakout_features(
                ...
                breakout_thresholds_bps=[
                    float(value)
                    for value in calibration["breakout_move_bps_grid"]
                ],
                rule=rule,
                prefilter=calibration_prefilter(calibration, rule),
            )
            if not features.empty:
                feature_frames.append(features)
        print(f"V86 calibrated {date.date()}", flush=True)
    features = (
    ...
    if len(policies) != 1000:
        raise ValueError(f"V86 registered {len(policies)} policies, expected 1000")
    ...
    decision = (
        "V86_CALIBRATION_POLICY_SELECTED"
        if selected
        else "V86_NO_CALIBRATION_POLICY"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_crossasset_volatility_pending_breakout_v86_calibration_audit",
