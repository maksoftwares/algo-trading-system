...
def contract_paths(config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    return {
        "config/dukascopy_growth_risk_state_transition_v96.json": CONFIG_PATH,
...
def verify_sources(
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise FileNotFoundError(f"Missing V96 contract input {name}: {path}")
    source = config["source"]
    ...
    if source_config["start_utc"] != growth_risk["start_utc"] or source_config[
        ...
        raise ValueError("Growth-risk source period differs from V96")
...
def main() -> int:
    ...
    if len(manifest) != int(controls["registered_policy_count"]):
        raise ValueError("V96 policy count mismatch")
    if int(manifest["attempt_no"].min()) != int(controls["attempt_first"]):
        raise ValueError("V96 first attempt mismatch")
    if int(manifest["attempt_no"].max()) != int(controls["attempt_last"]):
        raise ValueError("V96 last attempt mismatch")
    ...
    census = {
        "schema_version": "xauusd_dukascopy_growth_risk_v96_signal_census",
    ...
    body = {
        "schema_version": "xauusd_dukascopy_growth_risk_v96_contract_lock",
        "created_utc": datetime.now(UTC).isoformat(),
