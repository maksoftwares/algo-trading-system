...
OUTPUT = ROOT / "outputs"
CONFIG_PATH = ROOT / "config" / "dukascopy_growth_risk_state_transition_v96.json"
PREFIX = "DUKASCOPY_GROWTH_RISK_STATE_TRANSITION_V96"
...
V94_OUTPUT = ROOT.parent / "dukascopy-growth-risk-leadlag-v94" / "outputs"
V94_RESULT_PATH = V94_OUTPUT / "DUKASCOPY_GROWTH_RISK_LEADLAG_V94_RESULT.json"
V94_SHARED_RESULT_PATH = (
    V94_OUTPUT / "DUKASCOPY_GROWTH_RISK_LEADLAG_V94_SHARED_AUDIT.json"
)
V94_ARTIFACT_MANIFEST_PATH = (
    V94_OUTPUT / "DUKASCOPY_GROWTH_RISK_LEADLAG_V94_ARTIFACT_MANIFEST.json"
)
V95_FAILURE_PATH = (
    ROOT.parent
    / "dukascopy-growth-risk-state-transition-v95"
    / "PREOUTCOME_LOCK_FAILURE.json"
)
...
def _verify_v94_terminal_failure() -> dict[str, Any]:
    for path in (V94_RESULT_PATH, V94_ARTIFACT_MANIFEST_PATH):
        if not path.is_file():
            raise FileNotFoundError(
                "V96 is conditional on a completed terminal V94 failure: "
                f"missing {path}"
            )
    result = json.loads(V94_RESULT_PATH.read_text(encoding="utf-8"))
    artifact_manifest = json.loads(
        V94_ARTIFACT_MANIFEST_PATH.read_text(encoding="utf-8")
    )
    if (
        int(result.get("attempt_first", 0)) != 125001
        or int(result.get("attempt_last", 0)) != 126000
        or int(result.get("registered_policy_count", 0)) != 1000
    ):
        raise ValueError("V94 predecessor attempt registry is invalid")
    if artifact_manifest.get("contract_sha256") != result.get("contract_sha256"):
        raise ValueError("V94 result and artifact manifest contracts differ")

    decision = str(result.get("decision", ""))
    evidence_paths = [V94_RESULT_PATH]
    terminal_reason = decision
    if not decision.endswith("_FAIL_TERMINAL"):
        if not V94_SHARED_RESULT_PATH.is_file():
            raise RuntimeError("V94 has not failed terminally; V96 remains sealed")
        shared = json.loads(V94_SHARED_RESULT_PATH.read_text(encoding="utf-8"))
        if (
            decision != "V94_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
            or bool(shared.get("passed"))
            or shared.get("decision") != "V94_SHARED_V59_V60_FAIL"
            or shared.get("contract_sha256") != result.get("contract_sha256")
        ):
            raise RuntimeError("V94 has not failed terminally; V96 remains sealed")
        evidence_paths.append(V94_SHARED_RESULT_PATH)
        terminal_reason = str(shared["decision"])

    artifacts = artifact_manifest.get("artifacts", {})
    for path in evidence_paths:
        recorded = artifacts.get(path.name, {})
        if recorded.get("sha256") != _sha256(path):
            raise ValueError(f"V94 terminal evidence is not artifact-bound: {path.name}")
    return {
        "v94_contract_sha256": str(result["contract_sha256"]),
        "v94_terminal_reason": terminal_reason,
        "v94_result_sha256": _sha256(V94_RESULT_PATH),
        "v94_shared_result_sha256": (
            _sha256(V94_SHARED_RESULT_PATH)
            if V94_SHARED_RESULT_PATH in evidence_paths
            else None
        ),
    }


def _verify_v95_preoutcome_failure(config: Mapping[str, Any]) -> dict[str, Any]:
    if not V95_FAILURE_PATH.is_file():
        raise FileNotFoundError(
            "V96 requires the committed V95 pre-outcome lock failure: "
            f"missing {V95_FAILURE_PATH}"
        )
    failure = json.loads(V95_FAILURE_PATH.read_text(encoding="utf-8"))
    if (
        failure.get("status") != "TERMINAL_PREOUTCOME_LOCK_FAILURE"
        or failure.get("failure_stage") != "SOURCE_ONLY_POLICY_MANIFEST_ADMISSION"
        or failure.get("failed_mechanic") != "RISK_GROWTH_CONVERGENCE"
        or int(failure.get("source_eligible_policy_count", -1)) != 0
        or int(failure.get("required_policy_count", -1)) != 200
        or bool(failure.get("outcomes_opened"))
        or bool(failure.get("strategy_scoring_performed"))
        or bool(failure.get("thresholds_changed_after_failure"))
    ):
        raise ValueError("V95 pre-outcome failure evidence is invalid")
    source = config["growth_risk_source"]
    if (
        failure.get("growth_risk_feature_sha256") != source["feature_sha256"]
        or failure.get("growth_risk_manifest_sha256") != source["manifest_sha256"]
    ):
        raise ValueError("V95 failure and V96 source hashes differ")
    return {
        "v95_status": str(failure["status"]),
        "v95_failure_stage": str(failure["failure_stage"]),
        "v95_failure_sha256": _sha256(V95_FAILURE_PATH),
    }
...
def _verify_contract(config: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    if (
        ...
        raise FileNotFoundError("Run lock_contract.py before opening V96 outcomes")
    lock_module = _load_module(
        "dukascopy_growth_risk_v96_contract_verify", ROOT / "lock_contract.py"
    )
    ...
    if _canonical_hash(body) != claimed:
        raise ValueError("V96 contract hash mismatch")
    paths = lock_module.contract_paths(config)
    if set(paths) != set(lock["files"]):
        raise ValueError("V96 contract file set changed")
    for name, path in paths.items():
        if _sha256(path) != lock["files"][name]:
            raise ValueError(f"V96 contract input changed: {name}")
    if _sha256(MANIFEST_PATH) != lock["policy_manifest_sha256"]:
        raise ValueError("V96 policy manifest changed")
    if _sha256(CENSUS_PATH) != lock["signal_census_sha256"]:
        raise ValueError("V96 signal census changed")
...
def _verify_advancement(stage: str, contract_hash: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Missing V96 {stage} advancement lock")
    payload = json.loads(path.read_text(encoding="utf-8"))
    claimed = str(payload["advancement_sha256"])
    body = dict(payload)
    body.pop("advancement_sha256")
    if _canonical_hash(body) != claimed:
        raise ValueError(f"V96 {stage} advancement hash mismatch")
    if payload["contract_sha256"] != contract_hash:
        raise ValueError(f"V96 {stage} advancement contract mismatch")
    if payload["metrics_sha256"] != _sha256(_metrics_path(stage)):
        raise ValueError(f"V96 {stage} metrics changed")
    if payload["trades_sha256"] != _sha256(_trades_path(stage)):
        raise ValueError(f"V96 {stage} trades changed")
    return payload
...
def _stage_manifest(stage: str, contract_hash: str) -> pd.DataFrame:
    ...
    if not selected_ids:
        raise RuntimeError(f"V96 {stage} is sealed; {previous} selected no policy")
    selected = manifest.loc[manifest["policy_id"].isin(selected_ids)].copy()
    if set(selected["policy_id"]) != set(selected_ids):
        raise ValueError("V96 advancement references an unknown policy")
...
def _write_marker(stage: str, contract_hash: str) -> None:
    if _marker_path(stage).exists() or _metrics_path(stage).exists():
        raise RuntimeError(f"V96 {stage} outcomes were already opened")
    _write_json(
        _marker_path(stage),
        {
            "schema_version": "xauusd_dukascopy_growth_risk_v96_stage_open",
            "stage": stage,
...
def _write_advancement(
    ...
    payload: dict[str, Any] = {
        "schema_version": "xauusd_dukascopy_growth_risk_v96_advancement",
        "stage": stage,
...
def _decision(stage: str, selected_count: int) -> str:
    if selected_count:
        if stage == "final":
            return "V96_FINAL_SURVIVOR_REQUIRES_SHARED_V59_V60_AUDIT"
        return f"V96_{stage.upper()}_PASS_ADVANCE"
    return f"V96_{stage.upper()}_FAIL_TERMINAL"
...
def _load_result(contract_hash: str, config: Mapping[str, Any]) -> dict[str, Any]:
    if RESULT_PATH.is_file():
        ...
        if result["contract_sha256"] != contract_hash:
            raise ValueError("V96 result contract mismatch")
        return result
...
def _render(result: Mapping[str, Any]) -> str:
    lines = [
        "# Dukascopy Growth-Risk State Transition V96 Result",
        "",
    ...
    lines.extend(
        [
            "",
            "Only completed growth-risk source and XAU bars are used; execution is side-correct on the next XAU M5 bar.",
            "",
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    lock, lock_module = _verify_contract(config)
    predecessor_evidence = {
        **_verify_v94_terminal_failure(),
        **_verify_v95_preoutcome_failure(config),
    }
    ...
    campaign = _load_module(
        "dukascopy_growth_risk_v96_run_campaign", ROOT / "src" / "campaign.py"
    )
    data = _load_module(
        "dukascopy_growth_risk_v96_run_data",
    ...
    frame = campaign.prepare_features(bundle.bars["H1"], growth_risk_m5, config)
    ...
    result["data_evidence"] = {
        ...
        "dukascopy_growth_risk_path": str(
            paths["external/growth_risk_feature_cache"]
        ),
        "dukascopy_growth_risk_sha256": config["growth_risk_source"][
            "feature_sha256"
        ],
