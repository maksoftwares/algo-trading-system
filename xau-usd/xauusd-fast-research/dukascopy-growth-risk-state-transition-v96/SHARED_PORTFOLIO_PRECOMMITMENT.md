# V96 Shared-Portfolio Precommitment

The shared-account test is fixed before any V96 XAU outcome opens.

- V59/V60 inputs and hashes must remain byte-identical.
- The router may accept at most two V96 entries per UTC date.
...
- Absolute daily P&L correlation between V96 and the baseline must not exceed
  `0.50` in any window.
- The V60 floating-equity replay must include every accepted V96 trade and keep
  buffered drawdown at or below USD `449.7675`.
- No V59/V60 trade may be removed to make room for V96.
