...
V89 = _load_module(
    "dukascopy_growth_risk_v96_v89_metrics",
...
def parameter_space(mechanic: str) -> list[dict[str, Any]]:
    ...
    if mechanic in {"RISK_STATE_ACCELERATION", "GROWTH_STATE_ACCELERATION"}:
        return combine(_space(
...
def source_event_mask_direction(
    ...
    if mechanic in {"RISK_SIGN_REVERSAL", "GROWTH_SIGN_REVERSAL"}:
    ...
    elif mechanic in {"RISK_STATE_ACCELERATION", "GROWTH_STATE_ACCELERATION"}:
        score = growth if mechanic == "GROWTH_STATE_ACCELERATION" else risk
        prior = prior_growth if mechanic == "GROWTH_STATE_ACCELERATION" else prior_risk
        direction = pd.Series(np.sign(score.fillna(0.0)).astype(int), index=frame.index)
        prior_direction = pd.Series(
            np.sign(prior.fillna(0.0)).astype(int), index=frame.index
        )
        ...
        mask &= prior.abs().ge(prior_threshold)
        mask &= score.abs().ge(
            prior.abs() * float(params["acceleration_ratio"])
        )
    ...
    mask &= contiguous
    growth_mechanics = {"GROWTH_SIGN_REVERSAL", "GROWTH_STATE_ACCELERATION"}
    required_prefixes = ("copper", "usdcnh") if mechanic in growth_mechanics else PREFIXES
...
def generate_manifest(
    ...
    for mechanic in MECHANICS:
        ...
        if admitted != policies_per_mechanic:
            raise ValueError(
                f"Only {admitted} source-eligible V96 policies for {mechanic}; "
    ...
    if len(manifest) != expected or manifest["policy_id"].duplicated().any():
        raise ValueError("Invalid V96 policy manifest")
    return manifest
