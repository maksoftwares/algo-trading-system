    ...
    return pd.DataFrame(
        {
            ...
            "z_delta_192": [0.2] * 6,
            "z_lag_1_192": [1.5] * 6,
            "variance_ratio_4_192": [0.9] * 6,
            "variance_ratio_delta_4_192": [0.1] * 6,
            "return_acf_1_192": [0.0] * 6,
            "hurst_192": [0.5] * 6,
...
def _tiered_expansion_params() -> dict[str, object]:
    return {
        "stationarity_window": 192,
        "strong_z_abs_min": 1.25,
        "strong_z_delta_min": 0.5,
        "strong_variance_ratio_max": 1.1,
        "moderate_z_abs_min": 0.75,
        "moderate_z_delta_min": 0.1,
        "moderate_variance_ratio_max": 0.9,
...
def test_stationarity_features_become_finite_after_warmup() -> None:
    ...
    assert np.isfinite(result.loc[900, "z_delta_192"])
    assert np.isfinite(result.loc[900, "hurst_192"])
    assert np.isfinite(result.loc[900, "return_acf_1_192"])
    assert np.isfinite(result.loc[900, "variance_ratio_delta_4_192"])


def test_deterministic_design_has_1000_unique_rows() -> None:
    rows = parameter_space("CHOP_Z_EXPANSION_TIERED_ENVELOPE", 1000)
    assert len(rows) == 1000
    assert len({repr(sorted(row.items())) for row in rows}) == 1000


def test_tiered_expansion_signal_is_restricted_to_chop() -> None:
    mask, direction = signal_mask_direction(
        _signal_frame(),
        "CHOP_Z_EXPANSION_TIERED_ENVELOPE",
        _tiered_expansion_params(),
    )
    assert mask.tolist() == [False, True, True, True, False, True]
    assert direction.loc[mask].eq(1).all()


def test_counterflow_signal_trades_against_attempted_fade() -> None:
    frame = _signal_frame()
    frame["return_3"] = -0.2
    frame["tick_imbalance_15m"] = -0.1
    params = {
        "stationarity_window": 192,
        "variance_horizon": 4,
        "strong_z_abs_min": 1.25,
    ...
    mask, direction = signal_mask_direction(
        frame, "CHOP_COUNTERFLOW_TIERED_ENVELOPE", params
    )
    assert mask.tolist() == [False, True, True, True, False, True]
    assert direction.loc[mask].eq(1).all()
