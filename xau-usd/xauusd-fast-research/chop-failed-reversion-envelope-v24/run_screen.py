...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Failed-Reversion Envelope V24 Result",
        "",
    ...
    lines.extend(
        (
            ...
            "Failed-reversion envelopes use completed M15 bars and H1 risk.",
            "Composite branches share one position and cannot duplicate P&L.",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_failed_reversion_envelope_v24.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V24 outcomes already exist")
    ...
    decision = (
        "CHOP_FAILED_REVERSION_ENVELOPE_V24_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_FAILED_REVERSION_ENVELOPE_V24_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_FAILED_REVERSION_ENVELOPE_V24_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_failed_reversion_v24_artifacts",
            "files": {
                name: {
