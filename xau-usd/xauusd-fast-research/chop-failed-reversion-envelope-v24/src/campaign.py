...
SOURCE_MECHANICS = {
    "CHOP_COUNTERFLOW_TIERED_ENVELOPE": "COUNTERFLOW_TIERED",
    "CHOP_COUNTERFLOW_DUAL_WINDOW_ENVELOPE": "COUNTERFLOW_DUAL_WINDOW",
    "CHOP_Z_EXPANSION_TIERED_ENVELOPE": "Z_EXPANSION_TIERED",
    "CHOP_Z_EXPANSION_DUAL_WINDOW_ENVELOPE": "Z_EXPANSION_DUAL_WINDOW",
    "CHOP_FAILED_REVERSION_DUAL_MODE_ENVELOPE": "FAILED_REVERSION_DUAL_MODE",
}
    ...
    while len(result) < count:
        ...
        for name, items in options.items():
            digest = hashlib.sha256(
                f"chop-failed-reversion-envelope-v24|{mechanic}|{nonce}|{name}".encode(
                    "ascii"
                )
            ).digest()
...
def parameter_space(mechanic: str, candidate_count: int = 400) -> list[dict[str, Any]]:
    ...
    if source == "COUNTERFLOW_TIERED":
        options = dict(
            stationarity_window=(192, 384),
            variance_horizon=(4, 16),
            strong_z_abs_min=(1.25, 1.50),
            strong_variance_ratio_max=(1.10, 1.30),
            moderate_z_abs_min=(0.75, 1.00),
            moderate_variance_ratio_max=(0.70, 0.90),
            mean_slope_abs_max=(0.35, 0.70),
            return_acf_max=(-0.10, 0.0, 0.10),
            m5_confirmation_window=(3, 12),
            counter_move_max=(0.03, 0.06),
            counter_flow_max=(-0.03, -0.01, 0.0),
        )
    elif source == "COUNTERFLOW_DUAL_WINDOW":
        options = dict(
            stationarity_window=(192, 384),
            variance_horizon=(4, 16),
            variance_ratio_max=(0.90, 1.10, 1.30),
            z_abs_min=(0.75, 1.00, 1.25),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            return_acf_max=(-0.10, 0.0, 0.10),
            counter_move_3_max=(0.0, 0.03, 0.06),
            counter_move_12_max=(0.0, 0.03, 0.06),
            counter_flow_max=(-0.03, -0.01, 0.0),
        )
    elif source == "Z_EXPANSION_TIERED":
        options = dict(
            stationarity_window=(192, 384),
            strong_z_abs_min=(1.00, 1.25),
            strong_z_delta_min=(0.50, 0.75),
            strong_variance_ratio_max=(1.10, 1.30),
            moderate_z_abs_min=(0.75, 1.00),
            moderate_z_delta_min=(0.10, 0.25),
            moderate_variance_ratio_max=(0.90, 1.10),
            mean_slope_abs_max=(0.15, 0.35, 0.70),
            m5_confirmation_window=(3, 12),
            m5_alignment_min=(0.0, 0.03),
            flow_alignment_min=(0.0, 0.01),
        )
    elif source == "Z_EXPANSION_DUAL_WINDOW":
        options = dict(
            stationarity_window=(192, 384),
            z_abs_min=(0.75, 1.00, 1.25),
            z_delta_min=(0.25, 0.50),
            variance_ratio_max=(1.10, 1.30),
            mean_slope_abs_max=(0.15, 0.35),
            m5_alignment_3_min=(0.0, 0.03),
            m5_alignment_12_min=(0.0, 0.03),
            flow_alignment_min=(0.0, 0.01),
        )
    elif source == "FAILED_REVERSION_DUAL_MODE":
        options = dict(
            stationarity_window=(192, 384),
            z_abs_min=(1.00, 1.25),
            variance_ratio_max=(1.10, 1.30),
            ...
            m5_confirmation_window=(3, 12),
            counter_move_max=(0.03, 0.06),
            counter_flow_max=(-0.03, -0.01),
            direct_move_min=(0.0, 0.03),
            direct_flow_min=(0.0, 0.01),
        )
    else:
...
def add_m15_stationarity_features(
    ...
    for window in (192, 384, 768):
        ...
        z_score = deviation / standard.replace(0.0, np.nan)
        result[f"z_{window}"] = z_score
        result[f"z_delta_{window}"] = z_score - z_score.shift(1)
        for lag in (1, 2, 4):
            result[f"z_lag_{lag}_{window}"] = z_score.shift(lag)
        ...
        for horizon in (4, 16):
            ...
            variance_ratio = horizon_variance / (
                horizon * one_variance.replace(0.0, np.nan)
            )
            result[f"variance_ratio_{horizon}_{window}"] = variance_ratio
            result[f"variance_ratio_delta_{horizon}_{window}"] = (
                variance_ratio - variance_ratio.shift(1)
            )
...
def signal_mask_direction(
    ...
    if source == "COUNTERFLOW_TIERED":
        ...
        strong = (
            z.abs().ge(float(params["strong_z_abs_min"]))
            & variance_ratio.le(float(params["strong_variance_ratio_max"]))
        )
        moderate = (
            z.abs().ge(float(params["moderate_z_abs_min"]))
        ...
        m5_move = move_3 if int(params["m5_confirmation_window"]) == 3 else move_12
        confirmation = (
            direction.mul(m5_move).le(float(params["counter_move_max"]))
        ...
        confirmation = (
            (
                ...
                | direction.mul(move_12).le(
                    float(params["counter_move_12_max"])
                )
            )
            & direction.mul(imbalance).le(float(params["counter_flow_max"]))
        )
        ...
        z_expansion = direction.mul(frame[f"z_delta_{window}"])
        variance_ratio = frame[f"variance_ratio_4_{window}"]
        strong = (
            z.abs().ge(float(params["strong_z_abs_min"]))
            & z_expansion.ge(float(params["strong_z_delta_min"]))
            & variance_ratio.le(float(params["strong_variance_ratio_max"]))
        )
        moderate = (
            z.abs().ge(float(params["moderate_z_abs_min"]))
            & z_expansion.ge(float(params["moderate_z_delta_min"]))
            & variance_ratio.le(float(params["moderate_variance_ratio_max"]))
        )
        mask = (
            (strong | moderate)
        ...
        confirmation = (
            (
                ...
                | direction.mul(move_12).ge(
                    float(params["m5_alignment_12_min"])
                )
            )
            & direction.mul(imbalance).ge(float(params["flow_alignment_min"]))
        )
        ...
        expansion = (
            ...
            & direction.mul(m5_move).ge(float(params["direct_move_min"]))
