...
CONFIG = ROOT / "config" / "comex_flow_transition_v44.json"
OUTPUTS = ROOT / "outputs"
AUDIT = OUTPUTS / "COMEX_FLOW_TRANSITION_V44_CALIBRATION_AUDIT.json"
CANDIDATES = OUTPUTS / "COMEX_FLOW_TRANSITION_V44_CALIBRATION_CANDIDATES.csv"
...
def main() -> None:
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            feature_frames.append(build_transition_features(session, rule=rule))
    ...
    decision = (
        "V44_CALIBRATION_PASS_READY_TO_LOCK"
        if selected is not None
        else "V44_CALIBRATION_FREQUENCY_STRUCTURE_FAIL"
    )
    selected_candidates = (
    ...
    payload: dict[str, object] = {
        "schema_version": "xauusd_comex_flow_transition_v44_calibration_audit",
