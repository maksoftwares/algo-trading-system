...
def _paths(stage: str) -> tuple[Path, Path, Path]:
    prefix = f"COMEX_FLOW_TRANSITION_V44_{stage.upper()}"
    return (
...
def main() -> None:
    ...
    for number, path in enumerate(files, start=1):
        ...
        if quality["eligible_full_weekday"]:
            features = build_transition_features(session, rule=rule)
    ...
    if not candidates.empty:
        ...
        if candidates["candidate_id"].duplicated().any():
            raise RuntimeError("V44 stage candidates contain duplicate IDs.")
    ...
    decision = (
        f"V44_{stage.upper()}_PASS"
        if result["gate_passed"]
        else f"V44_{stage.upper()}_FAIL_TERMINAL"
    )
    payload: dict[str, Any] = {
        "schema_version": "xauusd_comex_flow_transition_v44_stage_audit",
