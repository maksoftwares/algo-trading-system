...
def parameter_space(mechanic: str, config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    rotation = {
        "deviation_atr_min": [0.4, 0.6, 0.9, 1.2, 1.6],
        "deviation_atr_max": [1.5, 2.5, 4.0, 8.0],
        "target_fraction": [0.25, 0.5, 0.75],
        "confirmation": ["CANDLE", "WICK", "EITHER"],
        "wick_min": [0.1, 0.25, 0.4],
        "hour_window": hours,
        "geometry_id": geometries,
    }
    if mechanic == "CHOP_WEEK_OPEN_PARTIAL_ROTATION":
        return [
            params
            for params in _space(minimum_week_bars=[8, 24, 48], **rotation)
            if float(params["deviation_atr_min"])
            < float(params["deviation_atr_max"])
        ]
    if mechanic == "CHOP_WEEK_VWAP_PARTIAL_ROTATION":
        return [
            params
            for params in _space(minimum_week_bars=[8, 24, 48], **rotation)
            if float(params["deviation_atr_min"])
    ...
    if mechanic == "CHOP_PRIOR_WEEK_MID_PARTIAL_ROTATION":
        return [
            params
            for params in _space(**rotation)
            if float(params["deviation_atr_min"])
            < float(params["deviation_atr_max"])
        ]
    if mechanic == "CHOP_PRIOR_WEEK_EDGE_REENTRY":
        return [
            params
    ...
    if mechanic == "CHOP_LONDON_WEEK_IMPULSE_FADE":
        return [
            params
            for params in _space(
                momentum_bars=[1, 2, 4, 8],
                momentum_atr=[0.3, 0.5, 0.75, 1.0, 1.4],
                deviation_atr_min=[0.4, 0.6, 0.9, 1.2],
                deviation_atr_max=[1.5, 2.5, 4.0, 8.0],
                target_fraction=[0.25, 0.5, 0.75],
                confirmation=["CANDLE", "WICK", "EITHER"],
                wick_min=[0.1, 0.25, 0.4],
...
def add_anchor_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    week_weight = weight.groupby(week, sort=False).cumsum()
    result["week_vwap"] = (typical * weight).groupby(
        week, sort=False
    ).cumsum() / week_weight
    weekly = week_group.agg(
        current_week_high=("mid_high", "max"),
        current_week_low=("mid_low", "min"),
    )
    prior_week = weekly.shift(1).rename(
        columns={
            "current_week_high": "prior_week_high",
            "current_week_low": "prior_week_low",
        }
    )
    result = result.join(prior_week, on="utc_week")
    result["prior_week_mid"] = (
        result["prior_week_high"] + result["prior_week_low"]
    ) / 2.0
    result["weekday"] = starts.dt.weekday
...
def signal_mask_direction_target(
    ...
    if mechanic in (
        "CHOP_WEEK_OPEN_PARTIAL_ROTATION",
        "CHOP_WEEK_VWAP_PARTIAL_ROTATION",
        "CHOP_PRIOR_WEEK_MID_PARTIAL_ROTATION",
    ):
        anchor_column = {
            "CHOP_WEEK_OPEN_PARTIAL_ROTATION": "week_open",
            "CHOP_WEEK_VWAP_PARTIAL_ROTATION": "week_vwap",
            "CHOP_PRIOR_WEEK_MID_PARTIAL_ROTATION": "prior_week_mid",
        }[mechanic]
        anchor = frame[anchor_column].astype(float)
        deviation = (close - anchor) / atr
        direction = -np.sign(deviation).fillna(0).astype(int)
        target = close + float(params["target_fraction"]) * (anchor - close)
        structure = (
            deviation.abs().between(
                float(params["deviation_atr_min"]),
                float(params["deviation_atr_max"]),
                inclusive="both",
            )
        ...
        if "minimum_week_bars" in params:
            structure &= frame["week_bar_number"].ge(
                int(params["minimum_week_bars"])
            )
    elif mechanic == "CHOP_PRIOR_WEEK_EDGE_REENTRY":
        direction = _false_break_direction(
            frame,
            frame["prior_week_high"],
            frame["prior_week_low"],
            float(params["sweep_atr"]),
            float(params["reentry_atr"]),
            float(params["wick_min"]),
        )
        anchor = frame["prior_week_mid"].astype(float)
        target = close + float(params["target_fraction"]) * (anchor - close)
        structure = pd.Series(True, index=frame.index)
    elif mechanic == "CHOP_LONDON_WEEK_IMPULSE_FADE":
        anchor = frame["week_open"].astype(float)
        prior_deviation = (close.shift(1) - anchor.shift(1)) / atr.shift(1)
        away = np.sign(prior_deviation).fillna(0).astype(int)
        direction = -away
        prior_momentum = frame[
            f"return_{int(params['momentum_bars'])}_local"
        ].shift(1) / atr.shift(1)
        current_deviation = (close - anchor) / atr
        target = close + float(params["target_fraction"]) * (anchor - close)
        structure = (
            current_deviation.abs().between(
                float(params["deviation_atr_min"]),
                float(params["deviation_atr_max"]),
                inclusive="both",
            )
            & prior_momentum.mul(away).ge(float(params["momentum_atr"]))
            & _rotation_confirmation(
                frame,
                direction,
                str(params["confirmation"]),
