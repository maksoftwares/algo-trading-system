...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Weekly Partial-Rotation V13 Result",
        "",
    ...
    lines.extend(
        (
            "",
            "All anchors are frozen at the completed signal bar.",
...
def main() -> int:
    ...
    config = json.loads(
        (ROOT / "config" / "chop_weekly_partial_rotation_v13.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V13 outcomes already exist")
    ...
    decision = (
        "CHOP_WEEKLY_PARTIAL_V13_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_WEEKLY_PARTIAL_V13_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_WEEKLY_PARTIAL_V13_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_weekly_partial_v13_artifacts",
            "files": {
                name: {
