...
def verify_lock(config: dict[str, Any], lock: dict[str, Any]) -> None:
    ...
    if canonical_json_sha256(definition) != lock["definition_contract_sha256"]:
        raise ValueError("Action V4 definition-contract digest is invalid")
    if sha256_file(CONFIG_PATH) != lock["config_sha256"]:
        raise ValueError("Action V4 configuration changed after lock")
    if sha256_file(ROOT / "PREREGISTRATION.md") != lock["preregistration_sha256"]:
        raise ValueError("Action V4 preregistration changed after lock")
    for name, spec in lock["implementation"].items():
        path = REPO_ROOT / str(spec["path"])
        if sha256_file(path) != str(spec["sha256"]):
            raise ValueError(f"Action V4 implementation changed after lock: {name}")
...
def write_result_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Action Models V4 Corrected-Data Replay Result",
        "",
...
def main() -> int:
    ...
    if not lock_path.is_file():
        raise FileNotFoundError("Run lock_contract.py before Action V4 evaluation")
    ...
    if methodology_hash != lock["methodology_contract_sha256"]:
        raise ValueError("Action V4 methodology contract changed after lock")
    features = list(v4_config["model_features"])
    if lock["model_feature_sha256"] != lock["expected_model_feature_sha256"]:
        raise ValueError("Locked Action V4 feature hash is inconsistent")
    ...
    if sorted(splits["fold_id"].unique()) != sorted(fold_ids):
        raise ValueError("V4 split fold IDs changed")
    ...
    for lane_index, lane in enumerate(lanes):
        ...
        decision = (
            "ACTION_MODEL_V4_LANE_GATE_PASS_DEVELOPMENT_ONLY"
            if all(checks.values())
            else "ACTION_MODEL_V4_LANE_GATE_FAIL"
        )
        acceptance[lane] = {
            "decision": decision,
            "checks": checks,
    ...
    passed = [
        ...
        if value["decision"] == "ACTION_MODEL_V4_LANE_GATE_PASS_DEVELOPMENT_ONLY"
    ]
    decision = (
        "ACTION_MODEL_V4_FAMILY_GATES_PASS_DEVELOPMENT_ONLY"
        if passed
        else "ACTION_MODEL_V4_MODEL_EVIDENCE_GATE_FAIL"
    )
    result = {
    ...
    paths = {
        ...
        "comparison_v3": output / config["outputs"]["comparison_v3"],
    }
    ...
    reference_result = json.loads(
        inputs["previous_action_v3_result"].read_text(encoding="utf-8")
    )
    write_json(
        paths["comparison_v3"], build_result_comparison(result, reference_result)
    )
