...
def _exit_prices(row: pd.Series, direction: str, exit_exec: float) -> tuple[float, float, float]:
    spread = max(0.0, float(row["ask_close"] - row["bid_close"]))
    mid = exit_exec + spread / 2.0 if direction == "LONG" else exit_exec - spread / 2.0
    return mid, spread, _point_size(row)


def _simulate_trade(bars: pd.DataFrame, entry_index: int, signal: pd.Series, stress_slippage_r: float) -> dict[str, Any]:
    ...
    elif signal["strategy_id"] == "CHOP_IMPULSE_EXHAUSTION_REVERSION_V1" and not (0.5 * atr_value <= distance <= 2.0 * atr_value):
        rejection = "IMPULSE_STOP_OUTSIDE_0P5_TO_2_ATR"
    if rejection:
        return {"accepted": False, "rejection_reason": rejection}

    max_index = min(len(bars) - 1, entry_index + int(signal["max_hold_bars"]) - 1)
    exit_index = max_index
    exit_reason = "MAX_HOLD"
    ambiguous = False
    exit_exec = float(bars.iloc[max_index]["bid_close"] if direction == "LONG" else bars.iloc[max_index]["ask_close"])
    favourable = adverse = 0.0
    for index in range(entry_index, max_index + 1):
        row = bars.iloc[index]
        if index > entry_index and not bool(row["chop_active_at_open"]):
            exit_index = index
    ...
    exit_mid, exit_spread, point_size = _exit_prices(exit_row, direction, exit_exec)
    ...
    entry_time, exit_time = entry_row["bar_start_utc"], exit_row["timestamp_utc"]
    return {
...
def run_cell(bars: pd.DataFrame, candidates: pd.DataFrame, timeframe: str, cooldown_hours: int, stress_slippage_r: float) -> BacktestResult:
    ...
    for strategy_id, strategy_signals in candidates.groupby("strategy_id", sort=True):
        ...
        for _, signal in strategy_signals.sort_values("signal_time", kind="mergesort").iterrows():
            ...
            if reason:
                ...
                continue
            outcome = _simulate_trade(bars, entry_index, signal, stress_slippage_r)
            ...
            open_until = int(outcome["exit_index"])
