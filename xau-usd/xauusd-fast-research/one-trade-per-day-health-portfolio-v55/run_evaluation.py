...
from risk_overlay import govern_addons_soft_risk  # noqa: E402
...
def main() -> int:
    config_path = ROOT / "config" / "one_trade_per_day_health_portfolio_v55.json"
    ...
    decision = (
        "V55_ONE_TRADE_PER_DAY_HISTORICAL_GATE_PASS"
        if gate["passed"]
        else "V55_ONE_TRADE_PER_DAY_HISTORICAL_GATE_FAIL_TERMINAL"
    )
    result = {
        "schema_version": "xauusd_one_trade_per_day_health_portfolio_v55_result",
        "decision": decision,
        ...
        "half_risk_addons": int(
            decisions.loc[
                decisions["accepted"].astype(bool)
                & decisions["risk_multiplier"].eq(0.5)
            ].shape[0]
        ),
    ...
    lines = [
        ...
        f"Half-risk add-ons: **{result['half_risk_addons']}**",
        "",
    ...
    manifest = {
        "schema_version": "xauusd_one_trade_per_day_health_portfolio_v55_manifest",
        "artifacts": artifacts,
    }
