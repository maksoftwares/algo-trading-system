...
from research import evaluate_gate, run_research, stage_metrics  # noqa: E402
...
def render_report(payload: dict[str, Any], stages: pd.DataFrame) -> str:
    ...
    for row in stages.to_dict("records"):
        ...
        lines.append(
            f"| {row['stage']} | {row['trades']} | {row['long_trades']}/{row['short_trades']} | "
            f"{row['trades_per_source_day']:.3f} | {pf} | {stress_pf} | {row['average_stress_r']:.3f} | "
            f"{row['stress_net_r']:.3f} | {row['stress_drawdown_r']:.3f} | {'PASS' if row['pass'] else 'FAIL'} |"
        )
    lines.extend(["", "## Interpretation", "", payload["interpretation"], "", "Research only. No Python prediction, EA, demo, live, or broker authorization is granted.", ""])
    return "\n".join(lines)


def main() -> int:
    config_path = ROOT / "config" / "macro_h4_confluence_portability_v1.json"
    ...
    if passed:
        decision = "RETROSPECTIVE_MACRO_H4_SURVIVOR_REQUIRES_PROSPECTIVE_VALIDATION"
        interpretation = (
            "The frozen macro specialist passed every chronological and all-in-cost gate, including the post-2024 exam. "
            "It is a low-frequency research survivor pending exact MT5 parity, independent review, and prospective shadow evidence."
        )
    else:
        decision = "MACRO_H4_CONFLUENCE_PORTABILITY_REJECTED"
        interpretation = "The unchanged macro specialist failed at least one frozen gate and is not tuned or rescued in V1."
    ...
    run.signals.to_csv(output / config["outputs"]["signals"], index=False)
    run.trades.to_csv(output / config["outputs"]["trades"], index=False)
