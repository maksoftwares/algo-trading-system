...
from crossasset_volatility import (
...
def test_feature_is_causal_and_requires_crossasset_volatility() -> None:
    ...
    assert row["source_direction_relation"] == "AGREE"
    assert int(row["xau_current_timestamp_ms"]) == base + 900
    assert int(row["decision_timestamp_ms"]) == base + 1000

    disagreeing_xag = quote_frame([(base, 20.0), (base + 900, 20.01)])
    disagreeing = build_volatility_features(
        date,
        dxy,
        disagreeing_xag,
        xau,
    ...
    assert disagreeing.iloc[0]["direction"] == "LONG"
    assert disagreeing.iloc[0]["source_direction_relation"] == "DISAGREE"
...
def test_policy_grid_has_exactly_one_thousand_members() -> None:
    calibration = {
        ...
        "minimum_xau_move_bps_grid": [1, 2, 3, 4, 5],
...
def test_source_audits_are_sliced_on_registered_stage_boundaries() -> None:
    config = json.loads(
        (ROOT / "config" / "crossasset_volatility_xau_momentum_v84.json").read_text(
            encoding="utf-8"
        )
    )
    assert SCOPES == (
    ...
    assert source_audit_decision("development") == (
        "V84_DEVELOPMENT_SOURCE_AUDIT_PASS"
    )
    assert source_audit_output_path(config, "exam").name == (
        "CROSSASSET_VOL_XAU_V84_EXAM_SOURCE_AUDIT.json"
    )
