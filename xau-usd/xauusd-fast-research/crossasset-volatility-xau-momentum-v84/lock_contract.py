...
CONFIG = ROOT / "config" / "crossasset_volatility_xau_momentum_v84.json"
PACKAGE_FILES = (
    "README.md",
    "PREREGISTRATION.md",
    ...
    "requirements.txt",
    "config/crossasset_volatility_xau_momentum_v84.json",
    "src/__init__.py",
    "src/crossasset_volatility.py",
    "run_source_audit.py",
    "run_calibration.py",
    "lock_contract.py",
    "run_stage.py",
    "tests/conftest.py",
    "tests/test_crossasset_volatility.py",
)
...
def build_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if sha256_file(feature_path) != calibration["feature_sha256"]:
        raise ValueError("V84 calibration features changed")
    if sha256_file(grid_path) != calibration["grid_sha256"]:
        raise ValueError("V84 calibration grid changed")
    ...
    for dependency in config["locked_dependencies"].values():
        ...
        if sha256_file(path) != dependency["sha256"]:
            raise ValueError(f"V84 dependency changed: {path}")
    ...
    if sha256_file(atr_path) != config["spot_source"]["m5_feature_sha256"]:
        raise ValueError("V84 completed-M5 ATR cache changed")
    contract: dict[str, Any] = {
        "schema_version": "xauusd_crossasset_volatility_xau_momentum_v84_contract_lock",
...
def verify_lock(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if lock != expected:
        raise ValueError("V84 immutable contract verification failed")
    return lock
...
def main() -> int:
    ...
    if path.exists():
        raise FileExistsError("V84 contract already exists")
    economic_outputs = [
        output / f"CROSSASSET_VOL_XAU_V84_{stage.upper()}_{suffix}"
        for stage in (
            "development",
            "confirmation",
            "validation",
    ...
    if existing := [str(path) for path in economic_outputs if path.exists()]:
        raise ValueError(f"V84 economic outputs existed before lock: {existing}")
