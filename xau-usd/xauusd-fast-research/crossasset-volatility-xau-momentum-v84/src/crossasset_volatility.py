...
def build_volatility_features(
    ...
    *,
    ...
    for horizon in horizons_ms:
        ...
        xau_magnitude = np.abs(xau_move)
        xau_sign = np.sign(xau_move)
        ...
        valid &= xau_magnitude >= float(prefilter["minimum_xau_move_bps"])
            ...
            pd.DataFrame(
                {
                    ...
                    "xag_magnitude_bps": xag_magnitude[chosen],
                    "xau_magnitude_bps": xau_magnitude[chosen],
                    "source_quote_count": source_quote_count[chosen],
                    "source_direction_relation": np.where(
                        np.sign(xag_move[chosen]) == -np.sign(dxy_move[chosen]),
                        "AGREE",
                        "DISAGREE",
                    ),
                    "direction": np.where(
                        xau_sign[chosen] > 0, "LONG", "SHORT"
                    ),
                }
            )
        )
...
def policy_id(policy: Mapping[str, Any]) -> str:
    return (
        ...
        f"__AU{int(round(float(policy['minimum_xau_move_bps']) * 100)):03d}"
...
def generate_candidates(
    ...
    mask &= features["xau_magnitude_bps"] >= float(
        policy["minimum_xau_move_bps"]
    )
    ...
    if selected["candidate_id"].duplicated().any():
        raise ValueError("V84 candidate IDs are not unique")
...
def select_policy(
    ...
    eligible.sort(
        key=lambda row: (
            ...
            -float(row["minimum_xau_move_bps"]),
