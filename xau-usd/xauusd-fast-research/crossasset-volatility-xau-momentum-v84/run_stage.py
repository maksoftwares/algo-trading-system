...
from crossasset_volatility import (  # noqa: E402
...
def output_paths(config: Mapping[str, Any], stage: str) -> tuple[Path, Path, Path]:
    ...
    prefix = f"CROSSASSET_VOL_XAU_V84_{stage.upper()}"
    return (
...
def require_firewall(config: Mapping[str, Any], stage: str) -> None:
    for prior in STAGES[: STAGES.index(stage)]:
        ...
        if not path.is_file():
            raise RuntimeError(f"V84 {stage} sealed because {prior} has not run")
        audit = load_json(path)
        if (
            canonical_hash(audit, "audit_sha256") != audit.get("audit_sha256")
            or not bool(audit.get("gate_passed"))
        ):
            raise RuntimeError(f"V84 {stage} sealed because {prior} failed")
...
def verify_stage_source_audit(
    ...
    if (
        ...
        raise ValueError(f"V84 {stage} source audit is invalid")
    if stage == "development" and sha256_file(path) != contract[
        "development_source_audit"
    ]["sha256"]:
        raise ValueError("V84 locked development source audit changed")
    return audit, path
...
def run_stage(stage: str) -> dict[str, Any]:
    ...
    if any(path.exists() for path in (candidate_path, label_path, audit_path)):
        raise FileExistsError(f"V84 {stage} outputs already exist")
    ...
    for number, date in enumerate(
        ...
        if number % 25 == 0:
            print(f"V84 {stage}: processed through {date.date()}", flush=True)
    candidates = (
    ...
    decision = (
        f"V84_{stage.upper()}_PASS"
        if bool(result["gate_passed"])
        else f"V84_{stage.upper()}_FAIL_TERMINAL"
    )
    audit: dict[str, Any] = {
        "schema_version": "xauusd_crossasset_volatility_xau_momentum_v84_stage_audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one sealed V84 stage")
