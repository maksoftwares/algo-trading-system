...
from correction import execution_arrays, load_config  # noqa: E402
...
def _render(result: dict[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# XAUUSD M15 Regime Target Campaign V2 Result",
        "",
        ...
        "",
        f"Negative next-bar gaps after normalization: **{result['clock_audit']['negative_next_bar_gaps']}**",
        "",
        "Historical eras are discovery evidence. No result authorizes training or execution.",
        "",
        "## Regime summary",
        "",
...
def _artifact_manifest(directory: Path, names: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "xauusd_m15_regime_target_campaign_v2_artifacts",
        "files": {
            name: {
...
def main() -> int:
    ...
    frame = CAMPAIGN.prepare_features(m15, h4, config, ADAPTIVE, REGIMES)
    ...
    for offset, manifest_row in enumerate(manifest.itertuples(index=False), 1):
        trades = CAMPAIGN.simulate_variant(
            frame, arrays, manifest_row, config["execution"], outcome_cache
        )
    ...
    decision = (
        ...
        else "NO_M15_REGIME_TARGET_V2_ECONOMIC_FINALIST"
    )
    result = {
