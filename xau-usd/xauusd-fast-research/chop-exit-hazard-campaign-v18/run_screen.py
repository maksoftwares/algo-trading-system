...
def _render(result: Mapping[str, Any], shortlist: pd.DataFrame) -> str:
    lines = [
        "# Chop Exit-Hazard Campaign V18 Result",
        "",
...
def main() -> int:
    ...
    config = json.loads(
        (
            ROOT / "config" / "chop_exit_hazard_campaign_v18.json"
        ).read_text(
            encoding="utf-8"
        )
    )
    ...
    if result_path.exists():
        raise FileExistsError("V18 outcomes already exist")
    ...
    decision = (
        "CHOP_EXIT_HAZARD_V18_FDR_FINALIST_FOUND"
        if not fdr.empty
        else (
            "CHOP_EXIT_HAZARD_V18_ECONOMIC_FINALIST_FOUND"
            if not economic.empty
            else "NO_CHOP_EXIT_HAZARD_V18_ECONOMIC_FINALIST"
        )
    )
    mechanic_summary = {
        mechanic: {
    ...
    write_json(
        ...
        {
            "schema_version": "xauusd_chop_exit_hazard_v18_artifacts",
            "files": {
                name: {
