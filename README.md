...
The 2026-06-04 direct-MT5 demo loss case study adds repeatable actual-broker analysis from `C:\Program Files\MetaTrader 5\terminal64.exe` / `1025742 Capital.ComMena-Demo`. The latest shadow-only review report is `xau-usd/xauusd-phase1/docs/PHASE2_DEMO_SHADOW_FILTER_REPORT_2026_06_04.md`; it measures, but does not enforce, a hypothetical block for `session_extreme_retest_v0` and XAUUSD morning/afternoon trades. The shadow report is retrospective evidence only and does not authorize canonical Phase 2 or router/session-filter changes.

The latest direct-MT5 trade export for reviewer inspection is packaged at `xau-usd/xauusd-phase1/docs/review_exports/PHASE2_DEMO_ACTUAL_TRADES_REVIEW_2026_06_04.zip`. It contains grouped actual broker trades, the loss case-study ledger, and the shadow-filter ledger. This export is experimental demo evidence only.
