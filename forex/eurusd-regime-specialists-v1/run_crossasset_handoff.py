...
from eurusd_regime_specialists.crossasset_handoff import (  # noqa: E402
    OWNERS,
    census,
    generate_signals,
    load_config,
...
def main() -> int:
    ...
    signals = generate_signals(m5, state, cfg)
    capacity = census(signals, m5, cfg)
    output = ROOT / "outputs" / "crossasset_handoff"
    ...
    if not capacity["passed"]:
        raise RuntimeError("Capacity gate failed; outcomes remain closed")
    result, trades = run_backtest(signals, m5, cfg)
    write_json(output / "RESULT.json", result)
    for name in [*OWNERS, "PORTFOLIO", "ALL_SPECIALIST_DIAGNOSTIC"]:
