...
FAMILY = "N17_NEUTRAL_DTCC_OTC_FX_OPTIONS_FLOW"
OUTPUT_ROOT = PACKAGE_ROOT / "outputs" / "neutral_dtcc_fx_options"
...
def load_config() -> dict[str, Any]:
    return json.loads(
        (
            PACKAGE_ROOT
            / "config"
            / "frozen_neutral_dtcc_fx_options.json"
...
def verify_lock() -> dict[str, str]:
    lock = json.loads(
        (
            PACKAGE_ROOT
            / "EURUSD_NEUTRAL_DTCC_FX_OPTIONS_PREREG_2026_07_28.sha256.json"
        ).read_text(encoding="utf-8")
    )
    if (
        lock.get("locked_before_dtcc_fx_options_outcome_pass")
        is not True
    ):
        raise RuntimeError("DTCC FX-options contract is not locked")
    checked: dict[str, str] = {}
    for relative, expected in lock["files"].items():
        actual = sha256_file(PACKAGE_ROOT / relative)
        if actual != expected:
            raise RuntimeError(
                "DTCC FX-options preregistration mismatch: "
                f"{relative}"
            )
    ...
    if sha256_file(parent_path) != cfg["parent_contract"]["sha256"]:
        raise RuntimeError("DTCC FX-options parent mismatch")
    manifest_path = Path(cfg["source_manifest"]["path"])
    if (
        sha256_file(manifest_path)
        != cfg["source_manifest"]["sha256"]
    ):
        raise RuntimeError("DTCC FX-options manifest mismatch")
    source_path = Path(cfg["source"]["path"])
    if sha256_file(source_path) != cfg["source"]["sha256"]:
        raise RuntimeError("DTCC FX-options source mismatch")
    return checked


def prepare_dtcc_source(
    frame: pd.DataFrame, baseline_sessions: int
) -> pd.DataFrame:
    required = {
        "trade_date",
        "available_time_utc",
        "call_qualified_trades",
        "put_qualified_trades",
        "call_eur_notional",
        "put_eur_notional",
        "call_usd_premium",
        "put_usd_premium",
    ...
    if missing:
        raise RuntimeError(
            f"Missing DTCC source columns: {sorted(missing)}"
        )
    result = frame.copy()
    ...
    result = result[
        result["qualified_total_trades"].astype(float).gt(0)
    ]
    result = (
    ...
    result["raw_notional_imbalance"] = (
        np.log1p(result["call_eur_notional"].astype(float))
        - np.log1p(result["put_eur_notional"].astype(float))
    )
    result["raw_premium_imbalance"] = (
        np.log1p(result["call_usd_premium"].astype(float))
        - np.log1p(result["put_usd_premium"].astype(float))
    )
    for measure in ("notional", "premium"):
        raw = f"raw_{measure}_imbalance"
        prior = f"prior_{measure}_imbalance_median"
        normalized = f"normalized_{measure}_imbalance"
        result[prior] = (
            result[raw]
            .shift(1)
            .rolling(
        ...
        result[normalized] = result[raw] - result[prior]
    result["composite_imbalance"] = 0.5 * (
        result["normalized_notional_imbalance"]
        + result["normalized_premium_imbalance"]
    )
    return result


def attach_dtcc_source(
    candidates: pd.DataFrame,
    source: pd.DataFrame,
    cfg: dict[str, Any],
) -> pd.DataFrame:
    prepared = prepare_dtcc_source(
        source,
    ...
    columns = [
        ...
        "put_usd_premium",
        "qualified_total_trades",
        "raw_notional_imbalance",
        "raw_premium_imbalance",
        "prior_notional_imbalance_median",
        "prior_premium_imbalance_median",
        "normalized_notional_imbalance",
        "normalized_premium_imbalance",
        "composite_imbalance",
    ]
    ...
    result["source_available"] = (
        ...
        & result["composite_imbalance"].notna()
    )
    result["active_participation"] = result["source_available"]
    result["trade_candidate"] = (
        result["source_available"]
        & result["composite_imbalance"].ne(0)
    )
    result["side"] = np.select(
        [
            result["trade_candidate"]
            & result["composite_imbalance"].gt(0),
            result["trade_candidate"]
            & result["composite_imbalance"].lt(0),
        ],
        ["LONG", "SHORT"],
        default="CASH",
    )
...
def build_candidates(
    ...
    *,
    ...
    attached = attach_dtcc_source(candidates, source, cfg)
        ...
        records.append(
            {
                ...
                **outcome,
                ...
                "call_qualified_trades": candidate[
                    "call_qualified_trades"
                ],
                "put_qualified_trades": candidate[
                    "put_qualified_trades"
                ],
                "call_eur_notional": candidate[
                    "call_eur_notional"
                ],
                "put_eur_notional": candidate["put_eur_notional"],
                "call_usd_premium": candidate["call_usd_premium"],
                "put_usd_premium": candidate["put_usd_premium"],
                "normalized_notional_imbalance": candidate[
                    "normalized_notional_imbalance"
                ],
                "normalized_premium_imbalance": candidate[
                    "normalized_premium_imbalance"
                ],
                "composite_imbalance": candidate[
                    "composite_imbalance"
                ],
            }
        )
...
def run_neutral_dtcc_fx_options() -> tuple[
    ...
    result = {
        ...
        "status": (
            ...
            if admitted
            else "REJECTED_NEUTRAL_DTCC_FX_OPTIONS_V1"
        ),
        "research_only": True,
        ...
        "causality": {
            "signal": (
                "Prior-session standalone new EUR/USD OTC vanilla "
                "call-versus-put notional and premium flow"
            ),
            "source_available_time": (
                "Each trade's public dissemination timestamp; daily "
                "aggregation becomes eligible at the next UTC midnight"
            ),
        ...
        "admitted": admitted,
        "verdict": (
            "The fixed DTCC OTC options-flow expert passed every "
            "historical gate; untouched prospective confirmation is "
            "mandatory."
            if admitted
            else "The fixed DTCC OTC options-flow expert failed at "
            "least one frozen chronological or robustness gate and "
