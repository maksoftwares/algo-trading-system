...
CONFIG_PATH = (
    PACKAGE_ROOT
    / "config"
    / "frozen_neutral_prior24_extreme_fade_execution.json"
)
LOCK_PATH = (
    PACKAGE_ROOT
    / "EURUSD_NEUTRAL_PRIOR24_EXTREME_FADE_EXECUTION_"
    "PREREG_2026_07_29.sha256.json"
)
OUTPUT_ROOT = (
    PACKAGE_ROOT
    / "outputs"
    / "neutral_prior24_extreme_fade_execution"
)
...
def verify_lock() -> dict[str, str]:
    ...
    if (
        ...
        raise RuntimeError("Prior-24-hour execution lock is incomplete")
    checked: dict[str, str] = {}
    for relative, expected in lock["files"].items():
        actual = sha256_file(PACKAGE_ROOT / relative)
        if actual != expected:
            raise RuntimeError(
                f"Prior-24-hour execution drift: {relative}"
            )
...
def load_candidates(config: dict[str, Any]) -> pd.DataFrame:
    ...
    if sha256_file(census_lock_path) != census_lock["sha256"]:
        raise RuntimeError("Prior-24-hour census result lock drift")
    ...
    if (
        ...
        raise RuntimeError("Prior-24-hour census did not pass")
    source = config["candidate_source"]
    path = PACKAGE_ROOT / source["path"]
    if sha256_file(path) != source["sha256"]:
        raise RuntimeError("Prior-24-hour candidate source drift")
    ...
    if not required.issubset(candidates.columns):
        raise RuntimeError("Prior-24-hour candidate schema drift")
    for column in (
        "signal_time_utc",
        "entry_time_utc",
    ...
    if (
        ...
        raise RuntimeError("Prior-24-hour candidate census drift")
    candidates["window"] = ""
...
def run_execution() -> tuple[
    ...
    trades, routing = engine.execute_candidates(
    ...
    status = (
        ...
        else "REJECTED_EXACT_PRIOR24_EXTREME_FADE"
    )
    result = {
        "schema_version": (
