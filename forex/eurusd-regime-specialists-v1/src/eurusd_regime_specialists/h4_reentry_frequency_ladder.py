...
def build_quota_signal_mask(
    h1: pd.DataFrame, candidate: dict[str, Any], quota: int
) -> pd.Series:
    ...
    return raw & raw.groupby(date).cumsum().le(int(quota))
...
def _render_report(result: dict[str, Any]) -> str:
    ...
    return f"""# EURUSD H4 re-entry frequency ladder result

Status: **{result["status"]}**

| Level | Quota | Trades | Gain | PF | +0.5 pip PF | Latest-12M PF | Eligible |
|---|---:|---:|---:|---:|---:|---:|---:|
{rows}
...
def run(config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for level_name, quota in config["signal_quota_ladder"].items():
        delayed_ledgers = {}
        delayed_caps = {}
        for delay in delays:
            pieces = []
            for regime in ("chop", "compression"):
                ...
                candidate["specialist_id"] = f"{level_name}_{regime.upper()}_REENTRY"
                raw, diag = simulate_short(
                    h1,
                    m5,
                    build_quota_signal_mask(h1, candidate, int(quota)),
                    candidate,
                    anchor,
            ...
            item["calendar_bootstrap"],
            {
                **config["frequency_preserving_edge_gates"],
                "minimum_trade_count_gain": float(
                    config["selection_rule"]["minimum_trade_count_gain_over_Q1"]
                ),
            },
        )
    ...
    eligible = [
        name
        for name in config["signal_quota_ladder"]
        if level_results[name]["eligible"]
    ]
    selected_level = eligible[-1] if eligible else "Q1_PROTECTED"
    ...
    result = {
        "schema_version": "eurusd_h4_reentry_frequency_ladder_result_v1",
        ...
        "eligible_nonbaseline_levels": [
            name for name in eligible if name != "Q1_PROTECTED"
        ],
        "selected_level": selected_level,
        "selected_trade_count_gain": selected["frequency"]["trade_count_gain"],
        "status": (
            "REENTRY_INCREASED_HISTORICAL_FREQUENCY_WITH_EDGE_PRESERVED_REQUIRES_FRESH_CONFIRMATION"
            if selected_level != "Q1_PROTECTED"
            else "NO_SAFE_REENTRY_FREQUENCY_EXPANSION"
        ),
    }
    ...
    (
        package_root / "EURUSD_H4_REENTRY_FREQUENCY_LADDER_RESULT_2026_07_30.md"
    ).write_text(
