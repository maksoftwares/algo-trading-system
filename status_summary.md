...
Generated UTC: `2026-07-07T20:50:00Z`
...
| Item | Result |
| --- | --- |
| Current final audit | Ordered plan evidence is consolidated in `xau-usd/xauusd-phase1/docs/A1_XAU_OWNER_GOAL_FRONTIER_EXHAUSTION_AUDIT_2026_07_06.md`. Step 1 split grid, Step 2 macro gate, Step 3 portfolio composition, and Step 4 daily-extreme new-family design all failed to produce a demo-ready owner-goal strategy. |
| Current best frontier | F67-H16 no-f33 composition remains the best exact-ledger frontier: `3751` signals, WR `50.23%`, W/L `2.0002`, active weekdays `86.39%`, PF `2.0336`, net `22294.46 USD`, last-12 WR/W-L/active `52.86%/2.2118/80.84%`, stress `-0.30/ticket` W/L `1.9029`, exit-time positive weeks `54.81%`, worst exit week `-878.18 USD`. This is research-only, not demo-ready. |
| H4/D1 reviewer repair exact probe | `NO_H4_D1_REVIEW_REPAIR_SURVIVOR`: a no-op exact-MT5 parity audit found refreshed-tester session drift at Friday server-hour `20:00`; adding archived-comparison session pin `InpBlockedEntryDayHoursCsv=5:20` restored signal/source/weekly/recent parity with only minor fill drift (`-8.14 USD`, W/L `-0.0004`). Corrected H4/D1 repair rerun still found no survivor. `supportive_guard` improved recent 3M from `-1226.32 USD` to `+279.22 USD`, May from `-1055.98 USD` to `-142.12 USD`, W/L to `2.0895`, stress W/L to `1.9720`, and positive weeks to `57.69%`, but worst week stayed `-878.18 USD`, so it still failed weekly shape. `weekly_loss_governor` failed W/L/stress/recent repair. Reports: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_H4_D1_NOOP_SESSION_PARITY_AUDIT_EXACT_202207_202606.md` and `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_H4_D1_REVIEW_REPAIR_EXACT_202207_202606.md`. |
| Partial-ladder exact probe | `NO_PARTIAL_LADDER_SURVIVOR`: four preregistered H4/D1 ladder cells ran in exact MT5 with position-ID deal-log reconstruction. Best row `p33_t2_run4_be` improved positive weeks to `57.21%` and June 2026 to `+164.14 USD`, but broke W/L to `1.9213` and stress W/L to `1.8106`. |
| Smooth second-book weekly diagnostic | `NO_70_80_WEEKLY_HIT`: existing exact-MT5 ledgers, reconstructed causal residual packages, top causal coverage candidates, all 42 loaded exact-MT5 individual variants, and fixed `0.5x-10x` second-book sizing were recomposed offline. Best 90% activity blend reached active weekdays `90.22%`, but positive calendar weeks stayed `54.29%` and W/L fell to `1.5950`. Best standalone smoother reached `61.90%` positive weeks but only `52.64%` active weekdays and W/L about `0.68`. |
| Weekly-state red-week rescue | `NO_WEEKLY_STATE_RESCUE`: current-week closed-P&L add-on gates, previous-red-week add-on gates, and baseline risk-off gates tested `760` rows. Best causal row reached only `58.10%` positive calendar weeks and `87.63%` active weekdays with W/L `1.7767`. A non-causal oracle that added current-pool trades only in known future baseline red weeks still topped out at `65.24%` positive weeks and `88.59%` active weekdays. |
| Weekly target gap | Baseline has `114/210` positive calendar weeks. To reach `70%`, we need `147/210`, so `33` current red weeks must flip; the easiest 33 require only `353.53 USD` total rescue in hindsight, but the current-pool oracle still failed, so the missing piece is a genuinely different red-week source with the right timing/coverage. |
| Built-in source inventory | Existing `InpSignalMode` values `0-11` have all been touched by the owner-goal program or earlier exact-MT5 probes. Weekly-level H4 rejection was not an untested shortcut: it already failed at `171` trades, WR `31.58%`, W/L `1.9852`, PF `0.9162`. |
| Next source plan | `xau-usd/xauusd-phase1/docs/A1_XAU_NEXT_RED_WEEK_SOURCE_PLAN_2026_07_06.md`: next exact-MT5 probe should be a genuinely new weekly-damage reversal/continuation source, judged by red-week flips/worsening as first-class metrics. |
| Weekly-damage H1 exact probe | `FREEZE_WEEKLY_DAMAGE_H1_SOURCE_CLASS`: V14/V14B exact MT5 probes over `2022-07-01 -> 2026-06-30` did not bridge the weekly target. Best baseline hybrid reached only `56.73%` positive weeks; best combo on top of weekly-state rescue reached `59.52%` positive weeks and `90.03%` activity with W/L `1.7708`. |
| Prior-day level M5 exact probe | `FREEZE_PRIOR_DAY_LEVEL_M5_SOURCE_CLASS`: V15/V15B exact MT5 probes bought activity but not weekly repair. Best V15B baseline hybrid reached `57.69%` positive weeks and `89.26%` activity from a standalone-negative short-only row; best combo on top of weekly-state rescue reached `59.05%` positive weeks and `90.41%` activity. |
| Asian-range M5 exact probe | `FREEZE_ASIA_RANGE_M5_SOURCE_CLASS`: V16 exact MT5 probes showed the clearest activity/weekly split. Best row `v16_asia_range_cont_rr2` was standalone-positive (`2658` trades, PF `1.0596`, net `904.53 USD`) and raised baseline-hybrid activity to `96.84%`, but positive weeks stayed `56.73%`; weekly-state combo fell to `57.14%` positive weeks while activity rose to `96.84%`. |
| Post-V16 risk-off premise audit | `NO_SIMPLE_RISK_OFF_PATH`: exact exit-time ledger anatomy shows `51/94` red weeks are frequency-dominated and `43/94` are H4/D1-dominated; `22/24` large red weeks below `-100 USD` are H4/D1-dominated. Best simple pair reaches `60.58%` positive weeks only by collapsing W/L to `1.3591`. |
| 12:20 deadline audit | `DEADLINE_MISSED_NO_DEMO_READY_OWNER_GOAL_STRATEGY`: best serious frontier remains V14 weekly-state combo at `59.52%` positive weeks / `90.03%` activity / W-L `1.7708`; best smoother diagnostic reaches `60.58%` positive weeks only with W-L `1.3591`. |
| Reviewer challenge prompt | `xau-usd/xauusd-phase1/docs/A1_XAU_OWNER_GOAL_EXHAUSTION_REVIEW_PROMPT_2026_07_07.md`: prepared but not spent; asks reviewer to confirm exhaustion, propose a genuinely different next test, or recommend target relaxation. |
| Anti-overfit decision matrix | `xau-usd/xauusd-phase1/docs/A1_XAU_ANTI_OVERFIT_DECISION_MATRIX_2026_07_07.md`: current-family overfit risk is now high. No demo spec and no further current-family tuning; next valid action is reviewer challenge, a genuinely new preregistered strategy class, explicit target relaxation, or retirement of the current family for the weekly-positive owner target. |
| Event-reaction new-class prereg | `xau-usd/xauusd-phase1/docs/A1_XAU_EVENT_REACTION_NEW_CLASS_PREREG_2026_07_07.md`: six fixed NFP/CPI/FOMC post-event impulse/fade cells were implemented as default-off MT5 signal mode `14` and run in exact MT5. No reviewer spend. |
| Event-reaction calendar provenance | `EVENT_CALENDAR_OFFICIAL_PROVENANCE_USED_IN_EXACT_MT5_V0`: official BLS NFP/CPI release-calendar rows and official Federal Reserve FOMC rows are frozen for `2022-07-01 -> 2026-06-30`. Calendar has `126` events: `47` NFP, `47` CPI, `32` FOMC. Calendar CSV SHA256 `39ebfd218ff3910643963f87b64de4431a35ad390a1862e82fe516203b4b69a1`. |
| Event-reaction V0 exact MT5 | `DIAGNOSTIC_CLUE_LOW_ACTIVITY_NO_DEMO_SPEC`: six fixed event cells ran over `2022-07-01 -> 2026-06-30`. Best quality clue is `event_impulse_fomc_rr2`: `16` trades, WR `56.25%`, PF `1.81`, net `+68.52 USD`, active weeks `7.62%`. Best activity cell reaches only `13.33%` active weeks. Result note: `xau-usd/xauusd-phase1/docs/A1_XAU_EVENT_REACTION_V0_EXACT_MT5_RESULT_2026_07_07.md`; runner report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_M5_MOMENTUM_VARIANT_BACKTEST_EVENT_REACTION_V0_202207_202606.md`. |
| Event red-week overlay audit | `NO_EVENT_OVERLAY_RED_WEEK_SURVIVOR`: exact-MT5 event V0 ledgers were recomposed onto the corrected supportive-guard book. FOMC impulse alone kept 15 event trades and netted `+79.91 USD`, but touched 6 red weeks, flipped 0, worsened 4, and reduced positive weeks to `57.21%`. All positive-net V0 event cells kept 86 event trades and netted `+184.63 USD`, but flipped only 3 red weeks while worsening 19; positive weeks stayed `57.69%`. Event overlay is too sparse/mistimed to rescue weekly shape. Report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_EVENT_RED_WEEK_OVERLAY_AUDIT_202207_202606.md`. |
| Non-uptrend range-fade red-week exact probe | `NO_NONUPTREND_RANGE_FADE_RED_WEEK_SURVIVOR`: added default-off generic D1 support-state gate (`InpD1SupportStateGateMode`) and exact-MT5 tested three fixed non-uptrend range-fade variants over `2022-07-01 -> 2026-06-30`, then recomposed them onto corrected `supportive_guard`. Best weekly-repair row was `nonup_all_range_fade_rr2`: `4395` signals, WR `47.62%`, W/L `2.1516`, active `89.55%`, PF `1.9682`, net `20439.99 USD`, stress W/L `2.0244`, positive weeks `58.65%` (`+0.96pp`), red weeks flipped `12`, but red weeks worsened `31` and WR failed. Cleanest core row `nonup_orrev_london_rr2_only` held WR/W-L (`50.22%`/`2.0955`) but did not improve weekly shape. Freeze this range-fade direction; do not tune hours/thresholds from this output. Report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_NONUPTREND_RANGE_FADE_RED_WEEK_EXACT_202207_202606.md`. |
| Downtrend short-engine exact probe | `DOWNTREND_SHORT_STANDALONE_CLUE_NO_COMBINED_SURVIVOR`: added bearish D1 gate mode `InpD1SupportStateGateMode=3` and exact-MT5 tested four short-only bearish-D1 variants. The best standalone clue is `down_m5_ema_h1h4_short_rr2`: `438` trades, WR `33.11%`, W/L `2.1595`, PF `1.0687`, net `+137.34 USD`, recent3 `+258.45 USD`, but positive weeks only `42.86%`. Combined with `supportive_guard`, it held W/L/stress (`2.1343` / `2.0127`) and improved recent3 to `+414.85 USD`, but WR fell to `48.74%`, positive weeks slipped to `57.42%`, and red weeks worsened `24`; no combined survivor. Report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_DOWNTREND_SHORT_ENGINE_EXACT_202207_202606.md`. |
| Bear-engine improvement exact probe | `NO_BEAR_ENGINE_IMPROVEMENT`: five structural exact-MT5 variants tried to improve the `down_m5_ema_h1h4_short_rr2` clue without hour-picking. None beat the reference on both more trades and better WR while preserving payoff. `bear_break_run_h1h4_rr2` was the best net/payoff diagnostic (`445` trades, WR `32.13%`, W/L `2.3536`, PF `1.1144`, net `+208.00 USD`) but WR was worse than the reference `33.11%`. `bear_ema_pullback_h1h4_rr2` added trades (`487`) but WR fell to `31.42%` and net was `-186.96 USD`. Combined rows all failed WR (`45.62%-48.82%`). Report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_BEAR_ENGINE_IMPROVEMENT_EXACT_202207_202606.md`. |
| Bear quality-first exact probe | `NO_BEAR_QUALITY_FIRST_HIT`: six preregistered exact-MT5 quality-first bearish variants accepted lower trade count to seek higher WR/payoff. Tightening existing M5 bear entries cut trades but worsened WR/net: `bear_quality_m5_ema_slope50` was `209` trades, WR `30.14%`, W/L `1.9311`, net `-114.17`; `bear_quality_m5_ema_slope100` was `134` trades, WR `28.36%`, W/L `1.8855`, net `-113.67`; `bear_quality_break_run_tight` kept payoff (`2.0058`) but WR fell to `27.60%` and net `-145.26`. The only profitable quality-ish row, `bear_quality_h4_pullback_d1bias`, had just `18` trades, WR `33.33%`, W/L `2.6756`, PF `1.3378`, net `+51.40`, so it is too sparse. Combined rows did not improve weekly shape; all-cells combined worsened positive weeks to `54.81%`. Report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_BEAR_QUALITY_FIRST_EXACT_202207_202606.md`. |
| Short hedge exact probe | `SHORT_HEDGE_COMBINED_REVIEW_CANDIDATE`: added default-off structural short modes `SIGNAL_BEAR_BREAKDOWN_RETEST=15` and `SIGNAL_BEAR_SWEEP_RECLAIM=16`, froze the old bear-continuation branch as a control, and ran three exact-MT5 variants over `2022-07-01 -> 2026-06-30`. Best row is `short_hedge_v2_breakdown_retest`: standalone `329` trades, WR `32.83%`, W/L `2.8332`, PF `1.3846`, net `+441.42 USD`, stress `-$0.30/trade` PF `1.2823`, stress W/L `2.6239`, stress net `+342.72`, Q2/recent3 `+283.39`, top1 contribution `8.06%`, top-day `28.42%`; it passed standalone hedge checks. Combined with `supportive_guard`: `3953` signals, WR `49.00%`, W/L `2.1637`, PF `2.0934`, net `+21064.67`, stress W/L `2.0390`, positive weeks `58.17%` (`+0.48pp`), recent3 `+494.57`, new kept net `+381.00`. Current guarded long-box baseline has no Q2 long-box loss, so Q2 repair is recorded as positive Q2 addition, not loss reduction. Research-only pending reviewer approval; no demo spec. Local report: `xau-usd/xauusd-phase1/outputs/reports/A1_XAU_SHORT_HEDGE_EXACT_202207_202606.md`; GitHub review packet: `xau-usd/xauusd-phase1/docs/reviewer_audit_2026_07_08_short_hedge/README.md`. |
| Current decision point | Do not draft a demo spec yet. The short-hedge V2 breakdown-retest is the first short-side candidate that passes both standalone hedge checks and combined-book shape, but it is a single exact-MT5 pass and needs reviewer scrutiny before any forward/demo discussion. Freeze V1 control and V3 sweep-reclaim as weaker; next valid work is reviewer review of V2, then only small preregistered robustness checks if the reviewer accepts the method. |
...
| Next action | Do not draft a demo spec. The H4/D1 supportive-state guard is a useful clue for recent-regime damage but still fails the weekly/worst-week repair gate. If reviewer agrees, freeze H4/D1 masking/repair and move to a genuinely new preregistered red-week rescue source class, or explicitly relax the weekly-positive/activity targets. |
