...
def asymmetric_candidates() -> list[FrequencyCandidate]:
    rows: list[FrequencyCandidate] = []
    counter = itertools.count(4001)
...
def main() -> None:
    ...
    output = ROOT / "outputs" / "frequency_v2_asymmetric_hunt"
    output.mkdir(parents=True, exist_ok=True)
    universe = asymmetric_candidates()
    ...
    frozen_ids: list[str] = []
    for _, group in development_frame[development_frame["gate_pass"]].groupby(
        ["family", "regime", "threshold"]
    ):
        ordered = group.sort_values(
            ["top_5pct_removed_profit_factor", "profit_factor", "trades"],
            ascending=False,
        )
        frozen_ids.extend(ordered.head(4)["candidate_id"].tolist())
    ...
    # Retain the Generation-2 quality sleeve as the immutable portfolio seed.
    seed_path = (
        ROOT / "outputs" / "frequency_v2_scheduled_hunt" / "VALIDATION_PORTFOLIO_TRADES.csv"
    )
    seed = pd.read_csv(seed_path, parse_dates=["entry_time", "exit_time"]).to_dict("records")
    seed_ids = list(dict.fromkeys(trade["candidate_id"] for trade in seed))
    combined_trades = {
        **validation_trades,
        **{
            candidate_id: [trade for trade in seed if trade["candidate_id"] == candidate_id]
            for candidate_id in seed_ids
        },
    }
    priority = seed_ids.copy()
    remaining = sleeves.copy()
    selection_path = [
        {
            "selection_order": 0,
            "candidate_id": "GENERATION_2_QUALITY_SEED",
            **metrics(seed, days["validation"]),
        }
    ]
    while remaining and len(priority) < 30:
        best = None
        for candidate, _ in remaining:
            trial_priority = priority + [candidate.candidate_id]
            trial = route_portfolio(combined_trades, trial_priority, maximum_trades_per_day=2)
            result = metrics(trial, days["validation"])
            quality_ok = (
                result["profit_factor"] >= 1.30
        ...
        if result["profit_factor"] < 1.30 or result["average_r"] <= 0:
            break
        ...
        selected_regimes = {
            candidate_map[x].regime for x in priority if x in candidate_map
        } | {"trend_up", "trend_down"}
        if result["trades_per_active_day"] >= 1.0 and len(selected_regimes) >= 3:
            break

    portfolio = route_portfolio(combined_trades, priority, maximum_trades_per_day=2)
    portfolio_metrics = metrics(portfolio, days["validation"])
    added_specs = [asdict(candidate_map[x]) for x in priority if x in candidate_map]
    distinct_regimes = len({row["regime"] for row in added_specs} | {"trend_up", "trend_down"})
    gate_pass = bool(
    ...
    verdict = {
        "schema_version": "eurusd_frequency_v2_asymmetric_hunt_v1",
        ...
        "generation_2_seed_ids": seed_ids,
        "selected_priority": priority,
        "added_specialists": added_specs,
        ...
        "verdict": (
            ...
            if gate_pass
            else "ASYMMETRIC_GENERATION_FAILED_CONTINUE_RESEARCH"
        ),
    }
