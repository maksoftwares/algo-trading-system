...
#property description "Tester-only, zero-order EURUSD M15 bid OHLC and spread exporter"

input string InpTargetSymbol = "EURUSD";
input string InpOutputFileName = "EURUSD_M15_CAPITAL_BROKER.csv";
...
void OnTick()
{
   datetime current = iTime(InpTargetSymbol, PERIOD_M15, 0);
   if(current == 0 || current == lastM15)
      return;
   lastM15 = current;

   MqlRates bar[];
   ArraySetAsSeries(bar, true);
   if(CopyRates(InpTargetSymbol, PERIOD_M15, 1, 1, bar) != 1)
      return;
